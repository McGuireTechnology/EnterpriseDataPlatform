function New-Result{
    <#
    .SYNOPSIS
        Creates new data structure with predefined fields to store the result
        of indicator execution
    .NOTES
        PowerShell Duck typing and implicit string to enum conversion allow to
        use multiple types and preserve type safety
    #>
    [CmdletBinding()]
    param()

    $resultType = "Semperis.PSSecurityIndicatorResult.SecurityIndicatorResult"
    if(-not($resultType -as [type])){
        Write-Warning "Type [$($resultType)] is not loaded"
        $resultType = 'System.Collections.Generic.Dictionary[string, object]'
        Write-Warning "Using [$($resultType)] instead"
    }

    $result = New-Object -TypeName $resultType
    $result.ResultMessage = 'No evidence of exposure.'
...
function Out-Log {
    <#
    .SYNOPSIS
        Provides a function to standardized log entries
    .DESCRIPTION
        When specified outputs verbose logging in a standardized format
    .PARAMETER Message
        Descriptive message to write to log
    .PARAMETER Operation
        Particular operation that trigged this log
    .PARAMETER Data
        Object containing relevent information to reproduce the operation
    .EXAMPLE
    ...
    process {
        $logEnabled = $VerbosePreference -ne 'SilentlyContinue'
        if(-not $logEnabled) {
            return
        }
        $timestamp = (Get-Date).ToUniversalTime().ToString("O")
        $scriptName = "$($self.ScriptName)"
        $operationText = if($Operation) { "${Operation}: " }
        $messageText = if($Message) { "${Message} "  }
        $dataObject = if($Data) {
            $Data.Remove('Verbose')
            $Data.Remove('TenantAppSecret')
            $Data | ConvertTo-Json -Compress
        }
        $logText = "[$timestamp $scriptName] ${operationText}${messageText}${dataObject}"
        Write-Verbose -Message $logText
    }
}

function ConvertTo-ErrorResult {
    <#
    .SYNOPSIS
        Provides unified global exception handling for all indicators
    #>
    param (
        $ErrorObject
    )
    ...
    $result = New-Result
    $result.Status = 'Error'
    $result.ResultMessage = $ErrorObject.Exception.Message
    $result.Remediation = 'None'
    $result.Score = 0

    if($ErrorObject.FullyQualifiedErrorId -eq 'NoEntraIdCredentialsProvided'){
        $result.Status = 'NotRelevant'
        $result.ResultMessage = 'Entra ID not connected. Please complete the Identity analytics server (Hybrid server) setup.'
        return $result
    }

    Write-Error "$($ErrorObject.Exception.Message) $($ErrorObject.ScriptStackTrace)" -ErrorAction Continue
...
function Get-Score{
    ...
    process {
        ...
        $maxWeight = 11
        $finalCount = $objectsCount - $ignoredCount
        $value = $finalCount * $Impact
        $score = [Math]::Floor((1.0-(($value / ($maxWeight - $Impact)) * 0.1))*100)
        if($score -le 0){$score = 0}
        $scoreOutput = [PSCustomObject][Ordered] @{
            Score = $score
...
function Resolve-Configuration {
    <#
    .SYNOPSIS
        Resolves path to specified indicator configuration and loads the file in memory
    .NOTES
        When configuration is found in Global or Environment Variable (e.g. equal to ""):
        No disk searches and reads are performed
    #>
    ...
    # Global Variable (Per Powershell Session/Tenant)
    $configurationJson = $Global:IOE_RUNTIME_CONFIG
    if ($configurationJson -ne $null)
    {
        return $configurationJson | ConvertFrom-Json
    }

    # Environment Variable (Per Indicator)
    $variableName = "IOE_$($ScriptName)_CONFIG"
    $configurationJson = [System.Environment]::GetEnvironmentVariable($variableName)
    if ($configurationJson -ne $null)
    {
        # In case when variable exist we do not need to load configuration from file
        return $configurationJson | ConvertFrom-Json
    }

    # File System (Per Indicator on Disk)
    $programData = Join-Path -Path $env:ProgramData -ChildPath 'Semperis\Indicators'
    if ($Path)
    {
        $configPath = "$((Get-location).Path)\Config"
        $pathsToSearch = @($configPath)
    }
    else
    {
        $pathsToSearch = @($programData)
    }

    $configFile = "$ScriptName.json"
    $fieldNames = ($Fields |
            Get-Member -MemberType 'NoteProperty' |
            Select-Object -ExpandProperty 'Name')

    # Lookup configuration
    foreach($directory in $pathsToSearch){
        $specificFile = Join-Path $directory -ChildPath $configFile
        if(-not (Test-Path $specificFile)){ continue }

        $configuration = Get-Content $specificFile | ConvertFrom-Json

        $configSchema = @{
            ScriptName = $ScriptName
            Configuration = $configuration
        }
        $updatedConfig = Update-Configuration @configSchema

        if($null -ne $updatedConfig){
            $configuration = $updatedConfig
            $configuration | ConvertTo-Json -Depth 10 | Out-File $specificFile
        }
    ...
    if($Path)
    {
        $isSchemaPath = (Test-Path $configPath)
        if($isSchemaPath){$shouldCreateSchema = $true}
        else
        {
            try{
        ...
        $schemaLocation = Join-Path $configPath -ChildPath $configFile
    }
    else{
        $shouldCreateSchema = (Test-Path $programData)
        $schemaLocation = Join-Path $programData -ChildPath $configFile
    }
    if($shouldCreateSchema)
    {
        ...
        foreach($fieldName in $fieldNames){
            $defaultSchema.exclude.Object[$fieldName] = $Script:FieldPredicate
        }
        $defaultSchema |
            ConvertTo-Json -Depth 10 |
            Out-File $schemaLocation -ErrorAction 'Continue'
...
function Test-Predicate {
    <#
    .SYNOPSIS
        Evaluates to $true if field value satisfies specified predicate object
    .PARAMETER FieldPredicate
        Collection of PowerShell derived comparison operators in form of an object:
        @{ 'operator-name' = $operands, ... }
    .NOTES
        Field predicate behaves as disjunction (OR): if any of the operators evaluates to true, result is true
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
        ...
        [Parameter(Mandatory)]
        [Alias('Predicate')]
        [PSCustomObject] $FieldPredicate
    )
    process {
        if($FieldPredicate -is [hashtable]) {
            # Value is expected to be of PSCustomObject type and PS does not do the conversion
            $FieldPredicate = [PSCustomObject]$FieldPredicate
        }
        $fieldValue = $InputObject."$FieldName"
        if($null -eq $fieldValue){
            if($InputObject | Get-Member -Name $FieldName){
                Write-Warning "[$($InputObject.GetType())] property [$FieldName] is null: Object not ignored"
            }
            else {
                Write-Warning "[$($InputObject.GetType())] property [$FieldName] not present"
            }
            return $false
        }
        $operators = $FieldPredicate |
            Get-Member -MemberType NoteProperty |
            Select-Object -ExpandProperty 'Name'
        ...
        foreach($operator in $operators){
            $operands = $FieldPredicate."$operator"
            if(-not $operands) {
                # Ignore empty conditions and treat them as unspecified
                continue
            }
            switch ($operator) {
                'eq' {
                    # Equivalent to -in for arrays
                    if($operands -eq $fieldValue) { return $true }
                }
                'ne' {
                    return -not(Test-Predicate -Predicate @{ eq = $operands } @sameField)
                }
                'like' {
                    foreach($operand in $operands){
                        if($fieldValue -like $operand) { return $true }
                    }
                }
                'notlike' {
                    return -not(Test-Predicate -Predicate @{ like = $operands } @sameField)
                }
                'in' {
                    if($fieldValue -in $operands) { return $true }
                }
                'notin' {
                    return -not(Test-Predicate -Predicate @{ in = $operands } @sameField)
                }
                'match' {
                    foreach($operand in $operands){
                        if($fieldValue -match $operand) { return $true }
                    }
                }
                'notmatch'{
                    return -not(Test-Predicate -Predicate @{ match = $operands } @sameField)
                }
                'ge' {
                    foreach($operand in $operands){
                        if($fieldValue -ge $operand) { return $true }
                    }
                }
                'le' {
                    foreach($operand in $operands){
                        if($fieldValue -le $operand) { return $true }
                    }
                }
                default {
                    Write-Error "Property [$FieldName] unsupported predicate '$operator'"
                }
            }
        }
    }
...
$Script:NameChanges = @{
    # Distinguished Names
    'MemberDistinguishedName' = 'DistinguishedName'
    'ComputerDistinguishedName' = 'DistinguishedName'
    'AttackerDistinguishedName' = 'DistinguishedName'
    'UserDistinguishedName' = 'DistinguishedName'
    'PreWin2KGroup.GroupDistinguishedName' = 'DistinguishedName'
    'PrivilegedGroupChanges.GroupDistinguishedName' = 'DistinguishedName'
    # SamAccountName
    'FSPInPrivilegedGroup.MemberSamAccountName' = 'SamAccountName'
}

function Update-Configuration {
    <#
    .SYNOPSIS
        Performs configuration update based on static history of field name changes
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    $fieldPredicates = $Configuration.exclude.object
    if(-not $fieldPredicates) {
        return $null
    }
    ...
    $fieldNames = $fieldPredicates |
        Get-Member -MemberType 'NoteProperty' |
        Select-Object -ExpandProperty 'Name'

    foreach($fieldName in $FieldNames){
        $predicate = $Configuration.exclude.Object."$fieldName"

        $newName = $null
        if($NameChanges.ContainsKey("$ScriptName.$fieldName")){
            # Specific Rename
            $newName = $NameChanges["$ScriptName.$fieldName"]
        }
        elseif($NameChanges.ContainsKey($fieldName)){
            # Global Unification
            $newName = $NameChanges[$fieldName]
        }
        elseif($fieldName -like '* *') {
            # Display Name as Identifier
            $newName = $fieldName -replace ' ', ''
        }
        elseif($fieldName -cmatch '^[a-z]') {
            # camelCase to PascalCase
            $newName = $fieldName.Substring(0, 1).ToUpper() + $fieldName.Substring(1)
        }
        ...
        $updatedConfig.exclude.object[$fieldName] = $predicate
    }

    if($schemaChanged){
        Write-Verbose "[$ScriptName]: Configuration changes detected"
        $updatedConfig.exclude.object = [PSCustomObject]$updatedConfig.exclude.object
        return [PSCustomObject]$updatedConfig
    }
}

function Set-IgnoredFlag {
    <#
    .SYNOPSIS
        Processes collection of objects, evaluating optional $Configuraton.excluded.Object filter for each field
    .NOTES
        New 'Excluded' property is added if input object satisfies the filter
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory, ValueFromPipeline)]
    ...
    process {
        ...
        $fieldPredicates = $Configuration.exclude.object
        if($fieldPredicates){
            $jsonKeys = $fieldPredicates |
                Get-Member -MemberType 'NoteProperty' |
                Select-Object -ExpandProperty 'Name'

            foreach($fieldName in $jsonKeys) {
                $condition = @{
                    InputObject = $InputObject
                    FieldName = $fieldName
                    FieldPredicate = $fieldPredicates.$fieldName
                }
                if(Test-Predicate @condition){
                    $ignored = $true
                    break
                }
            }
        ...
        $property = @{
            NotePropertyName  = 'Ignored'
            TypeName = 'bool'
            NotePropertyValue = $ignored
        }
        $InputObject | Add-Member @property
    }
}
...
function Get-ParameterValue {
    <#
    .SYNOPSIS
        Returns configured parameter value in the context of currently executed indicator or default value
    ...
    $indicatorName = $self.ScriptName
    if($null -eq $indicatorName){
        Write-Warning "Cannot define Indicator script context. Ensure that $Global:Self is being set"
        return $DefaultValue
    }

    $configuration = Resolve-Configuration -ScriptName $indicatorName
    $value = $configuration.param.$Name
    if($null -eq $value){
        return $DefaultValue
    }
    Write-Verbose "[$indicatorName]: Using configured parameter [$Name] with value [$value]"
    return $value
}

