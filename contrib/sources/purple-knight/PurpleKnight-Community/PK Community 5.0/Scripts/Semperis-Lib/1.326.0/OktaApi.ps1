...
function Invoke-OktaApiRequest {
    <#
    .SYNOPSIS
        Invokes web request to Okta API
    #>
    [CmdletBinding()]
    param(
        [Uri] $Uri,
        ...
        # in my testing the minimum sleep interval to avoid API throttling on
        # endpoints that default to 20 per minute (like /api/v1/logs) was 6
        [int] $SleepInterval = 6,
    ...
    try {
        # avoid progress output for Invoke-WebRequest
        $ProgressPreference = 'SilentlyContinue'

        $response = Invoke-WebRequest @request
        $results = $response.Content | ConvertFrom-Json

        if (-not $OnlyFirstPage) {
            $next = Get-OktaNextPageLink -Headers $response.Headers

            while ($next -and (--$MaxPages -ne 0)) {
                # sleep within page retrieval to attempt to avoid rate limits
                Start-Sleep -Seconds $SleepInterval

                $request["Uri"] = $next
                $response = Invoke-WebRequest @request
                $newResults = $response.Content | ConvertFrom-Json

                # break out if no results, otherwise it continues forever
                if ($newResults.Count -lt 1) {
                    break
                }
                $results += $newResults
        ...
        # reset progress variable to default
        $ProgressPreference = 'Continue'

        return $results
    }
    catch {
        # deal with different errors nicely
        $errorMessage = "Unable to query ($($request.Uri)):"
        if ($_.ErrorDetails.Message -match "Invalid search criteria.") {
            throw "$($errorMessage) Invalid filter."
        }
        elseif ($_.ErrorDetails.Message -match "The endpoint does not support the provided HTTP method") {
            throw "$($errorMessage) Invalid URI. Please check and try again."
        }
        elseif ($_.ErrorDetails.Message -match "You do not have permission to perform the requested action") {
            throw "$($errorMessage) Insufficient permissions to query this API endpoint."
        }
        elseif ($_.ErrorDetails.Message -match "Api validation failed: RequestParams") {
            throw "$($errorMessage) Invalid request parameters."
        }
        elseif ($_.ErrorDetails.Message -match "API call exceeded rate limit due to too many requests.") {
            throw "$($errorMessage) The request has failed due to Okta API throttling. Please try again later."
        }
        elseif ($_.Exception.Response.StatusCode -eq 404) {
            throw "$($errorMessage) HTTP page not found error returned."
        }
        elseif ($_.Exception.Response.StatusCode -eq 403) {
            throw "$($errorMessage) HTTP forbidden error returned."
        }
        throw
    }
    finally {
        # reset progress variable to default if exception occurred
        $ProgressPreference = 'Continue'
    }
}

function Get-OktaNextPageLink {
    <#
    .SYNOPSIS
        Extract next link from API return headers
    #>
    [CmdletBinding()]
    param(
        $Headers
    ...
    $next = $null
    foreach ($link in $Headers.Link) {
        if ($link -match 'rel="next"') {
            $next = $link.SubString(1, $link.IndexOf('>; rel="next"') - 1)
        }
    }
...
function Get-OktaSuperAdmins {
    [CmdletBinding()]
    param(
        [string] $TokenValue
    )
    $superUsers = [System.Collections.ArrayList]@()
    ...
    $users = Invoke-OktaApiRequest @getUsers

    foreach($user in $users)
    {
        $uid = $user.id
        $uriRoles = "/api/v1/users/$($uid)/roles"
        ...
        $userRoles = Invoke-OktaApiRequest @getUserRoles

        foreach($role in $userRoles)
        {
            if($role.type -eq "SUPER_ADMIN")
            {
                [void]$superUsers.Add($user.profile)
            }
        }
    }
...
function ConvertFrom-OktaUrlOrSPN {
    <#
    .SYNOPSIS
        Extract Okta Company name from request URL or SPN
    #>
    [CmdletBinding()]
    param(
        [Uri] $UrlOrSpn
    ...
    # first deal with URL
    $url = $true
    if ($UrlOrSpn.AbsoluteUri) {
        $hostPart = $UrlOrSpn.Host
    }
    # else assume it's an SPN
    else {
        $hostPart = ($UrlOrSpn.OriginalString -split '/')[1]
        $url = $false
    }

    $company = ($hostPart -split '\.')[0]
    # if URL ends with '-admin', strip it
    if ($company.EndsWith("-admin") -and $url) {
        $company = $company.SubString(0, $company.Length - 6)
    }

    return $company
}

function Get-OktaOrg {
    <#
    .SYNOPSIS
        Retrieve Okta Company information from API
    #>
    [CmdletBinding()]
    param(
        [Uri]
    ...
    return (Invoke-OktaApiRequest @getOrg)
}

function Get-OktaKerberosRealms {
    <#
    .SYNOPSIS
        Retrieve configured Kerberos realm information from Okta
    #>
    [CmdletBinding()]
    param(
    ...
    $oktaRealms = @{}
    if ($PSBoundParameters['DomainUrl'] -and $PSBoundParameters['TokenValue']) {
        $uri = "/api/internal/v1/adKerberosRealm"
        ...
        $kerbRealms = Invoke-OktaApiRequest @getOktaRealms
        foreach ($kerbRealm in $kerbRealms) {
            $oktaRealms[$kerbRealm.domain] = $kerbRealm
        }
    }
...
$CredsHighPrivs = @("okta.users.credentials.expirePassword","okta.users.manage","okta.users.credentials.manage","okta.users.credentials.resetFactors","okta.users.credentials.resetPassword")

Export-ModuleMember -Function Invoke-OktaApiRequest
Export-ModuleMember -Variable CredsHighPrivs
# SIG # Begin signature block
# MIIuIwYJKoZIhvcNAQcCoIIuFDCCLhACAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA+pKD8WPXtLmM/
# PZm76uVvOkTmFidF+2cu+YPqifUOeqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgXmxfaTDENvLafZnwEw+Dl8VJ1fw1PfTnsEX6sacWQjAwDQYJKoZI
# hvcNAQEBBQAEggIAcb78ZXTwtM9KSGZbcJYeEG/eZlEkmdiqHQBPNM9hNYDbuSYR
# cFUUSZAEqnLw+51NTmQhjCxWSrWnUl1arD++wjBecWQR5KJivgQTfOWJMuB0RxyT
# 9OjbaMY9JM3vSYP+KlmKAAj9mDVTt+Ur1a1Eo2t1M0rMrX8vaIFvtWnWi6TaUGAq
# 73KD5laN+APay3mEZ0h4TT2MiGNLQiyttKg1QeoPB9cC81Zyq/nx9q1cHiMe82+i
# JZS6TRdbcByXvtiTSJXtX/5SNUxKHhoO/PDr4xwdv3ikBx1cHq5vTqiDnXzqlISU
# N514DRKU+eZlU5O1mrwhK85NbOIp5yDHKuIv2bQ/fpwsxrNoqdM/vfoSoDgFrxY0
# Sie1OzSn3uzQ3KuuARwKG2/G/sjlc8WMjDRRum0k4RKh3C7rksX/icg4Oi1l8P4J
# 6n/zTIW05XrHid4ZdL/2YL/A6bxwymtLAugRpY3CexfSx4xUj34fsamIspz05UfO
# bHlVpfo6u9NrTRQo/1Lh48deNpLvuxvpJOIb4SgoYPGjITPclaluVtbNaheiuxzT
# Vv7/pRlFusW7zuNXQ2ZVar73IKxk3hBv3SoGroCYLXVcbwqMZfew0E/vpbvtDAxA
# XtcLRCeogbcs4KAKIlTIWGqQlOdNZcylDj9Mmifqxt1KqT+3lRBfqKsRQ7Khgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBBA56vkFzwO2Nn3uew4EjdowfIkygVH+Pe
# 035XaU27VgIUE57zmldHSaWcUGQC1UT/coWpaDQYDzIwMjUwNjEwMDc1MDIwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAVrVLY9KQarUxxEd6pu8y2qV+y8b/AQvfajL2Krkz8
# QjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgIBfN8j8GMdEgKjCHAKbCQJYvQ4tBejT6ZgGm/jiwZ0iksc/EN118Ayk
# gBwSfXBvNTMudXf61f6WGgU8csE05lnkDqnQkU6ZUDLOwx/Sz8nwD1h91lRnQmFS
# KIjFoKvSHMI16aq8Kexe3qjUBZsvV1xYTUTf1Ndj8sdYHlCgyfAgPKujzqSrBdIZ
# vazOscHPLaQyG00/LcZuLhush8eHXWEgcdb0yWwFBxS/mksNvBUN9yFt7LwkC9MY
# aWWJxgRBOLoyTz8TqOQ/YTdh5ahKxAgz3ig0L8bL1seKyuvP1BHYUPoxQqXFOVMb
# XZOHdYJzVY33lEYGVnrJD4nnSso6Be3ZddQpwJ6ohFje3bJQ5oO91HalT4Ttd+2q
# Q8YzPz2NCn5KBGI2wR08u2H2bdrHE2YNEeKVN43wUVmrSu9wxji5Fa6W5qJPLTnj
# 6YM9TLrSPsHLDonVhJ4o3Cacne3DAILDXoP2tijXvM05kEXXlHaZPQ0mMsgVghcM
# m5iU5QAbEw==
