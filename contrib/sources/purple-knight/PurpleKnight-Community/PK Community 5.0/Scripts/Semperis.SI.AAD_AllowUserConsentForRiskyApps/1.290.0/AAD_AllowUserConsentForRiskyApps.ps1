...
$Global:self = @{
    ID = 209
    UUID = '356e37ca-c8eb-4c3a-bdfc-1b6b6f142338'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000209'
    Name = 'User consent is allowed for risky applications'
    ScriptName = 'AAD_AllowUserConsentForRiskyApps'
    Description = '<p>This indicator checks for an Entra ID authorization policy that allows users to grant consent for risky applications.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Risk-based step-up consent is a configuration setting in Entra ID that prevents users from consenting to delegated application permissions from applications that Entra ID may consider risky. When disabled, organizations are at a greater risk of users falling victim to OAuth consent phishing, also known as Illicit Consent Grants. Organizations should develop and implement a business process around the lifecycle of app registrations in their Entra tenant. Ensure that the added read permissions align with organizational policy. Organizations may want to consider using admin consent workflows as an additional defense against Illicit Consent Grants.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/enterprise-apps/configure-risk-based-step-up-consent" target="_blank">Configure risk-based step-up consent - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/graph/api/authorizationpolicy-update?view=graph-rest-beta&amp;tabs=http" target="_blank">Update authorizationpolicy - Microsoft Graph beta | Microsoft Learn</a></p>'
    ResultMessage = 'User consent for risky applications is allowed'
    Remediation = '<p>Enable <b>risk-based step-up consent</b> in Entra ID.</p>
      <p>The configuration for enabling risk-based step-up consent is not exposed in the Entra admin center, and must be configured either through PowerShell or the Graph API.</p>'
    Types = @('IoE')
...
try {
    ...
    if ($authorizationPolicies.allowUserConsentForRiskyApps -eq $true) {
        $res.Score = 0
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCALIvFGf6QcIh22
# ck4CE7+LgC7PcQeaR4QNORPnsqAZoqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgjJBVjKgqRMnSTCuF+40P6edIQaNOshp9EoIw2Ow75gYwDQYJKoZI
# hvcNAQEBBQAEggIAdEnyzWtdPczgZ8fAOq83HyHC93LUnEq0mutFaB4wkXdPDT/m
# uhb78V3PkjFZk1t2GELIMCMYzhxz3+7PVrK3K+tm612E+wGMrQ5ZM5/KXSH2diG+
# BE4yuXdpvIrlsBULW9E/Yxaz13K9RdQcVuBjnQ71Zfq6N172Bb9wKvrPteUG5eEx
# CkSTMPZjJv8S/azHbRXJi45L8G/5uixOGmQOqnXp32J0bwgJ62esxcFBjj7YE+We
# LPtSBvRCtI1fclsRMIJf4sutCdlXlmfB4NeoeN0PRWskAANt35Z1lrVD6cip8q3U
# 5dPrkpjltsOM2dh6g4E5JgRjY6VrL8xXsAAEHe4Bij2wlqoL7iuiHoQLT/tgpIya
# ARpk9NbXmQSFZI1gEcDBpw3xXe/s6tFqGmV9+XtukmXDvZ15hEhogvPgcHRW2Kvu
# NGhZ8YOPxihpttR0laOJUvM/UkhwTxoHHWl4ph7czBTaqtqm4ucHP9ZQSAGfeERL
# w9+8XQ7hjYbOc6k8q1wVOvsvfEekc5C0Ymo3ibat79dDjg5hiPWWv+UKHm4CJfGQ
# 7XVZQ6lA/iFsZXtZXLdLdDG8xk4S5/fDBe9AyfdMJ3u02PQVW70MgfpSmvdnFCV4
# JqW1tnarIli2zVw9ooON7LLgxSkWNHWrlB6kEx78kFf9fI02cy7RO34ifkqhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAiI96IjejDVlC6LT7u4/15BIfukimYsMLN
# 3fNeSccK5AIUTECb0yy1jIVF7WEbt2gGfKcoVw8YDzIwMjUwNTEzMTg0MzI1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAuOL0hWi8GfwViOi/cdfqRksRKxeA0afLcacSxZN23
# oDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgKrXahajC8c5rkNvhjIvm7GVaLzpKHLkKz4br/+QkwnpQSkpP+Pan6dO
# rRR2fAbR5bHKTrOjKgEDNFAP8AePLIra+V2Wv5HgEyvtDy4YOSgX/+Q1YSUBmFAd
# Vsn+CQpPKXwob5pK2uFBr7HomrcWPUpa0rqGXyG7x2LdeXRqgITHHISRHXRwEiVp
# 6skC3ob7o1jSeZGgsWo6tdRRA3T/K0wrIleF4nwIrUNTzP/4BlvSOd0LJE6Es4P5
# HIPU/ngfC7ZL7NTsZDo3vEFGrXYZpWoi2iE4ZBhDdbIolLLeQCJzHYYse8FCfs9O
# nzY5RrZpYHMc22n8NMLhc9VbbL7nZfaYSTZecZNEDQdDTbGAGfBYPIp1CKuWNFH/
# caBqL5fHCx5co+XaHxtF7CkfWTYNojiHvVnyrznOmiAltRDtl+7afT5pRVCUvb6l
# ECpg8zfvNYMeuiC5D09SskiRe7lBpkOOYAjqfe0VC5rSkFgsr9dmgsgNWvt/LwwG
# jB6vw6MM3w==
