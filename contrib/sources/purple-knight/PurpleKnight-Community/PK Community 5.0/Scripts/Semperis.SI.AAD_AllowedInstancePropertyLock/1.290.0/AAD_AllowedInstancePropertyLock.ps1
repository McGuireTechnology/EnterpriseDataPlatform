...
$Global:self = @{
    ID = 223
    UUID = '445677dc-509f-4cc2-809d-a2aa850e1d7f'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000223'
    Name = 'Application instance property lock disabled'
    ScriptName = 'AAD_AllowedInstancePropertyLock'
    Description = '<p>This indicator checks for applications that have app instance property lock disabled.</p>
    ...
    Schedule = '1w'
    Impact = 4
    LikelihoodOfCompromise = '<p>Application instance property lock is a feature in Entra ID that restricts the ability to add or modify sensitive properties on service principals.</p>
      <p>Attackers can look for privileged service principals, and attempt to use them to move laterally, including by adding credentials onto the existing service principal, if they gain access to a user with the ability to do so. App instance property lock can prevent this.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity-platform/howto-configure-app-instance-property-locks" target="_blank">How to configure app instance property lock in your applications - Microsoft identity platform | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} applications that have a sensitive property lock disabled.'
    Remediation = '<p>Enable the application instance lock feature and all of its properties for app registrations.</p>
      <p>To enable the instance property lock of an application, Sign in to <a href="https://entra.microsoft.com/" target="_blank"> Microsoft Entra admin center</a> as at least a Cloud Application Administrator. In the <b>Applications</b> tab, click on <b>App Registrations</b> and select the application to be configured. Select <b>Authentication</b>, and then select <b>Configure</b> under the App instance property lock section. In the App instance property lock pane, enter the settings for the lock.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'Volume'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Application.Read.All')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Lateral Movement', 'Impact', 'Credential Access') },
        @{ Name = 'MITRE D3FEND'; Tags = @('Harden - Application Configuration Hardening') }
    )
    Products = @(
...
try {
    ...
    try {
        $appResponse = Invoke-GraphApiRequest @getApps
    }
    catch {
    ...
    $appVal = $appResponse.value
    foreach ($app in $appVal) {
        $lock = $app.servicePrincipalLockConfiguration
        $enabledStatus = "Disabled"
        $volume = "Partial lock"
        if($lock){
            if(!(($lock.isEnabled -and $lock.credentialsWithUsageVerify -and $lock.credentialsWithUsageSign -and $lock.tokenEncryptionKeyId ) -or ($lock.allProperties -and $lock.isEnabled))){
                if(($lock.isEnabled -eq $false) -and ($lock.allProperties) ){
                    $volume = "Full lock"
                }
                if(($lock.isEnabled -eq $false) -and ($lock.credentialsWithUsageVerify -and $lock.credentialsWithUsageSign -and $lock.tokenEncryptionKeyId) ){
                    $volume = "Full lock"
                }
                if($lock.isEnabled){
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBkowUUBYe9XVEK
# JeIBFsgNBTiP5XSO2vcA5y03q1bZEKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ