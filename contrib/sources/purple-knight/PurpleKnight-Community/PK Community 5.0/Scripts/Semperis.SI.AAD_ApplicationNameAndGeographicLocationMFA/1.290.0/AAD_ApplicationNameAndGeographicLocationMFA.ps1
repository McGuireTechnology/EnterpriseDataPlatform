...
$Global:self = @{
    ID = 206
    UUID = 'bfdae692-a6bd-40c3-ac0d-17e9e1cd43a6'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000206'
    Name = 'Application name and geographic location additional contexts are disabled on MFA'
    ScriptName = 'AAD_ApplicationNameAndGeographicLocationMFA'
    Description = '<p>This indicator checks if the Microsoft Authenticator App settings are configured to display the name of the target application and geographical location of the request when a user receives a push notification.</p>
    ...
    LikelihoodOfCompromise = '<p>Push notification bombing is a tactic where an attacker will spam a user with excessive push notifications; users who are enabled for passwordless through the Microsoft Authenticator App can receive these with just knowing their username. While number matching in Entra ID primarily prevents users from accepting malicious push notifications, users can have better insight into the requests with additional context. When additional context is enabled, a user will see the geographical location from where the request came, as well as the name of the requesting application. Users can use this to verify that the request is legitimate.</p> 
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/authentication/how-to-mfa-additional-context" target="_blank">Use additional context in Microsoft Authenticator notifications - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Application name and geographic location additional contexts on MFA are disabled.'
    Remediation = 'Enable <b>Show application name in push and passwordless notifications</b> and <b>Show geographic location in push and passwordless notifications</b> in Entra ID. 
      <br></br>The configuration for additional context settings passwords can be found in <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/AuthenticationMethodsMenuBlade/~/AdminAuthMethods" target="_blank">Authentication methods - Microsoft Entra admin center</a> (direct link).
      <br></br>To browse to the settings in the Microsoft Entra admin center, expand the <b>Protection</b> menu on the left, select <b>Authentication methods</b>, and then <b>Policies</b>. Once in Authentication methods, select <b>Microsoft Authenticator</b> on the right blade. The <b>Microsoft Authenticator settings</b> blade will open, and from the top menu select <b>Configure</b>. In the list of settings the status for <b>Show application name in push and passwordless notifications</b> and <b>Show geographic location in push and passwordless notifications</b> should both be changed to <b>Enabled</b>. The <b>Target</b> for the settings should be <b>Include</b> and set to <b>All users</b>.'
    Types = @('IoE')
...
try {
    ...
    try {
        $microsoftAuthPolicy = Invoke-GraphApiRequest @getMicrosoftAuthPolicies
    }
    catch {
    ...
    $displayLocationInformation = $false

    $displayAppInformationRequiredState = $microsoftAuthPolicy.featureSettings.displayAppInformationRequiredState
    $displayLocationInformationRequiredState = $microsoftAuthPolicy.featureSettings.displayLocationInformationRequiredState

    if ($displayLocationInformationRequiredState.state -eq "enabled" -and $displayLocationInformationRequiredState.includeTarget.targetType -eq "group" -and $displayLocationInformationRequiredState.includeTarget.id -eq "all_users" ) {
        $displayLocationInformation = $true
    }
    if ($displayAppInformationRequiredState.state -eq "enabled" -and $displayAppInformationRequiredState.includeTarget.targetType -eq "group" -and $displayAppInformationRequiredState.includeTarget.id -eq "all_users" ) {
        $displayAppInformation = $true
    }


    if ((!$displayAppInformation) -and (!$displayLocationInformation))
    {
        $res.Score = 0
        $res.ResultMessage = "Application name and geographic location additional contexts on MFA are disabled."
        $res.Remediation = $self.Remediation
        $res.Status = 'Failed'
    }
    elseif ($displayAppInformation -and (!$displayLocationInformation))
    {
        $res.Score = 0
        $res.ResultMessage = "Geographic location additional context on MFA is disabled."
        $res.Remediation = $self.Remediation
        $res.Status = 'Failed'
    }
    elseif ($displayLocationInformation -and (!$displayAppInformation)){
        $res.Score = 0
        $res.ResultMessage = "Application name additional context on MFA is disabled."
        $res.Remediation = $self.Remediation
        $res.Status = 'Failed'
    }
    else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC