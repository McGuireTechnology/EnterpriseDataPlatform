...
$Global:self = @{
    ID = 235
    UUID = '1ab76648-ae7d-4431-80a8-93437401b772'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000235'
    Name = 'Certificate-Based Authentication Persistence'
    ScriptName = 'AAD_CBAPersistence'
    Description = '<p>This indicator looks for the presence of specific Entra ID Microsoft Graph app roles and permissions that, when combined, can enable a user to establish persistence through certificate-based authentication (CBA).</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>If an attacker is able to gain access to an Account with Authentication Policy Administrator role or an application with a combination of <em>Policy.ReadWrite.AuthenticationMethod</em> and <em>Organization.ReadWrite.All</em> permissions, they can log into Entra ID as any user, even a Global Administrator.</p>'
    ResultMessage = 'Found {0} principals with at least one of the required permissions to be able to persist using CBA.'
    Remediation = '<p>To mitigate the risk associated with Certificate-based authentication (CBA) persistence, it is recommended that the organization review and evaluate the necessity of the permissions and role assignments that can be leveraged for a CBA attack.</p>
      <p>Limit the assignment of the following permissions only to trusted apps that need the ability to add a Root CA if CBA is enabled:</p>
      <ul>
        <li>Authentication Policy Administrator</li>
        <li>Application with Policy.ReadWrite.AuthenticationMethod and Organization.ReadWrite.All permissions</li>
       </ul> 
      <p>Remove the following roles and permissions from any account that does not require them for operational purposes:</p>
      <ul>
    ...
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/RoleManagement.Read.All', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/Application.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    $AAD_APP_ROLES = @{
        "d01b97e9-cbc0-49fe-810a-750afd5527a3" = @{Name="RoleManagement.ReadWrite.Directory";Type="Delegated"}
        "9e3f62cf-ca93-4989-b6ce-bf83c28f9fe8" = @{Name="RoleManagement.ReadWrite.Directory";Type="Application"}
        "84bccea3-f856-4a8a-967b-dbe0a3d53a64" = @{Name="AppRoleAssignment.ReadWrite.All";Type="Delegated"}
        "06b708a9-e830-4db3-a914-8e69da51d44f" = @{Name="AppRoleAssignment.ReadWrite.All";Type="Application"}
        "7e823077-d88e-468f-a337-e18f1f0e6c7c" = @{Name="Policy.ReadWrite.AuthenticationMethod";Type="Delegated"}
        "29c18626-4985-4dcd-85c0-193eef327366" = @{Name="Policy.ReadWrite.AuthenticationMethod";Type="Application"}
        "46ca0847-7e6b-426e-9775-ea810a948356" = @{Name="Organization.ReadWrite.All";Type="Delegated"}
        "292d869f-3427-49a8-9dab-8c70152b74e9" = @{Name="Organization.ReadWrite.All";Type="Application"}
    }

    $getAdmins = @{
        AccessToken = $graphAccess
        RoleID = $AAD_CBA_ROLES.Keys
    ...
    foreach ($admin in $AADAdmins) {
        ...
        if ($unresolvedAdmin) {
            ...
            $thisOutput = [PSCustomObject] @{
                ObjectName = $identityInfo.displayName
                ObjectId = $admin.ID
                RoleDefinitionId = $admin.Role
                Type = $admin.State
                ObjectType = $identityInfo.Type
            }
            [void]$outputObjects.Add($thisOutput)
        }
    }
    ...
    $getServicePrincipals = @{
        Uri = "https://graph.microsoft.com/v1.0/servicePrincipals?`$select=id,appDisplayName&`$filter=tags/any(t:t eq 'WindowsAzureActiveDirectoryIntegratedApp')"
        AccessToken = $graphAccess
    }
    try {
        $servicePrincipals = Invoke-GraphApiRequest @getServicePrincipals
    }
    catch {
    ...
    foreach ($servicePrincipal in $servicePrincipals.value) {
        $getServicePrincipalAppRoles = @{
            Uri = "https://graph.microsoft.com/v1.0/servicePrincipals/$($servicePrincipal.id)/appRoleAssignments"
            AccessToken = $graphAccess
        }
        try {
            $servicePrincipalAppRoles = Invoke-GraphApiRequest @getServicePrincipalAppRoles
        }
        catch {
            return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Applications' -TenantAppId $TenantAppId -AccessToken $graphAccess
        }

        if ($servicePrincipalAppRoles.value) {
            foreach ($appRole in $servicePrincipalAppRoles.value) {
                if ($AAD_APP_ROLES.ContainsKey($appRole.appRoleId)) {
                    $thisOutput = [PSCustomObject] @{
                        ObjectName = $servicePrincipal.appDisplayName
                        ObjectId = $servicePrincipal.id
                        RoleDefinitionId = $AAD_APP_ROLES[$appRole.appRoleId].Name
                        Type = $AAD_APP_ROLES[$appRole.appRoleId].Type
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC2Sy0MqMPw/0ZX
# zR2SXBdb3xmCQ3009t4q4bJmqBJh+qCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgDg3rQjhzHLmuWrzu+y2F8HijSBp1gsJo9vN8AxBS3GEwDQYJKoZI
# hvcNAQEBBQAEggIAbXf77V/r2/tDir1nw09xtrCwitm2Q5Z2NVSKj/BqaYB3Yja8
# GJBvu5Mm0gw+lYpxEuzQ2uuFj88QVXiIb1uiB57GJNLzPftYtXz3N8WFlskOmg5J
# qetoxY+BiFEgahPBs5/Yrt3z3TaHqr4oZKLo7k1t1L/ZgBwRDKgesZStkViJP4rY
# tNwEnmXsXjkIAIa0sae1oHu49PdUBmp/MII1yYJ66YQOBekTlS1PAqCs1iX9y2cV
# /GEGxt0hxvrMjNiAbLQFj7MPe+JiafbQ9LdGXuI9wzlJbx+L+hZT2jzSXSn2rVP7
# Y1MbUNDQUo8hY31Yp8Fi+QQnpjn6CMyaBItmytKctB9p4Ho4Zm0Z29coca1H5C4y
# b2pTePSxvp2twXogUV0WznwKbEyZ/D9+F/gzjH/3uWdcjCSHmX8vP57kJecORTHZ
# rbNxYI9viEBTad8QKjDRMyzrJx2rCpJJ90wZaTJoNs/ewGCUr56/eDRtEfEUKTlH
# 9pfpQMeTScmXTWeCapODfFMulbiRF4K5fQA8i1kF6Jk1FsnQbniMzU1drYgA7PRT
# FFnlZUVsPQOWS9AckUxdh/C743SXWElRgAZuUM2ehh8oM0VOK5tM+s3wm64vv0Do
# 1zCwKnfUKjoYVhN9ybmFktVaEt99Egq5Z3rNR+5rFuk9z3GymMMQ2JaO2yKhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCD4PGWnCSyp0Hx3hcC0yjFLPLz+nagKbP+2
# XSbRqE5ERgIUAfbh8ynh+lf0zgf5F+n7iD9XW9AYDzIwMjUwNTEzMTg0MzQxWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBXSRuTxQ6Lpsm0kc4ffajJrQqYsaLRdvEkkPNGiYsn
...
# CwUABIIBgBq7CpOW1a18io99zaZmjOjcs0MXSVZhoUoEsuvMGUNJJyBRG+kmKh0u
# 6m2xfnoaIisyY/7nGnMzEaQI1LnJWZnOKmRfVp6upd0JEw1x4pCifosOFld8jjV/
# n59DMFcA62aOHMOfKbVaX+/KqqmR3ngRl4jZHRc39nwk/DDQXVgBvKgEbwPZ/23h
# NQUxS02aKzN2QoNg6ONMK/QYriuMMWHsLArWjciwTKQrop5IyWqoag57Xu+q/KVr
# 0EYVKFRJS1hiWJ8YnKsLppVjpFFqv+X2aGnzIEC2wWXeob5IYa9Eb2cXMCOXuiqi
# Qkj/3rdISMcmWmlAt5f/clJwk458/svfB4q/TRv9PlJuK/Br/NK5gWRI9KiUGxHw
# LyINyjO/nWnaMGGeJKBDJJC7lDypl9pIcAI6LDOIW000ZUqLKWwJl8kbnrZ3kLja
# wJiPbKFUgbQMb2GEA/YEHTCqNqOOn20yf+OB4ZXjplYM0l6aS1pPHiZW2ldl7jqk
# wupFbYkrnw==
