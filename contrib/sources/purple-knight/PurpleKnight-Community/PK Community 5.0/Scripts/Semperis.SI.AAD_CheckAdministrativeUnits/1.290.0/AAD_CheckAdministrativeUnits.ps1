# Check if Administrative Units are configured
...
$Global:self = @{
    ID = 119
    UUID = 'be5021e0-da08-4266-9c43-0b8010838af6'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000119'
    Name = 'Administrative units are not being used'
    ScriptName = 'AAD_CheckAdministrativeUnits'
    Description = '<p>This indicator checks for the use of administrative units in the Entra tenant.</p>
    ...
    LikelihoodOfCompromise = '<p>Administrative Units are an Entra ID feature that allow for restricting administrative scope of privileged users. Organizations that leverage administrative units can have more granularity in role assignment. As an example, an organization may create different administrative units for different geographical locations, and then assign the Helpdesk Administrator role on a regional basis, instead of across the entire Entra tenant.</p>
      <p>An attacker will target privileged user accounts to abuse the privileges assigned to them. By using administrative units, an organization can limit the surface area of attack.</p>
        <h3>References</h3>
        <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/administrative-units" target="_blank">Administrative units in Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'There are ZERO Administrative Units created.'
    Remediation = '<p>Review the organizational structure and current role assignments, and consider leveraging administrative units to limit privileged user scope.</p>
      <p>Like organizational units in Active Directory, administrative unit design and implementation will be specific to the organizations structure.</p>
      <p>Administrative units can be configured in the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/AdminUnitManagementBlade" target="_blank">Microsoft Entra admin center</a> (direct link). They are located by expanding <b>Identity</b>, expanding <b>Roles &amp; admins</b> and selecting <b>Admin units</b>.</p>'
    Types = @('IoE')
    ...
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/AdministrativeUnit.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    $context = @{
        ...
        RequiredPrivileges = 'Read Administrative Units'
    }
    $permissions = Get-GraphApiRequiredPermission @context

    if ($permissions.Message -eq "Insufficient permissions") {
        $res.Status = 'Error'
        $res.ResultMessage = $permissions.Message
        $res.Remediation = $permissions.Remediation
    }
    else {
        ...
        $administrativeUnitResult = Invoke-GraphApiRequest @getAdministrativeUnits
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD1nq/32U/g7iRB
# /kHS2+Tt/jIbcXJM/BdWMVAVq8erYqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ