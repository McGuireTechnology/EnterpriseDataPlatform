# Check if Private IP Addresses (RFC 1918) are listed as trusted locations within Conditional Access Policies
...
$Global:self = @{
    ID = 170
    UUID = '0fe1c7ee-8406-4362-84dc-aa71ba3a9508'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000170'
    Name = 'Conditional Access policies contain private IP addresses'
    ScriptName = 'AAD_CheckConditionalPrivateAddress'
    Description = '<p>This indicator checks if any named locations contains private IP addresses and are assigned to enabled Conditional Access policies.</p>
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>Named locations should define public egress IP addresses or ranges, as Entra ID will see the public address of a user when evaluating Conditional Access policies.</p>
      <p>If a private IP address or range is defined in a named location, it may result in the associated Conditional Access policies not operating as intended. Depending on how the policy is defined, it may present a security posture lower than intended.</p>
      <p>The attack surface would be specific to the intention of the impacted Conditional Access policy.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/conditional-access/concept-assignment-network#user-ip-address" target="_blank">Using networks and countries/regions in Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'There are Conditional Access policies referencing named locations containing private IP Addresses. These private addresses will not be considered while access is determined.'
    Remediation = '<p>Review the impacted Conditional Access policy, and verify the policy conditions are working as expected.</p>
      <p>If the policy is working as expected, the Named location should be removed from the policy, as private IP addresses or ranges are not used in Conditional Access evaluation.</p>
      <p>If the policy is not working as expected and is designed to evaluate based on network location, the Named location should be changed to reflect the egress IP addresses or ranges.</p>
      <p><b>Organizations must ensure that all Conditional Access policy changes are documented and verify that they do not cause unintended negative impact</b></p>
      <p>Named locations can be reviewed in the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_ConditionalAccess/ConditionalAccessBlade/%7E/NamedLocations/fromNav/" target="_blank">Entra admin center</a> (direct link). To browse to them, on the left menu, expand <b>Protection</b> and then expand <b>Conditional Access</b>. In the blade on the right, select <b>Named locations</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'ConditionalPolicyName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ConditionalPolicyId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LocationName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LocationId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LocationUsage'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LocationRange'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Policy.Read.All', 'AAD.GraphAPI/AuditLog.Read.All')
    SecurityFrameworks = @(
    ...
    Products = @(
        ...
        @{ Name = 'PK'; MinVersion = '2.1'; MaxVersion = '10'; Licenses = @('Community', 'Post-Breach', 'BPIR') }
    )
...
try {
    ...
    $conditionalAccessPolicies += $conditionalAccessResult.value | Where-Object state -eq 'enabled'

    # If any Conditional Access Policies were returned, get all named locations
    if ($conditionalAccessPolicies) {
        ...
        try {
            $locationResult = Invoke-GraphApiRequest @getLocations
        }
        catch {
            return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Policy' -TenantAppId $TenantAppId -AccessToken $graphAccess
        }
        ...
        $namedLocations += $locationResult.value
    }

    $rfc1918 = '(^127\.)|(^192\.168\.)|(^10\.)|(^172\.1[6-9]\.)|(^172\.2[0-9]\.)|(^172\.3[0-1]\.)'

    foreach ($conditionalAccessPolicy in $conditionalAccessPolicies) {
        # PROCESS INCLUDE LOCATIONS
        if ($conditionalAccessPolicy.conditions.locations.includeLocations) {
            foreach ($policyLocation in ($conditionalAccessPolicy.conditions.locations.includeLocations | Where-Object { $_ -ne '00000000-0000-0000-0000-000000000000' })) {
                if ($policyLocation -eq 'AllTrusted') {
                    foreach ($trustedLocation in ($namedLocations | Where-Object {$_.isTrusted -eq $true})) {
                        foreach ($ipRange in $trustedLocation.ipRanges) {
                            if ($ipRange.'@odata.type'.split(".")[-1] -eq "ipv4CidrRange") {
                                $range = $ipRange.cidrAddress
                                if ($range -match $rfc1918) {
                                    $thisOutput = [PSCustomObject][Ordered] @{
                                        ConditionalPolicyName = $conditionalAccessPolicy.displayName
                                        ConditionalPolicyId = $conditionalAccessPolicy.id
                                        LocationName = $trustedLocation.displayName
                                        LocationId = $trustedLocation.id
                                        LocationUsage = "Include"
                                        LocationRange = $($range)
                                    }
                    ...
                    $thisLocation = $namedLocations | Where-Object {$_.id -eq $policyLocation}
                    foreach ($ipRange in $thisLocation.ipRanges) {
                        if ($ipRange.'@odata.type'.split(".")[-1] -eq "ipv4CidrRange") {
                            $range = $ipRange.cidrAddress
                            if ($range -match $rfc1918) {
                                $thisOutput = [PSCustomObject][Ordered] @{
                                    ConditionalPolicyName = $conditionalAccessPolicy.displayName
                                    ConditionalPolicyId = $conditionalAccessPolicy.id
                                    LocationName = $thisLocation.displayName
        ...
        # PROCESS EXCLUDE LOCATIONS
        if ($conditionalAccessPolicy.conditions.locations.excludeLocations) {
            foreach ($policyLocation in ($conditionalAccessPolicy.conditions.locations.excludeLocations | Where-Object { $_ -ne '00000000-0000-0000-0000-000000000000' })) {
                if ($policyLocation -eq 'AllTrusted') {
                    foreach ($trustedLocation in ($namedLocations | Where-Object {$_.isTrusted -eq $true})) {
                        foreach ($ipRange in $trustedLocation.ipRanges) {
                            if ($ipRange.'@odata.type'.split(".")[-1] -eq "ipv4CidrRange") {
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            if ($StartAttackWindow -and $EndAttackWindow) {
                $StartAttackWindow = (Get-Date $StartAttackWindow).ToUniversalTime()
                $EndAttackWindow = (Get-Date $EndAttackWindow).ToUniversalTime()
                $resultLogs = @()
                $newValues = @()
                $newValues += $outputObjects.LocationId | Select-Object -Unique
                if ($EndAttackWindow -gt (Get-Date).AddDays(-30).ToUniversalTime()) {
                    if ($StartAttackWindow -lt (Get-Date).AddDays(-30).ToUniversalTime()) {
                        $StartAttackWindow = (Get-Date).AddDays(-30).ToUniversalTime()
                    }
                    $startActivity = (get-date $StartAttackWindow -format o).split(".")[0] + "Z"
                    $endActivity = (get-date $EndAttackWindow -format o).split(".")[0] + "Z"
                    $AuditFilterAdd = "activityDateTime ge $startActivity and activityDateTime le $endActivity and activityDisplayName eq `'Add conditional access policy`'"
                    $AuditFilterUpdate = "activityDateTime ge $startActivity and activityDateTime le $endActivity and activityDisplayName eq `'Update conditional access policy`'"
                    $resultLogs += Get-AADAuditLog -AuditFilter $AuditFilterAdd -Token $graphAccess -NewValues $newValues -Regex
                    $resultLogs += Get-AADAuditLog -AuditFilter $AuditFilterUpdate -Token $graphAccess -NewValues $newValues -Regex
                }
                foreach ($outputObject in $outputObjects) {
                    $locationType = "$($outputObject.LocationUsage)Locations"
                    $logEntry = $resultLogs | Where-Object {$_.targetResources.id -eq $($outputObject.ConditionalPolicyId)} | Where-Object { (($_.targetResources.modifiedProperties.newValue | ConvertFrom-Json).conditions.locations.$locationType).contains($($outputObject.LocationId)) } | Sort-Object { $_.activityDateTime -as [datetime] } -Descending | Select-Object -First 1
                    if ($logEntry) {
                        $thisTimeStamp = (Get-Date "$($logEntry.ActivityDateTime)").ToUniversalTime()
                        $updateSearchParameters = @{
                            StartAttackWindow =$StartAttackWindow
                            EndAttackWindow = $EndAttackWindow
                            ThisOutput = $outputObject
                        }
                        Update-GraphApiAADIndicatorOutputAttackWindow @updateSearchParameters -DateToCheck $thisTimeStamp
                    }
                    else {
                        $outputObject | Add-Member -Type NoteProperty -Name "EventTimestamp" -Value ""
                        $outputObject | Add-Member -Type NoteProperty -Name "InAttackWindow" -Value "False"
                    }
                }
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBpa/DFdwqnvtoQ
# X+zIOg/Htwcah7fHBOl3pLYIpH49q6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgjITkjSXW5j6Te5UZY5bDMaHqeWN9aXmTGR9RB9W3QHUwDQYJKoZI
# hvcNAQEBBQAEggIAhcTqtSQHFZkxLjEn9G0rbP5No8zvWe285XDIQCmfp3cc4WuH
# V97LCojQFGNfGLCrgQMCiSB+Wv7R/HEI+z7WHONnbP5+XjQTfdQ8sjE7zrJXfw+6
# nstml8VxGeTTZWa26S1U+4xwrwiBTfTg0xmWpUM6LTWz3sX96ZHY+tkFUBV2Degw
# hsDDIQy5UjwL2vgNLbbWYh4LhSu1J+qT6lZ/ynYpSwY16ywwJB6vHjQSIWp3jCIi
# Ou4o5fSzS6aJNrwZcKDYyd9kDpVTHyew5FnbFLEDDbzslEUtpIMHMm/v2NhBxjuq
# 7Y9TDUhYNnjHOMLtbCayagaTbABtZMhWV+1v8zFTa88r03CPTh+oaQBOxu25AN0E
# x0mjP54Agh/fxbc9AdXOU8ZkZphKhl7Wy1hgGiTziWC14J1tju+gRMllNxUlgdRG
# PH+XIDvlsZ3u38AaXyMOfgSSx9vowYlRf8f9RCgPjPFvjjKlBtarrYMRrj9NXBK9
# MRk7p6ooZOgmASvX5U3fbLKkpJesKl31YpLUj8bKbsbMsU05pz1eS9QfiZ4lf4Qh
# ufeA+vMIZGxQZ4VjjxEyMg+yXu3dsQQpNY+JNYBUiU3j3sSvnqt0l3pGJbENhH5v
# mwUpG2aDRnmtlgNj4ZS+LIJUtOiAxLU7XV9uKVjIjphUYiPQYMeQur2PL+Ohgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBD7yBrSm0KVuYJFt13gHyduLe6iyTMiecc
# sSAUE/84UgIUYZij7F/w2clYp7KMnIB6quefoW4YDzIwMjUwNTEzMTg0MzU0WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCQxjpPulVjcmhBo9qdqSqOr3yOCDBiP0zgXi2547IC
# TTCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgBkJnKagi6mN1m+SjA8zhNrR9WnWT8B7ET3GX2FV0Xk+6xPhC1+2acnh
# PLtPChitgdEa15kVRaWC7yUcJsBpy0nVKUMywoQO3vwAgUiFsdWykapCrgIR/Obu
# QmonXk6Oox8RjKtwAOehQ3hTuT0CQRrukQ0MQNjS08IFQCEZx1enDdqyvW/aL84K
# utNBi4IrhIqaYi+WLBvNDGyElKF5CRdtKbESn+CLB1zBCT6AY7nYBJe9O1zIMkTg
# 5kiJAQcX7RQEUR1/o4SZM5ezbx1NuPsEq4qgk9+tEF9YM7/mC0Xuy1GrL5PGRuJm
# UYVoSNUKR/jXxwVBB9Lfb397o/EwFo3i2ffZBdJ51SsDo6hlJp2ZFIFklnJpQYI8
# JeAvZF2E3/L4Z1FhxPpOibefTSJ+uLOCunqSh0IbirWMdcFVxy2rCQ6L2g+pyInK
# 4PWm6Gy340Zf9TNGiohtPuJvrEINbxmj9nBosyAaMsxWbkjn8XKieDeqz6vpIOBA
# U26du3wLtw==
