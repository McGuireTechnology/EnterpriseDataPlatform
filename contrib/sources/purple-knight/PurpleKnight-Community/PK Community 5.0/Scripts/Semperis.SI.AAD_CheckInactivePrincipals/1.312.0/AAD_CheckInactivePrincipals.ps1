...
$Global:self = @{
    ID = 208
    UUID = '47bfb163-2e5d-42c8-9218-0b1e9609d0f6'
    Version = '1.312.0'
    CategoryID = 6
    ShortName = 'SI000208'
    Name = 'Users or devices inactive for at least 90 days'
    ScriptName = 'AAD_CheckInactivePrincipals'
    Description = '<p>This indicator checks for users or devices that have not signed in to Entra ID in the past 90 days.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>As part of an identity governance process and lifecycle, organizations should evaluate users and devices that have been inactive for 90 days or more, as it may be likely that the user or device is &quot;stale&quot; and no longer in use.</p>
      <p>Inactive users many times still have access to all the corporate data and systems associated with their role. An attacker may target inactive users to compromise their account, and then exfiltrate data, perform reconnaissance, move laterally, and abuse the users mailbox for actions such as business email compromise. As inactive users are not regularly monitored by the business units associated with their job function, these actions may go unnoticed by the organization.</p>
      <h3>References</h3><p><a href="https://learn.microsoft.com/en-us/entra/identity/monitoring-health/howto-manage-inactive-user-accounts" target="_blank">How to manage inactive user accounts - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/devices/manage-stale-devices" target="_blank">How to manage stale devices in Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final" target="_blank">NIST SP 800-53 Rev 5</a></p>'
    ResultMessage = 'Found {0} users or devices that have been inactive for more than 90 days'
    Remediation = '<p>Evaluate the indicated inactive users and devices for management as per your organizational policy.</p>
      <p>If the organization does not have a policy defining account management, NIST SP 800-53 Rev 5 covers account lifecycle controls under AC-2 (3).</p>
      <p>Indicated user objects can be found under the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserManagementMenuBlade/%7E/AllUsers/menuId/" target="_blank">Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Users</b>, and select <b>All users</b>. You can search for the impacted users in the right blade.</p>
      <p>Indicated user objects can be found under the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_Devices/DevicesMenuBlade/%7E/Devices/menuId/Devices" target="_blank">Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Devices</b>, and select <b>All devices</b>. You can search for the impacted users in the right blade.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'ObjectId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LastSignIn'; Type = 'DateTime'; IsCollection = $false },
        @{ Name = 'Created'; Type = 'DateTime'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/User.Read.All', 'AAD.GraphAPI/Device.Read.All')
    SecurityFrameworks = @(
...
$daysBack = Get-ParameterValue -Name 'IntervalInDays' -DefaultValue 90
$startActivity = (get-date (Get-Date).addDays(-$daysBack).ToUniversalTime() -format o).split(".")[0] + "Z"
...
try {
    ...
    $staleDate = (Get-Date).AddDays(-$daysBack).ToUniversalTime()

    $devices = [System.Collections.ArrayList]@()

    $getDevices = @{
        Uri = "https://graph.microsoft.com/v1.0/devices?`$select=createdDateTime,approximateLastSignInDateTime,displayName,id,operatingSystem&`$filter=createdDateTime le $startActivity and accountEnabled eq true"
        AccessToken = $graphAccess
    }

    try {
        $deviceResult = Invoke-GraphApiRequest @getDevices
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Devices' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $devices.AddRange($deviceResult.value)

    foreach ($device in $devices) {
        $deviceCreated = [DateTime]::Parse($device.createdDateTime)

        if ($device.approximateLastSignInDateTime) {
            $approxSignIn = [DateTime]::Parse($device.approximateLastSignInDateTime)
        }
        else {
            $approxSignIn = "Never"
        }

        if ($approxSignIn -eq "Never" -or ($approxSignIn -lt $staleDate)) {
            if ($device.operatingSystem) {
                $operatingSystem = $device.operatingSystem
            }
            else {
                $operatingSystem = "No Value"
            }

            $thisOutput = [PSCustomObject][Ordered] @{
                PrincipalName = $device.displayName
                PrincipalType = "Device"
                OperatingSystem = $operatingSystem
                ObjectId = $device.id
                LastSignIn = $approxSignIn
                Created = (Get-Date $deviceCreated).ToUniversalTime()
            }
    ...
    $getUsers = @{
        Uri = "https://graph.microsoft.com/beta/Users?`$select=userPrincipalName,createdDateTime,signInActivity&`$filter=createdDateTime le $startActivity and accountEnabled eq true"
        AccessToken = $graphAccess
    }

    try {
        $userResult = Invoke-GraphApiRequest @getUsers
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Users' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $users.AddRange($userResult.value)

    foreach ($user in $users) {
        $userUPN = $user.userPrincipalName
        $userCreated = [DateTime]::Parse($user.createdDateTime)

        if ($user.signInActivity) {
            if ($user.signInActivity.lastSignInDateTime) {
                $userActive = [DateTime]::Parse($user.signInActivity.lastSignInDateTime)
            }

            if ($user.signInActivity.lastNonInteractiveSignInDateTime) {
                $userInActive = [DateTime]::Parse($user.signInActivity.lastNonInteractiveSignInDateTime)
            }

            if ($userActive -gt $userInActive) {
                $userSignOn = $userActive
            }
            else {
                $userSignOn = $userInActive
            }
        }
        else {
            $userSignOn = "Never"
        ...
        if ($userSignOn -eq "Never" -or ($userSignOn -lt (Get-Date).AddDays(-$daysBack).ToUniversalTime())) {
            $thisOutput = [PSCustomObject][Ordered] @{
                PrincipalName = $userUPN
                PrincipalType = "User"
                OperatingSystem = "N/A"
                ObjectID = $user.id
                LastSignIn = $userSignOn
                Created = $userCreated
            }
            [void]$outputObjects.Add($thisOutput)
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA9w5ZXOhoYca/j
# mzzQ2P/OAamae8r2CPdpfp0393fT0qCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ