...
$Global:self = @{
    ID = 118
    UUID = 'cd191d30-9e68-4658-bec2-7d5c4c97c715'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000118'
    Name = 'Check if legacy authentication is allowed'
    ScriptName = 'AAD_CheckLegacyAuthentication'
    Description = '<p>This indicator checks if legacy authentication is blocked either by Conditional Access policies or Security Defaults.</p>
    ...
    Schedule = '1w'
    Impact = 4
    LikelihoodOfCompromise = '<p>Legacy authentication presents risk that an attacker may use compromised credentials to access resources, as legacy authentication does not support security mechanisms like Conditional Access or MFA.</p>
      <p>While Microsoft has now restricted the use of legacy authentication in Entra ID, organizations should ensure it is blocked with their own policies for defense-in-depth.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/conditional-access/policy-block-legacy-authentication" target="_blank">Block legacy authentication with Conditional Access - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Legacy Authentication is not being blocked.'
    Remediation = '<p>Create a Conditional Access policy blocking legacy authentication.</p>
      <p>Microsoft provides both a Conditional Access policy template, as well as step-by-step instructions for creating a Conditional Access policy to block legacy authentication.</p>
      <p>If legacy authentication is still required for limited resources that may utilize it, such as SMTP with Exchange Online, additional Conditional Access policies should be created for those resources to limit authentication to trusted IP ranges.</p>
      <p><b>Creation and deployment of Conditional Access policies can negatively impact users if not tested. Organizations need to ensure that policies are tested before broad deployment.</b>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'LegacyAuthenticationMethod'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
...
try {
    ...
    $missingPolicy = [System.Collections.ArrayList]@("exchangeActiveSync","other")

    if ($conditionalAccessPolicies) {
        $eligiblePolicies = $conditionalAccessPolicies | Where-Object {$_.grantControls.builtInControls -eq "block" -and $_.state -eq "enabled" -and $_.conditions.applications.includeApplications -eq "All"}
        foreach ($eligiblePolicy in $eligiblePolicies) {
            if ($eligiblePolicy.conditions.clientAppTypes -eq "exchangeActiveSync") {
                $missingPolicy.remove("exchangeActiveSync")
            }

            if ($eligiblePolicy.conditions.clientAppTypes -eq "other") {
                $missingPolicy.remove("other")
            }
        }
    }
    else {
    ...
    if ($missingPolicy.Count -gt 0) {
        foreach ($missingPolicyItem in $missingPolicy) {
            $thisoutput = [PSCustomObject][Ordered] @{
                LegacyAuthenticationMethod = $missingPolicyItem
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAc4c68/7gMermm
# nehuo10ndiaYem8rBi6K+QIRB2GNCqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg