# Check if MFA Trusted IPs are being used within an enabled Conditional Access Policy
...
$Global:self = @{
    ID = 136
    UUID = 'edeb01fd-375d-4046-9be4-c443d178872c'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000136'
    Name = 'Conditional Access policies that contain MFA Trusted IPs'
    ScriptName = 'AAD_CheckLegacyMFA'
    Description = '<p>This indicator checks if any enabled Conditional Access policies contain references to MFA Trusted IPs.</p>
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>MFA Trusted IPs is a legacy mechanism originally designed for specifying IP addresses to exclude from per-user MFA. Conditional Access policies also have the ability to refer to these IP address ranges as <em>Multifactor authentication trusted IPs</em> in the Locations condition. Newer security mechanisms, like Continuous Access Evaluation (CAE), help enforce Conditional Access policies that are location-based by IP address or range. Continuous Access Evaluation is not capable of using IP ranges specified in MFA Trusted IPs.</p>
      <p>An organizations using MFA Trusted IPs may see behavior of Conditional Access policies and Continuous Access Evaluation working as intended, the security posture of target users being lower than expected, and prone to a larger surface area of attack.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/conditional-access/concept-continuous-access-evaluation#supported-location-policies" target="_blank">Continuous access evaluation in Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/conditional-access/concept-assignment-network#multifactor-authentication-trusted-ips" target="_blank">Network in Conditional Access policy - Microsoft Entra ID | Microsoft Learn</a></p>
       <p><a href="https://learn.microsoft.com/en-us/entra/identity/authentication/howto-mfa-mfasettings#trusted-ips" target="_blank">Configure Microsoft Entra multifactor authentication - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'There are Conditional Access policies referencing legacy MFA Trusted IPs.'
    Remediation = '<p>IP addresses and ranges in <b>Multifactor authentication trusted IPs</b> should be creates as a Named location to be used by the impacted Conditional Access policies.</p>
      <p>To configure, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_ConditionalAccess/ConditionalAccessBlade/%7E/NamedLocations/fromNav/" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Protection</b>, expand <b>Conditional Access</b>, and select <b>Named locations</b>.</p>
      <p>To view existing trusted IPs, select the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/MultifactorAuthenticationConfig.ReactView" target="_blank">Configure multifactor authentication trusted IPs</a> (direct link) link on the upper portion of the right blade, then click on Service settings. A new named location should be created containing the same ranges.</p>
      <p><b>Creation and deployment of Conditional Access policies can negatively impact users if not tested. Organizations need to ensure that policies are tested before broad deployment.</b></p>
      <p><b>Organizations that are using AD FS with Entra Multifactor authentication, per-user MFA, or Entra MFA server may need to retain their trusted IP settings.</b></p>'
    Types = @('IoE')
...
try {
    ...
    foreach ($conditionalAccessPolicy in $conditionalAccessPolicies) {
        ...
        if ($conditionalAccessPolicy.conditions.locations.includeLocations) {
            if ($conditionalAccessPolicy.conditions.locations.includeLocations.contains('00000000-0000-0000-0000-000000000000')) {
        ...
        if ($conditionalAccessPolicy.conditions.locations.excludeLocations) {
            if ($conditionalAccessPolicy.conditions.locations.excludeLocations.contains('00000000-0000-0000-0000-000000000000')) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    ConditionalPolicyName = $conditionalAccessPolicy.displayName
                    ConditionalPolicyId = $conditionalAccessPolicy.id
                    LocationName = "MFA Trusted IPs"
        ...
        if ($scoreOutput.Score -lt 100) {
            if ($StartAttackWindow -and $EndAttackWindow) {
                ...
                if ($EndAttackWindow -gt (Get-Date).AddDays(-30).ToUniversalTime()) {
                    ...
                    $resultLogs += Get-AADAuditLog -AuditFilter $AuditFilterAdd -Token $graphAccess
                    $resultLogs += Get-AADAuditLog -AuditFilter $AuditFilterUpdate -Token $graphAccess
                }
                foreach ($outputObject in $outputObjects) {
                    $locationType = "$($outputObject.LocationUsage)Locations"
                    $logEntry = $resultLogs | Where-Object {$_.targetResources.id -eq $($outputObject.ConditionalPolicyId)} | Where-Object {($_.targetResources.modifiedProperties.newValue | ConvertFrom-Json).conditions.locations.$locationType -eq "00000000-0000-0000-0000-000000000000"} | Sort-Object { $_.activityDateTime -as [datetime] } -Descending | Select-Object -First 1
                    if ($logEntry) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDfiurSp0YC0/Tt
# aCJBGOleenrhwVzm0r4OSZ1b++zHMqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgUjZBNF0aiAYV3lKFn9XbKqlPblUqd85WYweLFIynDC8wDQYJKoZI
# hvcNAQEBBQAEggIAUZ0rqEQotg6nGJFK19TVjLhvO0GCAnV36md8UXGlylWpJB1s
# rbsLd2PT95vEebCe6/EAxOqtuNVdgM/HJudxuEMXzEUpKekhHi0/7HzsKRnidyGI
# z2Hk+ViiAIo019dZWIPJJf3HMFgZ/SOezJeY2Ao/j8sNb6ExCUB8F9dplWOan5FW
# 5vVRJCloQXPqkgZn0YOBUlCfzEQGryt2AUfjonGbaG2w4d4IfPzb72ZPFMfFfdq4
# 3/slZrDrYD1AcdqQG8Tqj//6YsuoS0csX7Z8x9dXG8dUqkvRFCUXnR+MIzKJ+5Bp
# 5Djg5mk8oaj02kWYQzPJll9UQJECmSUFHxNuquBesEnjZ0JaNK7IqL7LTYpKcQpg
# UCTU2onw7OehOEKV/3cXXARQYeCsjxbg1knh9jTR6F6kxdkTzLLLCp7Sj9/io45s
# NsACNiA2mKnd0G2YS4VQFLV9eYYpIIByDTgAYTOCEagaB1wB5m+RdmEIuhVDptUh
# 5EmbOnSqcGKt7FXZQf5CFMtDoOlpkZ4lWzhdt1RPQ7q8DMsHmiM+YieKN3AC4Xev
# d+RTshiL5KB57GlGz0hWLRsFGwA8csiZyh4jho1eCyQy/ClykvX2EguhoJ9mWu3m
# r8UIux3z2mqA0A6bQ6MnQn3rAChjr+ILoPWbyXaPBm+csx732iJ6BP2je/Khgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBKdgxDpSma47x0Qfzu5UuM8s55QBtCUfrg
# Ppp+0eUUEgIUJFKVar5TQBsJJIDlUZNHV9xkfX0YDzIwMjUwNTEzMTg0NDEwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDZSRvoMLDdiFblkoNj/vDV9nbf0dlL6UC2X/mew/lo
# HjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgFm9xTpGtVeIKSV7eINcfTLUOX0HEHomreuIJ52/LTkbwPc6mZvNTzlk
# T12w70xKReEdW+BuAuQXtLquqSLmbnl09nWpRkmguqjdnz3mxBwYtBOnBEA96Vw9
# 2meDFCZltPQDtZF/Rls4js2Gz3WM1YvdTMfbQ7f68stBQPE8Ic1fzbk2bczBIVqG
# sAoJ0TcUU95EG5dlhpBqXUnhkgcS/hI34nEhO9ZoMUD+3ZfrouLGVDHk4/tbDbka
# JSQ0pLyKbtIiR3My3NbOE2dJyDTBFXXMlAHp4B2Md0EdOnFjOwqPsOhiiQt5BOpX
# FeztAhsAFc/evU8V/j6H4eerVi8i6wzeqZyo389rpgrVnMtF/PXqfvzMgo45t3r1
# J9FfQ54is9k1hRVMOb++UVpXURrqL+zdJqeUuULjrqLFO3HwUAfWv/nlGSH3/GyZ
# eOmWlO+2mcQdZrAUUb6IFjAzaooLwvxOG2xWG9vSur4CagVvrJK9TrZgYanyiTrM
# Cc1+ZjqijA==
