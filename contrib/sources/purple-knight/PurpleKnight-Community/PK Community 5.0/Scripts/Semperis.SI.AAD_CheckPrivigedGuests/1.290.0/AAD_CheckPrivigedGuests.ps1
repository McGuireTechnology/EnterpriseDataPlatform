# Check if privileged roles have been assigned to guest accounts
...
$Global:self = @{
    ID = 128
    UUID = '5c8bd887-55ac-47d6-9835-6ed5370cda26'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000128'
    Name = 'Privileged group contains guest account'
    ScriptName = 'AAD_CheckPrivigedGuests'
    Description = '<p>This indicator checks whether any privileged roles have been assigned to guest accounts.</p>
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>Guests are a feature of Entra ID that allows for business-to-business (B2B) collaboration with users external to your organization. Commonly used for Microsoft 365 and application sharing with partners, vendors, contractors, and other users who collaborate with the organization but are not employees, a guest user will be represented in the organizations Entra tenant. Guest users are not members of the organization, have a different risk profile, and are considered less trusted due to the organization not having full control over and insight into the user object and the devices they come from. Under majority circumstances, guests should not be allowed to invite other guests into the Entra tenant.</p>
      <p>An attacker may target a guest account for compromise, and use the associated privileges to further compromise the Entra tenant and associated services. The attack surface is relative to the role assigned to the guest.</p>
      <h3>References</h3>
    ...
    ResultMessage = '{0} privileged user(s) found as members of privileged groups.'
    Remediation = '<p>Review the guest users identified and their assigned roles in the Entra tenant, and determine if they should continue to hold these unused roles. The organization should fully understand the associated risk involved with the guest holding the role assignment. A privileged access strategy should follow a governance plan for assigning least privilege, such as that documented in NIST SP 800-53 Rev 5.</p>
    ...
    OutputFields = @(
        ...
        @{ Name = 'RoleTemplateId'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/User.Read.All', 'AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/AuditLog.Read.All')
    SecurityFrameworks = @(
...
DirectoryRoles:  https://learn.microsoft.com/en-us/graph/api/directoryrole-get?view=graph-rest-1.0

REQUIRED APP ROLE ACCESS
- READ USERS:  User.Read.All
- READ DIRECTORY ROLES:  RoleManagement.Read.Directory (Least Privileges) OR Directory.Read.All
#>
...
try {
    ...
    try {
        $directoryRolesResult = Invoke-GraphApiRequest @getDirectoryRoles
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Roles','Read Users' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $directoryRoles = [System.Collections.ArrayList]@()
    $directoryRoles += $directoryRolesResult.value | Where-Object roleTemplateId -in $AAD_PRIVILEGED_ROLES.keys
    ...
    if ($directoryRoles) {
        ...
        try {
            $guestUsersResult = Invoke-GraphApiRequest @getGuestUsers
        }
        catch {
        ...
        foreach ($directoryRole in $directoryRoles) {
            $roleDisplayName = $directoryRole.displayName
            $roleTemplateID = $directoryRole.roleTemplateId

            $getRoleMembers = @{
                Uri = "https://graph.microsoft.com/v1.0/directoryRoles/roleTemplateId=$roleTemplateID/members"
                AccessToken = $graphAccess
            }

            $roleMembersResult = Invoke-GraphApiRequest @getRoleMembers

            $roleMembers = [System.Collections.ArrayList]@()
            $roleMembers.AddRange($roleMembersResult.value)

            foreach ($roleMember in $roleMembers) {
                $memberUPN = "$($roleMember.userPrincipalName)"
                $memberID = "$($roleMember.id)"
                $checkMemberResult = [System.Collections.ArrayList]@()
                $checkMemberResult += $guestUsers | Where-Object userPrincipalName -eq $memberUPN

                if ($checkMemberResult.count -gt 0) {
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            if ($StartAttackWindow -and $EndAttackWindow) {
                ...
                $resultLogs = @()
                $newValues = @()
                $newValues += $outputObjects.RoleTemplateId | Select-Object -Unique
                if ($EndAttackWindow -gt (Get-Date).AddDays(-30).ToUniversalTime()) {
                    ...
                    $AuditFilter = "activityDateTime ge $startActivity and activityDateTime le $endActivity and activityDisplayName eq `'Add member to role`'"
                ...
                foreach ($outputObject in $outputObjects) {
                    $logEntry = $resultLogs | Where-Object {$_.targetResources.modifiedProperties.newValue -eq "`"$($outputObject.RoleTemplateId)`""} | Where-Object {$_.targetResources.id -eq "$($outputObject.UserId)"} | Sort-Object { $_.activityDateTime -as [datetime] } -Descending | Select-Object -First 1
                    if ($logEntry) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAbZFMCY1D1fQ6K
# sFrxohJ4bY07olvZw6GfSBhxNUMn46CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgep89+uNyHqOhADiO5FX8eCwoaXw1oZd9UZcLPxz+2nUwDQYJKoZI
# hvcNAQEBBQAEggIAQlbX5LT32tN/4XGGDk1FCjJTO5HPQS6nZizDQANleI4rlEex
# zPwwcykSZ7DJagxqLZi1sZal85e+FOPsUM4tgUgfm0F8yue8lVWAaPwzrisQirZH
# tFRwEQ4tvpzcEZC6Lhz0HwlaLJEVWlXSRKG4S6u8j8ZFGHeHYCA5of0vPQv3Kzmf
# PJ6k8ZQaQh9ZTCq035ugi0SiPYfHCr4Oru4U+kggZa5nrFQkWE7+Xzc6RV2QCeha
# bgHbPVayp3HRpJOXqlZvIyX/mONuZOFxZ9cheioxfRAjz0w4eT4CA04UKDXLIuGn
# DuFjGYwXh6A/R6Dqf0xeHpmu5EfgXPiOOA8dZO9h2LqsVoaK5kmmlpwAHeyXwfFE
# 1RzyVnibAfaTI94dtValYM0795Bm47Lb84RP58S73I1FqJnRs9D1a1pIEljRzm+k
# CL6lDmdUdiiSLDqgjzDxWvnyFuM+xsk0zrtUZ0QH1c7E7xRfUulKdDJtyklIzMEV
# r8Zkaosnots20hfTUqFCoqwPnRdGhYClI7tZ+DAGteXEgOdkvAM93rj8g/e1PGB3
# pzs1Oye9T0U1xfDgeOyoUG1LsNZ5E4Hu4JG/xIWdfWVFNcgyvclE3/rOXw98xHZH
# 0pN42qL8hniYRoGDvsSUpP6VZomlgh0dDMNeWoGZLaRXy5CLW9iM112CxsChgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCB29pze68T7SqeSYnL3dP4l3OiDIB+ABjx+
# kuBFbT2+twIUCfCxdbstvwSLRzU8tX32iYYRuSgYDzIwMjUwNTEzMTg0NDE1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCd1tiEJJC3+e2vDKIrs145J61G/Gl/pLN/HfcxLzSw
# HDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgFXCIwjWHYIbuDOrQfFL+VGu9F5cOMWHAqvW5iSQ0EtPY50pWqpU0Wvg
# +FrpEIMIlBQTkv4hQqfVYR8Qsj6uejfLua8ykmjcwFs+k0Chscxfv2kqMbJvPJ5g
# MXBHObOhmWq3invy5+6ZjR6G/kA3meUZlCncQFDhu+0RWjE4+bcCgqiwzQzcc4ps
# BIOGrJAtwGM1CcqJh00M2qRlZYxlU/88pSZ14TYFQVPjhbzH63ZaKz+Gxg5Ssp3V
# cN5cE77JylmiNtX/+JhtBH4SBVOC8CxaLSHkpCIKMOmgD3RRmsDW3Z/OOhGirIja
# KJZ25XAaJ7KTrfC8ZWtausKyeQ4l2YpfZKiqK9GQGq72sF4gPmrhertWvD7n37s/
# FwpgA+DnLL0TM9UHk9Mh4xHYMPG711RPodf4xKbBp0uWI5sGmaIZ400nSiFy+VB7
# 8CFAaJjDGgsmmBvtsgFNuAWy0sp7SqKmUfLMHzptxA62m9M/6JwXZ200SNv7ler/
# V6MiO1DL1w==
