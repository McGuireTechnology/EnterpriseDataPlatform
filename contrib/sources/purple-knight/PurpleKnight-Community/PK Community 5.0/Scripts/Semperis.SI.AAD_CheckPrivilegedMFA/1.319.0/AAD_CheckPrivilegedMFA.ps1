# Check if Privileged Users have MFA configured
...
$Global:self = @{
    ID = 121
    UUID = '55019a5c-f4d1-4089-ac9e-5782400caced'
    Version = '1.319.0'
    CategoryID = 6
    ShortName = 'SI000121'
    Name = 'MFA not configured for privileged accounts'
    ScriptName = 'AAD_CheckPrivilegedMFA'
    Description = '<p>This indicator checks that MFA (Multi-factor Authentication) is enabled for users assigned privileged roles.</p>
    ...
    LikelihoodOfCompromise = '<p>Users that hold privileged roles in Entra ID are high-value targets for an attacker. It is critical for organizations to require MFA for all users holding privileged roles.</p>
      ...
      <p><b>Implementing changes to Conditional Access policies and enabling Security Defaults can negatively impact users if not properly tested.</b></p>
      <h3>References</h3>
      ...
      <p><a href="https://learn.microsoft.com/en-us/entra/fundamentals/security-defaults" target="_blank">Providing a default level of security in Microsoft Entra ID - Microsoft Entra | Microsoft Learn</a></p>'
    ResultMessage = '{0} privileged user(s) found without MFA configured.'
    Remediation = '<p>Users identified as impacted must be configured to require MFA at minimum for authentication in Entra ID.</p>
      ...
      <p>Microsoft provides both a Conditional Access policy template, as well as step-by-step instructions for creating a Conditional Access Policy requiring MFA for administrators. If a Conditional Access policy already exists for privileged users, the identified users can be added to that policy.</p>
      <p>Organizations that do not have licensing for Conditional Access policies should enable Security Defaults.</p>'
    Types = @('IoE')
    ...
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/Reports.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    try {
        ...
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Roles','Read Reports' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }
    ...
    if ($directoryRoles) {
        ...
        $getMFAReport = @{
            Uri = "https://graph.microsoft.com/beta/reports/credentialUserRegistrationDetails?`$filter=isRegistered eq false"
            AccessToken = $graphAccess
        }
        ...
        foreach ($directoryRole in $directoryRoles) {
            ...
            $roleMembersResult = Invoke-GraphApiRequest @getRoleMember
            ...
            foreach ($roleMember in $roleMembers) {
                ...
                $thisUserMFA = $userMFAResults | Where-Object userPrincipalName -eq $memberUPN

                if ($thisUserMFA) {
                    if ($outputObjects.UserPrincipalName -notcontains $memberUPN) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDjemMtRdPD65jk
# GYIhBZaDHB40w5hfuapNRKPj4Rk0faCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ