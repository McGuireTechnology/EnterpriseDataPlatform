# Check if risky API permissions exist
...
$Global:self = @{
    ...
    OutputFields = @(
        @{ Name = 'ServicePrincipalDisplayName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ServicePrincipalId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RiskyAppRoleName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RiskyAppRoleId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RiskWarning'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Application.Read.All', 'AAD.GraphAPI/AuditLog.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    try {
        $applicationsResult = Invoke-GraphApiRequest @getApplications
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Applications' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }
    ...
    $registeredAppIDs = @()
    if ($applications) {
        $registeredAppIDs += $applications.appId
    }
    ...
    foreach ($registeredAppID in $registeredAppIDs) {
        $getAppIDs = @{
            Uri = "https://graph.microsoft.com/v1.0/servicePrincipals/?`$filter=(appid eq '$($registeredAppID)')"
            AccessToken = $graphAccess
        }

        $appIDResult = Invoke-GraphApiRequest @getAppIDs
        $servicePrincipals += $appIDResult.value
    }
    ...
    foreach ($servicePrincipalID in $servicePrincipalIDs){
        ...
        $appIDRoleResult = Invoke-GraphApiRequest @geSPAppRoles
    ...
    forEach ($servicePrincipalAppRole in $servicePrincipalAppRoles){
        if ($servicePrincipalAppRole.appRoleId -eq "9e3f62cf-ca93-4989-b6ce-bf83c28f9fe8" -Or $servicePrincipalAppRole.appRoleId -eq "06b708a9-e830-4db3-a914-8e69da51d44f") {
            if ($servicePrincipalAppRole.appRoleId -eq "9e3f62cf-ca93-4989-b6ce-bf83c28f9fe8") {
                $thisAppRoleName = "RoleManagement.ReadWrite.Directory"
                $thisWarning = "This role can directly modify Global Admin assignments"
            }
            else {
                $thisAppRoleName = "AppRoleAssignment.ReadWrite.All"
            ...
            $thisoutput = [PSCustomObject][Ordered] @{
                ...
                ServicePrincipalId = $servicePrincipalAppRole.principalId
                RiskyAppRoleName = $thisAppRoleName
                RiskyAppRoleId = $servicePrincipalAppRole.appRoleId
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            if ($StartAttackWindow -and $EndAttackWindow) {
                ...
                $resultLogs = @()
                $newValues = @()
                $newValues += $outputObjects.RiskyAppRoleID | Select-Object -Unique
                if ($EndAttackWindow -gt (Get-Date).AddDays(-30).ToUniversalTime()) {
                    ...
                    $AuditFilter = "activityDateTime ge $startActivity and activityDateTime le $endActivity and activityDisplayName eq `'Add app role assignment to service principal`'"
                    $resultLogs += Get-AADAuditLog -AuditFilter $AuditFilter -Token $graphAccess -NewValues $newValues
                }
                foreach ($outputObject in $outputObjects) {
                    $logEntry = $resultLogs | Where-Object {$_.targetResources.modifiedProperties.newValue -eq "`"$($outputObject.ServicePrincipalId)`""} | Where-Object {$_.targetResources.modifiedProperties.newValue -eq "`"$($outputObject.RiskyAppRoleID)`""} | Sort-Object { $_.activityDateTime -as [datetime] } -Descending | Select-Object -First 1
                    if ($logEntry) {
            ...
            $res.ResultMessage = "There are API permissions on application(s) which may be risky if not intended."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC