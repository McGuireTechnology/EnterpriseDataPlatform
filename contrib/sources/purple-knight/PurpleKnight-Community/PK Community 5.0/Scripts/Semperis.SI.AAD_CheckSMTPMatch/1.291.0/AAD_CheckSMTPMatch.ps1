...
$Global:self = @{
    ID = 105
    UUID = '326a11f8-0dbd-40a1-b3f2-0b4440981f99'
    Version = '1.291.0'
    CategoryID = 6
    ShortName = 'SI000105'
    Name = 'Non-synced Entra user that is eligible for a privileged role'
    ScriptName = 'AAD_CheckSMTPMatch'
    Description = '<p>This indicator checks for Entra users that are eligible for a high-privilege role and have the proxyAddress attribute, but are not synchronized with an AD account.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>An attacker can abuse SMTP matching to synchronize a user they have compromised in Active Directory to a user in Entra ID that is eligible for a high-privilege role. This process overwrites the user password in Entra ID and can result in privilege escalation over the Entra tenant.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/smtp-matching-abuse-in-azure-ad/" target="_blank">SMTP Matching Abuse in Entra ID | Semperis blog</a></p>'
    ResultMessage = 'Found {0} user(s) eligible for high privilege roles that are not synchronized with AD.'
    Remediation = '<p>Ensure that the users indicated in the results are configured for MFA, which will prevent the ability for an attacker to authenticate as the compromised user in Entra ID, even if the attacker attempts SMTP matching abuse.</p>
      <p>Further, organizations can disable soft-matching in their Entra tenant to protect against this takeover. Disabling soft-matching should not be a replacement for enablement of MFA on privileged users in Entra ID.</p>
      <p>Full details on the attack, detection and preventative remediation can be found in the referenced Semperis article.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'ProxyAddresses'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Roles'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/User.Read.All', 'AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/Organization.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    # Check if tenant is doing synchronization
    $getOrganization = @{
        Uri = "https://graph.microsoft.com/v1.0/organization/$TenantId"
        AccessToken = $graphAccess
    }
    try {
        $organizationResponse = Invoke-GraphApiRequest @getOrganization
    }
    catch {
    ...
    if (!($organizationResponse.onPremisesSyncEnabled) -or $organizationResponse.onPremisesSyncEnabled -eq $false) {
    ...
    try {
        ...
        $returnedAdmins = $adminResult[0] | Where-Object {$_.State -eq "Eligible"}
    }
    catch {
    ...
    foreach ($returnedAdmin in $returnedAdmins) {
        $properties = @("onPremisesSyncEnabled","displayName","userPrincipalName","id","proxyAddresses")
        $identityDetails = Search-AADIdentity -IdentityId $returnedAdmin.id -Properties $properties -AccessToken $graphAccess

        if ($identityDetails) {
            if (!$identityDetails.onPremisesSyncEnabled -and $identityDetails.proxyAddresses) {
                $notEligibleOnPremCount++
                $userRoleName = ($returnedAdmins | Where-Object { $_.id -eq $returnedAdmin.id}).Role | Sort-Object | Get-Unique
                $finalRoles = $userRoleName -join "; "
                $finalProxy = $identityDetails.proxyAddresses -join "; "
                $thisOutput = [PSCustomObject][Ordered] @{
                    DisplayName = $identityDetails.displayName
                    UserPrincipalName = $identityDetails.userPrincipalName
                    ProxyAddresses = $finalProxy
                    Roles = $finalRoles
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage