# Check if Security Defaults are enabled (only applicable when there are zero Conditional Access Policies)
...
$Global:self = @{
    ID = 120
    UUID = '8fdabdf5-73e5-47c3-a70b-972f72a61b6f'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000120'
    Name = 'Security defaults not enabled'
    ScriptName = 'AAD_CheckSecurityDefaults'
    Description = '<p>This indicator checks whether security defaults are enabled when there are no Conditional Access policies configured.</p>
    ...
    Remediation = '<p>Enable security defaults or Conditional Access policies.</p>
      <p>If the organization is licensed for Entra ID P1 or higher, or associated EMS or M365 licensing, it is preferred to use Conditional Access for securing authentication.</p>
      <p>If the licensing is not available, then an organization should enable security defaults.</p>
      <p>To enable, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/TenantOverview.ReactView" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Users</b>, and select <b>Overview</b>.</p>
      <p>In the blade on the right, select <b>Properties</b>. Under the <b>Security defaults</b> section towards the bottom, select <b>Manage security defaults</b>. Set <b>Security defaults</b> to <b>Enabled</b>. Select <b>Save</b>.</p>
      <p><b>If Conditional Access is currently used by the organization, it should not enable Security defaults</b>.</p>'
    Types = @('IoE')
    ...
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Initial Access', 'Credential Access') }
    )
    Products = @(
...
<#
REFERENCE:
https://learn.microsoft.com/en-us/graph/api/resources/identitysecuritydefaultsenforcementpolicy?view=graph-rest-1.0
...
try {
    ...
    if ($tenantFail -eq "True") {
        $res.Score = 0
        $res.Remediation = "If Conditional Access policies will not be used, enable Security Defaults."
        $res.Status = 'Failed'
        $res.ResultMessage = "There are ZERO Conditional Access Policies enabled and Security Defaults are not configured."
    }
    else {
        $res.Score = 100
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBXxeWmrC6m/gK7
# j8DTcwwKEz9G+lfIWbPFhQCyQm1JN6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgOeYLgWbiSGgO6Kax2ZQrZqJDxQYwnjMHFOJ7GLRbgJ8wDQYJKoZI
# hvcNAQEBBQAEggIAX96jbj3H//hwMZQ2n9xt9I2zz/IOSOs1EBqL3W7q6BWrP7RN
# DR/yntGk9PukNDHWMz+R+lWWYGwRnhGLLIIuSalP+mPAgFcQoidO5fhguBfeIhEU
# or61CGnTylh5H6rhU0IFSncyzSCa7ImeFx/LloBREVKAC54HqYOtOh/mXKt9L4Mz
