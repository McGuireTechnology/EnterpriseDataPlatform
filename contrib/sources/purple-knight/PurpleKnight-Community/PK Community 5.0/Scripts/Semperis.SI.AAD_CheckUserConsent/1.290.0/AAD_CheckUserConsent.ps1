# Check if users can consent to ANY apps
...
$Global:self = @{
    ID = 122
    UUID = '8766ebb0-f5ef-4511-ad60-5f2c79f2dac4'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000122'
    Name = 'Unrestricted user consent allowed'
    ScriptName = 'AAD_CheckUserConsent'
    Description = '<p>This indicator checks if users are allowed to add applications from unverified publishers to Entra ID.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Publisher verification is a process from Microsoft to verify the authenticity of applications from 3rd party developers and the developers organization.</p>
      <p>Attackers use attacks such as Illicit Consent Grants, also known as OAuth consent phishing, to compromise user accounts. In these attacks, the attacker tricks a user to consent to a malicious application that is not verified by Microsoft.</p>
      <h3>References</h3><p><a href="https://learn.microsoft.com/en-us/entra/identity/enterprise-apps/configure-user-consent" target="_blank">Configure how users consent to applications - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity-platform/publisher-verification-overview" target="_blank">Publisher verification overview - Microsoft identity platform | Microsoft Learn</a></p>'
    ResultMessage = 'Users are allowed to consent to risky 3rd party apps.'
    Remediation = '<p>Do not allow users to consent to applications from unverified publishers.</p>
      <p>To disable consent for all applications, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/ConsentPoliciesMenuBlade/%7E/UserSettings" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Applications</b>, and select <b>Enterprise applications</b>.</p>
      <p>In the blade on the right, under <b>Security</b> select <b>Consent and permissions</b>. In the new blade that opens, under <b>User consent settings</b> ensure that <b>Allow user consent for apps</b> is <b>not</b> selected.</p><p><b>Organizations must understand that changes they make to the application consent process can impact users onboarding new applications, and the organizations must communicate out the changes as needed to impacted users.</b></p>'
    Types = @('IoE')
    ...
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Persistence', 'Lateral Movement') }
    )
    Products = @(
    ...
    if ($authorizationPolicy -match "microsoft-user-default-legacy") {
        ...
        if ($StartAttackWindow -and $EndAttackWindow) {
            ...
            $thisoutput = [PSCustomObject][Ordered] @{
                Risk = "Unrestricted user consent allowed"
            }
            ...
            if ($EndAttackWindow -gt (Get-Date).AddDays(-30).ToUniversalTime()) {
                ...
                $resultLogs += Get-AADAuditLog -AuditFilter $AuditFilter -Token $graphAccess -NewValues @("ManagePermissionGrantsForSelf.microsoft-user-default-legacy") -Regex
            }
            foreach ($outputObject in $outputObjects) {
                $logEntry = $resultLogs | Sort-Object { $_.activityDateTime -as [datetime] } -Descending | Select-Object -First 1
                if ($logEntry) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC+RcHNO/qKzjXc
# AYt8g9TXurvni/Dju1yMaaNHWcFU3KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ