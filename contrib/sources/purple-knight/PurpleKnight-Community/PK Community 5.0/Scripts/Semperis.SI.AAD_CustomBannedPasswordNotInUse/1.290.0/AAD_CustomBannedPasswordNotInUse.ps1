...
$Global:self = @{
    ID = 26
    UUID = '520293f5-38ff-4ee4-ba3e-3f374ebb76d7'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000026'
    Name = 'Custom banned password protection not in use'
    ScriptName = 'AAD_CustomBannedPasswordNotInUse'
    Description = '<p>This indicator checks if custom banned password protection is enabled in Entra ID.</p>
    ...
    Schedule = '1w'
    Impact = 2
    LikelihoodOfCompromise = '<p>Custom banned password protection is a feature of Entra password protection that allows organizations to specify custom banned words, helping to prevent users from creating weak passwords that may contain words relative to the organization.</p>
      <p>Attackers often use password guessing attacks to take control over user accounts. Targeted password attacks on organizations may include passwords containing words relative to the organization, such as the company name, products or services offered, locality references, and others.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/authentication/tutorial-configure-custom-password-protection" target="_blank">Configure custom Microsoft Entra password protection lists - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/authentication/concept-password-ban-bad" target="_blank">Password protection in Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Custom banned passwords not in use.'
    Remediation = '<p>Enable <b>Enforce custom list</b> in Entra ID and populate the <b>Custom banned password list</b>.</p>
      <p>To enable, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/AuthenticationMethodsMenuBlade/%7E/PasswordProtection/fromNav/" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Protection</b> and select <b>Authentication methods</b>.</p>
      <p>In the blade on the right, select <b>Password protection</b>.</p>
      <p>Set <b>Enforce custom list</b> to <b>Enabled</b> if it is not already enabled.</p>
      <p>Enter custom banned words.</p>
      <p>Organizations should also routinely verify the list and make additions or changes as needed.</p>'
    Types = @('IoE')
    ...
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Directory.Read.All')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Credential Access', 'Initial Access') },
...
<#
REFERENCE:
https://learn.microsoft.com/en-us/graph/api/usersettings-get?view=graph-rest-beta
...
try{
    ...
    $checkPremium  = $license.assignedplans | Where-Object { ($_.servicePlanId -eq 'eec0eb4f-6444-4f95-aba0-50c24d67f998') -or ($_.servicePlanId -eq '41781fb2-bc02-4b7c-bd55-b576c07bb09d')}
    $isPremium = $checkPremium.Count
    if($isPremium -eq 0)
    {
        $res.Status = 'NotRelevant'
        $res.ResultMessage = 'This indicator is relevant only when a P1 or P2 license is in place'
        return $res
    }
    ...
    try {
        $settingsResponse = Invoke-GraphApiRequest @getSettings
    }
    catch {
    ...
    $settings.AddRange($settingsResponse.value)

    $flag = $false
    foreach($setting in $settings)
    {
        if($setting.displayName -eq "Password Rule Settings")
        {
            $settingValues = $settings.values
            $checkEnable = $settingValues | Where-Object  { $_.name -eq 'EnableBannedPasswordCheck'}
            $checkList = $settingValues | Where-Object  { $_.name -eq 'BannedPasswordList'}
            if($checkEnable.value -eq $true -and $checkList.value -ne "")
            {
                $flag = $true
            }
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC