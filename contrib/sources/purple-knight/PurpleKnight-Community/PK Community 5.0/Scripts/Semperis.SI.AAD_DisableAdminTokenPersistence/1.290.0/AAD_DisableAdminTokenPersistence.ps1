...
$Global:self = @{
    ID = 140
    UUID = '9a0da97a-9ae9-40f9-b7f3-dac25928cd3c'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000140'
    Name = 'Conditional Access Policy that disable admin token persistence'
    ScriptName = 'AAD_DisableAdminTokenPersistence'
    Description = '<p>This indicator looks for Conditional Access policies that disable token persistence for users with admin roles and have a sign-in frequency that is less than or equal to 9 hours.</p>
    ...
    LikelihoodOfCompromise = '<p>Sign-in frequency (SIF) is a Conditional Access policy setting that will invalidate the refresh tokens for a user after a defined period of time. While sign-in frequency can have negative impact on personas of enterprise users, it serves as a defense-in-depth tool for protecting privileged users.</p>
      <p>Privileged users are high-value targets for attackers, and one modern attack mechanism is token theft and token replay, allowing an attacker to impersonate the user if their tokens are stolen from their device.</p>
      <h3>References</h3>
        <p><a href="https://learn.microsoft.com/en-us/entra/identity/conditional-access/howto-conditional-access-session-lifetime#policy-1-sign-in-frequency-control" target="_blank">Configure authentication session management - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'No Conditional Access policies are configured to disable admin token persistence with a sign-in frequency set to 9 or less hours.'
    Remediation = '<p>Deploy a Conditional Access policy that enforces sign-in frequency for privileged users.</p>
      <p>To create a Conditional Access policy, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_ConditionalAccess/ConditionalAccessBlade/%7E/Policies/fromNav/" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Protection</b> and select <b>Conditional Access</b>.</p>
      <p>In the blade on the right, select <b>Policies</b>. Select <b>New Policy</b>. A new policy should be created that is scoped to privileged roles with a time interval of 9 hours or lower:</p>
      ...
      <p><b>Conditional Access policies can negatively impact user authentication, and organizations must verify their policies before deployment</b>.</p>'
    Types = @('IoE')
...
try {
    ...
    if (!$securityDefaultsEnabled) {
        ...
        $adminSessionPolicyExists = $false
        $includedRoles = @()
        foreach ($conditionalAccessPolicy in $conditionalAccessPolicies) {
            if ($conditionalAccessPolicy.sessionControls.signInFrequency.isEnabled -eq $true -and $conditionalAccessPolicy.sessionControls.signInFrequency.value -le 9 -and $conditionalAccessPolicy.sessionControls.signInFrequency.type -eq "hours" -and $conditionalAccessPolicy.sessionControls.persistentBrowser.isEnabled -eq $true -and $conditionalAccessPolicy.sessionControls.persistentBrowser.mode -eq "never") {
                if ($conditionalAccessPolicy.state -eq "enabled" -and $conditionalAccessPolicy.conditions.clientAppTypes -eq "all" -and $conditionalAccessPolicy.conditions.applications.includeApplications -eq "All") {
                    $notAllAdminRolesIncluded = $false
                    ...
                    if ($conditionalAccessPolicy.conditions.users.includeUsers -eq "All") {
                        $adminSessionPolicyExists = $true
                        break
                    }
                    else {
                        foreach ($role in $AAD_PRIVILEGED_ROLES.Keys) {
                            if (-not ($role -in $includedRoles)) {
                                $notAllAdminRolesIncluded = $true
                                break
                            }
                        }
                    }
                    if ($notAllAdminRolesIncluded -ne $true) {$adminSessionPolicyExists = $true}
                }
            }
        }
    }

    if ($adminSessionPolicyExists -eq $false) {
        $res.Score = 0
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC