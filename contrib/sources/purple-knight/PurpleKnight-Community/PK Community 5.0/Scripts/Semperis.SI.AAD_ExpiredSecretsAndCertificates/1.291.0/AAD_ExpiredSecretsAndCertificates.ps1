...
$Global:self = @{
    ID = 315
    UUID = '804a13c6-46bf-4f00-96c7-dc3a1ca149c8'
    Version = '1.291.0'
    CategoryID = 6
    ShortName = 'SI000315'
    Name = 'Application expired secrets and certificates'
    ScriptName = 'AAD_ExpiredSecretsAndCertificates'
    Description = '<p>This indicator checks for certificates or secrets that have reached their expiration date. It does not indicate a direct risk or likelihood of compromise.</p>
    ...
    Schedule = '1w'
    Impact = 2
    LikelihoodOfCompromise = 'None'
    ResultMessage = 'Found {0} expired secrets and certificates in Azure Active Directory.'
    Remediation = '<p>Remove the expired credentials from the indicated applications in Entra ID. Expired credentials are not useable in Entra ID.</p>
      <p>To remove, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_RegisteredApps/ApplicationsListBlade/quickStartType%7E/null/sourceType/Microsoft_AAD_IAM" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Applications</b>, and select <b>App registrations</b>.</p>
      <p>In the blade on the right, search for the indicated app registrations. Note that you might need to select <b>All applications</b> to search.</p>
      <p>Select the impacted app registration name, which will open a new blade. In this blade, under <b>Manage</b> select <b>Certificates &amp; secrets</b>.</p>
      <p>You will need to select <b>Certificates</b> or <b>Client secrets</b> depending on the type of expired credential. In the list you''ll see the expired credential marked with a red warning indicator. Verify that the credential is the correct expired one to delete, and select the trash can on the right.</p>
      <p><b>Deleting actively used, non-expired credentials will have negative impact on the applications using them.</b></p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'SecretName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ExpirationDate'; Type = 'DateTime'; IsCollection = $false },
        @{ Name = 'StartDate'; Type = 'DateTime'; IsCollection = $false },
...
try {
    ...
    try {
        $response = Invoke-GraphApiRequest @getApplications
    }
    catch {
    ...
    foreach ($application in $applications) {
        ...
        if ($application.passwordCredentials) {
            foreach ($secret in $application.passwordCredentials) {
                $expireDate = [DateTime]::Parse($secret.endDateTime).ToUniversalTime()
                $startDate = [DateTime]::Parse($secret.startDateTime).ToUniversalTime()
        ...
        if ($application.keyCredentials) {
            foreach ($certificate in $application.keyCredentials) {
                $expireDate = [DateTime]::Parse($certificate.endDateTime).ToUniversalTime()
                $startDate = [DateTime]::Parse($certificate.startDateTime).ToUniversalTime()
                $secretDisplayName = $certificate.displayName
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBjiKQPC4COd+2j
# yajpSUuI8o49C7LB6psXCFPceIPXgqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ