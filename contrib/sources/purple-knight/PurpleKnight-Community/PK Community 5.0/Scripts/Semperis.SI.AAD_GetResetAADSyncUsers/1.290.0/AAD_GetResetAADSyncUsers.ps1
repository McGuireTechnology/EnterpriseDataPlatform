# Check if stale AAD Connect Sync accounts have had reset password
...
$Global:self = @{
    ID = 135
    UUID = '4f964cc3-7ce9-4b44-b6d0-799622027adc'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000135'
    Name = 'Entra Connect sync account password reset'
    ScriptName = 'AAD_GetResetAADSyncUsers'
    Description = '<p>This indicator checks if there have been any password resets against Entra Connect sync <em>Sync_</em> accounts not logged on in the past three days, which are considered stale.</p>
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Entra Connect uses a special user account in Entra ID as a service account for synchronization of users from Active Directory. These accounts hold a highly-privileged role in Entra ID, which can be used by an attacker for lateral movement, privilege escalation, and other forms of compromise is Entra ID.</p>
      <p>Entra Connect accounts should have continual authentication traffic, as sync cycles occur every 30 minutes. An account that has not been logged into over three days could be considered stale. Unused accounts have a higher chance of going unnoticed if compromised.</p>'
    ResultMessage = 'There have been {0} password change(s) on Azure AD Connect synchronization accounts which have not logged on within the past 3 days.'
    Remediation = '<p>Review the accounts listed in the results of this indicator.</p>
      <p>If the Entra Connect server was offline for known reasons and the password was recently changed by the organization running a password rotation on the account, then this result might be expected.</p>
      <p>If the account listed is associated with a no longer used Entra Connect server, it should be deleted from Entra ID.</p>
      <p>If this account is believed to be suspicious, the Audit logs for the account activity can be viewed in Entra ID.</p>
      <p>To view the audit logs, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserManagementMenuBlade/%7E/AllUsers/menuId/" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Users</b>, and select <b>All users</b>.</p>
      <p>In the blade on the right, search for the user indicated in the results. Select the user account.</p>
      <p>In the user blade that opens, select <b>Audit logs</b>. The audit log details of changes made will be shown.</p>
      <p><b>If the account is believed to be compromised, organizations must perform a thorough audit analysis of the user account</b></p>
      <p><b>Removing an active Entra Connect synchronization user account will break synchronization</b></p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'PasswordResetBy'; Type = 'String'; IsCollection = $false },
        @{ Name = 'AADConnectServer'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LastSignIn'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LastNonInteractiveSignIn'; Type = 'String'; IsCollection = $false },
        @{ Name = 'DirectorySynchronizationAccountsMember'; Type = 'Boolean'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/AuditLog.Read.All', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/User.Read.All', 'AAD.GraphAPI/RoleManagement.Read.Directory')
    SecurityFrameworks = @(
...
$staleAccountCount = 0
$3DaysAgo = (Get-Date).AddDays(-3).ToUniversalTime()

try {
    $res = New-Result
    $startSearchWindow = (get-date).AddDays(-7).ToUniversalTime()

    # Determine if the beginning of the search needs to be modified
    if ($StartAttackWindow) {
        $StartAttackWindow = (Get-Date $StartAttackWindow).ToUniversalTime()
        if ($StartAttackWindow -lt $startSearchWindow) {
            $startSearchWindow = $StartAttackWindow
        }
    }

    $startActivity = (get-date $startSearchWindow -format o).split(".")[0] + "Z"
    ...
    $getPasswordReset = @{
        Uri = "https://graph.microsoft.com/v1.0/auditLogs/directoryAudits?&`$filter=targetResources/any(x: startswith(x/userPrincipalName, 'Sync_')) and category eq `'UserManagement`' and activityDateTime ge $startActivity and (startswith(activityDisplayName, `'Reset password`') or startswith(activityDisplayName, `'Reset user password`') or startswith(activityDisplayName, `'User Password Reset`'))"
        AccessToken = $graphAccess
    }

    try {
        $passwordResetResults = Invoke-GraphApiRequest @getPasswordReset
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Audit Logs' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $passwordResets = @()
    $passwordResets += $passwordResetResults.value

    # roleTemplateId for "Directory Synchronization Accounts" - d29b2b05-8046-44ba-8758-1e26182fcf32
    $syncMembers = @()
    $membersRequested = $false

    foreach ($passwordReset in $passwordResets) {
        $userID = $passwordReset.targetResources[0].id
        $userUPN = $passwordReset.targetResources[0].userPrincipalName

        $syncMember = $false

        $getAADUserActivity = @{
            Uri = "https://graph.microsoft.com/beta/users/$($userID)?`$select=signInActivity"
            AccessToken = $graphAccess
        }

        try {
            $AADUserActivityResult = Invoke-GraphApiRequest @getAADUserActivity
        }
        catch {
            return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Users' -TenantAppId $TenantAppId -AccessToken $graphAccess
        }

        if ($AADUserActivityResult.signInActivity.lastSignInDateTime) {
            $lastSignIn = (Get-Date $($AADUserActivityResult.signInActivity.lastSignInDateTime)).ToUniversalTime()
        }
        else {
            $lastSignIn = $null
        }

        if ($AADUserActivityResult.signInActivity.lastNonInteractiveSignInDateTime) {
            $lastNotInteractiveSignIn = (Get-Date $($AADUserActivityResult.signInActivity.lastNonInteractiveSignInDateTime)).ToUniversalTime()
        }
        else {
            $lastNotInteractiveSignIn = $null
        }

        if ($lastSignIn -lt $3DaysAgo -and $lastNotInteractiveSignIn -lt $3DaysAgo) {
            $staleAccountCount++

            # Get members of 'Directory Synchrozation Accounts' role
            if ($membersRequested -eq $false) {
                $membersRequested = $true

                $syncRoleID = 'd29b2b05-8046-44ba-8758-1e26182fcf32'

                $getsyncRole = @{
                    Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments?`$filter=roleDefinitionId eq '$($syncRoleID)'"
                    AccessToken = $graphAccess
                }

                try {
                    $syncRoleResult = Invoke-GraphApiRequest @getsyncRole
                }
                catch {
                    return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Roles' -TenantAppId $TenantAppId -AccessToken $graphAccess
                }

                if ($syncRoleResult.value) {
                    $syncMembers += $syncRoleResult.value.principalId
                }
            }

            $part = $userUPN.split("@")[0].TrimStart("Sync_")
            $AADComputer = $part.Substring(0,($part.Length-13))

            if ($syncMembers.Contains($userID)) {
                $syncMember = $true
            }

            if ($passwordReset.initiatedBy.user) {
                $initiatedBy = $passwordReset.initiatedBy.user.userPrincipalName
            }
            else {
                $initiatedBy = "AppID $($passwordReset.initiatedBy.app.appId)"
            }

            $thisOutput = new-object psobject -Property @{
                UserPrincipalName = $userUPN
                PasswordLastSet = (Get-Date $($passwordReset.ActivityDateTime)).ToUniversalTime()
                PasswordResetBy = $initiatedBy
                AADConnectServer = $AADComputer
                LastSignIn = $lastSignIn
                LastNonInteractiveSignIn = $lastNotInteractiveSignIn
                DirectorySynchronizationAccountsMember = $syncMember
            }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage