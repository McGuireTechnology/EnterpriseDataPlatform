...
$Global:self = @{
    ID = 137
    UUID = 'ca3c2e9e-40bd-4090-868b-db2446fac627'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000137'
    Name = 'Global Administrators that signed in during the last 14 days'
    ScriptName = 'AAD_GlobalAdministratorLastSignIn'
    Description = '<p>This indicator looks for Global Administrators that have signed in during the past 14 days.</p>
    ...
    Schedule = '1w'
    Impact = 1
    LikelihoodOfCompromise = '<p>Users that hold the Global Administrator role are the most privileged users in Entra ID. Organizations should work to limit the necessity of using a Global Administrator, such as using less permissive roles when possible.</p>
      <p>An attacker will find users with the Global Administrator role as high-valued, and a compromised Global Administrator can lead to several attacks against the organization, that can include data exfiltration, lateral movement into other systems in Azure, multi-cloud, and on-premise systems, and can result in complete tenant compromise where an organization becomes locked out of their Entra tenant by the attacker.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/permissions-reference#global-administrator" target="_blank">Microsoft Entra built-in roles - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} from {1} Global Administrators that signed in (interactive or non interactive) in the last 14 days.'
    Remediation = '<p>Review the Global Administrators listed in the results of this indicator.</p>
      <p><b>If an organization believes that there is highly suspicious activity or a confirmed compromise of a Global Administrator, they must treat this as a critical security event.</b></p>
      ...
      <p>In the user blade that opens, select <b>Audit logs</b>. The audit log details of changes made by the user will be shown.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'DaysFromLastSignin'; Type = 'Integer'; IsCollection = $false },
        @{ Name = 'LastNonInteractiveSignIn'; Type = 'String'; IsCollection = $false },
        @{ Name = 'DaysFromNonInteractivelastSignIn'; Type = 'Integer'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.All', 'AAD.GraphAPI/User.Read.All', 'AAD.GraphAPI/AuditLog.Read.All')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Credential Access', 'Privilege Escalation') },
        @{ Name = 'MITRE D3FEND'; Tags = @('Detect - User Behavior Analysis') }
    )
    Products = @(
...
<#
REFERENCES:
https://learn.microsoft.com/en-us/graph/api/directoryrole-get?view=graph-rest-1.0&tabs=http
https://learn.microsoft.com/en-us/graph/api/user-get?view=graph-rest-beta&tabs=http

REQUIRED APP ROLE ACCESS
- READ ROLES:  RoleManagement.Read.All
...
try {
    ...
    $getGA = @{
        Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments?`$filter=roleDefinitionId eq '62e90394-69f5-4237-9190-012177145e10'"
        AccessToken = $graphAccess
    }
    try {
        $GAResponse = Invoke-GraphApiRequest @getGA
    }
    catch {
    ...
    $GA = [System.Collections.ArrayList]@()
    $GA.AddRange($GAResponse.value)

    $gaCount = 0
    $now = (get-date).ToUniversalTime()
    foreach ($member in $GA) {
        $getSignIn = @{
            Uri = "https://graph.microsoft.com/beta/Users?`$filter=id eq '$($member.principalId)'&`$select=signInActivity"
            AccessToken = $graphAccess
        }
        try {
            $signInResponse = Invoke-GraphApiRequest @getSignIn
        }
        catch {
        ...
        $signInTemp = $signInResponse.value.signInActivity
        $userData = $signInResponse.value
        if ($signInTemp -and $userData) {
            $lastSignIn = $signInTemp.lastSignInDateTime
            $lastNonInteractive = $signInTemp.lastNonInteractiveSignInDateTime

            if ($lastSignIn) {
                $newActive = (New-TimeSpan -Start $lastSignIn -End $now).Days
            }

            if ($lastNonInteractive) {
                $newNonActive = (New-TimeSpan -Start $lastNonInteractive -End $now).Days
            }

            $gaCount += 1

            if (($lastSignIn -and $newActive -le 14) -or ($lastNonInteractive -and $newNonActive -le 14)) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    UserPrincipalName = $userData.userPrincipalName
                    LastSignIn = $lastSignIn
                    DaysFromLastSignIn = $newActive
                    LastNonInteractiveSignIn = $lastNonInteractive
                    DaysFromNonInteractiveLastSignIn = $newNonActive
                }
                Update-GraphApiAADIndicatorOutputAttackWindow -ThisOutput $thisOutput -DateToCheck $lastSignIn `
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = $self.ResultMessage -f $outputObjects.Count,$gaCount
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDvZ3KK3VrmMuRq
# i9ZF03Mw2CwelaXgXR5vqzlSH+Zu56CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgqbFhDCSBP8KzPICZPOHjGJkQiTwp+adetSk6Of2+dwEwDQYJKoZI
# hvcNAQEBBQAEggIAd/30UPLR9WoKsuewV8cdcLyq9L8xUbaQR+a5A+fKneh9KvGK
# q8SuNmnFiDfHSXjaxS8xwB6Qvz3l4BjujdDJp3w/SyqhWx8Z13ILbR70FmvacPpZ
# 0OwYJ0BAdH3HISId520ukRCbbcJJB01DtB9NbkfSG/nYznt/7RLJRqipWI0iHNKk
# YQaS7Vfh+5jl8XS4D7ZHjR7Kp5Q77zJnrdfmDhgVZmsdFwLmZbwmXOmNc4sHg5zI
# F1faYoRAE9Fw6AQ1ItZUU+mexMwhTAZTAu3ZTYPPJubRLSUgvhFluJ7HntcEuE34
# BtEE5affb/w973rGeBa3i4PivAcu1K6UexC6HQLRDxSSorpzPcuNfUYHYa6xOJ1u
# Fc8FeoE7sdeJNOMiQljfUf4zxv4mKB39U9/Q7O+FPvUdixjtvxduoB3K6EK9ScH6
# 0gQE6altULAczMS8pZutcnqzkOD9FEXqdeLG6ZjUX/77H82C626FhZ2mlBPx44mu
# bNKdiFIo6KAcElExILVg3WUNSscpGFXv0VwUWH137cfzghmovLGcreAEqBvaUQg5
# pv1v5NF/eJNOgqcujqsMB02gfC376tRR651O87LtVWLbR1EGRdi4A/0+OIcc91fM
# iWAfTMC8Jpu4YCGY57Yv+d81gA1bsA1Fo04jflcrYcZuv8SWTMBIgm8rLB2hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCByIA7tURYy0vWKpvo3Ndqh8KkA7z4pKK0P
# Ow0QOKC9GQIUIv2DFoZrNHv+mEWuexr9D5cQ7G4YDzIwMjUwNTEzMTg0NTA1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCU4dzFKvnvkurEMMHxZPu4ApkZY0ybw5YFs8MsYx4D
# 1zCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgB+Q/1T3q0Hhkdt5WOHU883fORIB3273An2gpAYEfJUwxIU1k6BJoyRj
# KHiCzKlFvEQnGB8l45uOuzuib21YxM3yGJIyS/sgbqAiKeV54yR0CZIAEq9GZw5a
# 1qK/yK0DMAytw3hvxzgE4GiVKJeDnBOxxdUv7IjK6QwjFuSMSncdKgS0C1GAmUwF
# U/D/VRV/vUg+Z41CFspX54UssypAzFMIuuLpSd7U003aKVv+Kym7GgRmNuSizj+Q
# 1M52+f16UZzP2ZXRzRLeWqDhueQNVfsavhyGlXSZvHpJ+KDKGUlR+zxp4E5HctoF
# v2aIbzsvLSxQGvdAMYw0W1Oq7WCEd5BlHayNEwRJczczrlucfLj1f60QNvEydPzR
# tUTncmE1duMpmNAfUNYLYR+t8ASwxEBEG5dTmzih8aVf8pAOFKmDrfdWEtt1T5Ha
# ANYF9UJ7gVotsrAh7ABbTxxG0BUkNGV9+OaoaZYmSRNfFVDf8VPu072BsiUONtjg
# AM7I203KKg==
