...
$Global:self = @{
    ID = 231
    UUID = 'a512f590-9e40-4c80-b448-ccb7086da504'
    Version = '1.291.0'
    CategoryID = 6
    ShortName = 'SI000231'
    Name = 'Entra tenant is susceptible to Hidden Consent Grant attack'
    ScriptName = 'AAD_HiddenConsentGrant'
    Description = '<p>This indicator checks app permissions and settings to determine if the Entra tenant is susceptible to Hidden Consent Grant attacks.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>A Hidden Consent Grant attack is a type of phishing attack where a malicious actor who controls an application with Directory.ReadWrite.All permissions gains access to an application and exploits granted permissions to escalate his privileges.</p>
      <p>The Entra tenant contains applications with different permission requirements. That is, some applications require more permissive permissions, but other applications may have misconfigured permissions or are using default permissions that may not be required. If a malicious actor gets control of an application that allows them to create an app with a redirect to a URL pointing to a controlled endpoint (a pre-requirement of the attack), they may escalate their privileges within the Entra tenant.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/app-consent-attack-hidden-consent-grant/" target="_blank">A New App Consent Attack: Hidden Consent Grant | Semperis blog</p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/delegate-app-roles#restrict-who-can-create-applications" target="_blank">Delegate application management administrator permissions - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/fundamentals/users-default-permissions#restrict-member-users-default-permissions" target="_blank">Default user permissions - Microsoft Entra | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} scenarios of IOE that may lead to hidden consent grant attack.'
    Remediation = '<p>Review apps with AppRoleAssignment.ReadWrite.All, DelegatedPermissionGrant.ReadWrite.All or Directory.ReadWrite.All application permissions and remove any unnecessary app privileges. Monitor and revoke any unauthorized OAuth consent grants.</p>
      <p>In addition, consider disabling the Users can register applications setting in the Entra tenant, which is enabled by default.</p>
      <p>To disable this settings:</p>
      <p>Open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserManagementMenuBlade/%7E/UserSettings/menuId/UserSettings" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Users</b>, and select <b>User settings</b>.</p>
      <p>In the blade on the right, toggle <b>Users can register applications</b> to <b>no</b>.</p>
      <p>Select <b>Save</b> at the bottom of the blade.</p>
      <p>Organizations should understand how this change impacts their users and communicate the change out according to defined business processes.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'DirectoryReadWriteAllApps'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ReplyURLConfiguredApps'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UsersCanRegisterApps'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/Policy.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    $directoryApps = [System.Collections.ArrayList]@()
    $usersCanRegisterApps = $false
    $authUrls = $false
    $directoryRWAll = '19dbc75e-c2e2-444c-a770-ec69d8559fc7'
    ...
    try {
        $applicationResult = Invoke-GraphApiRequest @getAppIDs
    }
    catch {
    ...
    $applicationsList = $applicationResult.value
    foreach ($application in $applicationsList) {
        if ($application.preferredSingleSignOnMode -ne 'saml') {
            $app = $application.appId
            $getApplicationsAssignments = @{
                Uri = "https://graph.microsoft.com/v1.0/servicePrincipals(appId='$($app)')/appRoleAssignments"
                AccessToken = $graphAccess
            }
            try {
                $applicationAssignmentsResult = Invoke-GraphApiRequest @getApplicationsAssignments
            }
            catch{
                return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
            }
            $valueAssignment = $applicationAssignmentsResult.value
            if ($valueAssignment -and ($directoryRWAll -in $valueAssignment.appRoleId)) {
                $directoryApps += $application.displayName
            }
            if ($application.replyUrls) {
                $authUrls = $true
            }
        }
    }

    if ($authUrls -or $usersCanRegisterApps) {
        foreach ($directoryApp in $directoryApps) {
            $thisOutput = [pscustomobject][ordered] @{
                DirectoryReadWriteAllApps = $directoryApp
                ReplyURLConfiguredApps = $authUrls
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDzNiBGTT+jk2sE
# G5iYrRSsQInT48wB9rUyNVnoYwGUvqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgyHsbrYt8+kuz1kVOqQ/xBiodFMgVnKthtGmmmaqXHTswDQYJKoZI
# hvcNAQEBBQAEggIAMr9Q0Uz/5mCMk1HYl5m6CtzUc0kC+T46JO9SOeF8QUOK+XAx
# c+kwzy7VZplyWe5QdOVGTuLToQeKQ5YVWKEHKUquRZZlfK+85XwOyO1FXNLpSPbS
# duynulQUTRzKofoXur8xcd5FqdZmYDMioGOzpAz134YexNkdvRNQVt+xBoM2vMBp
# Ngq3QuAq69T9gKFFkdEegstrmSvltsqPJZHhM7ltIY8O9yWTCAN/0MGlXwe9nfgW
# YlXyh7Yjth5cqhKLCKRI+e1oI5Vb+t3Pe/A6NXIZ0o1bQBLmjDTjkeEmy6acPwyX
# 5Zrt7O41Evp/6KJqEZ81ugGgGYYZTakE6MywCeXNtefGYz40Or4FB3OAdS5aOEGz
# 9vMUmTrb24R+rrhY0kn88CXSLrp1yQwJ4sDeEYWGonCZvI/r/JtUxUuM8rF4VMKD
# G6Gf3CzwWIsx/k1Ek6EhQ+DPLOkbTEXxLErbLcJxlzF5neIfYPNGfi1oUGlUG1Hq
# CmtIbpfXSNIvyQ1zBhgAe+BuvOZWOV6mPiwGq/7EHI9j0B/aES6TpPNbA+m7QbRw
# eE5cE4cOKayIQQ+PDJsrvBpYqg12667wytDlFfTXjCIUt9m3Ccz9slLQtIdBlnxa
# uBM0ZRdtx5qfx1OYfZblVp6Am5kV5fnNnwbRllm6N6IVPLsWLjmNC3UBzuqhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCd/hxrePoE3IqSNGFkRF4XjxivRffOuCuL
# fMRL0oPxXgIUZ1I9VgehxF5VMR4PMsA+6frGajEYDzIwMjUwNTE1MTIzNjU2WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCzwT567nV5XDJM+7HfFHkBKjOPSBkaGq62fbHAnG6+
...
# CwUABIIBgLyzfwCzZhTFIYsuF55xi8GAj8Md2I21ZYMV/dCM47xUmsDljiwYTVLV
# Yq4TeNGe32TBaTZBGzEEsqc0/xQWo23ibeDxnnzj3ZRe4MENx88zMItmAKNhZjzW
# E2/sJUCFng7MCCbyveWddr7kYiQ+XDgmb+1hgtZmQiGNiPnDQX4Ucm/1CVGUgYTw
# V2+mYj9TOX7oPog6yesGEt72rUQwRdrQApTC/em6MNJTrGmSOK9F70ESvcXwvG/a
# uGlXSe90wprSxwyQ6zEHRMwDYjutWNARQbcS5clyEdhUng9jDo6zlFEunUIKgP6+
# FTYoWfCOC2cP4BizHnHOgBGY4yImp921GZ8Jw3RTlvhqbYTYX8kAEph99gZLzeMI
# kcw5tJqMZ3+0ViynGQavY7bOFIMr+6chiJM7sm2+j60xRXXbonFgVhgeb3OPTBjs
# JM2gOh9+HsipZIS95xovRK5I8gz04I7M1rAt+hk9+lYspJ1VM3LPO0O9kU3Ycn/9
# d0ca4wZqMg==
