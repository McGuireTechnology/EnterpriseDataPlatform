...
$Global:self = @{
    ID = 112
    UUID = 'b7f86499-830c-445d-bca9-4d289fc8dc89'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000112'
    Name = 'Guest accounts that were inactive for more than 30 days'
    ScriptName = 'AAD_InactiveGuests'
    Description = '<p>This indicator checks for Guest accounts that have not signed in, using an interactive or non-interactive sign in, in the past 30 days.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Guests are a feature of Entra ID that allows for business-to-business (B2B) collaboration with users external to your organization. Commonly used for Microsoft 365 and application sharing with partners, vendors, contractors, and other users who collaborate with the organization but are not employees, a guest user will be represented in the organizations Entra tenant.</p>
      <p>Guest users are not members of the organization, they have a different risk profile, and are considered less trusted due to the organization not having full control over and insight into the user object and the devices they come from. Because of the nature of guests, they might become <em>stale</em>, that is, their guest account is no longer used or needed in the Entra tenant.</p>
      <p>Unused guest accounts should be cleaned up in an Entra tenant, to ensure that an attacker cannot compromise the guest and access resources in the tenant.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/users/clean-up-stale-guest-accounts" target="_blank">Monitor and clean up stale guest accounts - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} Guests that are inactive for more than 30 days'
    Remediation = '<p>Review the guest users indicated in the results, and determine if the guest objects should be deleted from Entra ID. If the guest object needs to be kept for infrequent use, the object can alternatively be disabled, and then re-enabled when the guest needs access again.</p>
      <p>If licensed for Entra Identity Governance, organizations can leverage inactive guest insights and configure multi-stage guest reviews in Identity Governance, building a robust lifecycle around guest user objects.</p>
      <p>To review guest users, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserManagementMenuBlade/%7E/UserSettings/menuId/UserSettings" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Users</b>, and select <b>All users</b>.</p>
      <p>In the blade on the right, search for the guest object identified. Select the guest user. In the user properties, select <b>Delete</b> towards the top on the right blade to delete the user.</p>
      <p>If you want to disable the user, under <b>Account status</b>, select <b>Edit</b>, and in the new blade that opens uncheck <b>Account enabled</b>. Select <b>Save</b> at the bottom.</p>
      <p><b>Deleting active guest users objects in Entra ID will negatively impact the users ability to access resources</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'UserPrincipalName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Mail'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LastSignIn'; Type = 'String'; IsCollection = $false },
        @{ Name = 'LastNonInteractiveSignIn'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/User.Read.All', 'AAD.GraphAPI/AuditLog.Read.All')
    SecurityFrameworks = @(
...
- Users Read :  User.Read.All
- Audit Log Read : AuditLog.Read.All
#>
...
try {
    ...
    $getGuests = @{
        Uri = "https://graph.microsoft.com/beta/users?`$filter=(userType+eq+'Guest'+and+externalUserState+eq+'Accepted')&`$select=createdDateTime,externalUserState,id,userPrincipalName,mail,signInActivity"
        AccessToken = $graphAccess
    }

    try {
        $guestsResponse = Invoke-GraphApiRequest @getGuests
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Users','Read Audit Logs' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $guests = [System.Collections.ArrayList]@()
    $guests.AddRange($guestsResponse.value)

    $monthAgo = [DateTime]::Today.AddDays(-30)

    foreach ($guest in $guests) {
        $guestActive = $false

        try {
            $lastSignIn = [DateTime]::Parse($guest.signInActivity.lastSignInDateTime)
        }
        catch {
            $lastSignIn = "This user has never logged in"
        }

        try {
            $lastNonInteractiveSignInDateTime = [DateTime]::Parse($guest.signInActivity.lastNonInteractiveSignInDateTime)
        }
        catch {
            $lastNonInteractiveSignInDateTime = "This user has never logged in interactively"
        }

        if (($lastSignIn -gt $monthAgo -and $lastSignIn -ne "This user has never logged in") -or ($lastNonInteractiveSignInDateTime -gt $monthAgo -and $lastNonInteractiveSignInDateTime -ne "This user has never logged in interactively")) {
            $guestActive = $true
        }
        ...
        if (!($guestActive)) {
            $thisOutput = [PSCustomObject][Ordered] @{
                ...
                LastNonInteractiveSignIn = $lastNonInteractiveSignInDateTime
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBq6TUJW0GAZEI4
# TBq3r0qfKm8WzaqADwr8oeHwcHW2k6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgNDL5igH7vyHTPsbqn1g4dYXyfcfdxwhyvEN54K9LWvIwDQYJKoZI
# hvcNAQEBBQAEggIAe1LZgXmYNIRYgUG8WzxankgESBQJy2uIbevYCualbM0oA43s
# GfkLAuzKh0hksCA2+r/e7BD0GYJ2/p4ezL9WFjc+oZbc7VqZf48uUchVBOxSzSqo
# FwPLXeS1K/zg1kh+J4H+7tXq+DbpYUZ7aoPeqDdGJOx1pyqGgYZBe48U1r3XDfGb
# E9KRgAtlzdvkOg3KNfH75VeqSle4vqvog1XjE2T0gcn8wnp8aXLzh6WG+wUFrDcN
# 3ZpPqRcvzM+z8maz8hyEBLnymmq1sHWIhgQ385sNKIMX93okfh9p8TR+qljIUeht
# tMJQsipULPMYPk18WFkreP0+t1PsQDkAHmwP3loeCN74+Qgnm/6/2Ru3r76FdBZ1
# 8JO7W0xEI1Mk3D6zTl/W4TYy3OhvCuoU+xF9HHJx96670Jw7CESf/Ptq98V+vCwB
# ECrkx2X276nM0XMB3c0hGT7E5frBqWaQtG5L9xR6ZvQ5v8fI1/V8hr1DxabM7V6O
# 7T172rzhx1GBLUFYZtXYmJUrCVD1/b8WHnd/s6to2dl0hXTBm561ciVzIllHLpB/
# /OjuhBpzum8ZG4Apt4vVqrEXDrLZsbUgBcVwayrUhr4fTDjZ2+m+2Xa8h6E0CT46
# OaTvMtt6lOcqmrD8WP8HF5462SpSYPEoz2VEFSAug3lmwJxfoNVsZnVM2Gmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDCKdwI0gEIBRFO2vsl4T5H7LFcBfAyNyYu
# FP7Ks33c2gIUR/c3vtlA02FmAPrslOACxsi/oJAYDzIwMjUwNTEzMTg0NTE3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCALT8F/zL1VdrxyxAt2Ir6JOUg21WFkQkXwTccpIzoD
...
# CwUABIIBgB7HvHtp/7tzCeyhBWnWAkZflHnDLcv9OKIx7/gZYl4BYkttiCaizyCQ
# e9YEIFaqTeEqnWb6jA47ijXhQT0SHwZai/MvnSfMMKuiBSDBkB54wK8om0Z1NbVu
# QEo1qaRNCEh2CFPlwSJHzPm5Om4bVf2Bw3+5wwKYjT3DoDjcRcawl++om960HZPP
# cEgOPojfpkuyegMdRluCQp/7Jj9RsgWeURRcyeuHmvG7t1kCYzjb3rBc9foU24Nx
# y6v5UExQwLf1Eh20nbFalJd2pXYdBtp2DIj+EnGiBupHKVlysVP8qXsCuBKmMnfU
# 8Q4X9aTrrGbFXY6pEILPYhg0CySdqpy3f9YDnWlL5IZGJ7s7KsrUgJq+PpXyv3wv
# co9wZgU841sDn1jMZ2xlnLIFBAFMIFJ6o2bgGVA94fBk8ZQjvZU+RvZS495t+Xex
# jrGiASMcUmO7wY5LX2GEkkmMYTvy0wgkF4r+ih/VWYOHRMKwbxpVDrwsda0b9SCn
# WCthPxjjzg==
