...
$Global:self = @{
    ID = 222
    UUID = '0ba04531-98ec-4cd3-a7fe-5bac5e99b454'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000222'
    Name = 'Less than 2 Global Administrators exist'
    ScriptName = 'AAD_LessThan2GAs'
    Description = '<p>This indicator checks for the presence of less than 2 Global Administrators.</p>
    ...
    Schedule = '7d'
    Impact = 2
    LikelihoodOfCompromise = '<p>Users that hold the Global Administrator role are the most privileged users in Entra ID. According to Microsoft''s recommendations, organizations should have at least 2 Global Administrators.</p>
      <p>Having at least 2 Global Administrators ensures continuous access management and prevents single points of failure in administrative tasks.</p>
      <h3>References</h3>
      ...
      <p><a href="https://learn.microsoft.com/en-us/entra/fundamentals/concept-secure-remote-workers" target="_blank">Secure your organization''s identities with Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>
    ...
    ResultMessage = 'Found {0} Global Administrators in Entra ID.'
    Remediation = '<p>Create more than one Global Administrator.</p>
      ...
      <p>From this view you can add/remove users as necessary from the role. Organizations that have Entra ID P2 licensing available should use access reviews to manage the lifecycle of privileged users.</p>
      <p><b>Organizations must ensure that any addition/removal of the Global Administrator role does not impact the ability to manage their Entra tenant</b>.</p>'
    Types = @('IoE')
...
<#
REFERENCES:
https://learn.microsoft.com/en-us/entra/fundamentals/concept-secure-remote-workers
...
try {
    ...
    try {
        ...
        $globalAdmins = $adminResult[0]
        $globalAdminsUnresolved = $adminResult[1]
    }
    catch {
    ...
    if ($globalAdmins.Count -lt 2) {
        if ($globalAdmins.Count -eq 1) {
            # There is 1 GA only
            $admin = $globalAdmins[0]

            # Check if this is an unresolved admin
            $unresolvedAdmin = $globalAdminsUnresolved | Where-Object { $_.ID -eq $admin.ID }
        ...
        if ($globalAdminsUnresolved) {
            if ($globalAdminsUnresolved.Count -eq 1) {
                ...
                $res.ResultMessage += " Unable to resolve $($globalAdminsUnresolved.Count) privileged objects (details available in the indicator 'Unresolved Entra ID privileged role members')."
            }
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCWPkV94m7o5cZO
# 3a8L/V9WPbJ3j8gCQOxy0/bAurAIyqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ