...
$Global:self = @{
    ID = 216
    UUID = '4e9faa14-dd01-4722-b1ed-dd7b5a2ecf45'
    Version = '1.317.0'
    CategoryID = 6
    ShortName = 'SI000216'
    Name = 'MFA bombing attack occurred in the past day'
    ScriptName = 'AAD_MFABombingOnPrivilegedAccounts'
    Description = '<p>This indicator detects MFA bombing attempts on privileged accounts by monitoring login activity to identify unusual patterns of login activity of more than 5 failed MFA attempts within the last day. Note that this indicator will not detect activity of users using passwordless authentication.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = 'MFA bombing is a tactic where an attacker spams the user with MFA requests and the user unknowingly or unwillingly accepts the request. MFA bombing attacks can be successful in certain cases, particularly if the system''s MFA protections are not configured properly or are not robust enough to withstand a high volume of login attempts.'
    ResultMessage = 'Found {0} suspected MFA bombing attempts.'
    Remediation = '<p>To mitigate the risk of MFA bombing attacks, consider implementing the following measures:</p>
       <ol>
        <li>Reset the MFA method for the affected accounts.</li>
        <li>Set up MFA challenge policies that require users to complete additional authentication steps (e.g. answering security questions, entering a code sent to their phone) after a certain number of login failures.</li>
        <li>Increase the rate limiting on MFA attempts. This will make it harder for an attacker to brute force MFA codes.</li>
        <li>Use a hardware token for MFA. Hardware tokens are more secure than SMS or phone call-based MFA, as they cannot be easily intercepted or spoofed.</li>
        <li>Use a trusted IP range for MFA. If you have a specific range of IP addresses that you trust (e.g. your office location), you can configure MFA to only be required when logging in from outside of that range.</li>
      </ol>
        Note: It is important to carefully consider the potential impact of these changes on your users before implementing them. Some of these measures may be more effective at preventing MFA bombing attacks, but they may also be more inconvenient for your users.'
    Types = @('IoC')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'UserPrincipalName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Created'; Type = 'String'; IsCollection = $false },
        @{ Name = 'CorrelationId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'FailedMFACount'; Type = 'Integer'; IsCollection = $false },
        @{ Name = 'IPAddress'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/AuditLog.Read.All', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/RoleManagement.Read.All')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Initial Access') },
        @{ Name = 'MITRE D3FEND'; Tags = @('Harden - Multi-factor Authentication') }
    )
    Products = @(
...
try {
    ...
    # Get all privileged admins without ReturnUnresolved switch
    $getAdmins = @{
    ...
    $requestID = 1
    $batchItemCount = 1
    $itemsRemaining = $AADAdmins.Count

    $startActivity = (Get-Date).AddDays(-1).ToUniversalTime()

    $filterForMfaPrivilegedAccounts = "("
    foreach ($admin in $AADAdmins) {
        if (!($admin)) {
            $itemsRemaining--
            continue
        }

        $identityInfo = Search-AADIdentity -ID $admin.ID -AccessToken $graphAccess
        if ($identityInfo.userPrincipalName) {
            # Exclude Guest Accounts
            if ($identityInfo.userPrincipalName -match "#EXT#") {
                $itemsRemaining--

                # Check if we're done
                if ($itemsRemaining -eq 0) {
                    if ($filterForMfaPrivilegedAccounts[-1] -eq "+") {
                        $filterForMfaPrivilegedAccounts = $filterForMfaPrivilegedAccounts -replace ".{4}$", ""
                    }
                    $filterForMfaPrivilegedAccounts += ")"
                    $thisRequest = [PSCustomObject]@{
                        id = $requestID
                        method = 'GET'
                        url = "/auditLogs/signIns?&`$filter=createdDateTime ge $($startActivity.ToString('o')) and $($filterForMfaPrivilegedAccounts)"
                    }

                    $batchRequests += $thisRequest
                }

                continue
            }

            if ($batchItemCount -lt 50 -and $itemsRemaining -gt 1) {
                # loop here 50 times
                $filterForMfaPrivilegedAccounts += "userPrincipalName+eq+`'$($identityInfo.userPrincipalName)`'"
                $filterForMfaPrivilegedAccounts += "+or+"
            }
            else {
                # We hit here when we have 50 items in batch or have went through all admins
                $filterForMfaPrivilegedAccounts += "userPrincipalName+eq+`'$($identityInfo.userPrincipalName)`'"
                $filterForMfaPrivilegedAccounts += ")"
    ...
    $allBatchRequests = [PSCustomObject]@{ requests = $batchRequests }
    $batchBody = $allBatchRequests | ConvertTo-Json

    $getMicrosoftFailedMFABatch = @{
    ...
    try {
        $failedMFAs = Invoke-GraphApiRequest @getMicrosoftFailedMFABatch
    }
    catch {
    ...
    if ($failedMFAs) {
        foreach ($event in $failedMFAs) {
            $countFailedMFA = 0
            $authenticationDetails = $event.authenticationDetails
            foreach ($detailedEvent in $authenticationDetails) {
                # Reason for the failed event
                $authStep = $detailedEvent.authenticationStepResultDetail
                if ($authStep -eq "MFA denied; user declined the authentication" -or $authStep -eq "MFA denied; user did not respond to mobile app notification" -or $authStep -eq "MFA denied; Phone App Reported Fraud") {
                    $countFailedMFA++
                }
            }
            if ($countFailedMFA -gt 5) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    UserPrincipalName = $event.userPrincipalName
                    Created = $event.createdDateTime
                    CorrelationId = $event.correlationId
                    FailedMFACount = $countFailedMFA
            ...
            if ($identityInfo.userPrincipalName -match "#EXT#") {
                ...
                if ($itemsRemaining -eq 0) {
                    ...
                    $thisRequest = [PSCustomObject]@{
                        id = $requestID
                        method = 'GET'
                        url = "/auditLogs/signIns?&`$filter=status/errorCode eq 500121 and createdDateTime ge $($startActivity.ToString('o')) and $($filterForMfaPrivilegedAccounts)"
                    }
    ...
    $getMicrosoftFailedMFALongBatch = @{
        Uri = "https://graph.microsoft.com/beta/`$batch"
        AccessToken = $graphAccess
        Body = $batchBody
        Method = 'POST'
    ...
    try {
        $failedMFAsLong = Invoke-GraphApiRequest @getMicrosoftFailedMFALongBatch
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Audit Logs' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }
    ...
    $mfaFailed = @{}
    if ($failedMFAsLong) {
        foreach ($event in $failedMFAsLong) {
            if (-not $mfaFailed[$event.userPrincipalName]) {
                $mfaFailed[$event.userPrincipalName] = [PSCustomObject][Ordered]@{
                    UserPrincipalName = $event.userPrincipalName
                    Created = @($event.createdDateTime)
                    CorrelationId = @($event.correlationId)
                    FailedMFACount = 1
                ...
                $mfaFailed[$event.userPrincipalName].Created += $event.createdDateTime
                $mfaFailed[$event.userPrincipalName].CorrelationId += $event.correlationId
                $mfaFailed[$event.userPrincipalName].FailedMFACount++
                $mfaFailed[$event.userPrincipalName].IPAddress += $event.ipAddress
            }
        }
        foreach ($upn in $mfaFailed.Keys) {
            if ($mfaFailed[$upn].FailedMFACount -gt 5) {
                $outputObject = [PSCustomObject][Ordered]@{
                    UserPrincipalName = $mfaFailed[$upn].UserPrincipalName
                    Created = ($mfaFailed[$upn].Created -join ", ")
                    CorrelationId = ($mfaFailed[$upn].CorrelationId -join ", ")
                    FailedMFACount = $mfaFailed[$upn].FailedMFACount
                    IPAddress = ($mfaFailed[$upn].IPAddress -join ", ")
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB6ITIRTckGTQBv
# egd3WV9BaPEZDS9U1rcL+iaPUi5R+aCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgeQKLdtwsGi4KMMLEUQI7tIcwYyb7FnJzgGoFq3/mZKIwDQYJKoZI
# hvcNAQEBBQAEggIAQEy/ML/4swjwZhm708iJfgYNiMtQEwfBI8fn81M63zgufUoL
# 6ZTf/vii3v+bVEjgqNcQs+kkoQqa9LZEPnhH/VQJwOKxptBnwLMUMD4T+f0TAwEj
# xo4mpHn/vlpR4LbnmsQ/QzRuRrTCtk4JCnqr6FY4H6Npux/woUbojWUGrJQMuY1N
# iP8yuPnVLDHhVMZyYlWR+Cie2x1KJ/4DPeaZcViwpDsyraPyDS0dLfxcmgwyXscD
# zX3qujYL8vH4V4K2MURi+6UTX0ZNa/ipcfy7sQRGPEtIlzL46xfIMHnZ5lrsdWyL
# RMLxzfMAcNrtFw1nySPgHyhi4mmio2xueWUNFlo9DHsPe9Bx2dTOKIH0NymvW4FE
# 9N3bSeWGKqKVXhuJC3m4OU5rMMbgzzqqMwKQBGnal5iFdU3UWdLHlO7KdL7Uanft
# PKg0WLieXI+kfAp+WUE9QzVqY5XgAuWXuDeZewqUfBXA7TeNnhavdvFsPDsvMNnq
# LfGdqsMUg5f4YHZiUJRC+pAM6bXI8o9rGoreBUc59/yi4cORvtyyd8/HQffJpHxb
# 8AVSLLZzDK/RhvEtZ3vUcrBMSoO+eEqxtmZre5jLJOHIleYpBmotk9hil0BwcUOp
# KYKKiI1pESONjNcavwpLoePUisYm/kXsJBxtCD7b62dvSG2Rk2r5IT2QQr6hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAgtAwIMexBtx3pxgLjGwrF1GlReR5xQCPU
# Wof6ETMTQgIUZhuYC6n3yK9hHA9cZuqyOST1dfUYDzIwMjUwNjEwMDc1MDM2WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCC95o+jfBYtQULUSaw8RhMpWZXhseoIF6femIL8oevz
# rTCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgCNE/PsK7SJv513giEXO1z7zgvL2/9EAbF44qscUsA09f9r3CO9stVBz
# fUfBfHY/IrxlBe790l/obNGcMjRznqbUbpOiEsBR9pdRepnV97dTkiRQb/Ka/f7E
# f1AVHs14vZm3R06wuPAeZk9tbKiF12eVnB8zAQvKYyj2xaE3aKg2DZ5wtcZXBylL
# WoYgeorTXNFBp90nom4mNiPMtqq1wXgxJhL8SRe6YmJGWYsn1g7qJhMMYEK2i1Yd
# IhruObsNfNv1mQhmTn3Vodrta32pADHmhM2NtsEW4eDe5nslsplZ+Axo99hXXIZr
# G5k/FExfMMcfAGiKo73P2H2VccHjjHa0nbD9VtK8F/Yg2SzFiHu3jUDBgrbKBPS+
# ChN3YdpxKNQR5IV5FGQwxIb1RNMkSzJy9/PSzUBtxgdMY3dVS9ISb4dqQEFuVAm9
# YriiUZs8N9bOXHVne+mn8GUWoQoVDOSsYAJ4Pa3XWRksGZyKPERrmaZd7CgCJYMV
# olM+gUOHlA==
