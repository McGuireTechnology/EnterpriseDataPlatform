...
$Global:self = @{
    ID = 233
    UUID = 'fc2734ac-cb93-4673-b616-61df1e958ee4'
    Version = '1.313.0'
    CategoryID = 6
    ShortName = 'SI000233'
    Name = 'Detect MFA policy changes for groups and users'
    ScriptName = 'AAD_MFAGroupChange'
    Description = '<p>This indicator looks for changes (removal of users and groups) from MFA policies. It relates to events invoked more than 4 hours after the policy''s creation.</p>
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>MFA policies are considered as a sensitive asset in the identity security of the organization, in cases of changes in membership of users and groups in MFA policies, it may indicate on compromise of users (in case it is a malicious action).</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/monitoring-health/concept-audit-logs" target="_blank">What are Microsoft Entra audit logs?</a></p>'
    ResultMessage = 'Found {0} users/groups involved in MFA policy membership changes.'
    Remediation = '<p>Review the changes in the MFA policy in the audit log (with the Category of Policy and activity of "Update policy") and make sure they were made by a valid user.</p>'
    Types = @('IoE')
    ...
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/AuditLog.Read.All', 'AAD.GraphAPI/Policy.Read.All', 'AAD.GraphAPI/GroupMember.Read.All', 'AAD.GraphAPI/Reports.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    $nowFormatted = $now.ToString("yyyy-MM-ddTHH:mm:ss.fffZ")
    $dayAgo = $now.AddDays(-1)
    $dayAgoFormatted = $dayAgo.ToString("yyyy-MM-ddTHH:mm:ss.fffZ")

    $getConditionalChanges = @{
        Uri = "https://graph.microsoft.com/v1.0/auditLogs/directoryAudits?`$filter=(category eq 'Policy' and activityDisplayName eq 'Update%20conditional%20access%20policy' and activityDateTime ge $dayAgoFormatted and activityDateTime le $nowFormatted and result eq microsoft.graph.operationResult'Success')"
        AccessToken = $graphAccess
    }

    try {
        $conditionalChangesResult = Invoke-GraphApiRequest @getConditionalChanges
    }
    catch {
    ...
    $conditionalChangesVal = $conditionalChangesResult.value
    $users = @()
    foreach ($groupDetails in $conditionalChangesVal) {
        if ($groupDetails.targetResources) {
            $policyId = $groupDetails.targetResources.id | Select-Object -Unique
            $getCreation = @{
                Uri = "https://graph.microsoft.com/v1.0/identity/conditionalAccess/policies?`$select=displayName,state,createdDateTime&`$filter=id eq '$($policyId)'"
                AccessToken = $graphAccess
            }
            try {
                $creationResult = Invoke-GraphApiRequest @getCreation
            }
            catch {
                return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Policy' -TenantAppId $TenantAppId -AccessToken $graphAccess
            }

            if (-not $creationResult.value) {
                continue
            }

            $creationResult = $creationResult.value
            $creation = $creationResult.createdDateTime
            $statusPolicy = $creationResult.state
            $eventTime = $groupDetails.activityDateTime|Select-Object -Unique

            # In rare cases (legacy policies), createdDateTime is empty
            if ($creationResult.createdDateTime) {
                $creation = $creationResult.createdDateTime
                $timeDifference = New-TimeSpan -Start $creation -End $eventTime
            ...
            if (($hours -gt 4) -and ($statusPolicy -eq "enabled")) {
                $target = $groupDetails.targetResources
                $modify = $target.modifiedProperties

                foreach ($modifyDetails in $modify) {
                    $oldModifyDetails = $modifyDetails.oldValue | ConvertFrom-Json
                    $newModifyDetails = $modifyDetails.newValue | ConvertFrom-Json
                    if ($oldModifyDetails.grantControls -and $newModifyDetails.grantControls -and  `
                        ($oldModifyDetails.state -eq "enabled") -and ($newModifyDetails.state -eq "enabled") -and  `
                        ((Test-ConditionalMFAStrength -grantControls $oldModifyDetails.grantControls) -and (Test-ConditionalMFAStrength -grantControls $newModifyDetails.grantControls)) ) {
                        if ($oldModifyDetails.conditions -and $oldModifyDetails.conditions.users) {
                            $oldIncludeUsers = $oldModifyDetails.conditions.users.includeUsers
                            $oldIncludeGroups = $oldModifyDetails.conditions.users.includeGroups
                            $oldExcludeUsers = $oldModifyDetails.conditions.users.excludeUsers
                            $oldExcludeGroups = $oldModifyDetails.conditions.users.excludeGroups
                        }
                        if ($newModifyDetails.conditions -and $newModifyDetails.conditions.users) {
                            $newIncludeUsers = $newModifyDetails.conditions.users.includeUsers
                            $newIncludeGroups = $newModifyDetails.conditions.users.includeGroups
                            $newExcludeUsers = $newModifyDetails.conditions.users.excludeUsers
                            $newExcludeGroups = $newModifyDetails.conditions.users.excludeGroups
                        }
                        if ($oldIncludeUsers -or $newIncludeUsers ) {
                            $removedUsers = $oldIncludeUsers | Where-Object {$newIncludeUsers -NotContains $_} | Where-Object {$_ -ne "All"}
                        }

                        if ($oldIncludeGroups -or $newIncludeGroups ) {
                            $removedGroups = $oldIncludeGroups | Where-Object {$newIncludeGroups -NotContains $_}
                        }

                        if ($oldExcludeUsers -or $newExcludeUsers ) {
                            $addedExcludedUsers = $newExcludeUsers | Where-Object {$oldExcludeUsers -NotContains $_}
                        }

                        if ($oldExcludeGroups -or $newExcludeGroups ) {
                            $addedExcludedGroups = $newExcludeGroups | Where-Object {$oldExcludeGroups -NotContains $_}
                        }

                        foreach ($uid in $removedUsers) {
                            $users += $uid
                        }

                        foreach ($uid in $addedExcludedUsers) {
                            $users += $uid
                        }

                        foreach ($gid in $removedGroups) {
                            $getTransitiveMembers = @{
                                Uri = "https://graph.microsoft.com/v1.0/groups/$gid/transitiveMembers/microsoft.graph.user"
                                AccessToken = $graphAccess
                            }
                            try {
                                $transitiveMembersResult = Invoke-GraphApiRequest @getTransitiveMembers
                            }
                            catch {
                            ...
                            $transitiveMembersValue = $transitiveMembersResult.value
                            if ($transitiveMembersValue) {
                                foreach ($user in $transitiveMembersValue.id) {
                                    $users += $user
                                }
                            }
                        }

                        foreach ($gid in $addedExcludedGroups) {
                            $getTransitiveMembers = @{
                                Uri = "https://graph.microsoft.com/v1.0/groups/$gid/transitiveMembers/microsoft.graph.user"
                                AccessToken = $graphAccess
                            }
    ...
    foreach ($uid in $users) {
        ...
        if ($checkMFAResult) {
            if ($checkMFAResult.isMfaRegistered) {
                $status = "Enabled"
            }
            else {
                $status = "Disabled"
            ...
            $thisOutput = [PSCustomObject] @{
                UserPrincipalName = $checkMFAResult.userPrincipalName
                DisplayName = $checkMFAResult.userDisplayName
                Status = $status
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD81092qFuur1H0
# vL8q6G+gV8YBlAFobmfgQSh1UxoxlaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgOpK0O2YIT5r8f9dXKsDyJy+xFXpJdhe6XKTxWiwx4DEwDQYJKoZI
# hvcNAQEBBQAEggIAeXlIf4Xr0OxX9PkTiGmsieBPlrnTiULxg27UKeMtrH1Mdm81
# oqWvDGvJEUnjFR34WJHrO6mTHAMuTNGgKhNOVWqnNzxOoRe1g4KeKOYY24wB1lzO
# g/hgRPUsCrcN/tbmUFtBl/EInAj+Ccyy7XLYsfg4bZ9SddePuL4dV+VpOUbL6ZQw
# CGhwt3dVRwXlYryNvw1vCQrfOj3stK1qW8Yud9nSIDrppYTniQoR6NvV0Z+/xHfm
# hBx+JW5P2rWcOAURxfgjoOTKUdSWAvY/GYRprKKUpSyRUORf4F8zdWL0ibIri+ku
# EQ+aiOwjSCip34fDFt7PMbVFMAqJo+ugrALCeqkUOBL+2IyPqvN8QGBRVLPGVTvH
# eXCAApflynnD9lSE7a5kagxvHaRwdMEkKZ5Oeejpolni/Ui7ZHEB1K4N13SlM+pO
# ZbFeyFg+dvY/62eRwc+vuvmA62vJoFFFbOVig30cfo+yL6JFUcmDj0ekV5U2flDf
# /Izhno7CXgbI/Ynt4oCCe+EgkVqsbfjSda2ZYYIJAY9El+iByNyxrbT8weLZ3Ec1
# 9ImbO85XXnZc5EnLzQ2keE8pptWWR2KL6bd1CCamfk0fdCl7zcFoKj2j4qILUhXy
# bDHZogxbtxH/j2jogrrZ5lVk0/HgvSIznJeo+74FqISdHr0CA6sMN67xn7uhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCC0tgeVoO8VOkmZWKDGDi6kSlI5+3KKs5WC
# ZV4fY7IX1QIUYkdcKchhAbK1r8Ff28zVPXmxq3QYDzIwMjUwNjEwMDc1MDQxWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDLN3+S7xCmB4/H2CZdtATjQxAGMi+Id0RmQuJRbx3N
# sTCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgAoFcgUWOUp7/xhS7ATisDOoQvYa8hMFHTnKzrOVU9bNZhxgBQM3TPPH
# ZqMj0tlAO966Av2+/pSBbajny4du3D1oBev/Az9NwZ9Tp0MPdtb9k2byRsP+qSYZ
# ypShH180r9e7BI4j6ojyrmLVNL7VMr0DxsVyM9cwsRnQM6URIQfYCjNrUIEiE9FB
# gjQhfy+WjDzRJC3966FZP+gCq2GMGycNjzLu/bP+MxULGB/xLOatOaJ1PoE+JXnD
# yQQ5Gx1ZJuKDmNk71SpqEm1heY0jzdzrRli7XKI2GOIq/xDBx1mRsY1ednFQ3U3+
# qbSfM+heOgsnOP40ejlO/GnUBIu3MzPEIuwSSAynFYgZAG/vSkI3OOAyrADU5Z02
# 97B3TaIEEPertJ/k+Gfrc32O7gKcNPBRudpeykBHaz0GMdwTg6n0rIPfR7bSgQDS
# zHz29B/XH5LA5I/+NsbOWsRqf45UhURHKaBBQEOhaPLr+nUYFw19tJLKNY//giys
# No/+0VNAtw==
