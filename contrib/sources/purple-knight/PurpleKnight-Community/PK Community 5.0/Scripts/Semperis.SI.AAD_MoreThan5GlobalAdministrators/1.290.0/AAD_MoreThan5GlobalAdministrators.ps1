...
$Global:self = @{
    ID = 163
    UUID = '62386a3a-9d27-4969-af4e-a3528763ba59'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000163'
    Name = 'More than 5 Global Administrators exist'
    ScriptName = 'AAD_MoreThan5GlobalAdministrators'
    Description = '<p>This indicator checks for the presence of 5 or more Global Administrators. Please note:  If a given user has <b>both</b> an active and eligible assignment for a given role, the user will appear twice in the results.</p>
    ...
    Schedule = '1d'
    Impact = 2
    LikelihoodOfCompromise = '<p>Users that hold the Global Administrator role are the most privileged users in Entra ID. Organizations should limit the number of Global Administrators to less than five.</p>
      <p>An attacker will find users with the Global Administrator role as high-value, and a compromised Global Administrator can lead to several attacks against the organization, that can include data exfiltration, lateral movement into other systems in Azure, multi-cloud, and on-premise systems, and can result in complete tenant compromise where an organization becomes locked out of their Entra tenant by the attacker.</p>
      <h3>References</h3
      ...
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/security-emergency-access" target="_blank">Manage emergency access admin accounts - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} Global Administrators in Azure AD.'
    Remediation = '<p>Review the users assigned the Global Administrator role, and determine if the role assignment can be removed.</p><br /><p>To review and manage role assignments, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserManagementMenuBlade/%7E/UserSettings/menuId/UserSettings" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, select <b>Show more</b>, expand <b>Roles &amp; admins</b>, and select <b>Roles &amp; admins</b>.</p>
      <p>In the blade on the right, find the <b>Global Administrator</b> role. Select the role.</p>
      <p>From this view you can remove users as necessary from the role. Organizations that have Entra ID P2 licensing available should use access reviews to manage the lifecycle of privileged users.</p>
      <p><b>Organizations must ensure that any removal of the Global Administrator role does not impact the ability to manage their Entra tenant</b>.</p>'
    Types = @('IoE')
...
try {
    ...
    try {
        $adminResult = Get-PrivilegedAdmins @getGAs
        $AADAdmins = $adminResult[0]
        $AADAdminsUnresolved = $adminResult[1]
    }
    catch {
    ...
    if ($AADAdmins.Count -gt 5) {
        foreach ($admin in $AADAdmins) {
            # Check if this admin is in the unresolved list
            $unresolvedAdmin = $AADAdminsUnresolved | Where-Object { $_.ID -eq $admin.ID }

            if ($unresolvedAdmin) {
                $thisOutput = [PSCustomObject] @{
                    UserPrincipalName = "Object not found"
                    UserId = $admin.ID
                    AssignmentType = $admin.State
                }
        ...
        $gaAfterIngore = $outputObjects.Count - $scoreOutput.Ignoredcount
        if ($gaAfterIngore -gt 5) {
        ...
        if ($AADAdminsUnresolved) {
            if ($AADAdminsUnresolved.Count -eq 1) {
                $res.ResultMessage += " Unable to resolve 1 privileged object (details available in the indicator 'Unresolved Entra ID privileged role members')."
            }
            else {
                $res.ResultMessage += " Unable to resolve $($AADAdminsUnresolved.Count) privileged objects (details available in the indicator 'Unresolved Entra ID privileged role members')."
            }
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC