...
$Global:self = @{
    ID = 234
    UUID = 'da8da200-a31e-4154-a84c-c20fc224cc8f'
    Version = '1.290.0'
    CategoryID = 7
    ShortName = 'SI000234'
    Name = 'Password hash synchronization is not enabled'
    ScriptName = 'AAD_PasswordHashSync'
    Description = '<p>This indicator checks whether password hash synchronization is enabled in the tenant.</p>
      <h3>Required permissions</h3>
      <ul>
        <il>OnPremDirectorySynchronization.Read.All</il>
      </ul>'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>Password hash synchronization (PHS) is a feature of Entra ID that allows for authentication resilience over other methods of authentication, such as pass-through authentication (PTA) or federated authentication.</p>
      <p>Organizations that must use PTA or federated authentication can still enable PHS to take advantage of features in Entra Identity Protection to check for leaked credentials. Further, PHS can be used as a fallback in PTA or federated authentication outage scenarios, even though the change from PTA or federated authentication to PHS is not immediate.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/troubleshoot/entra/entra-id/user-prov-sync/pwd-hash-sync-auto-enable" target="_blank">Password Hash Synchronization is automatically enabled in Microsoft Entra connector | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-password-hash-synchronization" target="_blank">Implement password hash synchronization with Microsoft Entra Connect Sync - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} tenants with password hash synchronization disabled.'
    Remediation = '<p>Enable password hash synchronization in the Entra tenant.</p>
      <p>When dealing with password hash synchronization, organizations have options to migrate from pass-through authentication (PTA) or federated authentication, or alternatively organizations can enable password hash synchronization and still leave the authentication method of PTA or federated in place.</p>
      <p><b>Organizations enabling password hash synchronization must ensure they are not changing the authentication method without proper planning and testing.</b></p>
      <p>Depending on whether the organization is looking to migrate from PTA or federated authentication, or if they are looking to enable password hash synchronization without changing authentication, the means of making this change are different in Entra connect.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'TenantId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'PasswordSyncEnabled'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/OnPremDirectorySynchronization.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    try {
        $syncParamsResult = Invoke-GraphApiRequest @getSyncParams
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Sync' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    foreach ($sync in $syncParamsResult) {
        $syncVal = $sync.value
        $id = $syncVal.id
        $syncFeatures = $syncVal.features
        if ($syncFeatures) {
            $passwordSync = $syncFeatures.passwordSyncEnabled
            if ($passwordSync -eq $false) {
                $thisOutput = [PSCustomObject] @{
                    TenantId = $id
                    PasswordSyncEnabled = $passwordSync
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDIWXSWNnnsuPen
# ec0aqcgVOZ/rk3rUd9bVJ6LKFJ1TLaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ