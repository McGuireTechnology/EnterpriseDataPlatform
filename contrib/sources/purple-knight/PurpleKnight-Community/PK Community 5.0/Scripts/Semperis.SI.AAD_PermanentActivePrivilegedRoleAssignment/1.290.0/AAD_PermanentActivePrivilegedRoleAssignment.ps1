...
$Global:self = @{
    ID = 229
    UUID = '4c280de7-6abd-4093-8773-277dbf166698'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000229'
    Name = 'Permanent Active Privileged Role Assignment'
    ScriptName = 'AAD_PermanentActivePrivilegedRoleAssignment'
    Description = '<p>This indicator checks for the presence of permanent active privileged role Assignments.</p>
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>Privileged roles in Microsoft Entra Privileged Identity Management (PIM) that are assigned as "permanently active" pose significant security risks. According to Microsoft Learn, "Privileged role administrators can make users active for Microsoft Entra admin roles. An active administrator can activate the role when they need it, and then their permissions expire once they''re done." However, allowing principals to have permanent active for privileged roles is not the most secure practice, as it provides continuous access to elevated permissions.</p>
      <p>If an administrator intends to grant a user the ability to perform privileged actions, it is advisable to provide a limited time window for activity rather than indefinite access. Longer periods of activity increase the risk of privilege escalation and potential misuse.</p>
      <p>The likelihood of compromise increases significantly when privileged roles in PIM are assigned as permanently active. This configuration allows users continuous access to elevated permissions, increasing the risk of privilege escalation and potential exploitation.</p>
<h3>References</h3>
<p><a href="https://learn.microsoft.com/en-us/entra/id-governance/privileged-identity-management/pim-how-to-add-role-to-user" target="_blank">Assign Microsoft Entra roles in Privileged Identity Management | Microsoft Learn</a></p>
<p><a href="https://learn.microsoft.com/en-us/entra/id-governance/privileged-identity-management/pim-resource-roles-assign-roles" target="_blank">Assign Azure resource roles in Privileged Identity Management | Microsoft Learn</a></p>
<p><a href="https://github.com/cisagov/ScubaGear/blob/main/PowerShell/ScubaGear/baselines/aad.md#msaad74v1" target="_blank">CISA MS.AAD.7.4v1</a></p>'
    ResultMessage = 'Found {0} Permanent Active Privileged Role Assignments in Azure AD'
    Remediation = '<p>Limit the duration of activity for privileged roles in PIM to a reasonable timeframe (e.g., up to 90 days) based on the user''s role and necessity.</p>
<p>To configure activity durations, open the <a href="https://entra.microsoft.com/#view/Microsoft_Azure_PIMCommon/ResourceMenuBlade/~/members/resourceId//resourceType/tenant/provider/aadroles" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, select, expand <b>Identity governance</b>, and select <b>Privileged Identity Managemnet</b>.</p>
<p>In the blade on the right, select the specific privileged role you wish to configure. Adjust the activity settings to ensure they are time-bound.</p>
<p>Conduct regular reviews of all privileged roles to ensure that only necessary users have active access, and that this access is appropriately time-bound.</p>
<p>Educate administrators and users about the risks of permanently active roles and the importance of maintaining strict access controls.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'DisplayName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'PrincipalId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'IdentityType'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RoleDefinitionName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'AssignmentType'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/RoleEligibilitySchedule.Read.Directory', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/Application.Read.All', 'AAD.GraphAPI/GroupMember.Read.All', 'AAD.GraphAPI/User.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    # Endpoint "v1.0/roleManagement/directory/roleAssignmentSchedules" requires P2 (or Governance) license
    $getLicense = @{
        Uri = "https://graph.microsoft.com/v1.0/organization"
        AccessToken = $graphAccess
    }
    try {
        $licenseResponse = Invoke-GraphApiRequest @getLicense
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Organizations' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }
    $license = [System.Collections.ArrayList]@()
    $license.AddRange($licenseResponse.value)
    ...
    $validPlans = $license.assignedPlans | Where-Object { $_.capabilityStatus -in @('Enabled', 'Warning') } | Select-Object -ExpandProperty servicePlanId

    if (-not ($requiredPlans | Where-Object { $_ -in $validPlans })) {
        $res.Status = 'NotRelevant'
        $res.ResultMessage = 'This indicator is relevant only when a P2 or Governance license is in place'
        return $res
    }

    $getActiveRoles = @{
        Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignmentSchedules?`$select=principalId,roleDefinitionId,scheduleInfo"
        AccessToken = $graphAccess
    }
    try {
        $activeRoles = Invoke-GraphApiRequest @getActiveRoles
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Role Assignment Schedules' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    foreach ($role in $activeRoles.value) {
        $roleID = $role.roleDefinitionId
        if (($role.scheduleInfo.expiration.type -eq "noExpiration") -and ($AAD_PRIVILEGED_ROLES.ContainsKey($roleID))) {
            ...
            try {
                $IdentityInfo = Search-AADIdentity @getIdentityInfo
            }
            catch {
                return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects', 'Read Groups', 'Read Users' -TenantAppId $TenantAppId -AccessToken $graphAccess
            }
            ...
            $thisOutput = [PSCustomObject] @{
                ...
                AssignmentType = "Permenant Active"
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAztsh5SoMpnyt5
# UPL83xngWMSK0SxbdStPo4DGiyGwkqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ