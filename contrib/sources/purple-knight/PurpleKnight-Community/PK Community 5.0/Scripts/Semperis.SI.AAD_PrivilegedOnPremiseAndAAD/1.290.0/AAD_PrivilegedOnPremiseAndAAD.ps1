...
$Global:self = @{
    ID = 158
    UUID = 'c117b6a5-00c8-4e77-93d2-e291e36b462a'
    Version = '1.290.0'
    CategoryID = 7
    ShortName = 'SI000158'
    Name = 'Entra ID privileged users that are also privileged in AD'
    ScriptName = 'AAD_PrivilegedOnPremiseAndAAD'
    Description = '<p>This indicator checks for Entra ID privileged users that are also privileged in on-premise AD.</p>
    ...
    Schedule = '1h'
    Impact = 9
    LikelihoodOfCompromise = '<p>It is common in organizations that the people who manage Active Directory also manage Entra ID, and potentially M365 and Azure as well. In these scenarios users may require privileged credentials in all of these environments. It is critical for organizations to separate the privileged credentials out between the different identity stores, so that one user is not privileged in multiple directory systems.</p>
      <p>If a user holds privileges in Active Directory and Entra ID, and the account becomes compromised by an attacker, an attacker can move laterally between the two directory systems, easily compromising both and the resources attached to them. If the user holds highly privileged roles such as Domain Administrator and Global Administrator, attack damage from compromise can be severe.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/best-practices#9-use-cloud-native-accounts-for-microsoft-entra-roles" target="_blank">Best practices for Microsoft Entra roles - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/security-planning" target="_blank">Secure access practices for administrators in Microsoft Entra ID - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} Privileged Entra ID users that are also privileged in AD.'
    Remediation = '<p>Review the users identified as holding privileges in both Active Directory and Entra ID, and device a plan for separating out the privileged roles into multiple credentials.</p>
      <p>Separation of privileges is usually handled on a case-by-case basis, formulated by the level of privileges the role holds, as well as determining which identity system will be easier to create a new set of privileged credentials for the user in.</p>
      <p>A common practice for privilege separation into new sets of privileges users, is to create a new user in the target system that holds the same roles, and start using that user in the target system, validating that role functionality continues as needed. Then, the privileges are removed from the existing user that are no longer needed.</p>
      <p>If a privileged user from Active Directory was holding privileges in Entra ID and those privileges have been removed from Entra ID, the organization should also consider if they still need to synchronize the user from Active Directory. This is to defend the user account from potential attacks that target moving laterally between the two directory systems.</p>
      <p>If a new privileged user is created in Entra ID, the user should be created as a cloud-only account that is not synchronized from Active Directory, even if it has no privileges in Active Directory. This is to defend the user account from potential attacks that target moving laterally between the two directory systems.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI', 'AD.LDAP')
    OutputFields = @(
        @{ Name = 'DistinguishedName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserPrincipalName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'State'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RoleDefinitionId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Notes'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD', 'AD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/RoleManagement.Read.All')
    SecurityFrameworks = @(
...
https://learn.microsoft.com/en-us/graph/api/rbacapplication-list-roleeligibilityscheduleinstances?view=graph-rest-beta&tabs=http
...
$notInForest = [System.Collections.ArrayList]@()
$inForest = [System.Collections.ArrayList]@()
...
try {
    ...
    try {
        $adminResult = Get-PrivilegedAdmins @getAdmins
        $returnedAdmins = $adminResult[0]
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Roles','Read Directory Objects','Role Eligibility Schedule Instances' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $AADAdmins = [System.Collections.ArrayList]@()

    foreach ($returnedAdmin in $returnedAdmins) {
        $properties = @("onPremisesDomainName","onPremisesSecurityIdentifier","onPremisesImmutableId","id","userPrincipalName")
        $identityDetails = Search-AADIdentity -IdentityId $returnedAdmin.id -Properties $properties -AccessToken $graphAccess

        if ($identityDetails) {
            $identityObject = [PSCustomObject] @{
                onPremisesDomainName = $identityDetails.onPremisesDomainName
                onPremisesSecurityIdentifier = $identityDetails.onPremisesSecurityIdentifier
                onPremisesImmutableId = $identityDetails.onPremisesImmutableId
                UPN = $identityDetails.userPrincipalName
                ID = $identityDetails.id
                RoleDefinitionID = ($AAD_PRIVILEGED_ROLES.GetEnumerator() | Where-Object {$_.Value -eq $returnedAdmin.Role}).Name
                State = $returnedAdmin.State
            }
            [void]$AADAdmins.Add($identityObject)
        }
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        $baseDN = Get-DN $domain
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("mS-DS-ConsistencyGuid")
            baseDN = $baseDN
            scope = "subtree"
            filter = "(&(objectCategory=person)(adminCount=1)(mS-DS-ConsistencyGuid=*))"
        }
        $admins += Search-AD @searchParams
    }
    ...
    if (!$admins.Count -gt 0) {
        ...
        foreach ($domain in $DomainNames) {
            ...
            $searchParams = @{
                ...
                baseDN = $baseDN
                scope = "subtree"
                filter = "(&(objectCategory=person)(adminCount=1))"
            }
            $admins += Search-AD @searchParams
        }
    }
    ...
    foreach ($admin in $admins) {
        $isSync = $false
        foreach ($AADadmin in $AADAdmins) {
            if (-not $AADAdmin.onPremisesDomainName) {
                continue
            }

            try {
                $AADDomain=$AADadmin.onPremisesDomainName.ToLower()
            }
            catch {
                $AADDomain=$AADadmin.onPremisesDomainName
            }
            $AADsid = $AADadmin.onPremisesSecurityIdentifier
            $AADGuid = $AADadmin.onPremisesImmutableId
            if ($DomainNames.Contains($AADDomain)) {
                if ($matchedAttribute -eq "msdsConsistencyGuid") {
                    $guidBytes = [byte[]]$admin.Attributes."ms-ds-consistencyguid"[0]
                    $adminConsistencyId = [Convert]::ToBase64String($guidBytes)
                    if ($AADGuid -eq $adminConsistencyId) {
                        $isSync = $true
                    }
                }
                elseif ($matchedAttribute -eq "SID") {
                    $adminSID =  (New-Object System.Security.Principal.SecurityIdentifier($admin.Attributes.objectsid[$GET_STRING],0)).Value
                    if ($AADsid -eq $adminSID) {
                        $isSync = $true
                    }
                }
                if ($isSync -eq $true) {
                    $isObject = $outputObjects | Where-Object {$_.UserId -eq $AADadmin.ID -and $_.State -eq $AADadmin.State}
                    if ($isObject) {
                        $objectRole = $isObject.RoleDefinitionId
                        if (-not ($objectRole.Contains($AADadmin.RoleDefinitionID))) {
                            $newRole = $objectRole += " ,$($AADadmin.RoleDefinitionID)"
                            $isObject.RoleDefinitionId = $newRole
                        }
                    }
                    else {
                        $thisOutput = [PSCustomObject][Ordered] @{
                            DistinguishedName = $admin.DistinguishedName
                            UserPrincipalName = $AADadmin.UPN
                            UserId = $AADadmin.ID
                            State = $AADadmin.State
                            RoleDefinitionId = $AADadmin.RoleDefinitionID
                            Notes = $null
                        }
                        [void]$outputObjects.Add($thisOutput)
                        [void]$inForest.Add($thisOutput)
                        break
                    }
                }
            }
                ...
                if ($isObject) {
                    ...
                    $thisOutput = [PSCustomObject][Ordered] @{
                        ...
                        State = $AADadmin.State
                        RoleDefinitionId = $AADadmin.RoleDefinitionID
                        Notes = "This Azure AD high privileged user is synced to a different forest: $($AADadmin.onPremisesDomainName)"
                    }
                    [void]$outputObjects.Add($thisOutput)
                }
            }
    ...
    if ($inForest.Count -eq 0 -and $notInForest.Count -gt 0) {
        $res.Status = 'NotRelevant'
        $res.ResultMessage = 'This indicator is not relevant because all the tenants high privileged users are synchronized with a different forest.'
    ...
    if ($outputObjects) {
        ...
        $inforestCount = @($outputObjects | Where-Object {$_.Notes -eq $null} | Sort-Object -Property UserId -Unique).Count
        $inforest = $outputObjects | Where-Object {$_.Notes -eq $null} | Sort-Object -Property UserId -Unique
        $scoreOutput = $inforest | Get-Score -Impact $self.Impact
        if ($scoreOutput.Score -lt 100) {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($inforestCount) Privileged Entra ID users that are also privileged in AD."
        ...
        if ($notInForest.Count -gt 0) {
            $res.ResultMessage += " Found another $(@($notInForest | Sort-Object -Property ID -Unique).Count) Privileged Entra ID users that are also privileged in AD., but in a different forest."
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC7q6KBezs4eCGL
# +eAf1UGDbOAYbXVdkj4FWfiuAjXkE6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg