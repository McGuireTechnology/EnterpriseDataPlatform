...
$Global:self = @{
    ID = 162
    UUID = '5f6540a9-f226-4438-9e64-387a98beda2b'
    Version = '1.290.0'
    CategoryID = 7
    ShortName = 'SI000162'
    Name = 'AD privileged users that are synced to Entra ID'
    ScriptName = 'AAD_PrivilegedOnPremiseSyncedToAAD'
    Description = '<p>This indicator checks for AD privileged users that are synced to Entra ID.</p>
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>During deployments of Entra Connect or Entra Cloud Sync, organizations may intentionally or unintentionally synchronize privileged users from Active Directory to Entra ID. When this occurs, the attack surface is increased on these privileged users.</p>
      <p>Organizations should not synchronize privileged users from Active Directory to Entra ID unless there is a specific and designed intention, and the organization understands the risk of the increased attack surface.</p>
      <p>An attacker may attempt to compromise the user account in Entra ID, and then use that compromise to move laterally into Active Directory.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-sync-configure-filtering#organizational-unitbased-filtering" target="_blank">Microsoft Entra Connect Sync: Configure filtering - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/cloud-sync/how-to-configure#scope-provisioning-to-specific-users-and-groups" target="_blank">Microsoft Entra Cloud Sync new agent configuration - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} Privileged AD users that are synced to Entra ID.'
    Remediation = '<p>Review the users identified as holding privileges in Active Directory, and remove them from synchronization in Entra Connect or Entra cloud sync.</p>
      <p>In Entra Connect, user filtering should be configured on an Organizational unit level. This requires modification of the filtering on the Entra Connect server, as well as any staging servers. Details on adjusting the OU filtering can be found in the reference Microsoft documentation.</p>
      <p>In Entra cloud sync, user filtering is configured with scoping filters. Details on adjusting scoping filters can be found in the reference Microsoft documentation.</p>
      <p><b>Organizations must ensure that changes to synchronized users are understood and verified that they will not negatively impact the target users, and any impacted users should have the changes communicated as needed</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI', 'AD.LDAP')
    OutputFields = @(
        @{ Name = 'DistinguishedName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'MemberOf'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD', 'AD')
    Permissions = @('AAD.GraphAPI/User.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    $getAzureSyncedUsers = @{
        Uri = "https://graph.microsoft.com/v1.0/users?`$select=id,onPremisesDomainName,onPremisesSyncEnabled,userPrincipalName,onPremisesImmutableId,onPremisesSecurityIdentifier&`$filter=onPremisesSyncEnabled eq true"
        AccessToken = $graphAccess
    }

    try {
        $response = Invoke-GraphApiRequest @getAzureSyncedUsers
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Users' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $syncedUsers = @($response.value)
    $syncedAD = $false
    foreach ($sync in $syncedUsers) {
        $domainADName = $sync.onPremisesDomainName
        try {
            $domainAD=$domainADName.ToLower()
        }
        catch {
            $domainAD=$domainADName
        }
        if ($DomainNames.Contains($domainAD)) {
            $syncedAD = $true
            break
        }
    }
    if ($syncedAD -eq $false) {
        $res.Status = 'NotRelevant'
        $res.ResultMessage = 'This indicator is not relevant because the domain is not synchronized with the current tenant.'
        return $res
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("mS-DS-ConsistencyGuid", "memberof")
            baseDN = $baseDN
            scope = "subtree"
    ...
    foreach ($syncUser in $syncedUsers) {
        $AADsid = $syncUser.onPremisesSecurityIdentifier
        $AADGuid = $syncUser.onPremisesImmutableId
        foreach ($admin in $admins) {
            ...
            if ($isSync -eq $true) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    MemberOf = $admin.Attributes."memberof"[0] -join "; "
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Privileged AD users that are synced to Entra ID."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAjsWUnpIqjLfk9
# 5T0KiD7VSHrVt/rK94XRvnWx1aMiuqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgW8Z7voViAV60cnonkfhrGzhjufhVO7B2Eb7sXUp41QswDQYJKoZI
# hvcNAQEBBQAEggIASlMVAMp5/UNczbxTTRnNfFJ5pjR1F5QDpBxk0voXhaY5cZhP
# emJjIM4kdVHxCJxZSzzV7oMkPsXmnDE40CfMfgMTfAVC/EQbxTaes/u5WnAQHy8p
# VpqQ0UMx4gCp0pxmMDN61XgQD0WqP/U55ONKU2XKicohoB+TN6koZEgbEPAiJ5uH
# MlvPCvFkkV6Y8EmiIXup8RjqdR4iWlGUMdN+pnQefkEsg98iZV6YPSDt6yusGuw2
# EtUsVLmqja7Gm+0RJ6mHLBfZo5QELtGMf9PrClkjnBBj2eXBG80i/j4D9Wdpla++
# JlFRoSkc0XZTjLatm+cyAWL9SBpZKxv7z10zaF7pWq4p87THcRyssg41tgajOEnY
# bhPHv7weN4+LKe16tMqu1d947XD55L/yeAf4ZX/MYPKO3wlwxp+yZb7Ylh7FKqew
# XrHI/ZdJeZG4iAiHjzAVxzLeaSzh0dnlkVL7yhIGPnbeqzvNC4ldgEInkyszRZZW
# GVkbMhBiwFUnlY74jzY8c4rLx35yrQ3ZlLig/b0S03yDDHN+GjrAD8WReCK6jPVZ
# 1XO2mCkYPGdeWVrIYctPt8EU34stlOA51z0MkFGNRTjJmW8/96STHWaX5GF8J85i
# aSE0eP/rCdMqH6OQuyK0c//pZSctxYdqmXYL/+/91YlhxM5dyXZIN03RW7ehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDipaCuaTU9om42uz7j4QJecxf0RGR7zfw/
# mYKY45aWwAIUE491cjQfE4bZNzLG0+LsAuj7CBwYDzIwMjUwNTEzMTg0NjExWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDs/jaisWYGHAmpd+Bcj9Y8xIzvAZhewqbI7cp8bFft
...
# CwUABIIBgCRU7oDj+b5jgBJV4qtXI/HWC6N5+IWPHt3YexMPPJiuFzJIS6O8eTDK
# X0eADJJLmT1Qq1/45V/6G5kL872U+YK7Fckyta38l9YP1Tksss0m/sSMs3xBN7hb
# Xqa/zCGabeI4OcFMrP4zu2tGC4DEuxw1L8fT8yZ8YrpBuMcAXpviV2bkZzfMNCxp
# bv3Ufol0dYpLvg8HMjCPe8F9m7jbO8DUMkHhjKJNVcaeytDMjFN41wDdnDHxcVYj
# yFlRI0Dlr3yLIJPsOn0qNmHnYNu+rhUCW/PinOBw9FzWF3wnVh7oMGo/x1II29QX
# NZ5py4KK8yPJCeqL9eMz1gA5567vpcC/WLEZ6m35qtZcZHWPukkvUpnETRBX8Xmi
# Xn3BQisFl45Kia3uOREMLdXgRmljuSkkeBxoA5Ka8/pq2+j6UoDJrqBr3guYdyl9
# QNUAz6JgdjTKWE+GhYK74bsMqlbKgB0+8o1zF9HOrhFMnYDsjAiJBFARVWS7aGou
# yDwW3tnfrw==
