...
$Global:self = @{
    ID = 226
    UUID = 'e231380a-a77e-481f-ab03-6310b7b5cdfa'
    Version = '1.294.0'
    CategoryID = 6
    ShortName = 'SI000226'
    Name = 'Prohibited Entra ID Roles Assigned'
    ScriptName = 'AAD_ProhibitedPrivilegedRoles'
    Description = '<p>This indicator checks for the assignment of roles in Entra ID that are deprecated or prohibited from use.</p>
    ...
    Schedule = '1d'
    Impact = 3
    LikelihoodOfCompromise = '<p>Deprecated roles and roles marked as "never use" can pose significant security risks if assigned. These roles may have unintended permissions or may not be properly maintained, potentially leading to security vulnerabilities.</p>
<p>An attacker who gains access to an account with these roles could potentially exploit outdated or overly broad permissions, leading to unauthorized access, data breaches, or further privilege escalation within the Entra ID environment.</p>
<h3>References</h3>
<p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/best-practices#5-limit-the-number-of-global-administrators-to-less-than-5" target="_blank">Best practices for Microsoft Entra roles - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} principals with prohibited roles assigned. {1} ''Deprecated'' roles and {2} ''Prohibited'' roles detected.'
    Remediation = '<p>Review all principals with prohibited role assignments and remove these assignments immediately.</p>
      <p>To review and manage role assignments, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/RolesManagementMenuBlade/~/AllRoles" target="_blank">Microsoft Entra admin center</a>, go to Identity > Roles and admins.</p>
      <p>For each prohibited role:</p>
      <ol>
        <li>Identify alternative, supported roles that provide the necessary permissions.</li>
        <li>Remove the prohibited role assignment.</li>
        <li>Assign the appropriate alternative role.</li>
        <li>Verify that the principal can still perform their required tasks with the new role assignment.</li>
      </ol>
    <p>For deprecated roles, consult Microsoft documentation to find the recommended replacement roles.</p>
    <p>For "prohibited" roles, revoke all assignments and reassess the need for those permissions.</p>
    <p><b>Caution: Ensure that removing these role assignments does not disrupt critical operations. Always test changes in a non-production environment first.</b></p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'PrincipalName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'PrincipalId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'PrincipalType'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RoleName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RoleType'; Type = 'String'; IsCollection = $false },
        @{ Name = 'AssignmentType'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ObjectResolved'; Type = 'Boolean'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/RoleManagement.Read.All', 'AAD.GraphAPI/Directory.Read.All')
    SecurityFrameworks = @(
...
function Write-OutputObject {
    param (
        ...
        [System.Collections.ArrayList]$currentCollection,
        $token,
        [switch]$objectResolved
    )

    $existingObject = $currentCollection | Where-Object { $_.ID -eq $responseObject.id -and $_.Role -eq $responseObject.Role -and $_.State -eq $responseObject.State}

    if (!($existingObject)) {
        if ($responseObject.principal) {
            if ($responseObject.principal.userPrincipalName) {
                $objectUPN = $responseObject.principal.userPrincipalName
            }
            else {
                $objectUPN = "Object not found"
            }

            if ($responseObject.principal.displayName) {
                $objectDisplay = $responseObject.principal.displayName
            }
            else {
                $objectDisplay = "Object not found"
            }

            if ($responseObject.principal.'@odata.type') {
                $type = $responseObject.principal.'@odata.type'.split(".")[-1]
                $objectType = $type
            }
            else {
                $objectType = "Object not found"
            }
        }
        else {
            # Try to get the object type
            try {
                $objectInfo = Search-AADIdentity -IdentityId $responseObject.id -AccessToken $token
                $objectUPN = $objectInfo.userPrincipalName
                $objectDisplay = $objectInfo.displayName
        ...
        $thisOutput = [PSCustomObject] @{
            PrincipalName = $objectDisplay
            PrincipalUPN = $objectUPN
            PrincipalId = $responseObject.id
            PrincipalType = $objectType
            RoleName = $writeRoleName
            RoleType = $writeRoleType
            AssignmentType = $writeAssignmentType
            ObjectResolved = $objectResolved.IsPresent
        }
...
try {
    ...
    foreach ($roleType in $prohibitedRoles.Keys) {
        foreach ($roleId in $prohibitedRoles[$roleType].Keys) {
            $roleName = $prohibitedRoles[$roleType][$roleId]
            $getActiveAssignments = @{
                Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments?`$filter=roleDefinitionId eq '$($roleId)'&`$select=principalId&`$expand=principal"
                AccessToken = $graphAccess
            }

            $activeResults = Invoke-GraphApiRequest @getActiveAssignments

            $activeMembers = @($activeResults.value)
            if ($activeMembers) {
                foreach ($activeMember in $activeMembers) {
                    $getObject = @{
                        Uri = "https://graph.microsoft.com/v1.0/directoryObjects/$($activeMember.principalId)?`$select=id"
                        AccessToken = $graphAccess
                    }

                    try {
                        $objectResult = Invoke-GraphApiRequest @getObject

                        if ($objectResult.'@odata.type' -eq "#microsoft.graph.group") {
                            $getGroupMembers = @{
                                Uri = "https://graph.microsoft.com/v1.0/groups/$($objectResult.id)/members"
                                AccessToken = $graphAccess
                            }

                            try {
                                $groupMembersResult = Invoke-GraphApiRequest @getGroupMembers
                            }
                            catch {
                                return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Groups' -TenantAppId $TenantAppId -AccessToken $graphAccess
                            }

                            $groupMembers = $groupMembersResult.value
                            foreach ($groupMember in $groupMembers) {
                                $getMemberObject = @{
                                    Uri = "https://graph.microsoft.com/v1.0/directoryObjects/$($groupMember.id)?`$select=id"
                                    AccessToken = $graphAccess
                                }

                                try {
                                    $memberObject = Invoke-GraphApiRequest @getMemberObject
                                }
                                catch {
                                    if ($_.Exception.Response.StatusCode -eq 404) {
                                        ...
                                        Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $memberObject -writeAssignmentType "Active" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$false -token $graphAccess
                                        continue
                                    }
                                    else {
                                        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
                                    }
                                }

                                if ($memberObject.'@odata.type' -ne "#microsoft.graph.group") {
                                    $memberObjectResult = @{
                                        id = $memberObject.id
                                        Role = $roleName
                                        State = "Active"
                                    }
                                    Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $memberObjectResult -writeAssignmentType "Active" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$true -token $graphAccess
                                }
                            }
                        }
                        else {
                            $activeObject = @{
                                id = $activeMember.principalId
                                Role = $roleName
                                State = "Active"
                            }

                            if ($activeMember.principal) {
                                $activeObject.principal = $activeMember.principal
                            }

                            Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $activeObject -writeAssignmentType "Active" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$true -token $graphAccess
                        }
                    }
                    catch {
                        if ($_.Exception.Response.StatusCode -eq 404) {
                            ...
                            Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $activeObject -writeAssignmentType "Active" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$false -token $graphAccess
                        }
                        else {
                            return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
                        }
            ...
            $getEligibleAssignments = @{
                Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleEligibilityScheduleInstances?`$filter=roleDefinitionId eq '$($roleId)'&`$select=principalId&`$expand=principal"
                AccessToken = $graphAccess
            }

            $eligibleResults = Invoke-GraphApiRequest @getEligibleAssignments

            $eligibleMembers = @($eligibleResults.value)
            if ($eligibleMembers) {
                foreach ($eligibleMember in $eligibleMembers) {
                    $getObject = @{
                        Uri = "https://graph.microsoft.com/v1.0/directoryObjects/$($eligibleMember.principalId)?`$select=id"
                        AccessToken = $graphAccess
                    }
                            ...
                            foreach ($groupMember in $groupMembers) {
                                ...
                                try {
                                    ...
                                    if ($_.Exception.Response.StatusCode -eq 404) {
                                        ...
                                        Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $memberObject -writeAssignmentType "Eligible" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$false -token $graphAccess
                                        continue
                                    }
                                    else {
                                        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
                                ...
                                if ($memberObject.'@odata.type' -ne "#microsoft.graph.group") {
                                    ...
                                    Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $memberObjectResult -writeAssignmentType "Eligible" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$true -token $graphAccess
                                }
                            }
                        }
                        else {
                            $eligibleObject = @{
                                id = $eligibleMember.principalId
                                Role = $roleName
                                State = "Eligible"
                            }

                            if ($eligibleMember.principal) {
                                $eligibleObject.principal = $eligibleMember.principal
                            }

                            Write-OutputObject -writeRoleID $roleId -writeRoleName $roleName -responseObject $eligibleObject -writeAssignmentType "Eligible" -currentCollection $outputObjects -writeRoleType $roleType -objectResolved:$true -token $graphAccess
                        }
                    }
                    catch {
                        if ($_.Exception.Response.StatusCode -eq 404) {
    ...
    if ($outputObjects) {
        ...
        $assignmentsAfterIgnore = $outputObjects.Count - $scoreOutput.Ignoredcount

        if ($assignmentsAfterIgnore -gt 0) {
            $deprecated = @($outputObjects | Where-Object { $_.RoleType -eq "Deprecated" })
            $prohibited = @($outputObjects | Where-Object { $_.RoleType -eq "Prohibited" })
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = $self.ResultMessage -f $outputObjects.Count, $deprecated.Count, $prohibited.Count
            $res.Score = $scoreOutput.Score

            $unresolvedObjects = @($outputObjects | Where-Object { $_.ObjectResolved -eq $false })
            if ($unresolvedObjects) {
                if ($unresolvedObjects.Count -eq 1) {
                    $res.ResultMessage += " There is $($unresolvedObjects.Count) member which could not be resolved."
                }
                else {
                    $res.ResultMessage += " There are $($unresolvedObjects.Count) members which could not be resolved."
                }

                $res.Remediation = '<![CDATA[<p>Review all principals with prohibited role assignments and remove these assignments immediately.</p><br /><p>To review and manage role assignments, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/RolesManagementMenuBlade/~/AllRoles" target="_blank">Microsoft Entra admin center</a>, go to Identity > Roles and admins.</p><br /><p>For each prohibited role:</p><ol><li>Identify alternative, supported roles that provide the necessary permissions.</li><li>Remove the prohibited role assignment.</li><li>Assign the appropriate alternative role.</li><li>Verify that the principal can still perform their required tasks with the new role assignment.</li></ol><p>For deprecated roles, consult Microsoft documentation to find the recommended replacement roles.</p><p>For "prohibited" roles, revoke all assignments and reassess the need for those permissions.</p><br /><p>Two common causes of unresolved members are the deletion of eligible users and Partner Relationships that involve GDAP. Use the following methods to learn more:</p><h3>Deleted Users:</h3><ol><li>Navigate to the Microsoft Entra admin center</li><li>Use the provided UserPrincipalName and DisplayName to search in the Users blade</li><li>If confirmed as deleted, remove their role eligibility through PIM</li></ol><h3>For Partner Relationships:</h3><ol><li>Access the Microsoft Admin Center at <a href="https://admin.microsoft.com/Adminportal/Home?#/partners" target="_blank">https://admin.microsoft.com/Adminportal/Home?#/partners</a></li><li>Review all partner relationships and their associated role assignments</li><li>Verify that each partner relationship is legitimate and necessary</li></ol><p>Note: GDAP relationships cannot be viewed through Graph API from the managed tenant, but are visible in the admin portal to Global Administrators</p><p>Regular review of both scenarios is recommended as part of your security governance process.</p><br /><p><b>Caution: Ensure that removing these role assignments does not disrupt critical operations. Always test changes in a non-production environment first.</b></p>]]>'
            }
            else {
            ...
            $res.Status = 'Pass'
            $res.ResultMessage = "No prohibited role assignments found after applying ignore rules."
        }
    }
}
catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC