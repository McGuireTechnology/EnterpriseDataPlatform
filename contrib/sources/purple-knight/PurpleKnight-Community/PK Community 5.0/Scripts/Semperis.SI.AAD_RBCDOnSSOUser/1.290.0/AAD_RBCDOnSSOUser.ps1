...
$Global:self = @{
    ID = 114
    UUID = '3d6ff5b6-8d01-46b9-a261-7f5ab0c3b306'
    Version = '1.290.0'
    CategoryID = 7
    ShortName = 'SI000114'
    Name = 'Resource Based Constrained Delegation applied to AZUREADSSOACC account'
    ScriptName = 'AAD_RBCDOnSSOUser'
    Description = 'This indicator looks for Resource Based Constrained Delegation configured on the Entra Seamless SSO account.'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>When configuring Entra Seamless SSO, a computer account is created in Active Directory, AZUREADSSOACC. This computer account is used by Entra ID to decrypt Kerberos tickets that are used as part of the seamless SSO mechanism between Active Directory and Entra ID. If resource-based constrained delegation is configured on the AZUREADSSOACC computer account, an account with the delegation would be able to generate Ticket Granting Service (TGS) requests to Entra ID on behalf of the AZUREADSSOACC account, and impersonate any user in the Entra tenant that is synchronized from Active Directory.</p>
      <p>An attacker can use this method to move laterally from Active Directory into Entra ID.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-sso#sso-via-primary-refresh-token-vs-seamless-sso" target="_blank">Microsoft Entra Connect: Seamless single sign-on - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-sso-how-it-works#how-does-set-up-work" target="_blank">Microsoft Entra Connect: Seamless Single Sign-On - How it works - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} Domain(s) with Resource Based Constrained Delegation configured on AZUREADSSOACC account.'
    Remediation = '<p>Clear the msDS-AllowedToActOnBehalfOfOtherIdentity attribute on the AZUREADSSOACC computer account.</p>
      <p>There is no functional reason to allow resource-based constrained delegation on this computer account.</p><br /><p>To remove, run the following PowerShell command against the computer account:</p><br /><p>Set-ADComputer -Identity AZUREADSSOACC$ -Clear &quot;msDS-AllowedToActOnBehalfOfOtherIdentity&quot;</p>
    ...
    Permissions = @()
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Lateral Movement', 'Credential Access') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $azureUser = "AZUREADSSOACC"

        $results = @()
        $DN = Get-DN $domain

        # Get a list of AZUREADSSOACC account and its rbcd configuration
        $searchParams = @{
            dnsDomain = $domain
            attributes = "msds-allowedtoactonbehalfofotheridentity"
            baseDN = $DN
            scope = "Subtree"
            filter = "(name=$($azureUser))"
            PropertyFilter = "name"
        ...
        if ($results) {
            ...
            foreach ($result in $results) {
                $checkRbcd = $result.Attributes.'msds-allowedtoactonbehalfofotheridentity'
                if ($checkRbcd -ne "") {
                    ...
                    $delegatePrincipals = @()
                    foreach ($Ace in $delegateSD.DiscretionaryAcl) {
    ...
    if ($checkAADConnect -eq $false -and $outputObjects.Count -eq 0 -and $unavailableDomains.Count -eq 0) {
        ...
        $res.Score = 0
        $res.ResultMessage = 'This indicator is only relevant for environments with AAD Connect. No IDs for AAD Connect were detected in the environment.'
        return $res
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC