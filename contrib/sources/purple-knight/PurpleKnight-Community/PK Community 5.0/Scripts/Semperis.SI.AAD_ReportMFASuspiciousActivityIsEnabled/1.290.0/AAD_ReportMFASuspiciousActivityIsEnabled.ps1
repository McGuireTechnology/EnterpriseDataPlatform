...
$Global:self = @{
    ID = 93
    UUID = '7b9dd975-4b16-4fce-8e5f-41b36e7dc816'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000093'
    Name = 'Report suspicious MFA activity disabled'
    ScriptName = 'AAD_ReportMFASuspiciousActivityIsEnabled'
    Description = '<p>This indicator checks if the setting to report suspicious MFA activity is enabled.</p>
    ...
    Schedule = '1w'
    Impact = 1
    LikelihoodOfCompromise = '<p>Social engineering attacks such as phishing or MFA fatigue attacks can be used to trick a user into authorizing an MFA request they did not initiate. While systems like number matching reduce the ability for these attacks to work, in some instances the attacker may have gained access to the user''s password.</p>
      <p>When report suspicious activity is enabled, users can report suspicious MFA requests in interactive MFA scenarios with push notifications or phone calls. This allows the user to be proactively involved in securing their account.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en%2Dus/entra/identity/authentication/howto%2Dmfa%2Dmfasettings" target="_blank">Configure Microsoft Entra multifactor authentication settings | Microsoft Learn</a></p>'
    ResultMessage = 'Report suspicious activity is set to {0} for Group {1}.'
    Remediation = '<p>It is recommended to have the "Report suspicious activity" setting enabled for all users. Please note that if you are using the "Microsoft managed" option, the "Report suspicious activity" setting will be disabled.</p>
      <p>To configure the "Report suspicious activity" setting, open the <a href="https://entra.microsoft.com/" target="_blank">Microsoft Entra admin center</a> (direct link), browse to <b>Protection &gt; Authentication methods &gt; Settings &gt; Report suspicious activity</b>, and confirm that <b>State</b> is set to Enabled and <b>Target</b> is All users.</p>'
    Types = @('IoE')
    ...
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Policy.Read.All', 'AAD.GraphAPI/GroupMember.Read.All')
    SecurityFrameworks = @(
...
<#
REFERENCES:
https://learn.microsoft.com/en-us/entra/identity/authentication/howto-mfa-mfasettings#report-suspicious-activity

REQUIRED APP ROLE ACCESS
- READ AUTHENTICATION METHODS POLICY:  Policy.Read.All (Least privileged)
- READ GROUPS:  GroupMember.Read.All (Least privileged)
#>
...
try {
    ...
    try {
        $authenticationMethodsPolicy = Invoke-GraphApiRequest @getAuthenticationMethodsPolicy
    }
    catch {
    ...
    $state = $authenticationMethodsPolicy.reportSuspiciousActivitySettings.state
    $groupId = $authenticationMethodsPolicy.reportSuspiciousActivitySettings.includeTarget.id

    if (-not (($state -eq "enabled") -and ($groupId -eq "all_users"))) {
        if ($groupId -ne "all_users") {
            $getGroupInfo = @{
                IdentityId = $groupId
                AccessToken = $graphAccess
                ObjectType = 'group'
            ...
            try {
                $groupInfo = Search-AADIdentity @getGroupInfo
                $groupId = $groupInfo.displayName
            }
            catch {
        ...
        $res.Score = 0
        $res.ResultMessage = $self.ResultMessage -f "$($state)","$($groupId)"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBnk76xmk5mAchI
# k6ovb7yhIN0b0suFhjX++VZOSaEvl6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ