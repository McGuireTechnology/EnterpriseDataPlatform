...
$Global:self = @{
    ID = 134
    UUID = '22739cb9-e1f1-4789-88b4-b7c6c3c9cc0f'
    Version = '1.313.0'
    CategoryID = 6
    ShortName = 'SI000134'
    Name = 'Conditional Access Policy does not require MFA on privileged accounts'
    ScriptName = 'AAD_RequireMFAOnPrivilegedAccounts'
    Description = '<p>This indicator checks whether Conditional Access policies require mutli-factor authentication (MFA) for privileged accounts.</p>
      <h3>Required permissions</h3>
      <ul>
        <li><p>Policy.Read.All</li>
        </ul>'
    Weight = 4
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Users that hold privileged roles in Entra ID are high-value targets for an attacker. It is critical for organizations to require MFA for all users holding privileged roles.</p>
      <p>The risk associated with compromise of a privileged user is equal to the permissions and rights held within the role assigned to the user.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/azure/security/fundamentals/steps-secure-identity#before-you-begin-protect-privileged-accounts-with-mfa" target="_blank">Secure your Microsoft Entra identity infrastructure - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/conditional-access/policy-old-require-mfa-admin" target="_blank">Require MFA for administrators with Conditional Access - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Conditional Access policies do not require all privileged roles to use MFA'
    Remediation = '<p>Users identified as impacted must have a Conditional Access policy applied enforcing MFA.</p>
      <p><b>Break-glass accounts may be an exception to MFA requirements, and can be added to an ignore list for this indicator. Organizations need to monitor their break-glass accounts to ensure that the accounts are not used without awareness</b>.</p>
      <p>Microsoft provides both a Conditional Access policy template, as well as step-by-step instructions for creating a Conditional Access Policy requiring MFA for administrators.</p>
      <p>If a Conditional Access policy already exists for privileged users, the identified users or roles can be added to that policy instead.</p>
      <p>The policy implemented should implement MFA for the following roles:</p>
      <p>Application administrator, Application Developer, Authentication administrator, Authentication Extensibility Administrator, Authentication Policy Administrator, B2C IEF Keyset Administrator, Billing administrator, Cloud application administrator, Cloud Device Administrator, Conditional Access administrator, Directory Synchronization Accounts, Directory Writers, Domain Name Administrator, Exchange administrator, External Identity Provider Administrator, Global administrator, Global Reader, Helpdesk administrator, Hybrid Identity Administrator, Intune Administrator, Lifecycle Workflows Administrator, Partner Tier1 Support, Partner Tier2 Support, Password administrator, Privileged authentication administrator, Privileged Role Administrator, Security administrator, Security Operator, Security Reader, SharePoint administrator, User administrator.</p>
      <p>Additional privileged roles can be added if desired.</p>
      <p><b>Implementing changes to Conditional Access policies and enabling Security Defaults can both negatively impact users if not properly tested. Organizations need to verify all policy and security changes and communicate them out accordingly</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'MissingMFARole'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
...
try {
    ...
    if (!$securityDefaultsEnabled) {
        ...
        foreach ($conditionalAccessPolicy in $conditionalAccessPolicies) {
            if (-not $conditionalAccessPolicy.grantControls) { continue }
            if ((Test-ConditionalMFAStrength -grantControls $conditionalAccessPolicy.grantControls) `
                    -and ($conditionalAccessPolicy.conditions.clientAppTypes -eq "all" `
                        -or ($conditionalAccessPolicy.conditions.clientAppTypes.Contains("exchangeActiveSync") `
                            -and $conditionalAccessPolicy.conditions.clientAppTypes.Contains("browser") `
                            -and $conditionalAccessPolicy.conditions.clientAppTypes.Contains("mobileAppsAndDesktopClients") `
                            -and $conditionalAccessPolicy.conditions.clientAppTypes.Contains("other"))) `
                    -and $conditionalAccessPolicy.state -eq "enabled" `
                    -and $conditionalAccessPolicy.conditions.applications.includeApplications -eq "All") {

                $includedRoles = $conditionalAccessPolicy.conditions.users.includeRoles
                if ($conditionalAccessPolicy.conditions.users.includeUsers -eq "All") {
                    $coveredRoles = $AAD_PRIVILEGED_ROLES.GetEnumerator() | Select-Object -ExpandProperty Value
                    break
                }
                else {
                    foreach ($role in $AAD_PRIVILEGED_ROLES.Keys) {
                        if ($role -in $includedRoles) {
                            $roleName = ($AAD_PRIVILEGED_ROLES.GetEnumerator() | Where-Object {$_.Name -eq $role}).Value
                            if (!($coveredRoles.Contains($roleName))) {
        ...
        if (!($AAD_PRIVILEGED_ROLES.GetEnumerator().Name.Count -eq $coveredRoles.Count)) {
            foreach ($roleItem in ($AAD_PRIVILEGED_ROLES.GetEnumerator() | Select-Object -ExpandProperty Value)) {
                if (-not ($coveredRoles.Contains($roleItem))) {
                    $thisOutput = [PSCustomObject] @{
                        MissingMFARole = $roleItem
                    }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultObjects = $outputObjects | Sort-Object -Property MissingMFARole
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAt5IU60HBqqEiR
# 52h4rzEMJ5WlVfwlhlHX+mai1/kEWKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg/eND0A+QX1CI94Eopm1pGOJpmSfw8w/n3IEyCwdqWrQwDQYJKoZI
# hvcNAQEBBQAEggIAYKfeQZXonyxrVHnlAXR2BwsPvIUODXfOUVokx6m2CfhupkVT
# hxurCqxjER3wI0GIM1TqFamx5vPaGrJmZX7kgVFmLaooJaSlZWgFCMNUWLeeLUK7
# 8QEoJg+ANYRB9JwlupyCHU11sGN/nqNLp78E4fjYP1Bly0v4GfygoXM9cHn9Acjw
# xyzrYiZC02YKH7PQ3cOJ0oPcVfiWcgfx20BeV4hyhWmHgzMOeaNRukFSPG6vYQYb
# Nydrl+zUG0ejxcBNWfIdR8CDD7wNGBOZ54tV0GmNfmg70/0T9DizIGE2SiFK6TEB
# iVlEuSoNQf/X2jH0Cgm/pMB5VHmJF4kXGY7z0NAg9Jn8nHQz+AWo5XGBEyiJ2P+Q
# er/5QLcQAPx8zv2UCCATS+k/eMzcjrMWi416pwjeehd2L8ZhX26ZNb989iH9zurD
# W8BoeCOJG9hkEnP4t5iwbnBmjxC1vWWToEJMwKX/Oo+2VP1f5j99hkELaab1Oui1
# uz3bDfs8K02a+7tcjY29wSWhsVNFjjR4dZK73XG/3qf+LiyaibAoi5IBWwCd011p
# SbHjKSCXPi2PWs93NbwH08rgZ+BJiGTi7rOcxTY44VgajCTHC0eW534e3wEKhObb
# pO2Dx6E9djPvYlkoVI9PGXb4GUOhk/GWNa8nnB+e9EGp7eEXDLg8YLBfWDmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCmC62iAV776oD9B14ANSUJ2sAFSxTps785
# OagjadxwGgIUbpft5rHhxTfbupAPEdMuy9aKnuAYDzIwMjUwNjEwMDc1MTAyWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCqbZRx/vx/CYJppvgXLWJHvGhHQzc1lfKJ4PwfbKNB
# 7DCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgB5CKqvmZ2TmjKxB/J/tqt3GneAFiHaKqAXyOuSpjnuiCLVXDdC9mzK2
# mcRL9n3/KTRvbT9raConT+g6qr9aF0DPB0gP+PCgeMA+KgN+LHLz+9C0K1RzdjaM
# 0m+a9rQKq/RCRiO5t0OzGD9AsGL60mxGePwQeC0QBqICVyQBHuXBz+nGN+bRkz28
# Gr4Q2kmSjzQ+ItEayuiZ4k+hgrGXTEUSg1YwyeU69K8fRnuge1VLOIIiAeH/tuC0
# rrRAClkP6qkVEIyyiErgsZ9xP4mNfaRLzGC23U6zhIz252zPR5Gq/N65S0Okqhta
# 2u3IlghQ1lAos8hLjrC6VlBamnY2+UGvEIC/jZGQpDUOjPiNPrfe3B4lUVb7762W
# KuiIAk9KgyWrZE+y7WiddgPRzxE4++zk69J8LXdlFLxuanutuvxTj6u5ZvkIvfOS
# ihDhvcBDRpSUmEsHgK2l0DGMsPwb8V4x2Q2K8Hp2W6rbrcAkiHDce9vnNLpuPVMw
# kwCVxk4Q6Q==
