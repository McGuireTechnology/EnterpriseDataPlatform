...
$Global:self = @{
    ID = 173
    UUID = '84a90bda-81bf-4aa3-96e9-75a2fe0409ac'
    Version = '1.313.0'
    CategoryID = 6
    ShortName = 'SI000173'
    Name = 'Conditional Access Policy that does not require MFA when sign-in risk has been identified'
    ScriptName = 'AAD_RequireMFAOnRiskySignIns'
    Description = '<p>This indicator checks whether a Conditional Access policy exists that requires MFA if the authentication request risk is medium or high as determined by Entra ID Protection.</p>
    ...
    LikelihoodOfCompromise = '<p>Entra ID Protection uses several signals to determine the risk level when a user authenticates to Entra ID. This risk level on sign-in can be used to evaluate whether the authentication event occurring is genuine or malicious. </p>
      <p>An attacker may compromise a user account in different ways, either by trying to authenticate as the user with a stolen password, or perform actions such as token replay. These attacks can generate a risk level in Entra ID Protection, that can be used in Conditional Access to force a request for MFA, which can prevent the user from leveraging the stolen credentials to access organizational resources.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/id-protection/concept-identity-protection-risks" target="_blank">What are risks in Microsoft Entra ID Protection - Microsoft Entra ID Protection | Microsoft Learn</a></p>'
    ResultMessage = 'There are no Conditional Access Policies configured to require MFA if sign-in risk is detected to be medium or high.'
    Remediation = '<p>Deploy a Conditional Access policy requiring MFA for risky sign-ins.</p>
      <p>Microsoft provides both a Conditional Access policy template, as well as step-by-step instructions for creating a Conditional Access Policy requiring MFA for risky sign-ins.</p>
...
try {
    ...
    if (!$securityDefaultsEnabled)
    {
        ...
        $requireMFARiskySignIn = $false
        foreach ($conditionalAccessPolicy in $conditionalAccessPolicies) {
            if (-not $conditionalAccessPolicy.grantControls) { continue }
            if ((Test-ConditionalMFAStrength -grantControls $conditionalAccessPolicy.grantControls) -and $conditionalAccessPolicy.state -eq "enabled" -and ($conditionalAccessPolicy.conditions.signInRiskLevels -eq "high" -or $conditionalAccessPolicy.conditions.signInRiskLevels -eq "medium") -and $conditionalAccessPolicy.conditions.clientAppTypes -eq "all" -and $conditionalAccessPolicy.conditions.users.includeUsers -eq "All" -and $conditionalAccessPolicy.conditions.applications.includeApplications -eq "All") {
                $requireMFARiskySignIn = $true
            }
        }
    }

    if ($requireMFARiskySignIn -eq $false) {
        $res.Score = 0
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCFbFm/8UOSof/5
# MWMNkDc7LkcOWV9V2c+Wvru0gW6U1qCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ