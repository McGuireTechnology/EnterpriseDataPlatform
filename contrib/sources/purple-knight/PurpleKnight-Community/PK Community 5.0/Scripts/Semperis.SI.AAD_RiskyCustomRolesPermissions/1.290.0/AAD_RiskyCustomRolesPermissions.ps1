...
$Global:self = @{
    ID = 232
    UUID = 'd80e37b8-8d8a-4964-88ac-5c51af1b0f3b'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000232'
    Name = 'Entra custom Roles with risky permissions'
    ScriptName = 'AAD_RiskyCustomRolesPermissions'
    Description = '<p>This indicator checks if a custom role contains risky permissions</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Custom roles with risky permissions can be abused by attackers in various ways depending on the specific permission that was granted. Potentially, custom roles can be used to gain access to sensitive information or perform malicious actions. More details on this attack can be found in the reference Semperis article.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/resources/custom-roles-permissions-security-indicator/" target="_blank">Custom Roles Permissions - Semperis</a></p>'
    ResultMessage = 'Found {0} custom roles with risky permissions.'
    Remediation = '<p>Review the permissions assigned to the custom role.</p>
      <p>Organizations should determine if risky permissions cna be removed from the role. If not, use of the role should be limited as needed.</p>
      <p>To review and manage role assignments, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/RolesManagementMenuBlade/%7E/AllRoles" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, select <b>Show more</b>, expand <b>Roles &amp; admins</b>, and select <b>Roles &amp; admins</b>.</p>
      <p>In the blade on the right, find the custom role. Select the role. A new blade will open with the role settings. Under <b>Manage</b> in this blade, select <b>Description</b>.</p>
      <p>From this view you can review role permissions. To remove permissions, select <b>Edit</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'RoleName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RoleId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'DangerousPermissions'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory')
    SecurityFrameworks = @(
...
try {
    ...
    try {
        $response = Invoke-GraphApiRequest @getCustomRoles
    }
    catch {
    ...
    $customRoles = $response.value

    foreach ($customRole in $customRoles) {
        $dangerousPermissionsFound = @()
        $rolePermissions = $customRole.rolePermissions.allowedResourceActions
        foreach ($permission in $rolePermissions) {
            if ($permission -in $dangerousPermissions) {
                $dangerousPermissionsFound += $permission
            }
        }
        if ($dangerousPermissionsFound.Count -gt 0) {
            $thisOutput = [PSCustomObject][Ordered] @{
                RoleName = $customRole.displayName
                RoleId = $customRole.id
                DangerousPermissions = $dangerousPermissionsFound -join ','
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAgQ5zA/a8ne6uP
# hIXvOzgi2lpmgYDf00AZtuE70gGITKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg