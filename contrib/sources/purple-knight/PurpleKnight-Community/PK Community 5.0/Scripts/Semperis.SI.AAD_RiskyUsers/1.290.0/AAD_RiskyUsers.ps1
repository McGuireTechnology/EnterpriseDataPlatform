...
$Global:self = @{
    ID = 218
    UUID = '931ddace-2ed0-4f70-998d-04e213e03f71'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000218'
    Name = 'List of Risky users (medium or high level)'
    ScriptName = 'AAD_RiskyUsers'
    Description = '<p>This indicator checks for risky users in the tenant with medium or high level of risk.</p>
      <h3>Required permissions</h3>
      <ul>
        <li>IdentityRiskyUser.Read.All</li> 
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Risky users are individuals or accounts exhibiting behaviors increasing the likelihood of security incidents or breaches, such as weak authentication practices, susceptibility to phishing, unusual activity patterns, accessing resources from unsecured devices or networks, or holding elevated privileges. These users pose significant risks, potentially leading to credential compromises, data breaches, or insider threats.</p>
      <p>Risky users pose a significant threat in enterprise identity (Entra ID) systems due to their potential to compromise sensitive data, systems, and resources. These users may exhibit behaviors such as using weak passwords, falling victim to phishing attacks, accessing resources from unsecured devices or networks, or exhibiting anomalous activity patterns. Their actions can lead to unauthorized access, data breaches, system disruptions, and other security incidents, posing substantial risks to the confidentiality, integrity, and availability of enterprise resources. Moreover, compromised user accounts can serve as footholds for attackers to escalate privileges, move laterally within the network, and carry out further malicious activities, amplifying the impact of security breaches. </p>'
    ResultMessage = 'Found {0} risky users with high or medium risk.'
    Remediation = '<p>Remediating risky users in enterprise identity (Entra ID) systems involves implementing a multi-faceted approach that includes comprehensive risk assessment, user education and training on security best practices, enforcement of strong authentication measures such as multi-factor authentication, regular review and adjustment of user access privileges to adhere to the principle of least privilege, continuous monitoring and anomaly detection for early identification of suspicious activities, establishment of an effective incident response plan for prompt containment and remediation of security incidents, and collaboration with security partners and regulatory bodies to stay abreast of emerging threats and best practices. By implementing these measures, organizations can mitigate the risks associated with risky users and enhance the overall security posture of their Entra ID systems.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'TenantId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Risk'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/IdentityRiskyUser.Read.All', 'AAD.GraphAPI/Organization.Read.All')
    SecurityFrameworks = @(
...
try {
    ...
    # The riskyUsers Graph-API endpoint requires P2 license
    $getLicense = @{
    ...
    if ($license.assignedPlans.servicePlanId -notcontains "eec0eb4f-6444-4f95-aba0-50c24d67f998") {
        $res.Status = 'NotRelevant'
        $res.ResultMessage = 'This indicator is relevant only when a P2 license is in place'
        return $res
    }

    $getRiskyUsers = @{
        Uri = "https://graph.microsoft.com/v1.0/identityProtection/riskyUsers?`$select=riskLevel,userDisplayName,userPrincipalName&`$filter=riskLevel eq 'high' or riskLevel eq 'medium'"
        AccessToken = $graphAccess
    }

    try {
        $riskyUsersResult = Invoke-GraphApiRequest @getRiskyUsers
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Risky Users' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $riskyUsers = $riskyUsersResult.value
    foreach ($riskyUser in $riskyUsers) {
        if (($riskyUser.riskLevel -eq "high") -or ($riskyUser.riskLevel -eq "medium" )) {
            $thisOutput = [PSCustomObject][Ordered] @{
                UserPrincipalName = $riskyUser.userPrincipalName
                DisplayName = $riskyUser.userDisplayName
                TenantId = $TenantId
                Risk = $riskyUser.riskLevel
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUAB