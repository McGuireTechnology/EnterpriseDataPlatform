...
$Global:self = @{
    ID = 215
    UUID = '746e2299-10c9-4610-868d-349db02aa65d'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000215'
    Name = 'Enterprise applications using SAML for SSO'
    ScriptName = 'AAD_SSOConfiguredWithSAML'
    Description = '<p>This indicator checks for enterprise applications (service principals) in Entra ID configured to use SAML for SSO.</p>
    ...
    Schedule = '1d'
    Impact = 2
    LikelihoodOfCompromise = '<p>When an enterprise application is configured for SAML authentication, either a Microsoft Entra generated self-signed certificate can be used for response signing, or alternatively, an externally generated certificate can be imported.</p>
      <p>Applications using SAML can be prone to Silver SAML, a type of SAML response forging, if an attacker is able to obtain the signing certificate used by Entra ID. Externally generated signing certificates create an unnecessary attack surface for an attacker to be able to obtain the signing certificate. It is recommended that organizations use the generated self-signed certificates from Microsoft, as the private key cannot be exported from Entra ID, preventing the ability to perform a Silver SAML attack.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/meet-silver-saml/" target="_blank">Meet Silver SAML: Golden SAML in the cloud (semperis.com)</a></p>'
    ResultMessage = 'Found {0} service principals configured to use SAML.'
    Remediation = '<p>Verify that the certificate assigned to the application is a Microsoft generated self-signed certificate.</p>
      <p>To review the certificate, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserManagementMenuBlade/%7E/UserSettings/menuId/UserSettings" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Identity</b>, expand <b>Applications</b>, and select <b>Enterprise applications</b>.</p>
      <p>In the blade on the right, search for the application, and select it.</p>
      <p>In the new blade, under <b>Manage</b> select <b>Single sign-on</b>. Under <b>SAML Certificates</b>, you can select <b>Download</b> to download a copy of the current signing certificate, and then review the details of the certificate.</p>
      <p>If the certificate is self-signed, it will contain <b>CN = Microsoft Azure Federated SSO Certificate</b> as the issuer and subject. Note, however, that an externally generated certificate could be produced with the same issuer and subject name, so organizations should be aware of any certificate rotations.</p>
      <p><b>Organizations must coordinate certificate rotations with the application owner, as a certificate rotation must occur in both the Service Provider (SP) and Entra ID, or else authentication for the application will stop working</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'ApplicationDisplayName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ApplicationID'; Type = 'String'; IsCollection = $false },
        @{ Name = 'PreferredSingleSignOnMode'; Type = 'String'; IsCollection = $false },
        @{ Name = 'CertificatesDisplayNames'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
...
try {
    ...
    $getServicePrinicipalsWithSAML = @{
        Uri = "https://graph.microsoft.com/v1.0/servicePrincipals?`$filter=preferredSingleSignOnMode eq 'saml'"
        AccessToken = $graphAccess
    }

    try {
        $servicePrincipals = Invoke-GraphApiRequest @getServicePrinicipalsWithSAML
    }
    catch {
    ...
    foreach ($app in $servicePrincipals.value) {
        foreach ($keyCredential in $app.keyCredentials) {
            if ($keyCredential.usage -eq "Sign") {
                $thisOutput = [PSCustomObject][Ordered] @{
                    ApplicationDisplayName = $app.appDisplayName
                    ApplicationID = $app.id
                    PreferredSingleSignOnMode = $app.preferredSingleSignOnMode
                    CertificatesDisplayNames = $keyCredential.displayName
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC