# This script looks for AZUREADSSOACC computer objects that haven't changed their password in 90 days
...
$Global:self = @{
    ID = 133
    UUID = '86e00792-a38f-409b-bd74-76e7ceecb93e'
    Version = '1.300.0'
    CategoryID = 7
    ShortName = 'SI000133'
    Name = 'SSO computer account with password last set over 90 days ago'
    ScriptName = 'AAD_SSOOldPwdLastSet'
    Description = 'This indicator checks the Entra Seamless Single Sign-On computer account, AZUREADSSOACC, to determine if the password has been rotated in the last 90 days.'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>When configuring Entra Seamless SSO, a computer account is created in Active Directory, AZUREADSSOACC. This computer account is used by Entra ID to decrypt Kerberos tickets that are used as part of the seamless SSO mechanism between Active Directory and Entra ID. Because this is not an actual computer that is a member of Active Directory, the password for it cannot automatically be rolled over every 30 days. If an attacker compromises this account, they can generate Ticket Granting Service (TGS) request to Entra ID and impersonate any user in the Entra tenant that is synchronized from Active Directory.</p>
<p>An attacker can use this to move laterally from Active Directory into Entra ID.</p>
<h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-sso-faq" target="_blank">Microsoft Entra Connect SSO FAQ - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-sso" target="_blank">Microsoft Entra Connect: Seamless single sign-on - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_password_change_msa_no_change_90" target="_blank">Managed service accounts with passwords unchanged for more than 90 days | ANSSI</a></p>'
    ResultMessage = 'The computer account used for Azure SSO has not changed in the last {1} days.'
    Remediation = '<p>The Kerberos decryption key used for Entra Seamless Single Sign-On must be rolled.</p><br /><p>For details on the process of rolling the key, refer to the Microsoft reference article.</p>
      <p>Organizations should also investigate the necessity for Entra Seamless SSO. If devices are configured for hybrid join, Seamless SSO is not used. Refer to the reference Microsoft article for more information about SSO with a primary refresh token (PRT) vs. the Seamless SSO system.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $checkAADConnect = $false
    $daysToRemove = Get-ParameterValue -Name 'IntervalInDays' -DefaultValue 90
    $pwdLastSetThreshold = ((Get-Date).AddDays(-$daysToRemove)).ToUniversalTime().ToFileTime()
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("pwdlastset", "lastlogontimestamp")
            baseDN = $DN
            scope = "Subtree"
            filter = "(&(objectCategory=computer)(cn=AZUREADSSOACC))"
        }
        $results = Search-AD @searchParams

        if ($results.Count) {
            $checkAADConnect = $true
            foreach ($result in $results) {
                try {
                    $pwdLastSet = $result.Attributes."pwdlastset"[0]
                }
                catch {
                    $pwdLastSet = $null
                }

                if ($pwdLastSet -le $pwdLastSetThreshold -and $pwdLastSet -ne 0) {
                    $failedDomainCount++
                    if ($result.Attributes."pwdlastset"[0]) {
                        $passwordLastSet = [datetime]::FromFileTime($result.Attributes."pwdlastset"[0]).ToUniversalTime()
                        $daysSinceLastSet = ((get-date).ToUniversalTime() - $passwordLastSet).Days
                    }
                    else {
    ...
    if ($checkAADConnect -eq $false -and $outputObjects.Count -eq 0 -and $unavailableDomains.Count -eq 0) {
        $res.Status = 'NotRelevant'
        $res.Score = 0
        $res.ResultMessage = 'This indicator is only relevant for environments with Entra Connect. No IDs for Entra Connect were detected in the environment.'
        return $res
    }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage