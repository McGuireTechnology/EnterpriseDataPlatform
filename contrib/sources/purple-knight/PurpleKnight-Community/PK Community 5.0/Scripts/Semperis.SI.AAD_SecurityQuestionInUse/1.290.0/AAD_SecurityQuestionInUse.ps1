...
$Global:self = @{
    ID = 45
    UUID = '16503719-3c1a-4766-830c-7aa4133652ee'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000045'
    Name = 'Security questions are in use'
    ScriptName = 'AAD_SecurityQuestionInUse'
    Description = '<p>This indicator checks if security questions are in use for Entra self-service password reset.</p>
    ...
    Schedule = '1w'
    Impact = 2
    LikelihoodOfCompromise = '<p>Security questions, also known as knowledge-based authentication (KBA) are a mechanism in Entra ID that organizations can use for authentication flows during a self-service password reset scenario. If a user does not know their password, these alternative means of authentication can be used to allow the user to reset their password.</p>
      <p>Due to the rise of availability of information about users on the internet, as well as the increase in social engineering, it''s increasingly easier for an attacker to gather answers for KBA scenarios. Organizations that rely on KBA as a means of authentication for self-service password reset are at a greater risk of an attacker using this to compromise users. NIST 800-63B, section 5.1.1.2, indicates that providers should no longer use KBA for user verification.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/authentication/concept-authentication-security-questions" target="_blank">Security questions authentication method - Microsoft Entra ID | Microsoft Learn</a></p>
      <p><a href="https://csrc.nist.rip/publications/detail/sp/800-63b/final" target="_blank">NIST 800-63B</a></p>'
    ResultMessage = 'Security questions are configured for self service password reset.'
    Remediation = '<p>Consider deprecating security questions as method available for self-service password reset.</p>
      <p>If the organization does not have a policy around the use of knowledge-based authentication (KBA), known as security questions in Entra ID, NIST indicates in SP 800-63B, under section 5.1.1.2, that providers should not use KBA.</p>
      <p>To review the existing security question settings, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/PasswordResetMenuBlade/%7E/AuthenticationMethods/fromNav/" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Protection</b>, select <b>Password reset</b>.</p>
      <p>In the blade on the right, select <b>Authentication methods</b>. You can review the settings for the number of security questions required for reset, as well as view the questions available under <b>Select security questions</b>.</p>'
    Types = @('IoE')
...
try {
    ...
    $getRegistrationDetails = @{
        Uri = "https://graph.microsoft.com/beta/reports/credentialUserRegistrationDetails?`$filter=authMethods/any(s:s eq 'securityQuestion')"
        AccessToken = $graphAccess
    }
    try {
        $registrationResponse = Invoke-GraphApiRequest @getRegistrationDetails
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Reports' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }
    $registrations = [System.Collections.ArrayList]@()
    $registrations.AddRange($registrationResponse.value)

    if ($registrations) {
        foreach ($userRegistration in $registrations) {
            $thisOutput = [pscustomobject] @{
                UserPrincipalName = $userRegistration.userPrincipalName
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCOSHFJsI4yspcn
# 2LEAGe2s4JNTepbwIwwXBFfXsAnVr6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ