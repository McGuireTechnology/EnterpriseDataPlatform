...
$Global:self = @{
    ID = 165
    UUID = '4c4029ed-2fdd-46c8-8d96-eebefd2d9799'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000165'
    Name = 'Guest invites not accepted in last 30 day'
    ScriptName = 'AAD_StaleGuestsInvites'
    Description = '<p>This indicator checks for guest invites that were not redeemed within 30 days of the invitation.</p>
    ...
    Schedule = '1w'
    Impact = 2
    LikelihoodOfCompromise = '<p>Guests are a feature of Entra ID that allows for business-to-business (B2B) collaboration with users external to your organization. Commonly used for Microsoft 365 and application sharing with partners, vendors, contractors, and other users who collaborate with the organization but are not employees, a guest user will be represented in the organizations Entra tenant. Guest users are not members of the organization, have a different risk profile, and are considered less trusted due to the organization not having full control over and insight into the user object and the devices they come from. Unused guest invitations can result in excessive permissions on resources, however, there are also legitimate reasons guests may exist in a tenant without redeeming an invite.</p>
      <p>An attacker may target a guest account for compromise, and access the associated resources that the guest was assigned permission to.</p>'
    ResultMessage = 'Found {0} Guest invitations that were not accepted within 30 days of the invitation'
    Remediation = '<p>Guest accounts that have not accepted their invitation should be evaluated for cleanup.</p>
      <p>Guest users can server multiple purposes in an Entra tenant for an organization. If guest users have never redeemed their invitation, then the intended access to the resources they have been assigned permissions has never been used, and may result in excessive unnecessary permissions.</p>
      <p>Guest users though may be created for legitimate reasons in an Entra tenant, including:</p>
      <ul>
      <li>Membership in an Office 365 group</li>
      <li>Membership in an Exchange Online distribution list</li>
      <li>Membership in a Teams channel for email</li>
      <li>Guest accounts being used as contact objects</li>
      </ul>
      <p>Organizations must review the legitimacy of why the guest users exist in the tenant, and make a decision on how to clean up or keep the accounts.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'ExternalUserStateChangedDateTime'; Type = 'DateTime'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/User.Read.All')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Persistence', 'Initial Access') }
    )
    Products = @(
...
try {
    ...
    $getGuests = @{
        Uri = "https://graph.microsoft.com/v1.0/users?`$filter=(userType+eq+'Guest'+and+externalUserState+eq+'pendingAcceptance')&`$select=createdDateTime,externalUserState,ExternalUserStateChangeDateTime,id,userPrincipalName,mail"
    ...
    if ($guests) {
        foreach ($guest in $guests) {
            if ($guest.mail -match "teams.ms$") {
                continue
            }

            if ($guest.createdDateTime) {
                $createdTime = [DateTime]::Parse($guest.createdDateTime)
            }
            else {
                $createdTime = "NULL"
            }
            ...
            if ($guest.externalUserStateChangeDateTime) {
                $stateChangedTime = [DateTime]::Parse($guest.externalUserStateChangeDateTime)
            }
            else {
                $stateChangedTime = "NULL"
            }

            if ($stateChangedTime -le $monthAgo -or $stateChangedTime -eq "NULL") {
                $thisOutput = [PSCustomObject][Ordered] @{
                    UserPrincipalName = $guest.userPrincipalName
                    UserId = $guest.id
                    Mail = $guest.mail
                    Created = $createdTime
                    ExternalUserStateChangedDateTime = $stateChangedTime
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC