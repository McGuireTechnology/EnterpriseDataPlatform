...
Param(
    ...
    [Parameter(ParameterSetName='Execution')][Parameter(Mandatory = $False)]$StartAttackWindow,
    [Parameter(ParameterSetName='Execution')][Parameter(Mandatory = $False)]$EndAttackWindow,
    [Parameter(ParameterSetName='Metadata',Mandatory)][switch]$Metadata
)

$Global:self = @{
    ID = 225
    UUID = '36e3878e-e65c-47a1-a9b8-0444ce978683'
    Version = '1.291.0'
    CategoryID = 6
    ShortName = 'SI000225'
    Name = 'Unprivileged owner of a privileged group'
    ScriptName = 'AAD_UnprivilegedGroupOwner'
    Description = '<p>This indicator checks for the presence of an unprivileged owner of a group with privileged roles.</p>
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Assigning roles to a group instead of individual principals streamlines the process of adding or removing users and ensures consistent permissions for all group members. However, a group owner can add themselves to the group at any time, posing a privilege escalation risk if the owner is an unprivileged principal. Therefore, organizations should monitor group ownership closely to ensure it is not assigned to an unprivileged principal.</p>
      <p>An attacker compromising an unprivileged group owner''s account can exploit this to achieve privilege escalation by adding their compromised account (or any other account) to the privileged group and inherit all the permissions associated with that group.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/entra/id-governance/privileged-identity-management/pim-create-roles-and-resource-roles-review" target="_blank">Create an access review of Azure resource and Microsoft Entra roles in PIM - Microsoft Entra ID Governance | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/groups-concept" target="_blank">Use Microsoft Entra groups to manage role assignments - Microsoft Entra ID | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} unprivileged owners of a privileged groups in Entra ID'
    Remediation = '<p>Review the principals assigned the ownership of a given privileged group, and determine if the ownership can be removed.</p>
      <p>To review and manage role assignments, open the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/GroupsManagementMenuBlade/~/EnablePrivilegedAccessGroups/menuId/AllGroups" target="_blank">Microsoft Entra admin center</a> (direct link), expand <b>Privileged Identity Management</b>, select the <b>privileged group name</b>, expand <b>Roles</b> under the <b>Manage</b> blade, and select <b>Owner</b>.</p>
      <p>Select each unprivileged principal and consider its ownership.</p>
      <p>From this view you can remove principals as necessary from the group''s ownership. </p>
      <p><b>Organizations must ensure that any removal of the unprivileged owners does not impact the ability to manage their Entra tenant</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'GroupName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'GroupId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'AssignedRoles'; Type = 'String'; IsCollection = $false },
        @{ Name = 'EligibleRoles'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UnprivilegedOwners'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UnprivilegedEligibleOwners'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/GroupMember.Read.All', 'AAD.GraphAPI/PrivilegedEligibilitySchedule.Read.AzureADGroup')
    SecurityFrameworks = @(
...
<#
Review all Entra ID groups that have privileged (assigned/eligible) roles to ensure their (assigned/eligible) owners do possess
privileged roles themselves. Each entry in ResultObjects represents a privileged group that is overseen by an unprivileged owner.
...
try {
    ...
    try {
        $adminResult = Get-PrivilegedAdmins @getRoles
    }
    catch {
    ...
    $privilegedUsers = $adminResult[0]

    if ($privilegedUsers) {
        $privilegedUserIds = $privilegedUsers.ID

        # Get all groups that can be assigned roles
        $groupsQuery = @{
            Uri = "https://graph.microsoft.com/v1.0/groups?`$filter=isAssignableToRole eq true&`$select=id,displayName"
            AccessToken = $graphAccess
        }
        try {
            $groupsResponse = Invoke-GraphApiRequest @groupsQuery
            $assignableGroups = @($groupsResponse.value)
        }
        catch {
            return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects', 'Read Groups' -TenantAppId $TenantAppId -AccessToken $graphAccess
        }
        if ($assignableGroups) {
            # Get all roles and eligible roles assigned to those groups
            $roleAssignmentsQuery = @{
                Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignmentScheduleInstances"
                AccessToken = $graphAccess
            }
            try {
                $roleAssignmentsResponse  = Invoke-GraphApiRequest @roleAssignmentsQuery
                $roleAssignments = $roleAssignmentsResponse.value | Where-Object { $assignableGroups.id -contains $_.principalId }
            }
            catch {
                return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Roles','Read Directory Objects','Role Eligibility Schedule Instances' -TenantAppId $TenantAppId -AccessToken $graphAccess
            }
            ...
            try {
                $eligibleRolesResponse = Invoke-GraphApiRequest @eligibleRolesQuery
                $eligibleGroupRoles = $eligibleRolesResponse.value | Where-Object { $assignableGroups.id -contains $_.principalId }
            }
            catch {
                return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Roles','Read Directory Objects','Role Eligibility Schedule Instances' -TenantAppId $TenantAppId -AccessToken $graphAccess
            }
            foreach ($group in $assignableGroups) {
                ...
                $activeGroupRoles = @()
                $groupRoleAssignments = $roleAssignments | Where-Object { $_.principalId -eq $groupId }
                foreach ($assignment in $groupRoleAssignments) {
                    $roleName = $AAD_PRIVILEGED_ROLES[$assignment.roleDefinitionId]
                    if ($roleName) {
                        $activeGroupRoles += $roleName
                    }
                }
                $eligibleGroupRolesOut = @()
                $eligibleRoles = $eligibleGroupRoles | Where-Object { $_.principalId -eq $groupId }
                foreach ($eligibleRole in $eligibleRoles) {
                    $eligibleRoleName = $AAD_PRIVILEGED_ROLES[$eligibleRole.roleDefinitionId]
                    if ($eligibleRoleName) {
                        $eligibleGroupRolesOut += $eligibleRoleName
                    }
                }
                if ($activeGroupRoles -or $eligibleGroupRolesOut) {
                    # get assigned and eligible owners
                    $getOwners = @{
                        IdentityId = $groupId
                        AccessToken = $graphAccess
                    }
                    try {
                        $owners = Get-AADIdentityOwner @getOwners
                    }
                    catch {
                        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Groups', 'Read Privileged Eligibility Schedule' -TenantAppId $TenantAppId -AccessToken $graphAccess
                    }
                    $unprivilegedOwners = @()
                    $unprivilegedEligibleOwners = @()
                    foreach ($owner in $owners) {
                        if ($owner.OwnerID -notin $privilegedUserIds) {
                            if ($owner.OwnerType -eq 'Assigned') {
                                $unprivilegedOwners += $owner.OwnerID
                            }
                            elseif ($owner.OwnerType -eq 'Eligible') {
                                $unprivilegedEligibleOwners += $owner.OwnerID
                            }
                        }
                    }
                    # prepare given output object as a new variable to simplify unit tests and code coverage process
                    if($activeGroupRoles) {
                        $assignedRolesOut = $activeGroupRoles -join ", "
                    } else {
                        $assignedRolesOut = "N/A" }
                    if ($eligibleGroupRolesOut) {
                        $eligibleRolesOut = $eligibleGroupRolesOut -join ", "
                    } else { $eligibleRolesOut = "N/A" }
                    if ($unprivilegedOwners) {
                        $unprivilegedOwnersOut = $unprivilegedOwners -join ", "
                    } else { $unprivilegedOwnersOut = "N/A" }
                    if ($unprivilegedEligibleOwners) {
                        $unprivilegedEligibleOwnersOut = $unprivilegedEligibleOwners -join ", "
                    } else { $unprivilegedEligibleOwnersOut = "N/A" }

                    if ($unprivilegedOwners -or $unprivilegedEligibleOwners) {
                        $thisOutput = [PSCustomObject]@{
                            GroupName = $groupName
                            GroupId = $groupId
                            AssignedRoles = $assignedRolesOut
                            EligibleRoles = $eligibleRolesOut
                            UnprivilegedOwners = $unprivilegedOwnersOut
                            UnprivilegedEligibleOwners = $unprivilegedEligibleOwnersOut
                        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAWW/A8Hu/obTQr
# 3ThYmH+0Yxg/OxU/P9sYK6JpSOk2j6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgJEcisY8UJxklOqgwkUOBNZJ0K6LuFMTCP24scnoEXH8wDQYJKoZI
# hvcNAQEBBQAEggIAEQ5r1kPqSbFBUiwEegwCFQuGsioi12HCLkUBeuvIu2ndeXrl
# NQRkBGZfPT3WK4TPtfeTcWbhZapyABtffxFm+VhyiwNiVEJmu+V0UrrHPK17IUXb
# iaiYS/tHkwklwL4z+s5XlXgUN7BmuZlbIcwFUQOo/ANpmd0sYpTD6aMWHCJiJ6Yw
# xcAxvwOiyHeocvUeqxH7ZF8dhTcu950tvi7ruXQ4tYigRMZXLEh3H3L5jXTSBu70
# 0b6Nelqqh+Xgc3VKeeanVlQ1nfYnyuAMB9ODeI5J++X7ubipcfSm9t6uou4FsW4r
# 1SI8yn9qTTiQrul4ROhZW7PmCqDlNsP0Ufnf/yEu+t26b/SXaDsCKvVvaJ6wDYfh
# TzAmGWq1UqYXfGh0YDJJc5M/U79StOmcLRGgfWfwf3VHcV6pFl8fl+ubfmw5Pijy
# uqwGQ8Lx+Myoebgab0A1CejullVNz1LelOSgTcqxZTJsKnd7V7E9qsewEIViJXi5
# OfUU8BVdvm0RkhRrrAiQNiHp3VSVn0oiOTHIF/xn69S/pTWSREIJWnSX3PEaOYZD
# EB/FnaGloYmfrBc5RNqcYVk07gfKMfTrXFpYtdXBwxCBO7BMHEv5ipKQBDJghVEh
# xp2IjuuQAtogLZXDkCB40m1BJ5bx9PsMcUV/FRZ3s5O1gziugjUtk5Junvihgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBvzFIVGmM0vw1rTudT3Z5f5tReIqOP4XwG
# AG5oz5w2/gIUKjhIn4XUY9q0kLofWw4Uhpa3c8YYDzIwMjUwNTE1MTIzNzE0WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDaRX6cV7ywgd8T8lEdQtgtNkddTNbvYf6C6KbnGS28
...
# CwUABIIBgCu1wMa0Yh2DDtA8ChbtlWhi1arrN5DNisv0GZTp8AGBaRAdsRw6MWyG
# ChEEszy89Vk820acWGjAlpG3rvsHl+JKYr9lrg6//wT92a8OayzhcRDw/vMu/ubq
# oKYn2NQDcMwVt6z/EPzIz/Dkx/MR4RJMfH2j1p/4vEahnQF0l84MVqgyeiuIHjyF
# JP9Fl10jqav/APaF1LlQ2yted7bIbxQbQ+g3sa/PtmFVKmNxRIyHXHwYaa57LUL2
# CmiWjft48e/E82SCiTnbO+6UDD92NHRXgXt/j/S8jejv8pvGBOptXrYl3vIiCuPC
# CfTNoythnJBqFKA+ckS+Hd8OwVkIBAQQTl4bC3Dx3YoXrSx0Yz3YxVaecUY6Cl/L
# y2uMIkrYBZ2ZvRmJc4AzIAJx4HTRd1XP9dYWv5DLH4fc4t+INzUs1aOY/gk2FZvq
# K+VjowUcqZRsd9tQ2z7Skq5g5zu/43OfqhuvqNBYW4TDWqzkNnB4zI8qnkghtpdc
# peyCLf3mgw==
