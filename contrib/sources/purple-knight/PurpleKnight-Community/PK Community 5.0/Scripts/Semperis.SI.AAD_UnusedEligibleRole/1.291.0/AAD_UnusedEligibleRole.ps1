...
$Global:self = @{
    ID = 286
    UUID = '7933368e-a218-4fdb-9f99-83882c677201'
    Version = '1.291.0'
    CategoryID = 6
    ShortName = 'SI000286'
    Name = 'Users are not using their privileged roles'
    ScriptName = 'AAD_UnusedEligibleRole'
    Description = '<p>This indicator checks for users who have not activated their a role assigned to them for over 30 days.</p>
      <p><em>For users eligible via group membership, audit log retention in Entra ID may prevent gathering an accurate ''EligibleSince'' date.</em></p>
    ...
    Schedule = '1w'
    Impact = 2
    LikelihoodOfCompromise = '<p>Users holding excessive privileges that are not used expands an organizations attack surface area. Attackers can use any privileges assigned to users for attacks, including lateral movement, privileges escalation, implementing persistence, data exfiltration, and others with a compromised user that has privileges assigned.</p>
      <p>Access Reviews can be used in Entra Privileged Identity Management (PIM) to manage the lifecycle fo assigned privileges for users.</p>
      <h3>References</h3>
       ...
       <p><a href="https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final" target="_blank">NIST SP 800-53 Rev 5 </a></p>'
    ResultMessage = 'Found {0} users eligible for highly privileged roles that have not activated their role in the past 30 days.'
    Remediation = '<p>Review the users identified and the unused roles they hold, and determine if they should continue to hold these unused roles. A privileged access strategy should follow a governance plan for assigning least privilege, such as that documented in NIST SP 800-53 Rev 5.</p>
      <p>You can review existing role assignments in the <a href="https://entra.microsoft.com/#view/Microsoft_AAD_IAM/RolesManagementMenuBlade/%7E/AllRoles" target="_blank">Microsoft Entra admin center</a> (direct link).</p>
      <p>Organizations should use the Access Reviews feature of Entra Privileged Identity Management to help manage the lifecycle of privileged role assignments.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'Assignment'; Type = 'String'; IsCollection = $false },
        @{ Name = 'EligibleSince'; Type = 'DateTime'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleEligibilitySchedule.Read.Directory', 'AAD.GraphAPI/GroupMember.Read.All', 'AAD.GraphAPI/Directory.Read.All')
    SecurityFrameworks = @(
...
- Read Role Schedule Instances: RoleEligibilitySchedule.Read.Directory
...
function CheckActivation {
    ...
    $getAssignmentInstance = @{
        Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignmentScheduleInstances?`$filter=principalId eq '$($checkPrincipal.Id)'"
        AccessToken = $accessToken
    }

    try {
        $assignmentInstance = Invoke-GraphApiRequest @getAssignmentInstance
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Role Schedule Instances' -TenantAppId $TenantAppId -AccessToken $accessToken
    }

    $assignments = @()
    $assignments += $assignmentInstance.value

    $activateAssignments = $assignments | Where-Object {$_.assignmentType -eq 'Activated'}

    if (-not($activateAssignments)) {
        $displayName = $checkPrincipal.displayName
        $upn = $checkPrincipal.userPrincipalName
        $roleName = $AAD_PRIVILEGED_ROLES[$roleId]
        $thisOutput = [PSCustomObject][Ordered] @{
            UserPrincipalName = $upn
            DisplayName = $displayName
            UserId = "$($checkPrincipal.Id)"
            RoleName = $roleName
            Assignment = $eligibleVia
            EligibleSince = $startTime
        }
...
try {
    ...
    $getRoleEligibilitySchadule = @{
        Uri = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleEligibilityScheduleInstances?`$expand=principal"
        AccessToken = $graphAccess
    }

    try {
        $roleEligibility = Invoke-GraphApiRequest @getRoleEligibilitySchadule
    }
    catch {
        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Role Schedule Instances' -TenantAppId $TenantAppId -AccessToken $graphAccess
    }

    $eligibles = @()
    $eligibles += $roleEligibility.value

    $lastMonth = (Get-Date).AddDays(-30).ToUniversalTime()
    $now = (Get-date).ToUniversalTime()

    $resultLogs = @()

    foreach ($eligible in $eligibles) {
        $roleId = $eligible.roleDefinitionId
        if (!($AAD_PRIVILEGED_ROLES.Contains($roleId))) {
            continue
        }

        $principalId = $eligible.principalId
        $startTime = [DateTime]::Parse($eligible.startDateTime).ToUniversalTime()
        if ($eligible.endDateTime) {
            $endTime = [DateTime]::Parse($eligible.endDateTime).ToUniversalTime()
        }
        else {
            $endTime = $null
        }

        if ($lastMonth -gt $startTime -and (($endTime -eq $null) -or ($endTime -gt $now))) {
            $getObject = @{
            ...
            if ($objectResult) {
                if ($objectResult.'@odata.type' -eq '#microsoft.graph.group') {
                    $groupMembers = @()
                    $getGroupMembers = @{
                        Uri = "https://graph.microsoft.com/v1.0/groups/$($principalId)/members"
                        AccessToken = $graphAccess
                    }

                    try {
                        $groupResults = Invoke-GraphApiRequest @getGroupMembers
                    }
                    catch {
                        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Groups' -TenantAppId $TenantAppId -AccessToken $graphAccess
                    }

                    $groupMembers += $groupResults.value

                    if (-not($resultLogs)) {
                        $StartWindow = (Get-Date).AddDays(-30).ToUniversalTime()
                        $EndWindow = (Get-Date).ToUniversalTime()
                        $startActivity = (get-date $StartWindow -format o).split(".")[0] + "Z"
                        $endActivity = (get-date $EndWindow -format o).split(".")[0] + "Z"
                        $auditFilter = "activityDateTime ge $startActivity and activityDateTime le $endActivity and activityDisplayName eq `'Add member to group`'"
                        $resultLogs += Get-AADAuditLog -AuditFilter $auditFilter -Token $graphAccess
                    }

                    foreach ($groupMember in $groupMembers) {
                        $memberUpn = $groupMember.userPrincipalName
                        $memberLogs = $resultLogs | Where-Object {$_.targetResources.userPrincipalName -eq $memberUpn}

                        if ($memberLogs) {
                            $memberAdded = (Get-Date "$($memberLogs[0].ActivityDateTime)").ToUniversalTime()
                            if ($memberAdded -lt $lastMonth) {
                                $startTime = $memberAdded
                            }
                            else {
                                continue
                            }
                            ...
                            $startTime = "more than 30 days ago"
                        }

                        CheckActivation -checkPrincipal $groupMember -roleId $roleId -eligibleVia "Group" -accessToken $graphAccess
                    }
                }
                else {
                    CheckActivation -checkPrincipal $eligible.principal -roleId $roleId -eligibleVia "Direct" -accessToken $graphAccess
                }
            }
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC