# This indicator will search for privileged accounts in AAD that have associated mailbox.
[CmdletBinding()]
Param(
...
$Global:self = @{
    ID = 221
    UUID = 'dbc58c9d-2df6-49cc-a732-ba370486211e'
    Version = '1.311.0'
    CategoryID = 6
    ShortName = 'SI000221'
    Name = 'Privileged accounts with mailbox'
    ScriptName = 'AAD_privilegedAssociatedMailbox'
    Description = '<p>This indicator searches for privileged accounts in the Entra tenant that have an associated mailbox.</p>
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Privileged accounts in the tenant that have a mailbox may be a prime target to attackers. Attackers may try to use the mailbox address of privileged users to achieve tokens to gain access, obtain sensitive data, or move laterally through the environment.</p>
      <h3>References</h3>
<p><a href="https://learn.microsoft.com/en-us/entra/identity/authentication/howto-mfa-userstates" target="_blank">Enable per-user Microsoft Entra multifactor authentication to secure sign-in events</a></p>'
    ResultMessage = 'Found {0} privileged Entra ID accounts that have mailboxes.'
    Remediation = '<p>Monitor the detected privileged users'' mailboxes to ensure that they are not being used in a non-administrative way. In addition, use unified audit logs to investigate suspicious activity. Also regularly rotate passwords and avoid using default or easily guessable passwords. Furthermore, enforce strong password policies for administrative accounts and train administrators on security best practices, including identifying phishing attempts and avoiding suspicious links. Another important thing is to enable MFA for privileged accounts, to add an extra layer of security by requiring a second form of authentication.</p>
<p>To enable MFA for a user:</p>
<p>Sign in to the Microsoft Entra admin center as at least an Authentication Administrator. Browse to <b>Identity</b>, select <b>users</b> and then <b>All users</b>. Click on <b>Per-user MFA</b>. Find the privileged user you want to enable for per-user Microsoft Entra multifactor authentication. You might need to change the view at the top to users. Then Check the box next to the name(s) of the user(s) to change the state for. On the right-hand side, under quick steps and choose <b>Enable</b>.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'UserPrincipalName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserDisplay'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Mail'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.All', 'AAD.GraphAPI/RoleManagement.Read.Directory', 'AAD.GraphAPI/Directory.Read.All', 'AAD.GraphAPI/MailboxSettings.Read')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Initial Access', 'Execution', 'Credential Access') }
    )
    Products = @(
...
try {
    ...
    # Return friendly message if zero license plans are valid
    $getLicense = @{
    ...
    if (-not $validPlans ) {
        ...
        $res.ResultMessage = 'There are no valid license plans. Please review the tenant licenses.'
        return $res
    }
    ...
    foreach ($id in $adminsIDs.id) {
        $getUser = @{
            Uri = "https://graph.microsoft.com/v1.0/users/" + $id + "?`$select=mail,userPrincipalName,displayName"
            AccessToken = $graphAccess
        }
        try {
            $users = Invoke-GraphApiRequest @getUser
        ...
        $url = "https://graph.microsoft.com/v1.0/users/" + $id + "?`$select=mailboxSettings"
        $getmailBox = @{
            Uri = $url
            AccessToken = $graphAccess
        }
        ...
        try {
            $mailboxes = Invoke-GraphApiRequest @getmailBox
        }
        catch {
            if ([int]$_.Exception.Response.Statuscode -eq 404) {
                continue
            }
            return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read