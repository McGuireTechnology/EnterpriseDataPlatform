# This script looks for mismatch between user pwdLastSet & unicodepwd
...
try {
    ...
    $startSearchWindow = (Get-Date).AddDays(-30).ToUniversalTime()
    ...
    $pwdLastSetThreshold = $startSearchWindow.ToFileTimeUtc()
    ...
    foreach ($domain in $DomainNames) {
        ...
        $filter = "(&(samaccounttype=805306368)(pwdLastSet>=$pwdLastSetThreshold)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))"
        ...
        foreach ($result in $results) {
            ...
            $pwdMetaData = [datetime]::Parse($result.Attributes.attributemetadata.LastOriginatingChange)

            $attributeDifference = New-TimeSpan -Start $pwdMetaData -End $pwdLastSet

            if ($attributeDifference.TotalSeconds -gt 30) {
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) user(s) with a mismatch between pwdLastSet and unicodepwd.  This could indicate an attempt to bypass the organization's password policy."
            $res.Remediation = "Ensure that users change their password at least once every 6 months."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBtRSeMMttVHkqS
# igrk/XEMbYxE3x4WdPPR5Td8yuYHl6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgmXFYqVme/mHq94XJotxzg/xXvNcoxQLgHnqoX0z4xv0wDQYJKoZI
# hvcNAQEBBQAEggIAL26yti5alp8EGShBfU4bhNzdDtnwo8/DTneoJPdRWnTvkwft
# vRJHLl2oN9wyl1chQRe93gzM70FlMGvUpy2Cu6QfXPLcEeGtYe04qWNFLON7PTxC
# vSLhEMa9u1uHMqiEByb+wdsCy++eycJMSi++QgXKtt9lJBnYt+Zb3YvhpaOl5bhh
# ep2ZjFi+uf3yJStArxLt2zC4Y1bn0jjeTg0TpN1MLPUMd4fOuDa8Umg1XxZOh0NK
# F7kNGjf8+pU2iQqaGaVTyqpXOYdxRDMkIkZiMU5KxzlpIymbNFQnHihRSjxHhWPE
# E/JSaKvJTDrLtuRnpFC2qwoRvWvfK8ArMF/d/SiSrp/wtsqpJN+utTHFLN0OkCUJ
# h/5nXJ33JqX4IKGrzR5VZN3wg57RnFhw7zFIhyOSkLVUr6ocyrcysUVCZweJeOsX
# h/aGYdEe/wYb/H3kV+Y4qpvDfDCDh9tEJ5xJS0TmbOVYQvJik5H/U1eSvN7KgZ+q
# qbr3g6NBxwf6i17eoVUJdDrfH/XvRI0hVhLAz0dsLT8maPJId4ymEc9XNkJ2rUsY
# 0397f/QEz8oas3U4xOzjkd6bRAYP2rL9++OkvUTI+xy6nG690l5mES5n63alusdJ
# ekGU5zNB05ViStxASVl+fl0YggEny0Q7yumI4sTwfM5jog0Yx5zV4yWOmPuhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBnhGR3iTIkepXrV/TISFicKQuUPmWnTL7g
# CVmoauaP1QIUEMycl+/tMXi13RDf2fyrGRAb36EYDzIwMjUwNDE2MTMwNjIxWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBiFLSFA+EZsTsKvOcfTYXg9k7Ybqrdz4FntksSS9Uh
...
# CwUABIIBgDzojI9hGbAAxcQO+sYmiWy0JKhlIbqUn3AqXGgUcxIaXx+nfcx3wB75
# OsN4TlJOnWBvXRxdBt11efIPiLk3ZgButmPgBUpLWXBCJtCfcaea28nQ4MfpZ40b
# r+M9Vg6o9txuQd/bC/aeY8D7og8z1XSZH4NnNdpmEUFHEhwvvEC4u5R8HBZytCrY
# fQtiNNCC9+6sIpd8FA2agEZLailZo8riCe8xgT4wHscpnmkQWyBBXmQJ7S5bcj3H
# D8FI3MybBOhuIlly2eTxSlALtvn9sO1uat/h3ij4o0XYAp6yTlO5PM8X7RctjeGz
# P5VyY1gIglCSvKvcSMz62Y53e8XeUb6cWTjc9OojAXFKrRzZ1ri37RD+jvznJ9GO
# uMZI4TckRBwPonxwyxq5kXbS+fouxvrFg0kWDMAX11Fn3Sm7I5/KHN3Dan5WKkZ+
# LzP1tB8gFqh5O+fVSaMnOnRWa/Gnc9zqmU7AIRtvhoaHnv4eCSeHtV3qervXUgcS
# GHqaouhBxA==
