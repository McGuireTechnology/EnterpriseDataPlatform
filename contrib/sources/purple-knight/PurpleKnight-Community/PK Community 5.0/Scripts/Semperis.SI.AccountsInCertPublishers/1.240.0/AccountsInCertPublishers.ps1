...
try {
    ...
    $CDPDN = "CN=CDP,CN=Public Key Services,CN=Services,CN=Configuration,"+ $forestDN
    $cdpContainers = Search-AD -dnsDomain $ForestName -baseDN $CDPDN -scope "subtree" -filter "(ObjectClass=container)"

    $regex = "(?<=CN=)([^,]+)"

    foreach ($container in $cdpContainers) {
        $dn = $container.DistinguishedName
        $computerName = [RegEx]::Matches($dn, $regex).Value[0]
        $computerNamesInCDP += $computerName
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        $certPublishersSID = $domainSID + "-517"
        ...
        $searchParams = @{
            ...
            baseDN = $DN
            scope = "subtree"
            filter = "(&(objectSid=$($certPublishersSID))(objectCategory=group))"
        }

        $certPublisherGroupDN = (Search-AD @searchParams).DistinguishedName

        if ($certPublisherGroupDN) {

            $groupMembers = [System.Collections.ArrayList]@()
            $groupMembers = Search-AD -dnsDomain $domain -attributes "msds-membertransitive" -baseDN $certPublisherGroupDN -scope "base" -filter "distinguishedname=$($certPublisherGroupDN)"
            ...
            foreach ($member in $groupMembers.Attributes.'msds-membertransitive'.GetValues("string")) {
                $extractedNameFromDN = ($member -split ',')[0] -replace 'CN=', ''
                if ($computerNamesInCDP -notcontains $extractedNameFromDN) {
                    $thisOutput = [PSCustomObject][Ordered] @{
                        DistinguishedName = $member
                        DomainName = $domain
                        GroupDistinguishedName = $certPublisherGroupDN
                    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAHL6I0Jmr/Xx8M
# qUdSnO9oJgMEa+w2e3MLMsg0FGqobKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgbefTqEc6KkLg4JfQRafdo5q2XjenuUuvlfly5HlVwQQwDQYJKoZI
# hvcNAQEBBQAEggIAHxwvf1rOAwFLBRwSteoB0v2bhk6ZNN5k7X9GbHOPE2hVRI5l
# C8NO1yR+9cWygyl4C1kgFZbdZy4iIJF0NnZ+mDHRcv8RhASAd5blbp+HHlecc5lm
# Q9mPK8iEwIuZOR6EogIEFnGdpyeDS8mJCXfXwmmxkxgMGGhpQmUvoNYxQuaSuSA1
# lW6BZGUHjuA4h9diSt8TRAcceJawMzamtr8c3LIYt7ol5El4Ght0J3P1mJ8SUOxd
# 4lNITpbhXSo2xkUGoewkvBlMu34KbwKl3lWWVa4R+EAKg3UxVPtk3o6Auv5Jgv4A
# YbvlJEoHWB00ZJn1ElY34nlKnusVg8I74qZl1W8VQD7J0fTd/pL34g9JwopQHcXB
# x9tsD/KWEpFVjpo+URsB6mUE9+HOMgpoHng2wvPKcQeT3coulJLDDvHePQoBED5M
# L51onJWcdp9jV0fho7yjp0RUtkgOzK6Fo90f09ZQ5iFK6U0QcoZuzsWjruJo8MGN
# kIPB6iYstt/1t+6hapMXOyq14KBR0XgGX9zU/zzabT4JLSvu7CRh+EPGkyh20yj/
# auHpD7ukso/DoXcvHw/XEXgmnI85GozjvUgSPRVDIwxsmBCSVeo5rdwhUk02UJPT
# GUzlUEYLtfje0nKxfgrOnyR7dMtOIDde03flGQFV0qOdrahQaOf189kZGcehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCLKnXqOybraNFTgPfKxyXDpEl6dCQZOAWZ
# eAOizC4BuQIUQUCs7lM8odLdO2IeM8E4HbkO0V0YDzIwMjUwNDE2MTMwNjI1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBZJBClYsnLVNKvhPstooyJXtPlIzD4Mp4lrZxdH6mF
...
# CwUABIIBgKbSca224d0Iy1lgKMiAQdaM1V0y9dh0SVsWqPssLPn4fJPlz2TY06t6
# KjtlziyIkfqt99nOuVmbg8FRH3pz6grZeNyis0rm08MRF9Z0Ol6vq0J0VCNvDgoW
# bBmHgloG6AziF5XfV5SvDRzDi1UjC85x/kHMtvVNu1SHi62GeLKCA3yEnSjwbabB
# CPZOcDPxQbFyuYqNgRGtAcPamTVeVpR62osBSshhKqanAblCJJ+bLalDf4s3qZzD
# 6SugPuPY7oJUiSQw3y0BHpE0pQDVE7EFc6P0mV2W94FtYrXORlihCZndhUUcNdgn
# rjcIqgeafCcdaVo5OFwsswWiB3iNT0JK9gbkxX6y/SmuGLygoHEEmSfse4HfZbh2
# 3ajdPtrQqtWsnwzhzBeCwtki7FToaYoPvK+ZmZs0oqhf8XYxNYgo8/AzNKDbepDR
# rjaUTG4KiE+fFn9Tpfy3lvgIwo54+h1arymrGgl4x9vCP6x5znX98HwvNoNihrqU
# N1smXqNWmw==
