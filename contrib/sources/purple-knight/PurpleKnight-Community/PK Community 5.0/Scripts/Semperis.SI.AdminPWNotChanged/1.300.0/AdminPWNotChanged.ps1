# Built-in Administrator password has not been changed in last 6 months
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 4
    LikelihoodOfCompromise = '<p>The built-in Domain Administrator account is a well-known SID that is easily discoverable, regardless of attempts to obfuscate the account, such as renaming it.</p>
      <p>It is recommended that this account is not used for administration of Active Directory, but due to lack of use, this account in many organizations also goes unmonitored. Attackers may target this account for brute force password attacks.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/understand-security-identifiers" target="_blank">Security identifiers | Microsoft Learn</a></p>
    ...
    Remediation = '<p>It is recommended to rotate the password on the built-in Administrator account every 180 days, or whenever there is the possibility that the password has become known or compromised.</p>
      <p>The built-in Domain Administrator account is a well-known SID that is easily discoverable, regardless of attempts to obfuscate the account, such as renaming it.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $filter = "(objectSid=$domainSID-500)"
        ...
        foreach ($result in $results) {
            ...
            $daysSinceLastSet = ((get-date).ToUniversalTime() - $lastChange).Days
            if ($result.Attributes."pwdlastset"[0] -ne 0) {
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = $self.ResultMessage -f $outputObjects.Count, $daysToRemove
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBDkPNR+ZxJ+Q6Z
# R+Aa/vXgT7QHAGH2Pr2yAG6nxt+cK6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgqGNuIIyFeLe93HqpyNNHo6f3A7cDt8P8p6PremIWV5swDQYJKoZI
# hvcNAQEBBQAEggIALEAFyB02vjKyM6JVBlaL2S9p8FtYeMzRVUM2h9LIquC97QZl
# 9WIClQJJKFs3eRuSg+Fp00R1+mUk1/9IhARlv0OqiW90T8t0NeieY/hCUML4LRYA
# 6VpcrhzaenckaCPbYfGjpAEKM5Ne6Gis4kEi+kGtb9Kc1AXFfUnI0dZOT3z4bf7u
# AehYILuZLfzYqf40T2xfjK8nZr1ndne1f1uBfAO8QZdvdypO0rK47oAFflDpbkLW
# L4koatkEVqnI7oaJg2gh+6SLx8hkkdjBtidHh6ccodXuWtMgo7eqw8s0ob7rOnn+
# ue7wwN9mlUaYRQFTuj7hxQ5P6eEEuiQF41H14kmn+M5pJIA07T644ZpcaArOfllo
# K/GKWV1Wws9lu184yeWJnfG97Nelm3zxBBBi8EZgGxeDY3Gt1Jg1pN2Vmd/eXTrC
# blPoV3AML+aSfinyx5H+rsrRROGF8awnGRnT6PWOiPgP5oU8AA6ChVec8o0rzA4u
# 5JiLXy9GJsLHYwdIROSEKA59kINUql0A3/DeoU5t9dQ2ER7ZxWlEoDKD/etTVFH3
# 8N4M6FbRH1FNxJRCFdxHaB65iFdkVyaJoaSE5ZODy/chl4ipimtvHx4tSVoXSAN/
# 0z4wrOqByg34gOSsgLjyhKwoz0JjCWg6Og0NuM2FLV1Q+htijOhO/kb2psOhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAvsCKzceekPKPWlAIymh9k0Khzp8pVryEh
# 5doP5ou7BgIUAqMk2gE/bk0v5F3ao5Rwuvlbn+UYDzIwMjUwNTI4MTIzNzM2WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAc4dK4ESKSdPmvY3Nmz/PMS+/ngacLdeZQlrtrDv4R
...
# CwUABIIBgCUC+aL/AfLqgjnsSV198OARKnLNEl7fkVbsKzqoX1sBagBJlLi4acek
# J+6rjw75Gb23g08CBNW/ufxzVo7GjU/iEzXmlPK+rfYTXLoiTxT1JiOJRPQ79AAh
# zl3JsrEF9SCZbq/uYHdG60pz0lo3OT/FmkLAgGlyWBckRk/bRnFSaSWDmjkmjvOY
# Ok2wHOfeV1RoKSEWPjImAuSDu+6AVlL/LtDPR3bn2zpPlxNEYNuep6WfPrE5fkQJ
# z2apN4SSZWtOhK6W61xWiyJg2dpZRzbLk5LhyisiXAdOsLiXhQMpOp2I1n4iPxXZ
# IeBkuPoLqUVXMCZGqLMf16cr+KUZAQSYcJ92LB9LhG0/o8TGEe9WjxfWL65kwEav
# xjyKznZ2CI/0bT0IppPJcCP/FUfNzUXndyugxAg20OXzo5Y2rl0szIvIJHQI5Whb
# lW/0mazLpwsrui2ly1KHwu2choiTuLFNeQy2GtrOzPPrdK2j1iKrZXOS3/djv37m
# U0dAWZY8Jw==
