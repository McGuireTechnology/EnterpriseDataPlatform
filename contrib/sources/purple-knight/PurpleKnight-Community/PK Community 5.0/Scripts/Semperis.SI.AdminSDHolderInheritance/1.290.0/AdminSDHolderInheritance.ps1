...
$Global:self = @{
    ...
    Remediation = '<p>In addition to <b>disabling inheritance</b> on the AdminSDHolder object, organizations should investigate the ACL on the object to ensure it aligns with their security policy. Use attack path analysis software, such as Forest Druid, to analyze attack paths to privileged users in Active Directory.</p>
      <p>Organizations, if changing the permissions on AdminSDHolder, should have an established process in place for awareness.</p>
...
$attributes = @("msds-replattributemetadata", "ntSecurityDescriptor")
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $inheritedACEs = [System.Collections.ArrayList]@()
        ...
        $scriptblockFilter = {
            ...
            Update-AttributeMetadata -Entry $entry -PropertyFilter $propertyFilter -MinimumVersion 1
        }
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $baseDN -scope "Base" `
        ...
        if ($results.count -gt 0) {
            ...
            $inheritedACEs = @()
            $inheritedACEs += $securityDescriptor.Access | Where-Object {$_.IsInherited -eq $true}

            if ($inheritedACEs.Count -gt 0) {
                $thisoutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $results.DistinguishedName
                    Attribute = $results.Attributes.attributemetadata.AttributeName
                    LastChanged = [datetime]::Parse($results.Attributes.attributemetadata.LastOriginatingChange).ToUniversalTime()
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA4t/Vvqvhrc+yj
# UW/YEthYCFrHBbzCK1XL+Oieqnh5yaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQglO6I8zA2nAP7qVd5KoLjSQz6tizxur0UwVXZqW+pocQwDQYJKoZI
# hvcNAQEBBQAEggIASf2UShGud0yFRDEiMV37PCJcUpduEusXP7ExoMUP6hnKhsKK
# lijg+KtEcNDQeG3GSl0AwbacuY9ULlvpWl8FMI6IsfMoRdhkUVUHUGVxXfId8jlK
# 6dtzhWbeOh6V0BfnClRpHgT7H53wSlsthc5YoislIDv6HEzmjPj26JkRedG1yTEz
# hd/5B/HVQQnZik2nTqUcziOIfLijgfhzgvKwEInJCJpfTAk1Q8Uyng9g+UEG5M36
# KEkyXDvq48FA4MnsJxdnMkKRTaUzuCZ+CCPxmTO70AkYoOG/CWTs4tJNKQTCTUaa
# fg721Zm4UseabZE3P+/WO8h0jcsBPdSUQbTiaBHoNxqBsW+C/ASJKOACGq7cGI53
# F3TaMREcVlRa81SZIj7m+xPSyYvAgecRo0MLYfzvFKQu8z3ScESj3YLaxwKJWmMz
# mH2fMdGgtzdFAv+ghKXCv9zUphO5XvqUjSyz6lxs7jA4EJwqj8hTXK3t7iIR61J7
# 5C3QMgadFRE/X/7eNEOF3cZvVDJVfE3QwnjVz4tnxfEKRBsTLWapWqSMokJw2FsT
# cQmvWLAy00l87n3eJwyyyGNwlOZTD7oIm/mvE/8AdZs6cFkTu88+LJId+r3BlIYD
# AKYD4vHuSwCKjwWAVYw0cDTyYMVu80W7cXd/APMjzUdyFbEzHfdYBFup8pKhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCByCeO71iMlrfZUJhsV5rfYuTICLFdViwyD
# cdkRFpa15wIUM1jiM6VeCASe7WN3KRjWGgvRxT4YDzIwMjUwNTEzMTg0NzU5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBULjUsATaI8/jrIDR5LP6So2UUgLMhdwCiAlLoahcE
# cDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgL3gUIl7VrJWLRjwgDGNVOg1Kd5ncSh6WRmTuK+dYoY9a9cZSFDI3xl9
# s4tUMhfAi4+gZsfTVLcGItgSke3GMB32mSWSdpX2A9TudggUFW7VJ8rsZBJVTr3g
# ZBSolF7yW9L737ztsvkr9K9ltFgC+ilW3qXZxAU5pOPDKmPnT5dlQElRB1Wi/Dlh
# 3GnEM7sYrdQrmqbYT1JGEdw3tj3dKnzAP7sIDOwC4GSiL/wyaYTjovJjEctOGRYp
# L4jbgMKdxo5xoHYrVq82DgqNdY6eq5dRPzJaQhDmYkvUco72Ip3yslEC4v/TRaqB
# iGe8Kfb5o5w06WgHon5da0arOqRAzSwl0FipYR3JzX7CoppT9pcc/bEsQjbhckCs
# eOhxOich/UfI59AMJ0jZaLRYucEs7uH2m9w7arxeoBs0QstgrOK0q69QmpErj8kO
# lBzNvUD579bvRJfOsuEYpMLfuSnStkBHHOIi7jugrwsxugtHfwwdMtlsnaZ5ak5f
# cib2Pb57FQ==
