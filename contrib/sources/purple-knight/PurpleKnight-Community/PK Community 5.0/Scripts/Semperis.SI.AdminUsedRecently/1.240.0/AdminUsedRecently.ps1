...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $syncInterval = Search-AD -dnsDomain $domain -attributes "msds-logontimesyncinterval" -baseDN $DN -scope "base" -filter "(distinguishedname=$DN)"
        if ($syncInterval.Count) {
            if ($syncInterval.Attributes."msds-logontimesyncinterval"){
                if($syncInterval.Attributes."msds-logontimesyncinterval"[0] -gt 14) {
        ...
        <#
        There could be a case where the $EndAttackWindow is before the $startDate and the two time frames
        are not overlapping but we don't address this case because it should be interesting that the built-in
        admin was used after an attack has taken place
        #>
        ...
        $int8Date = $startDate.ToFileTimeUtc()
        $domainSID = Get-DomainSid -dnsDomain $domain

        # We are not using the UpdateMetadata attributes because we do not need metadata here
        $searchParams = @{
            dnsDomain = $domain
            attributes = "lastlogontimestamp"
            baseDN = "$DN"
            scope = "SubTree"
            filter = "(&(objectSid=$domainSID-500)(lastlogontimestamp>=$int8Date))"
        }
        ...
        if ($results.Count -gt 0) {
            ...
            foreach ($result in $results) {
                ...
                $thisOutput = new-object psobject -Property @{
                    ...
                    EventTimestamp = [datetime]::FromFileTime($result.Attributes."lastlogontimestamp"[0])
                }
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.Count) domains in which the built-in administrator was used recently."
            $res.Remediation = "Ensure that the built-in domain Administrator account is not used regularly and has a complex password known only to highly privileged admins."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAB0A43mI0RNMvO
# La8bbHDKtQkqpo2IVzZEie2I+ApxiaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgR6hZT7XEdTduS8PBeH2UcutigeY1I6B30zPFJV0ICc8wDQYJKoZI
# hvcNAQEBBQAEggIAKNXwSV6+qkTHY/ZrYXrzjqJ6WSTMvYeugjgsoDvIFssOiqEV
# Mnw56jdH9E3ZEecqSquCpjm07mS31T8bSc3xwgg5RXHnczZON2jHB4XJTxUnw72j
# UWPTRryw7wrnOBbrt4mmFgQ+P0NMB+2wNXeyVmej3i06rolxvXkOLCt8/tXJW+lr
# wJnQXzgIoVp4EoILW+lLWplClIwSwtdMgx3BK22Eg8/JuUYeTTg85dVMwPz/v+Ok
# 6FCcq+KJ6idKvWFB7Sse8Iu05es1uF0tYVYI7JcuRDCGjsk0K8LhK7YnJSp+Tsys
# imyFpz3EZcfNgvyqZrtOyTQkJJAR/IOdYS9/6lRmoH+pZNkrv3Bcsu13mW8j3sxX
# iM0PYwxBp2dBjRez+rgZfls86cShF8UVY7Kj+uE11YtCQ4S7LmBl81hhfG/UilE1
# oBxwiFAZUDUGfcIB/oqtMralR59BmUp4aWmJnhGcI1ohPuSZZmd9z5iNJhY3lhKk
# a8Bigvj0WDXQe8TPSvVtE634W7SshZa8k5dnukLwpXxfozWFmHR7mkvg1XklG7a6
# ZL40BO57MrM9fafXgNLi8STvJCOjy42UdUD6bGXR8W58RVelVu80cQTMxyqw9HM1
# Kv4arsMuJabBk56iQtHS+BmIC5JGRIyXmjyeUW/jemOUELFbM5l6S9yioMGhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCA8VWjRV30NF4STGkQcaoQQyCct5/f068o4
# 419hCJ4dmAIUAXWEkCrRVGB9CubAoH2RPhJVfdMYDzIwMjUwNDE2MTMwNjQ3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAKpdBtFfaZVSaeOvwbjgA4JtGbHMvbp5iIq7Wgir6D
# mDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgH5bu8lCx0MXNj2FFw+B+neXygwmUTMYnfFcS0UwkhU0In0+spoMQdbO
# 1B/CBVsC1C0AFbKA3lwFW48zwsYI4mkEyvIfKjY7FvuHzeinH+T6nbdQCwN/EsBw
# 9L+K0MBtfPDA/NKeIVAcynj6PPrgswcT3XTfsXFeJPirBtiNvVv91V3tuprBhiav
# VeKQ2LlQd9hHimSte7DyTZwJaTFbYgQaJ+nZKdYpJKzMhYuyCzl17Z39V5lCwu0P
# EvKCQ4gxoUo7HqNDPHOK2LfOHXgBnX3UpCI8gcvZkkwgX3XaZYc/LJirJIxNzk7X
# ItJelMYDHTg1BlSmztfLz17ZMHVD2wVUSLTrnBGbTKw9NPWs78wsgasJLcw29Gxd
# fejSLcB4JqHTuWBun349D/TWOVZVWgFkjGzLIuZA66xuGXqGUI6rXdNWXeKVYS4y
# R3eWaU/sWFtAwIs8QwqvVEDAdnTGAP2ucSpKoihTF131VULa6PbpnF21KqNZtUda
# ybIyDMnh/A==
