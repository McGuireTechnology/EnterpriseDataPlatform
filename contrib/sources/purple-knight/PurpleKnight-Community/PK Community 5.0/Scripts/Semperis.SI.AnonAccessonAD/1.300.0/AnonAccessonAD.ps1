...
#This script checks each domain to determine if anonymous access to AD has been enabled forest-wide
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>Anonymous access to Active Directory allows an attacker to enumerate accounts and perform attacks like password spray, as well as to enumerate the domain to gather information that can model attack paths. This is a significant risk as the complexity of AD often presents many opportunities for attackers and anonymous access allows them an easy way to find such opportunities.</p>
      <h3>References</h3>
      <p><a href= "https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/e5899be4-862e-496f-9a38-33950617d2c5" target="_blank">6.1.1.2.4.1.2 dSHeuristics | Microsoft Learn</a></p>
      <p><a href= "https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_compatible_2000_anonymous" target="_blank">The "Pre-Windows 2000 Compatible Access" group includes "Anonymous" | ANSSI</a></p>'
    ResultMessage = 'Found risky configuration in the forest that enables anonymous access to LDAP operations.'
    Remediation = 'Disable anonymous access unless it is absolutely needed. The dsHeuristics attribute on the CN=Directory Service,CN=Windows NT,CN=Services,CN=Configuration,DC=<forest_name> object should be set to disable anonymous access.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $searchParams = @{
        attributes = @("whenchanged", "dsheuristics")
        baseDN = "CN=Configuration,$DN"
        scope = "SubTree"
        filter = "(&(objectClass=nTDSService)(cn=Directory Service)(dSHeuristics=*))"
    }

    $result, $unavailableDomains = Search-ADConfig -forestName $ForestName -domainNames $DomainNames -searchADParameters $searchParams

    if ($result.Count -gt 0) {
        #now check to see if the 7th byte in dsHeuristics is a 2
        [array]$flag = $result.Attributes."dsheuristics"[0].ToString().ToCharArray()
        if ($flag[6] -eq "2") {
            $scoreObjects += 0
            $failcount++
            $thisOutput = new-object psobject -Property @{
                DistinguishedName = $result.DistinguishedName
                DSHeuristics = $result.Attributes."dsheuristics"[0]
                LastChanged = [datetime]::ParseExact($result.Attributes."whenchanged"[0], "yyyyMMddHHmmss.0Z", $null)
            }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found risky configuration in the forest that enables anonymous access to LDAP operations."
            $res.Remediation = "Disable anonymous access unless it is absolutely needed. The dsHeuristics attribute on the CN=Directory Service,CN=Windows NT,CN=Services,CN=Configuration,DC=<forest_name> object should be set to disable anonymous access. For more information see <a href=`"https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/e5899be4-862e-496f-9a38-33950617d2c5`">6.1.1.2.4.1.2 dSHeuristics</a> "
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCa+yl7sfY5WaVJ
# LHu35mN700ryFcMTCVN20TR59DVK7KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg