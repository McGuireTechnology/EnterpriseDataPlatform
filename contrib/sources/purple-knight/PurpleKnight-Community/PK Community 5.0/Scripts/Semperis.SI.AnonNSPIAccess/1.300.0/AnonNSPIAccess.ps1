...
#This script checks each domain to determine if anonymous NSPI access to AD has been enabled forest-wide
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>NSPI access is rarely ever enabled so if you find it enabled, this should be a cause for concern.</p>
       <h3>References</h3>
      ...
      <p><a href= "https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dsheuristics_bad" target="_blank">Dangerous dsHeuristics settings | ANSSI</a></p>'
    ResultMessage = 'Found risky configuration in the forest that enables anonymous access to NSPI RPC operations.'
    Remediation = 'Disable anonymous name service provider interface (NSPI) access to AD unless it is absolutely needed. The dsHeuristics attribute on the CN=Directory Service,CN=Windows NT,CN=Services,CN=Configuration,DC=<forest_name> object should be set to disable anonymous access.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_dsheuristics_bad', 'vuln2_dsheuristics_bad', 'vuln3_dsheuristics_bad') }
    )
    Products = @(
...
try {
    ...
    if ($result.Count -gt 0) {
        #now check to see if the 8th byte in dsHeuristics is nonzero
        [array]$flag = $result.Attributes."dsheuristics"[0].ToString().ToCharArray()
        if ($flag.Count -ge 8 -and $flag[7] -ne "0") {
            $scoreObjects += 0
            $failcount++
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.Remediation = "Disable anonymous name service provider interface (NSPI) access to AD unless it is absolutely needed. The dsHeuristics attribute on the CN=Directory Service,CN=Windows NT,CN=Services,CN=Configuration,DC=<forest_name> object should be set to disable anonymous access. For more information see <a href=`"https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/e5899be4-862e-496f-9a38-33950617d2c5`">6.1.1.2.4.1.2 dSHeuristics</a> "
            $res.ResultMessage = "Found risky configuration in the forest that enables anonymous access to NSPI RPC operations."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD8qLjR86jSOsmG
# kxno6bkm6hKqNJZYmvfYcEl+6QA09qCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgge0GV6bSo4I5S7DW5zw2gGNjdr9d+HpCqcH0BLUpvIwwDQYJKoZI
# hvcNAQEBBQAEggIAa3rNiFg3zqNHezXJEylKnMhnBTqDeiJdlINstq1TqLcOMshf
# HiK2XbrI5FzxnvYMEU8TsyPEb69AxPHHI1lpI0TyMerx0dAym+7+RHeI/grbW7h5
# nfKg5vSNhDHfz0ev+8nbg9qoBDQK8rKzTY7MhyXXUlf4O7xy2jFkwWChcavqr00m
# aM/wrmKD+u2k+CCP6e8vEgH2XOMZEKzO7NFOSyaQPLY4zoUKIdf13yp/bW/l0Pug
# erbJUzMYPju7eKzCNJtVztNI87+rEU1yi+VkXD1afo/pSf8tLxz//YuqOOGsvbVD
# ylsPr9tETZwWAKyyoVqIKEuKSnRlaDF9V6z/CVnmW8018/SYVZUl4jZrUI0jrfvV
# 4RlTqYC5nCeeu2Y0IO9EvfkSyExLlNRGxmsE3fDMddJbbCipE89lR0OWHHXAOdgb
# nAKnNSHTEdOZbTULky/tjHKPFlVBqmeantG0RJkj/uOiVGFZ8iVBvRknHFSuF/pL
# ECxAJ24PRp++qQXQ0vy9gkGhPbp2XYYmKDpMZXTeY9YeZXfglI2UD4jKXMWggQ9K
# MiwIuxH0rWeSUot6MG6ywCq6lgDcqtBPZ1fzzujQ9KzrA912pP08t/uVhQERBZf5
# 1kellYwkGHUyk62SCae7/S7OxI/o1HJfjnC0mgGAmy3OfUWCoGVZ0GW49J2hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCHbpP0svoVBJyF8WSOGVD+zqUxf4SHRBPL
# x25sS2AkQwIUJrgfBvArYdHYNyDJW/iJvS+Z2B8YDzIwMjUwNTI4MTIzNzU3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAo3L238T4VrfaFNVxYexIOV8pC/+CrQ8ULXio6jR1d
...
# CwUABIIBgGbnb04bTMUwwQ6CCfUNW4H2gw4vz+9G6HzjC8bC1TyAJp9Gyca5eacg
# qXR5Rc2MvxCx0t7eA6teaP0NyHu9AJBIYSvguEu8sVOXXMPjxf/ylmDWR7mFDp4K
# JoSMU3HH5jp17PsoXlJTvPf7vesgOslAJLIa6ng1wIvY6li2jkdtNaG6Z77G6U5I
# CPGaIJTcsezT9gXhpuaRAj0uHSB/26ljJaz2+RYkiGpu/w2L5ouIB4gxS7CBGAH2
# LFDWhP65aH3zgDk44MAqjthmY4K2wAcE0HCa1xAIKAYXt0PC0eHPiBxe+jl6Foyh
# E9YiWJNJF3wanHCQwJYrHgQ5WofAniCoKpXBwx1iYjbY3L6r3+WzZ37bPxg1w4dk
# UlTPdMFJ+gwAChVgA70nfMVjYVeF9rsvX21lWpi3goX+7Qxy+H3QrEHhbi4N8tA6
# qdD0Ybv5T4Mie+/LnCUSJCzZ71DGsb2+QMZMZKaU2PX/vYk9m/s3lS3YzEAJ1zOW
# ZNVK3NsDVQ==
