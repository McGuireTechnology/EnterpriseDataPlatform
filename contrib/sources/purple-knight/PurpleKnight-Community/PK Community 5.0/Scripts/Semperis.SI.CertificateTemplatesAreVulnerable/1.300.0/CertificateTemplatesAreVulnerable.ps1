...
$Global:self = @{
    ...
    LikelihoodOfCompromise = 'The following configurations of a certificate template can be exploited by adversaries:
      <ol>
        <li>Manager approval is disabled - new certificates are automatically approved if the user has the correct enrollment rights.</li>
        <li>No authorized signatures are required - CSRs (Certificate Signing Requests) are not signed by any existing authorized certificate.</li>
        <li>SAN (Subject Alternative Name) Enabled - Allowing the creator of a certificate template to specify the subjectAltName in the CSR, thus they can make the request as anyone, even a domain admin.</li>
        <li>Authentication EKU (Enhanced Key Usage) present - if present, the EKU created from the certificate template will allow the user to authenticate with it.</li>
        </ol>
        <h3>References</h3>
        ...
        <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_adcs_template_auth_enroll_with_name" target="_blank">Dangerous enrollment permission on authentication certificate templates | ANSSI</a></p>'
    ...
    Remediation = 'Multiple actions can be taken to ensure certificate templates will be less vulnerable:
      <ol>
        <li>Enable manager approval - make sure manager approval is enabled and required on the certificate, and approve each request manually after inspecting it.</li>
        <li>No authorized signatures are required - it is recommended to set it to 1 so that each request will have to be signed by an authorized certificate.</li>
        <li>SAN Enabled - evaluate if the certificate needs to specify a subjectAltName, if not disable this option.</li>
        <li>Authentication EKU present - make sure the certificate template is being used for authentication only.<br> For example, a certificate that is solely used for code signing should not also be used for authentication.</li>
        </ol>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBJ48WpRHddN4mm
# ud0GPF97eaqzlyBsrP4zjO2Yeg5wiqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgl1L7M83ahGAxrTZ5ZOJtmTDX7PRgQCmnPzuNAPzpAEowDQYJKoZI
# hvcNAQEBBQAEggIASZ0Tks+T7bCfrP3mmPblRigdv63SKCIbTkyZNPs4qOsyKRKN
# wTnz7af/7ERkYvSwg1lx7Y6xPbLbJECVTXm2UscUinkzg1rShs4YsvtN484TR3/Z
# riyx/lPujp/6AcOk0rT+b5zsqtLX4NLshzjx0sj7+oSeNRNVgZg5VVLdUskXMxgq
# Fzrtd17SxSFB20/pKTKpuW9sjLsInzLwDgpScj05iZ422K8zdFRjCZzdoPaNtMOV
# 6egBUFGxZLKUf03R6rTedzEWFTBWIUkmUPXaBCVSEudgr3JHUuixD0l5yQbF1Nif
# ZieVwGFo5NTEpQ0gbhAIErz3xEF5jX0OAUrVk2ZsNZdeZcM/5Bw/b8MnX5JS0ol1
# bpqXKO9hrv5W93B+RUacMjp1K5xyjjiQwyiEdVlgfLSSVooPpaqNsCiW91S6axYB
# 0GE5y2qiENS5Mer0NNDkAJjfEaGMf8xONJXq26XG4tKEyHvxued2AcZO4lhRMPXr
# BGnzXilG8uultAr2rZaYByKp83gyiA34Vnb0ITahh7CQjDtcMCsbXF1vmtAOVZCL
# ekKLWWa1N5Xk2fHGve0Z+ORAxYLfWTm8HnS3zVrIy2X9rNGvesirrVrtxiIHjKQV
# Tj2SWRs77oB13PALAXIKvAfm+TVpR/3SslPcWU5KVbQA77y7eNsJn7rziz+hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDCMoRKZUS5DCqt6PR8udDBBSlix69JQPqX
# kmT6l8CP7AIUF2Ga+BhweexyIayaGcrlPgF7tp8YDzIwMjUwNTI4MTIzODA4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCA9hgzSzRE5ACUZbRK+B5xXRbAvHMcptBFnkv9FkHDO
# 1jCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgKAHZADfKLGpzrMAukeVb4mtKai736LA476DVA1YhTbrige+Inn2cjuX
# ppwnhUYT3Z/urwSOKu6vwqtvg17vEInZ3M6TcJbpAP/8+Q0J4wxW7oQqoo/VQk4n
# VPVcxPZgtodH4bkTDp/eVmDn7W9vHHrahaLRPKYjbAqXSSLqeGui70e9lsm7z5JX
# iiNjNPrecPS4J+oyHfFlpPxHy5Ais8FI6WBEw+7jc+fse1mkZOuQxvAC4lOmOF+q
# C7CsPI3TLvyL7ir87C9/bdGCDjQdSoBTt+0/pjFWViioLqJ8mkT9MqgToi8LQjVY
# 6yVMnnB38X31s3O1Aj81ESV3WxPM/996Bbz8G133235u/9wwvV6uVxwVi2XpXf5J
# GbGu2ygJVjfCvQH+Wb3w+m1vjHa1bVePD94tgExTyPPHJ5r55H8yG0AYwfNn7ire
# Fs7ZzNEGjM0Wtfipc5H80aKcKdRVw8OX+aJg4+bFeOSo0xG5we3ubY6FLUaJyd3N
# CyxRjAy4yg==
