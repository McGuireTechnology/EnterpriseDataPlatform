...
$Global:self = @{
    ...
    Description = 'This indicator looks for non-default principals with the ability to write properties on a certificate template.'
    Weight = 7
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = 'Controlling certificate templates allows one to have the certificate authority issue an arbitrary certificate. It becomes possible to obtain a smartcard authentication certificate for any user, thus stealing his identity.
        <h3>References</h3>
        <p><a href="https://www.nccoe.nist.gov/sites/default/files/legacy-files/tls-serv-cert-mgt-nist-sp1800-16b-final.pdf" target="_blank">NIST-SP1800-16B</a></p>
        <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_adcs_template_control" target="_blank">Dangerous ACLs expose certificate template objects (attack path) | ANSSI</a></p>'
    ResultMessage = 'Found {0} certificate templates on which unprivileged users can write properties.'
    Remediation = '<p>Unprivileged users should not be able to write properties on certificate templates. Doing so potentially gives them the ability to escalate their access and create vulnerable certificates to enroll. Remove unnecessary permissions from the certificate template.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $guidHT = @{"00000000-0000-0000-0000-000000000000" = "All Properties"; "ea1dddc4-60ff-416e-8cc0-17cee534bce7" = "msPKI-Certificate-Name-Flag"
        "18976af6-3b9e-11d2-90cc-00c04fd91ab1" = "pKIExtendedKeyUsage"; "dbd90548-aa37-4202-9966-8c537ba5ce32" = "msPKI-Certificate-Application-Policy"
        "d15ef7d8-f226-46db-ae79-b34e560bd12c" = "msPKI-Enrollment-Flag"}

    # forest-wide Allowed SIDs
    $allowedSIDs = @("S-1-5-18", "S-1-5-32-544", "S-1-5-9")
    $domainAllowedSIDs = @("512","516")
    ...
    foreach ($domain in $DomainNames) {
        ...
        foreach ($domainAllowedSID in $domainAllowedSIDs) {
            $allowedSIDs += "$domainSID-$domainAllowedSID"
        }
    }

    # remove any unavailable domains, checking their availability is slow and
    # Get-ForestSID/Search-ADConfig will also check if not removed here
    $DomainNames = $DomainNames | Where-Object {$unavailableDomains -notcontains $_}

    # if all domains are unavailable, no need to run the rest of the indicator code
    if ($DomainNames.Count -gt 0) {
        $forestSID = Get-ForestSID -forestName $ForestName -domainNames $DomainNames
        $allowedSIDs += "$forestSID-519"

        $rightsFilter = "GenericAll|GenericWrite|WriteProperty|WriteDacl|WriteOwner"
        $excludeGuids = @("0e10c968-78fb-11d2-90d4-00c04f79dc55")

        $forestDN = Get-DN $ForestName
        $searchDN = "CN=Certificate Templates,CN=Public Key Services,CN=Services,CN=Configuration,$forestDN"
        $scriptBlockFilter = {
            ...
            Update-NTSecurityDescriptor -Entry $Entry -PropagationFlagsFilter "None|NoPropagateInherit" -AccessControlTypeFilter "Allow" `
                -RightsFilter $rightsFilter -ExcludedSIDs $allowedSIDs
        }
        ...
        $results, $unavailable = Search-ADConfig -forestName $ForestName -domainNames $DomainNames -searchADParameters $searchParams

        if ($results.Count -gt 0) {
            foreach ($template in $results) {
                if (!($template.Attributes.'aces')) {
                    continue
                }

                $usersACL = @{}

                foreach ($access in $template.Attributes.'aces') {
                    if ($access.ObjectType.Guid -in $excludeGuids) {
                        continue
                    }

                    $templateIdentity = $access.IdentityReference.Value

                    # Try to translate the GUID
                    if ($guidHT.ContainsKey($access.ObjectType.Guid)) {
                        $translatedGuid = $guidHT[$access.ObjectType.Guid]
                    }
                    else {
                        $translatedGuid = $access.ObjectType.Guid
                    }

                    if ($UsersACL.ContainsKey($templateIdentity)) {
                        $UsersACL[$templateIdentity] += ";" + $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $translatedGuid
                    }
                    else {
                        $UsersACL[$templateIdentity] = $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $translatedGuid
                    }
                ...
                if ($UsersACL.Count -gt 0) {
                    foreach($result in $UsersACL.GetEnumerator()) {
                        ...
                        Update-IndicatorOutputAttackWindow -ThisOutput $thisOutput -Metadata $template.Attributes.'attributemetadata'[0] `
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.count) certificate templates on which unprivileged users can write properties."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAz4Z6IrZguQzPy
# QmJA6ZHPHJHDhusZvCGqqgu137ipraCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgi6eiweG6NRAvUDltgIMS2lssFqR7wHZIcqS8iuogsR0wDQYJKoZI
# hvcNAQEBBQAEggIADa+PL4Py7+2V0GLavhb8SsjvHwQG8gRSUqkp4ZnrfDaEqAB5
# 6zJnla1dvQ+8lsU76QrCVtVk2qZGlttmn1EN1shSMjxqi8cv5hGpDYzNQZhvx9bQ
# IiwMO5oSEOpVRjQTOy5ow5vZRbglfrnN/ptorVuvQy8g+2wDeLd0RfS/6prlQ3J6
# jMo5Spyuux+YMhQyU+m5Vwwm1Nf+VB1pRmm88uNKmYcayZVDSiVFptTLUvouuZlE
# QWN8QXgorQmdE+xHVBapnzOiYOYLUFLj1m66rCOspT23pjjWPXh+5PD4HXyDb09o
# qOi2ThbfMWKiAZncuvTyq1N4LEOivpMFdSw/eZJIuc0R2u2D6f9EP5FAywC99WXp
# hp7vC2EzUzALtNl70OImXE8IPNIi0KS8EFElTe5mOpHxG6CZ0rFhqmQlY8b9g4wV
# gbsBhq9OT1apDvKnl+o+pZxCd2uuUUsaY2HyYo4IqYL7LlBvCYxuUHMo3MGyrQZo
# krKVyl56QSnIT25ey1CKiYdOZgkEQw+OayGVMQX9NKbTa3RNtNZ6ck+LENc0lDMP
# Mx+DoM/sl6b0f7bWbozxiLE44KfAbLj1T8If/AWDPdH5L8ok/x94VnGJR0XnBkJE
# HPCZzw856Fcq5PE195FJWdv+4BeVuADeIvr414VoUCRHU7YzcJUIDL8Xtzahgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDijqNQrPvrVvMLDrebug4JVTz5Rizv0pK3
# lrfmL6VB7AIUS5Eckc2R5O9FLX6LdiXWkNSpO0gYDzIwMjUwNTI4MTIzODEzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCam76GOjzb9iPMiWOTSmYqExvuxn0hyO4lgRP/3IYi
# DTCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgIYo9baYnhKGeMJzQcbdcsLCKjRxz20JzfjxmFqtXARRkhQijpgZaYGn
# htaJHtMHGfbRVvoiv68ZhfR5A5EAu1PdDJ8TKWJYXngWz2dfU4i2rLktLgQiubr0
# jS6I8aU0q82s4q1f2NIVh1FexXYg44jABU1b5/Yc1Fz5kGKFpV83GywlQTlNt5HK
# c2PzVqD0EIMblYqtzrRYJIWzzhOVYUtk9RSg7Xpnv8hx3dcCWTwJur7MkhcvYT6I
# k4zzS/3Ra7xhuLZgl9BmdRxBwQoiZQt2CBgEUAyL0Ahh6Mk51S9036a6dK6qUslv
# BCyscnHHFT5583BKH4syuC0j3y5nLw9iaQwS5L1tAykwgVLwje3Y9pRQV2Wp23pI
# V4cHkMU7pNXMxXyayAYFPCgh1GuxaGxAe81J6Ka1EN0xRyEuVQVKiEHIRqYq9q0H
# YGcqYI5ypXes4poAPvlpcUF2nMf9eMkM9eTjdAXsSgKIQ06lmIQjKfZbgrnWswC+
# L5wP8yzKFg==
