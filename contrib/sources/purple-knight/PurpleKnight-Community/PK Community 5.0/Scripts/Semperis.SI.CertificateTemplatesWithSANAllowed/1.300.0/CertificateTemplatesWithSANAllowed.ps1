...
$Global:self = @{
    ...
    Schedule = '1h'
    Impact = 8
    LikelihoodOfCompromise = '<p>When certificate templates allow requesters to specify a subjectAltName in the CSR, the result is that they can request a certificate as anyone. For example, a domain admin. When that is combined with an authentication EKU present in the certificate template it can become extremely dangerous.</p>
         <h3>References</h3>
...
try {
    ...
    $results = @()
    $builtInTemplatesCNsWithSAN = @("EnrollmentAgentOffline","WebServer","CA","SubCA","IPSECIntermediateOffline","OfflineRouter","CEPEncryption","ExchangeUser","ExchangeUserSignature","CrossCA",`
    ...
    $builtInTemplatesCNsWithoutSAN = @("EnrollmentAgentOffline","WebServer","CA","SubCA","IPSECIntermediateOffline","OfflineRouter","CEPEncryption","ExchangeUser","ExchangeUserSignature","CrossCA",`
            "CAExchange","EnrollmentAgentOffline","WebServer","CA","SubCA","IPSECIntermediateOffline","OfflineRouter","CEPEncryption","ExchangeUser","ExchangeUserSignature",`
    ...
    foreach ($cn in $builtInTemplatesCNsWithSAN) {
        $filter += "(cn="+$cn+")"
    }

    $scoringFilter = ""

    foreach ($cn in $builtInTemplatesCNsWithoutSAN) {
        $scoringFilter += "(cn="+$cn+")"
    }
    ...
    $searchParams = @{
        ...
        scope = "subtree"
        filter = "(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.804:=1)(!(|$filter)))"
    }
    ...
    if ($results) {
        foreach ($result in $results) {
            ...
            $cn = $result.Attributes.cn[$GET_STRING]
            $canBeUsedForAuth = $false
            $isPublished = $false
            foreach ($eku in $templateEKU) {
                $eku = [System.Text.Encoding]::ASCII.GetString($eku)
                if ($AUTH_EKUS.Contains($eku)) {
                    $canBeUsedForAuth = $true
                    break
                }
            }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) certificate templates that allow the requester to specify a subjectAltName in the CSR"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD/mqyzImp3I3Aw
# K5mm8WRaRWM3jg+HyhpPvCswJiwbuaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgdbqZFtfOsIb5aPqv0OCQFTS2IkrzEkk9ihIJTS9UB4swDQYJKoZI
# hvcNAQEBBQAEggIANwCEFgQtaA51eZ/82Xxn3OoXEnu/2uFP9kA6IkTtTvAP43dl
# i681lCK/uYgC716nIgEJ6i0BzV7/whqhO+R/Uv/W7uvYAE1QeR4GkRuOrHGTXRiY
# N5qEwsQR+6Ff2Jp731xQfgTXWJ/TNhEokd4YwvffCh+CmMKmG7MR0X2c6Z1GUWxk
