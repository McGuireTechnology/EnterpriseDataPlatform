# This script looks for non-privileged users with permissions on the NTAuthCertificates object
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>These control paths allow adding a malicious certificate authority, which allow an attacker to authenticate as arbitrary users or services.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_adcs_control" target="_blank">Dangerous ACLs expose certificate containers (attack path) | ANSSI</a></p>'
...
try {
    ...
    $guidHT = @{"00000000-0000-0000-0000-000000000000" = "All Properties"}

    # forest-wide Allowed SIDs
    $allowedSIDs = @("S-1-5-18", "S-1-5-32-544")
    $domainAllowedSIDs = @("512")
    ...
    if ($DomainNames.Count -gt 0) {
        ...
        $searchDN = "CN=Public Key Services,CN=Services,CN=Configuration,$forestDN"
        ...
        $searchParams = @{
            ...
            baseDN = $searchDN
            scope = "OneLevel"
            filter = "(distinguishedName=CN=NTAuthCertificates,$searchDN)"
        ...
        if ($results.Count -gt 0) {
            ...
            foreach ($access in $results.Attributes.'aces') {
            ...
            if ($UsersACL.Count -gt 0) {
                foreach ($result in $UsersACL.GetEnumerator()) {
                    $thisOutput = [PSCustomObject][Ordered] @{
                        DistinguishedName = "CN=NTAuthCertificates,$searchDN"
                        Identity = $result.Name
                        Access = $result.Value
                    }
                    Update-IndicatorOutputAttackWindow -ThisOutput $thisOutput -Metadata $results.Attributes.'attributemetadata'[0] `
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.count) principals that have non default permissions on the NTAuthCertificates object."
            $res.Remediation = "Unprivileged users should not have permissions on the NTAuthCertificates. Doing so potentially gives them the ability to escalate their access and make the domain trust a rouge CA. Remove unnecessary permissions from the object."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB4TBdQutb5pet3
# KRWc/X87Jn3pOyUmfG85a/VMUJD4qaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgMmdB5h8bLyzxhj8JwwuQuCZn1NIIwtEpF3ETaxMfpXcwDQYJKoZI
# hvcNAQEBBQAEggIAQ7MN8HpHY5oWauoTaZz89GL1ZQp5qXxs5YghYhU+Z8uBOK4w
# fhSn5G/bbrEqnxCwYIbHLE0kLnjMBIioBHmZpEPfXTB5t0zb0sv0bqlipf9fvAzI
# kT9Ih0ESRNA6oYBlAAGWMZrY6iWhyqk5NhBBg1RHUCSeja/qI4PAe63oCe3Swt8y
# jvQxWozY51CkX44Cp3l5F5TzvmpFcP8TugRXvhTb3UZWAVUbVtOMnU9CdWzk+hu6
# vthhdKaLyWhl5CKfGuHHsCCjYhYqlxvTjB20GBnlvCWnW5nDlgK6AiUZMXkCa1YN
# o1Vv9/mwU6SnWcxWIifBHrnj3TKXqM/p6D58+ULtRdck0oyE0o9pD2/3hCY20HeZ
# bK6tE2UC44qoz/65BDYSP8p7wCFgpt1C0LNUEPHSsg/I/mpSvEQWzFsKggq/y8kg
# sMYyS2op4xwvhSMeccUjA0k5WrvxhEZ0lWI51igP5WlSaYu+Wn6jrWIGyLeW/Qdd
# Tx0xAL8VcZsV/8zF3s2Pab4/o0mBKK+bqDZqiQVGvDS0BZppJSvlYo9pCK2bxpn2
# WwEkmHCvi8+UUa4gVM/fhQYZBNk62kQi92J/1PPcu/qzy8O7X7mf1MITgSKQhPW0
# +ohm/HCAXx2zOMvQi3Qbjl2zLYdX2h/DnGWWewm+dqUMhJP0ecJicKJE5Lahgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBt+bbdigOeubPK0te3d75NcBOuCSS5/39X
# ej0NpRuQqQIUKaYbXFFT5K1dROL6uR/GdVgWQ9AYDzIwMjUwNTI4MTIzODAzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAPzPDTwcuFe0G3WMgqNP5FaH50g2xM1w+ZjPK9s4OI
# pzCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgIzRzqb/oAsVRT0djgtuWc/2+AOEBzAtth24wNSawxUT7ULcjYfulW4d
# sB7TltFlsrLMAE4BMOTGLgbFm9RkinXMVeLL1c+pSJo4mTEiV+6ZbHD1Ua5FfuyY
# xboMnrVMXZOkRdAPNM87V24VSSbBOgt5TXQ5GRXR7R4BCstgfbPrgQlQJgJ5Oaoz
# w8CVq0Hv3Olgu7+EBcbgbFQrU8G1jmEPkJuU92kyFfh6whmQnkhNp9V10g0hrFxx
# Iu6hDyR4WIOBv2uHVwzXcFTqpCRlwAt0L/LsOpA4ASAU18hqAmfQv60NFviDSfy/
# 0da1IinmL1mAnanxwt+R6EnlP3NwE5qMd5xK6+SPVj4JKaZJBwAq5UPd43GJ3N0e
# CbXh9iYlbkkjRPsDVox4G/XPU+cTDD42ZHbUuBRea4EBv7jV/3vZu+LYrQEH9WM6
# 0RThFg2zpNkRMxR9/I3pcAhPVrsPyXsVBfLKPv8+LOTyDvymmxlU3jyyoKD+3KeY
# oswdt/K+TQ==
