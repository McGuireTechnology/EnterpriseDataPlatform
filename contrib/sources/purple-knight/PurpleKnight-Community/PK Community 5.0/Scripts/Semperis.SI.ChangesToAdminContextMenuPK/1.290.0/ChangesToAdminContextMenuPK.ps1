...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>Attackers may utilize context menus as a stealthy way of getting various users in a domain to execute code. Modifying this attribute requires special permissions granted by default only to Domain Admins and Enterprise Admins and also requires the user to click on the illicit context menu item. See the this blog post for additional information.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/active-directory-security-abusing-display-specifiers/" target="_blank">Semperis blog | Active Directory Security: Abusing Display Specifiers</a></p>'
...
try {
    ...
    $forestCreation = (Search-AD -dnsDomain $forestName -attributes "whencreated" -baseDN $DN -scope "base" -filter "objectClass=*").Attributes.'whencreated'[0]
    $forestCreationDateTime = ([datetime]::ParseExact($forestCreation,"yyyyMMddHHmmss.0Z",$null)).ToUniversalTime().AddHours(1)

    $daysToRemove = 90
    $startFilter = (Get-Date).AddDays(-$daysToRemove)
    # Change time filter based on attack window if needed
    if ($StartAttackWindow -and $StartAttackWindow -lt $startFilter) {
        $startFilter = $StartAttackWindow
    }
    # Change time filter to forest creation if it was created in the last 90 days
    if ($startFilter -lt $forestCreationDateTime) {
        $startFilter = $forestCreationDateTime
    }
    $whenChanged = Get-Date $startFilter -Format "yyyyMMddHHmmss.0Z"

    # find any display specifier that was changed in last 90 days
    $propertyFilter = "adminContextMenu"

    $searchParams = @{
        dnsDomain = $ForestName
        attributes = "msds-replattributemetadata"
        baseDN = "CN=DisplaySpecifiers,CN=Configuration,$DN"
        scope = "Subtree"
        filter = "(&(whenChanged>=$whenChanged)(objectClass=DisplaySpecifier))"
        PropertyFilter = $propertyFilter
        MinimumVersionThreshold = 2
        StartMetaFilter = $startFilter
    ...
    # Count is the number of changed adminContextMenu
    if ($outputObjects){
        $configArgs = @{
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.count) changes to Admin Context Menu on Display Specifier in the last $daysToRemove days."
            $res.Remediation = "Review the changes and ensure they are legitimate. It is also recommended to revert write permissions on Display Specifiers to default state (only privileged users have write permissions)."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBmQdfRnMZ8okYl
# bnqjHa9Dl+zlfjVr9EqpWKUGBy5ay6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg