# This script looks for any changes to default security descriptor in the last 90 days
...
$Global:self = @{
    ...
    Remediation = 'Ensure that the changes to the SD schema are appropriate and valid, else investigate the cause and source of the changes.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $daysToRemove = 90
    $startFilter = (Get-Date).AddDays(-$daysToRemove).ToUniversalTime()
    ...
    $propertyFilter = "defaultSecurityDescriptor"
    ...
    $searchParams = @{
        ...
        scope = "Subtree"
        filter = "(&(whenChanged>=$whenChanged)(objectClass=classSchema))"
    ...
    # Count is the number of changed defaultSecurityDescriptor
    if ($outputObjects) {
        $configArgs = @{
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects whose default security descriptor has changed in the last $daysToRemove days."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDdDQCUGOvH6gOE
# u5ZKUgmhYW/cO7td8qkW7lTu7gpExKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg/6w+PHfPb4NBwc1IxK7AnAJr3XQDiAM7pDHjbXfCBxwwDQYJKoZI
# hvcNAQEBBQAEggIATrrATmHs/qd+RgWNhXPpnAb5sVeYnoTOqNMLQLVQZ9G7aeFx
# tVgjF2smy9+C5W9VqTX1aTichoRNhVhp98lQke5PxHeaKpaAVUkCJqzyRRFgn2O3
# bOg1N9c2SqdmcX1z8iADQJJYKoOWmjpHVFkp9C6MEyVxb2/EbyJMwKtqXq0uZHv9
# 1iaKCOR8skcVB0j3ZEK2+t1KbmBIvftSwKX8gXmZ+PMvlxPq76yO0JUAggaWjDlE
# uN39vYeKctzEnb3InKBxeAPTspc1diWH0ST3uF2z0djbmhhQmr+qm3Wus1Fv4T/p
# o3YY9kMHIXPKatZ+rEqv66aONU/B2xeLW2sFpDJ8CYI+MWMikSMgdZe9s/aoGdkI
# z+s1T61F6gt35Et8/DHpyFk0RUXFu3CH1ZD0dnMYAueMDuJEeeph3umGbLPWh5tX
# H4Iipv8nvYHCNapvXzGNp1Dl3EEmEtcCrHJd5P5fYuh6ybTl+ewCsaeVMEZ3ICBG
# Lln3kusQZjaeVoSh6SfN3GnNeNaVh08fnOvT871yqBmD5n6pK+VPP+YnLmeTis2s
# 7iQESQubxpAwJF8byjDGLecH/Z54GC/k/odal6zvG8HkdKpZuvpXCGvzKu/kvQDk
# ase89hWvusjz5bMc/hrY9qBBuOA5n7nfmMP9s+w1EymzCgA5wTCOq9l8JD+hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAVIUkH/gO28TXAUL+BJjNnPBbs9yE8ZD+d
# HfgjlWkr7gIUOQWC8lvrzFXh8X2ccqyoiZUl6SEYDzIwMjUwNTEzMTg0ODMxWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBjCtfps5H3GX7k/ziwj+Smsp6rMMDx+/XFGTM+cIw5
# XDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgCDcIIlfMoteInvBWo++fs+IfKrWADDMjB8/r7i009XUNKhdfSMhFe/3
# QRl+esNz7c0VTKMNhUoR1Ki+z48xe2ISytgETWnPw9HCUWJoDRhNTeEywQDOyEaO
# lPEgKlPyi8N40eR+1JtyBnDQKL4R7iAdek2JUU1YGIFWgpl8HXONfdDAS4qG3tnM
# cm0D+gdZryZtp7CWSbL8VbWG+WRJOiEAHIQWUxBuVBxZWTSgjplho/IFBlapxSGm
# f5JufbM9xtOTHQNldqAsQrnlcCXX2UIh+8N+xCy3uxoWXMjjgt8BgFH+mPEdWeUH
# P2y3udRS2kgLfrK2soZ3aY+OapYbrNihV+5uFIOTjCFSs5d7ADMhLMbceU5xuIY7
# cK0niCDtjZ0d2XexQVi15i+UnWe2gxc+zC8NKrJ2kKEIhNaywHE2hBUsx0yDMADb
# xlU+Xb8OCT4HAPWhLv7yM1EXigLJEs56r8oB5YgN7uS6BrIkz7eDZeRmbKJY3Gdy
# PpSpXM2ILA==
