# This script looks for any changes to default domain or domain controllers policies in the last 7 days
...
$Global:self = @{
    ...
    OutputFields = @(
        ...
        @{ Name = 'Version'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AD')
    Permissions = @()
    SecurityFrameworks = @(
...
try {
    ...
    $filter = "(&(objectClass=groupPolicyContainer)(|(cn={31B2F340-016D-11D2-945F-00C04FB984F9})(cn={6AC1786C-016F-11D2-945F-00C04fB984F9})))"
    $propertyFilter = "versionNumber"
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            baseDN = "CN=Policies,CN=System,$DN"
            scope = "OneLevel"
            filter = $filter
        ...
        foreach ($result in $results) {
            ...
            $thisOutput = new-object psobject -Property @{
                GPOName = $result.DistinguishedName
                DomainName = $domain
                Version = $attrMetaData.Version
            }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.Remediation = "Review the changes and ensure that any changes to these two GPOs have gone through well-known change processes and that any changes made to these GPOs are well-documented. Investigate any undocumented changes."
            $res.ResultMessage = "Found $($outputObjects.Count) sensitive policies in the organization that have been changed in the last $daysToRemove days."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDRLnShS9m9py+P
# B1LXtuNAB895nyVvV0x3KYuHGHdMvKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgJ0s/8de706UAl37/VXJ8FVFpyKEyrBYdEWvM9fQpCeowDQYJKoZI
# hvcNAQEBBQAEggIABGKqOnE51DpsW0AAhXb8Yh8L2rnk5D4B62B5FBIHXTtbmRCM
# JkmFYpjsyhyVtk402ICo7m62+rdE4uKURZYMvM8t3MGhqUOa6zGY1j5hP2TdpMaL
# oOJoEzIQNpqHHNiY3rYMKQ35LELxI4t2kZ32Br8uDpi2yHsK3ScWm0WbLoJLKPI7
# I26HxoQSLV3W4SG82bYDSzFCVjbPAqkrvDDYkCSpIPi8ncwMVRpL6ipSyTQRO7Zx
# QlyRb5l+K2nxa5wGnLVD3WRqqw9jl9QSHesuXAxQ0fCjeFqlb6C10nrBThwpg881
# C0d8Sd2J716+qqfZTBkF8H9yvBZi2ySMLUuRkQQdi6btowo/KzCSBLoVHgKfCh/y
# JMlrwFXU82suQk5y+iFRHNB/UdR1dGZkhXUuxJLANrNWEvSFcj0jV+4Aku32bZBj
# dKQjV1soDIqfM/7nKhcvdPI1/hloczZlBy9XshdtTq4G3mLyd45NHeapHvlHNliO
# oZbo0Oty16FPPZP0I40lbYJkfQ5qf97blg0AWwAmt6vom7wFBn0QQc0jvmxWQ+f7
# lWlkelVTrjPyEg6Wx+7mhFscpBhKEP7pBc+CCtlnub7DbAlBmGehHuxMNYmqqR23
# +gmXVSBtvbJAtmcDxPkIFgFmC02CeLLBlgc41ZlbY5/VIZhvWb6tZjQ2mVqhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCB5ufrJMPbJGHehBtAPSpu/CjIE8BTxHMts
# Hbz20AMEJAIUGeVIbpJ55WWFMTakkbWXIefXTjAYDzIwMjUwNDE2MTMwNzQwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAOiT4mAHgAJBbcA89r9gmAGPLLcljdoBH4GD3mmP5F
...
# CwUABIIBgKS9ZhcgocjpYB2IDqocwOe0tpe3gPdYmwrbSB36gDKfOfYcy43dzXTb
# IE9cX+9XvEvqjU4Vc0wQClYqowXO11Ig0uhewlaUBIuK+IuUfX2ktu8mz1nOLxwb
# 2z8WtvBAfEpQNtHCCqA94m95E5UC8VD3i/fqJzKo7g8CNKo+F+qbvf1QmIE8RS2/
# Fnk8R+Ln0HoLP4TFdunAQzKEo07X2M47BtWvWL7SbCMSbKYrGcwfyoobGNfkfbjB
# IYUZWrPXwWR3Iir44wXH4IHEq9kCAtncaoyE0PdQtZLOHKUpFnFvlQjHHviw9VAQ
# BVHie+zbN0sZkgDdKKuWFgI7Q+OyMQ+I6dYcxwmKzFHZ9tkI9nDVnP+6k6o068i2
# 1dkyFA9/bCpgjztjRgHazVbNo+m1BGZwcb7w+MgJhpXC6CJtsw6RP1+BFojOc/IX
# HdrothGvKUGuzSdKjfjJHz/fp+e3kNsn5b1s60bnji7N/peaFv35hkyMTHYvKfoO
# NEEOh40y4Q==
