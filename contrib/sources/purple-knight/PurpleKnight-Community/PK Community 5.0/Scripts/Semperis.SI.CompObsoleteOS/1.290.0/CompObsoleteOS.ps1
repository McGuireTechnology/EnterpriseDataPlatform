# This script looks for computers with OS older than server 2012R2 or 8.1
...
try {
    ...
    $daysToRemove = 30
    $activeThreshold = (get-date).AddDays(-$daysToRemove).ToUniversalTime().ToFileTimeUtc()

    $attributes = @("operatingsystem", "lastlogontimestamp", "pwdlastset", "useraccountcontrol")
    $filter = "(&(objectCategory=Computer)(operatingsystem=Windows*)(|(operatingsystemversion=5*)(operatingsystemversion=6*))(!(operatingsystemversion=6.3*)))"
    $propertyFilter = "whenCreated"
    ...
    foreach ($domain in $DomainNames) {
        ...
        if ($results.Count) {
            foreach ($result in $results) {
                ...
                if ($pwdLastSet -ge $activeThreshold -or $lastLogonTimeStamp -ge $activeThreshold) {
                    $active = $true
                }
                if ($enabled -and $active) {
                    $domainFailCount++
                }
                if ($result.Attributes."operatingsystem"[0]) {
                    $OperatingSystem = $result.Attributes."operatingsystem".GetValues("string")[0]
                }
                ...
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    PasswordLastSet = [datetime]::FromFileTime($pwdLastSet).ToUniversalTime()
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) computers in the organization that have obsolete OS."
            $res.Remediation = "Where possible, update servers and workstations to later versions with better security features."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB5nRGA2cdvllRV
# Gz+SYFqNfwaSPyITljcBsEboe2DrrqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgXH5mw3Iv+0SJiQnVcU6DboU5BN943XKGDx+m5hvgl8MwDQYJKoZI
# hvcNAQEBBQAEggIAXnJL3qDthtgIBv/L5I3WdH/6+YWxOcedEhnQypgXIWPHbu3g
# T12LR1AslcvUiWCHEsZnvoGMnfoFBjvo3iPnEwgN6unoWmnKRSao9g0stt8Ibw+6
# Ert+BC30p0pMh87fgPwXXnbHa6P/qoWiWyIwat+fyglkZB94tfXGmbnnWmiApoXj
# XTAQVpmCRgcaytd17opyySd2Z9kr8MO3tTozqs51cIfq+8KmL0s73KJTuB2oLJiQ
# SZA8mGZOUGXS2qYTZOHnweiOAhz78n3mKqUZU2Vb05PBMg7bEdAsOstSmvJyiV6p
# YLIUJnAdQKmOcOJtasQqDNP+3muuGU6LgggnMNI0zY2AEcMi5OQX8tvoQ+GAPwEi
# ySn6vALbxsoL3trP7y3eTwcZE3837+gvKBMxyJ/KMr2eN+9lEKsmpfDQyh7HuxvO
# wObd1GCrT8IVzHhufnk8lDqM6YOhmQ3Zp1jLwn7AkbJEtyDtxk2Y5CYRqOgOnW7q
# VmAo18ZfOfpawnLTWY/xPhNuoOivSLyM6gikb1zuH3Wkpc9VGKEH9F0JG1Ku7gG+
# 0eI/sqHLq0YGbJ8LTKKMsGXHGkVpZiiO4B3dGRQRkgcMP7/VwgwFUY078/4i5mzM
# dSERTB5JymtAGf8C5Yy06IIt9PoTf62r10W4gc6+ayEx9+at1wTI+dgN+suhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDlms6k4iNciqpfHD+R4N55Vjaw43GoPBk2
# SZAwi57OjgIUeTh+fQM+r9ZzEtUsREgEUqf57UsYDzIwMjUwNTEzMTg0ODQ0WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDpNrUQu12ZKZ/mF7OfKD0ny2eZi43RvhsJdsY0yO0G
...
# CwUABIIBgJVRMIaiLijj/xwZOw8Pp+feCGkbB/uZZEzwhLAjyKkCKk18QNl1ebuf
# Owks2L9vi9h+DCShSgfo2Wy8s/4sqEZFodXtXBTczfrBL3Zeto7/q/JeeLnou/P7
# tVtjrpPl4Sm6FUakpJ1W4n1FbjBXA3U6omiGMAaDZBCaDCMexuhAaMfmMlcOu5Om
# gUyjhJuq319Qb1n4j4SZnKFTxgW/YWQqr7B3DFr7LPi8iPOjQZjCuizt7Vu4YzWR
# SOjuOj72cMTgEg7DUdMdj8svecTzRKBkG10MxNo1uOXqNDKG7GPb6kB45GA8WE3q
# 4UAaYkI0+kUY2kYX4xJ87q0YuMLMksaXVY7CmcUixhkFTUQRqlb2stuIqQtSHneM
# XC0AAy9wrxTb2dpTyDeaQWUs2HI6A4lsWNzApBb+i0w9afkyENIChWFUoVciiDID
# hINiHJ/79CX1FsDrHddDSrXtSxDvfH8TqvYYaZMlTICLoLA1E1VpKeCTSJnSPQ+m
# pAOC5FcjJg==
