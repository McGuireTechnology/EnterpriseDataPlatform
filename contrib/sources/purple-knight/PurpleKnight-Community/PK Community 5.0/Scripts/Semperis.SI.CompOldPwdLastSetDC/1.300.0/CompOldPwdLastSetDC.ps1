# This script looks for Domain Controllers that haven't changed their password in 45 days (default value)
...
$Global:self = @{
    ID = 63
    UUID = 'bd287262-50dd-4e99-9daa-7754eb27cddb'
    Version = '1.300.0'
    CategoryID = 3
    ShortName = 'SI000063'
    Name = 'Domain Controllers with old passwords'
    ScriptName = 'CompOldPwdLastSetDC'
    ...
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>A DC that is not updating its machine account password regularly could be more easily taken over. From an operational standpoint, it could also indicate a communication problem with the rest of the domain.</p>
      <h3>References</h3>
      <p><a href="https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf" target="_blank">NIST.SP.800-63-3</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_password_change_dc_no_change" target="_blank">Domain controllers with passwords unchanged for more than 45 days | ANSSI</a></p>'
    ResultMessage = 'Found {0} Domain Controllers whose password has not changed in the last {1} days.'
    Remediation = 'Ensure that any DCs found by this indicator have a valid trust relationship with the domain and are properly replicating to partner domain controllers. Use repadmin /showrepl to test replication and run nltest /sc_verify to check for a secure channel connection to the domain.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_password_change_dc_no_change') }
    )
    Products = @(
...
try {
    ...
    $attributes = @("pwdLastSet","serverReferenceBL")
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all enabled DCs that haven't changed their password in a while
        $searchParams = @{
            dnsDomain = $domain
            attributes = $attributes
            baseDN = $DN
            scope = "Subtree"
            filter = "(&(samaccounttype=805306369)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(pwdLastSet=0))(pwdLastSet<=$pwdLastSetThreshold)(|(primarygroupid=516)(primarygroupid=521)))"
        }
        ...
        if ($results.Count) {
            foreach ($result in $results) {
                if ($result.attributes.serverreferencebl) {
                    $nTDSDSADN = 'CN=NTDS Settings,' + $result.attributes.serverreferencebl.GetValues("string")[0]
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Domain Controllers whose password has not changed in the last $daysToRemove days."
            if ($DomainNames.Count -le 0) {
                throw [System.Exception] "Exception: Empty domain array"
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAIFf5klgRCkGqv
# QPqMBh1S5GgWP+28cSlx3EYiniEkR6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgamZtLaAUvSGDNhheUthWQZgg996qoZGHJsR1sLHNcUwwDQYJKoZI
# hvcNAQEBBQAEggIAGIy5x4FIb2NpoHWSPsFI9W4Gs7EQvOFL5DSGuL3FmPWz8TcG
# /IAYtbel0gPL6u8YLgCh29jUdpEcpAE5c/I6sSo8UD2DdYtMLJMdE/upJYzvVo1q
