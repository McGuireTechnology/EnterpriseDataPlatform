...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Attackers who control a service or user trusted for unconstrained delegation can dump local credentials and uncover cached TGT. These credentials could belong to users that accessed the service and who may be privileged.</p>
      <h3>References</h3>
      ...
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_delegation_t4d" target="_blank">Unconstrained authentication delegation | ANSSI</a></p>'
    ResultMessage = 'Found {0} objects configured with unconstrained Kerberos delegation.'
    Remediation = 'Accounts that require Kerberos delegation should be set to constrain that delegation to the particular service or services that require delegation. Attempts should be made to have Kerberos-enabled accounts not be privileged accounts.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
$filter = "(&(servicePrincipalname=*)(&(UserAccountControl>=524288)(UserAccountControl:1.2.840.113556.1.4.804:=524288)(!(useraccountcontrol:1.2.840.113556.1.4.804:=8192))(objectClass=user)))"
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            scope = "Subtree"
            filter = $filter
            PropertyFilter = "useraccountcontrol"
        ...
        if ($results.Count -gt 0) {
            ...
            foreach ($result in $results) {
                ...
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    DisplayName = $result.Attributes."samaccountname"[0]
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects configured with unconstrained Kerberos delegation."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCuZwoI3y+QmKVD
# +7qjVsHdOWOpJWlX2x+T5NCT3TuMyaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgoFYw0WB9ieeY/odlrQWvxEbSOCtH+tARM7/+qe/sLHswDQYJKoZI
# hvcNAQEBBQAEggIAdseo8YpbFkT5aaJWp8VqEJhVIj5HscbQCwWZ3a4ZNrufN/CG
# GLvYwU3y3WaUTjBqXRp0G1xCmcgyYUrZY/aSKFnjV89LQZYjw/q9OLbz/oBt1yO/
# 3T9v2R5I6cR9s5aPv38E6GMj9HwlwAxW9E4uIoL5Zol5FBQSulZsUMleWYdjPj0R
# QfMNFlhrGaaEq8zaVMlIXk/+IkgU/TW6/ZyBze7p6iHdhmdzd7MMfpUdP5PaT5/4
# IFN56oArwf8fLyMc2guYLjaSUN45L5YYMkChVPQS9y8J2S59d9yPCIFarY7/6W8m
# Xpqr1YgyteHQ2McH1rXNpYnLvSfJx8UUFjdUhY9ks3Zp9cY7V4H0gBJX0gFtElkO
# s4BH8Tuo5mbjQ4V5Yskbj/1VTe6LkaYCa/6MKbS93tMLJc8s9gAlOvpvhQrFDZXl
# nrb5vvg+nhRoA5NgL6OR87JHFf7pa6oOBUPcVxQL/gx/CgYyowJFP+VL7Ft0NOQY
# LO28E1scO0wkmvoUhv33PDE+EoIZethQbOmZ+o5AzLweHDb2i+LnilSIdRfr+vZ1
# BOUDW47+hPuh/1p8CegY8+Ka4K35JMRhOli68HKFgojtgdF9CoRdllaHdN7c/Kmk
# X0njkIiquL7jbuuUd024DPk36tBmqZfDwdtZ4NwnGrEFqaiLNd3Y+65gKkuhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBFkha0GNAw9HI4/b+YWGWDMs28kS6ZcHlg
# UE/AoiMxxQIUNt03DnDT/bigNROTx5oJ9O77ojAYDzIwMjUwNTI4MTIzODQxWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCua86s4VYMsob02iqWQZBEuKwaq86A0Hpvnlap5h94
...
# CwUABIIBgLdIKqC7n4waCMqsjjIxGUmZgMeJGHL5X05a2qSdLhuOGBFUZQSU0uRL
# TkaDkFAKfmFMjnMILcz2T1hHvhJorFRPqub/Y0rs3VqQdL6FxTbBjH617NTrN5rv
# l/oSg/XMx5/WIcuzrtbdpQpNfdAsB/Q2lBzrWu/2bPbnSmYuNyG5Fl9juZXKhBn1
# CXloHYQVHIYtA+2fHGnpuyAuuedxfsC0iSoDeUBSMU9B0tuUl23jiwnX+FyDxWgN
# WS9mBKz8APyAVyLvycNdsxjaQYnfbwKi2b08JQsSzKxCuyz0l9RnUZZJ/9ETkOyn
# SpBfmVSx6+rcCsoegMadIGTgibY2GnEXx6q9BdtfOrp5+DZMVmOnthx+m/YYLCxU
# qFD7IhWMoI6VH1QOYmV36WJCL12xLB9c9RWVkX+wG/0xk7ODYjicbTs7VURcJugJ
# Hi9mKGM6onSFIhBr7V71AQiGjn/+ZLmrZH2eQ7jK6eOxpzN44jkugc0C5Ng9WeyY
# Y4WkPCkaTw==
