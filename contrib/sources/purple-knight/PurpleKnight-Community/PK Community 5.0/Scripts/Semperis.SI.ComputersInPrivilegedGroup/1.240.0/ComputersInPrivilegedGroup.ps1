# This script looks for computers that are memebers of privileged groups
...
$script:outputObjects = [System.Collections.ArrayList]@()
$failedDomains = @()
$failedObject = [System.Collections.ArrayList]@()
$script:primaryMatches = @()

function ValidateDC {
    param (
        [string]$validateDN,
        [string]$validateDomain,
        [string]$validateDomainDN
    )

    $failCondition = $false
    $serverReferenceFilter = "(distinguishedName=$validateDN)"
    $validateObject = Search-AD -dnsDomain $validateDomain -attributes "serverreferenceBL","name","objectsid" -baseDN $validateDomainDN -scope "Subtree" -filter $serverReferenceFilter

    $name = $validateObject.Attributes."name"[0]

    if ($validateObject.Attributes.serverreferencebl) {
        $ntdsDN = $validateObject.Attributes.serverreferencebl[0]
        $ntdsFilter = "(objectClass=nTDSDSA)"
        $ntdsResult = Search-AD -dnsDomain $validateDomain -attributes "" -baseDN $ntdsDN -scope "OneLevel" -filter $ntdsFilter
        if (-not ($ntdsResult)) {
            return $true
        }
    }
    elseif ($name -eq "AzureADKerberos") {
        try {
            $scpResult = $null
            $scpResult = Search-AD -dnsDomain $validateDomain -attributes "keywords" -baseDN "CN=900274c4-b7d2-43c8-90ee-00a9f650e335,CN=AzureAD,CN=System,$validateDomainDN" `
                -scope "Base" -filter "(objectClass=serviceConnectionPoint)"
        }
        catch {
            # ServiceConnectionPoint was not found (Azure AD Kerberos)
            return $true
        }

        if ($scpResult.Attributes.keywords) {
            $resultSid = (New-Object System.Security.Principal.SecurityIdentifier @($validateObject.Attributes.objectsid.GetValues("byte[]")[0],0)).Value
            $scpSid = ($scpResult.Attributes.keywords[0] | ConvertFrom-Json).ComputerAccountSid
            if (-not ($scpSid -eq $resultSid)) {
                $failCondition = $true
            }
        }
    }
...
function Search-PrivilegedGroup {
    ...
    $targetGroupDN = $targetGroup.split(";")[0]
    $targetGroupSid = $targetGroup.split(";")[1]
    $targetGroupRid = $targetGroupSid.split("-")[-1]
    $groupFailure = $false
    ...
    try {
        $searchParams = @{
            ...
            attributes = ""
            baseDN = $targetDomainDN
            scope = "Subtree"
            filter = "(&(objectCategory=computer)(memberOf:1.2.840.113556.1.4.1941:=$targetGroupDN))"
        }
        ...
        if ($groupMembers) {
            foreach ($groupMember in $groupMembers) {
                if ($targetGroupRid -eq "516" -or $targetGroupRid -eq "521") {
                    # Check if returned result is an actual DC
                    $failCondition = ValidateDC -validateDN $groupMember.DistinguishedName -validateDomain $targetDomain -validateDomainDN $targetDomainDN
                }
                else {
                    $failCondition = $true
                }

                if ($failCondition -eq $true) {
                    if ($script:primaryMatches.Contains("$($groupMember.DistinguishedName);$targetgroupDN")) {
                        continue
                    }

                    # For matches to Administrators group, we only continue if there's a direct membership
                    # This avoids reporting twice when computer is member of Enterprise Admins or Domain Admins
                    $continueOutput = $true
                    if ($targetGroupRid -eq '544') {
                        $searchParams = @{
                            dnsDomain = $targetDomain
                            attributes = "memberOf"
                            baseDN = $groupMember.DistinguishedName
                            scope = "Subtree"
                            filter = "(DistinguishedName=$($groupMember.DistinguishedName))"
                        }

                        $directMembersResult = Search-AD @searchParams

                        if ($directMembersResult.Attributes.memberof) {
                            $directMembers = $directMembersResult.Attributes.memberof.GetValues("string")

                            if (!($directMembers.Contains($targetGroupDN))) {
                    ...
                    if ($continueOutput) {
                        ...
                        [void]$script:outputObjects.Add($thisOutput)
                    }
                }
            }
        }
...
try {
    ...
    $domainPrivilegedRIDs = @("512","516","526","521")
    $forestPrivilegedRIDs = @("519","527","518")

    $forestDN = Get-DN $ForestName
    $forestPrivilegedGroupsDN = @()
    ...
    if (Confirm-DomainAvailability $ForestName) {
        ...
        foreach ($groupRID in $forestPrivilegedRIDs) {
            $searchParams = @{
            ...
            if ($forestGroupObject) {
                $forestGroupObjectDN = $forestGroupObject.DistinguishedName
                $forestGroupObjectSid = "$forestSID-$groupRID"
                $forestPrivilegedGroupsDN += "$forestGroupObjectDN;$forestGroupObjectSid"
            }
        }
    }
    else {
    ...
    $privilegedRIDs = $domainPrivilegedRIDs + $forestPrivilegedRIDs

    $primaryGroupFilter = "(&(objectCategory=computer)(|"
    ...
    foreach ($domain in $DomainNames) {
        ...
        [void]$domainGroups.AddRange($forestPrivilegedGroupsDN)
        ...
        foreach ($primaryResult in $primaryResults) {
            if ($primaryResult.Attributes.primarygroupid[0] -eq "516" -or $primaryResult.Attributes.primarygroupid[0] -eq "521") {
                # Check if returned result is an actual DC
                $failCondition = ValidateDC -validateDN $primaryResult.DistinguishedName -validateDomain $domain -validateDomainDN $DN
            }
            else {
                $failCondition = $true
            }
            ...
            if ($failCondition -eq $true) {
                ...
                if ($domainPrivilegedRIDs -match $($primaryResult.Attributes.primarygroupid[0])) {
                    $primarySid = "$domainSID" + "-" + "$($primaryResult.Attributes.primarygroupid[0])"
                    $primaryDomain = $domain
                }
                else {
                    $primarySid = "$forestSID" + "-" + "$($primaryResult.Attributes.primarygroupid[0])"
                    $primaryDomain = $forestName
                }

                $primaryDN = Get-DN -dnsDomain $primaryDomain
                $primaryDNResult = Search-AD -dnsDomain $primaryDomain -attributes "" -baseDN $primaryDN -scope "Subtree" -filter "(objectsid=$primarySid)"

                if ($primaryDNResult) {
                    if ($script:primaryMatches.Contains("$($primaryResult.DistinguishedName);$($primaryDNResult.DistinguishedName)")) {
                        continue
                    }
                }

                $script:primaryMatches += "$($primaryResult.DistinguishedName);$($primaryDNResult.DistinguishedName)"

                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $primaryResult.DistinguishedName
                    GroupDistinguishedName = $primaryDNResult.DistinguishedName
                }
                [void]$script:outputObjects.Add($thisOutput)
            }
        }
        ...
        foreach ($domainPrivilegedRID in $domainPrivilegedRIDs) {
            ...
            if ($groupObject) {
                $groupObjectDN = $groupObject.DistinguishedName
                $groupObjectSid = "$domainSID-$domainPrivilegedRID"
                [void]$domainGroups.add("$groupObjectDN;$groupObjectSid")
            }
        }
        ...
        foreach($group in $domainGroups) {
            $groupResult = Search-PrivilegedGroup -targetDomain $domain -targetDomainDN $DN -targetGroup $group
            if ($groupResult) {
                if (!($failedDomains.Contains($domain))) {
                    $failedDomains += $domain
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage