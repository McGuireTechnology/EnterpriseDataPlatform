# This script looks for msds-allowedtodelegateto to krbtgt
...
$Global:self = @{
    ...
    Schedule = '1h'
    Impact = 9
    LikelihoodOfCompromise = '<p>In Active Directory it is possible to create a Kerberos delegation to the KRBTGT account. This type of delegation to a user or computer would allow that principal to generate a ticket granting service (TGS) request to the KRBTGT account as any user.</p>
      <p>An attacker may attempt to compromise any account that has a delegation to the KRBTGT account. This particular type of configuration allows for an attack similar to a Golden Ticket attack.</p>
      <h3>References</h3>
    ...
    Remediation = '<p>It is recommended to not allow constrained delegation to the KRBTGT account.</p>
      <p>This type of delegation to a user or computer would allow that principal to generate a ticket granting service (TGS) request to the KRBTGT account as any user.</p>
      <p>An attacker may attempt to compromise any account that has a delegation to the KRBTGT account. This particular type of configuration allows for an attack similar to a Golden Ticket attack.</p>
      <p>The delegations identified should be removed.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $krbtgtDelegFilter = New-Object System.Text.StringBuilder

        # Get a of all SPNs applied on the krbtgt account
        $searchParams = @{
            dnsDomain = $domain
            attributes = "serviceprincipalname"
            baseDN = $DN
            scope = "Subtree"
            filter = "(objectsid=$domainSID-502)"
        }
        $krbtgt = Search-ADHelper @searchParams
        if ($krbtgt.Count) {
            foreach ($spn in ($krbtgt.Attributes.'serviceprincipalname'.GetValues("string"))) {
                if ($spn -ne 'kadmin/changepw') {
                    [void]$krbtgtDelegFilter.Append("(msds-allowedtodelegateto=$spn)")
                }
            }
        }

        # Get netbios name of the domain
        $netbiosName = Get-NetBIOSName -dnsDomain $domain -forestName $forestName -DomainNames $DomainNames
        [void]$krbtgtDelegFilter.Append("(msds-allowedtodelegateto=krbtgt/$netbiosName)")

        # Get a list of all objects with constrained delegation configured to krbtgt
        $searchParams = @{
            dnsDomain = $domain
            attributes = "msds-allowedtodelegateto"
            baseDN = $DN
            scope = "Subtree"
            filter = "(|(msds-allowedtodelegateto=krbtgt/$domain)$($krbtgtDelegFilter.ToString()))"
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) account(s) with Constrained Delegation configured to krbtgt service."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBDLK0Zdx0OLoP5
# QusEroEe3nTwbW4oD3LAiLVK0hBPD6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ