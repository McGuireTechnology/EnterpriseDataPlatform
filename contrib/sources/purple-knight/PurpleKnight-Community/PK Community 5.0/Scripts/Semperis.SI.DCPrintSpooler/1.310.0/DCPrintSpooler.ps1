...
$Global:self = @{
    ...
    Description = '<p>This indicator checks for Domain Controllers running the print spooler service.</p><p>This indicator <b>requires</b> the local server to be running the print spooler service to function, or it will return <b>Failed to run</b>.</p>'
    Weight = 8
    Severity = 'Critical'
    Schedule = '1h'
    Impact = 8
    LikelihoodOfCompromise = '<p>The Windows print spooler service is vulnerable to remote code execution if unpatched, commonly referred to as <b>PrintNightmare</b>. These vulnerabilities are documented in CVE-2021-34527 and CVE-2021-1675.</p>
      <p>The print spooler service can also be used by an attacker in combination with unconstrained Kerberos delegation and TGT delegation enabled across trusts.</p><h3>References</h3><p><a href="https://www.semperis.com/blog/what-you-need-to-know-about-printnightmare-the-critical-windows-print-spooler-vulnerability/" target="_blank">What You Need to Know about PrintNightmare, the Critical Windows Print Spooler Vulnerability - Semperis</a></p>
      <p><a href="https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-34527" target="_blank">CVE-2021-34527 - Security Update Guide - Microsoft - Windows Print Spooler Remote Code Execution Vulnerability</a></p>
      <p><a href="https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-1675" target="_blank">CVE-2021-1675 - Security Update Guide - Microsoft - Windows Print Spooler Remote Code Execution Vulnerability</a></p>
      <p><a href="https://msrc.microsoft.com/update-guide/en-US/advisory/CVE-2019-0683" target="_blank">CVE-2019-0683 - Security Update Guide - Microsoft - Active Directory Elevation of Privilege Vulnerability</a></p>'
    ResultMessage = 'Found {0} DCs that have the Print Spooler service running.'
    Remediation = '<p>Organizations should disable the print spooler service on domain controllers. The print spooler service has a long history of abuse by attackers, with the most recent being <b>PrintNightmare</b>.</p>
      <p>If organizations are publishing printers to Active Directory, they should evaluate disabling print spooler on domain controllers and performing a manual process to instead prune printers.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
$spooler = @"
    ...
    namespace Spooler {
        public class winspool {
            ...
            public static bool openPrinterByName(string printerName) {
                bool bResult;
                IntPtr hPrinter;
                bResult = OpenPrinter2("\\\\" + printerName, out hPrinter, null, null);
...
try {
    if (-not ('Spooler.winspool' -as [Type])) {
        Add-Type $spooler
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            baseDN = $DN
            scope = "Subtree"
            filter = "(&(objectCategory=computer)(!(name=AzureADKerberos))(|(primaryGroupID=516)(primaryGroupID=521)))"
        }

        $dcs += Search-AD @searchParams
    }

    $isSpoolerRunning = Get-Service -Name spooler
    $failedDCsCount = 0

    # Check each DC if it responds to OpenPrinter
    $dcs | Foreach-Object {
        $dc = $_
        ...
        $openResult = [Spooler.winspool]::openPrinterByName("$hostname")
        if ($openResult) {
            $results += $Fqdn
        }
        elseif ($openResult -eq 0) {
            $results += "Failed"
        }
    }

    $failedToRun = $false
    if ($isSpoolerRunning.Status -eq "Stopped") {
        $failedToRun = $true
    }

    $dcsPrintSpoolerNotRunning = 0
    if ($results -gt 0 -and $failedToRun -ne $true) {
        foreach ($result in $results){if ($result -eq "Failed"){$failedDCsCount += 1}}
        if ($failedDCsCount -ne $results.Count) {
            foreach ($result in $results) {
                if ($result -ne "Failed") {
                    $dcsPrintSpoolerNotRunning += 1
    ...
    if ($failedToRun) {
        $res.ResultMessage = "Spooler service is not running on the local computer running this indicator. Please verify it's running on the local machine to use this indicator."
        $res.Remediation = "Start the spooler service by running : Start-Service spooler"
        $res.Status = 'Error'
    }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($dcsPrintSpoolerNotRunning) DCs that have the Print Spooler service running."
            $res.Remediation = "Print spooler services are enabled by default. If not absolutely required, disable the service on all domain controllers. If required, make sure the server is fully patched and follow Microsoft guidance <a href=`"https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-34527`" target=`"_blank`">here</a>."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDefbsqGlT9zZTN
# 2vnNRagm6VTBDicNif/coSTC2ADJFKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgrld1FIG4OJaXfTa7CIbCO6mZhLgsk+/MmStJ7oEIDvEwDQYJKoZI
# hvcNAQEBBQAEggIASh846kCwJUfZKrzDvCTxQGEYx4pAiYSnHwBqnJTUu/GKjRWH
# RHavXQj9xYre/2ymAQSuOkiZ0FqFcRQGa8OBPk+OTvCnnBAMlKog8Orv7D3VV2Qv
# +fSB4Jl1SC1Lp+ldYQOnHSb63Jaid1d5DqSQJIhJhbJ8wX4jFqKbbeuz/8dY835c
# L52EK3o7ls6aR+c76opmu64uGM+tsrhQW2Cr9u7Lc/d5bRWMhUGUWkqi863Wl97c
# JecOdh9BmTadlCsa7wy+x6FjpT0rvnsY0X99epcUCfx3CdCA7zs6qmlvqQE7ESul
# 82rLA+SuTFD9LIIHX3vBWPX8MjZ/kXT7a4ZKdpFRGWoFbGrmb4qR/FGh63Zi352k
# N2CQp+YGkxcl+w0AeEfxskNHmF+Ldg7B5MHeTcVg3x/vpkOQZMSB60tH8xjpnGAP
# jhy78maJdyNwarBxv6ztWRt4mHSfqEKpNd5172tA35E9NlfvW8HnbzrP17k46QfD
# 0xJOx0CnvTbHfeU1eexd6mPFP/vjBSA345fBlG0J8WUvG1QYRUs4/u0LK511TZlo
# YhzJ2BPDf5uOs/33ZFCvQ/VLsNRFd7SNDlGaCGoP0jdRyeBHZQcLWcxcma3KVLN6
# hlv2Bx+rTsFCJQs9n1eQnJs4sFAq1XVnQZLLBirwk1DxY6NfwbIRaBpZn5Ghgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAAleqY7JZdojwH1el1xUPs1rY4d/PTCteT
# nrssHc8VrgIURTtT0GxIFnX1E+B3rq4eWo2hYTMYDzIwMjUwNjEwMDc1MTIzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCQrjrNlDN0CDhtf2VgId2AKKSzQS0JEWPg8/q1bmXo
...
# CwUABIIBgCx0mbjkFaPomMb1viOR/8orl2CAQyqjO9RlkMojYKZh5pd0Ajw7IVkq
# CQS4B0nD4qOpATqSPM2AfbQf8GkB4oUj4KBv/Rczklna7zIJF073BMLtullIBLLf
# 78vOguw1jJ6mFl5lmDcH93tPk8OnVPXlQZmhpaGKcDB+wuaQUrEeTEMasp1Y+HtH
# +MABbKnugihe2Fl6MsFln6grows8aOVw4cXYadaDjYxPfutQumxBiTTrURHwJmnx
# fF5Wn06i/YbveWQZID1J4XzNUWe2xctAKEurRr9jPC/apM4hdyV/07ZgjxH7hPeL
# IMjoAPyGFKyBM5OgyZGo53YhVrrAlAAes+MH1tZD/cVVrxR9XHn35Q00PlFo8Cfo
# eGkSjhqGqIF+bma7Q7/AlQzQQqLzyl1Ex8x8LBwj0SYGYOg1tYSqvqgelO53iFvC
# NlIlaETyuE6KlsN1DpNlZyHjvVTgsWdCF7v+9ACOSOT4PpgsrJ5nzr4v7OoMbdEL
# TUypuyjP4Q==
