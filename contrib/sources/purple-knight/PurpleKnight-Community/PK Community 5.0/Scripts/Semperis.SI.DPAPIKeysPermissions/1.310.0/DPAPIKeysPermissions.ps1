# This script looks for DCs that permits non default principals to view private information (e.g. use LsaRetrievePrivateData to retrieve the DPAPI backup key)
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>An attacker could recover all domain data encrypted via DPAPI, if they gain access to such data.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_permissions_dpapi"> Dangerous ACLs expose DPAPI key objects (attack path)
 | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_permissions_dpapi', 'vuln2_permissions_dpapi') }
    )
    Products = @(
...
$DPAPI = @"
    ...
    namespace PInvokeDPAPI
    {
        public class LSA
        {
            ...
            public static extern int LsaQuerySecurityObject(
                IntPtr ObjectHandle,
            ...
            [DllImport("advapi32.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
            public static extern int LsaSetSecurityObject(
                IntPtr ObjectHandle,
                int SecurityInformation,
            ...
            [DllImport("advapi32.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
            public static extern int ConvertSecurityDescriptorToStringSecurityDescriptorA(
                IntPtr SecurityDescriptor,
            ...
            [DllImport("advapi32.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
            public static extern int ConvertSecurityDescriptorToStringSecurityDescriptorW(
                IntPtr SecurityDescriptor,
                int RequestedStringSDRevision,
                int SecurityInformation,
                out string StringSecurityDescriptor,
            ...
            public static string LsaGetSDDL(string hostname)
            {
                uint aWinErrorCode = 0;
                LSA_UNICODE_STRING systemName = InitLsaString(hostname);
                LSA_OBJECT_ATTRIBUTES oa = new LSA_OBJECT_ATTRIBUTES();
                IntPtr handle, sd;

                uint aOpenPolicyResult = (uint)LsaOpenPolicy(systemName, oa, 0x02000000, out handle);
                aWinErrorCode = LsaNtStatusToWinError( aOpenPolicyResult );
                if( aWinErrorCode == 0 )
                {
                    uint aQueryPolicyResult = (uint) LsaQuerySecurityObject(handle, 7, out sd);
                    aWinErrorCode = LsaNtStatusToWinError( aQueryPolicyResult );
                    if( aWinErrorCode == 0 )
                    {
                        string sddl;
                        int sddlSize;
                        ConvertSecurityDescriptorToStringSecurityDescriptorW(sd, 1, 7, out sddl, out sddlSize);
                        return sddl;
                    }
                }
                return null;
...
try {
    if (-not ('PInvokeDPAPI.LSA' -as [Type])) {
        Add-Type $DPAPI
    }
    ...
    # Check each DC if non default principals have POLICY_GET_PRIVATE_INFORMATION access on the LSA policy object
    $res.Score = 100
    $couldNotQuery = 0
    ...
    foreach ($dc in $dcs) {
        ...
        $lsaSddl = [PInvokeDPAPI.LSA]::LsaGetSDDL("$fqdn")
        if (!$lsaSddl) {
            $couldNotQuery += 1
            $thisOutput = [PSCustomObject][Ordered] @{
                HostName = $fqdn
                AllowedPrincipals = "Could not query security descriptor"
            }
            [void]$outputObjects.Add($thisOutput)
            continue
        }
        $sd = New-Object System.DirectoryServices.ActiveDirectorySecurity
        $sd.SetSecurityDescriptorSddlForm($lsaSddl)
        $allowedPrincipals = @()
        foreach ($access in $sd.Access) {
            if ($access.ActiveDirectoryRights -band 4) {
                ...
                if ($identitySID -eq "S-1-5-32-544") {
                    continue
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            if ($couldNotQuery -eq 0 ) {
                $res.ResultObjects = $outputObjects
                $res.ResultMessage = "Found $($outputObjects.Count) DCs that allows non default principals to read private data from the LSA policy database."
                $res.Remediation = "These permissions allow a principal to use LsaRetrievePrivateData to retrieve the domain DPAPI backup keys. Review these principals, investigate and remove any unneeded permissions using adsiedit.msc or the ldp utility."
                $res.Status = 'Failed'
                $res.Score = $scoreOutput.Score
            }
            else {
                $res.ResultObjects = $outputObjects
                $res.ResultMessage = "Found $($outputObjects.Count-$couldNotQuery) DCs that allows non default principals to read private data from the LSA policy database, And $($couldNotQuery) that couldn't be queried."
                $res.Remediation = "These permissions allow a principal to use LsaRetrievePrivateData to retrieve the domain DPAPI backup keys. Review these principals, investigate and remove any unneeded permissions using adsiedit.msc or the ldp utility."
                $res.Status = 'Failed'
                $res.Score = $scoreOutput.Score
            }
        ...
        if ($couldNotQuery -ge $outputObjects.Count) {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "There are {0} DCs that their LSA policy security descriptors couldn't not be read." -f $couldNotQuery
            $res.Status = 'Error'
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA993Cf71qT1L75
# vP+yCgd1p88bF+dUWDP4cB47Jzj8SqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgRj4HILJSVC23YSGIbHoHNWo24O3Xn50h9+CZjyXPYCwwDQYJKoZI
# hvcNAQEBBQAEggIASgOzUMq+kKEdsnKiLZGirAe7sQCzHd0fzCDKIxi2HxvmzNfj
# zSoGxMwAiJ5gn3wQWOipMkmyJQ9eJmxqPnQa3l3H/jczbhSDAsB1n57ccje9G+Nb
# djbXFSqLl0JAPo1YWVxmHlJ0dS+FQ3ouoOdwPER9oTrz5geJljeP7tZhE+tRjhEm
# D0JcfcAH7FbqszGOPXVNypaIjDvw8YeTMZN68GSBk5LZMH29mpeQI+YGCEhUX5uJ
# KPhK95q01oSjN3w1nUUwVydXh6X3cUopUU5UjDH4l4Xc/ci24N2hS64HQDONE3pu
# mk3+A8g8wxw4bpYROnOrV+2yUcJSnVCFoVaSaXzs6RC+r8m/F8jVtLxOF7qAfMNg
# E+szDh1p0Ty/pKjWnOz6sKW0Ia6l+x/7e3C1rBNKZBLdXE/fxAhWi32AGeMXJbuz
# qIdC0J6tDFF2AOyo13Ey8Fr7YvTrtmYEN1X8X1Y+s2DZ03tyU+GYUZFh5HmcIPc5
# NCeifsjRiSo6HW4CreW4vMSJCHqYjf9UmutzQM9d2qFBDop3H2vBWk0fz8mqYtFu
# 8p+oFisyjfmbB/Im5+iFRJE7d8BIiozwhHWqEr15EYEQFe+GYd7y5XOYzRAtiSw2
# M2xgr+FNJUkfYBirTydHfvUvgw0brjLMskjQWAYanUw2soQdXqc0KszkoGqhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDm0N0L5AdoEMhphdydU/JbF44EoVp69LDh
# bqKdD3HMzgIULueZ9+2z32UadLoRaQwfmFLaib0YDzIwMjUwNjEwMDc1MTQ5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDNHDIil76S4UKj6/sjZCXGJitvU72mZCn+6ZugdzQo
# FzCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgG/Dg6NTF49o0wkcn4AphwNU+cyZW6E92fstG4F2lHGv5p9vACu6uXnq
# 5m22nM1G5iTisqQAcOckFOA3dEMwdRWm94wgIiXMU7VOdG3V0Br0e+Gcy9O904DI
# 2YEYZONWXBvZJYWUlLMyu4C23QNTvPR43sb/6d0Nl9FajDJGa5eZbB3R8GSvZQNh
# PIYJIEw8aVj4fyOmKwNiP5o4VopDHaNeOCl2uBdihKiaubrGl1drrUl7Lvu3+TLf
# EdVhqWxFjehB09mEYILTw0wzVfNraI+iWaiYWJIU9WjEVUp1K4g7u4GPYT/cURzP
# EERraQxlO8q/VbLzdy8WKjib0uVgd/omgZPKeJ7qw4lSfeunUUvE9EeC/bFcZcyI
# h9S9hSm1H/mSgzEL8Y0dOC7wkpeO46O7grIuO4NCj40bAQZ1HA3xPpeO51l2gvGw
# lwzF7UEb8yA740qbeVWr91ZQxjc891wJWx9NzZFSPaG3LtF3CXYxBKXD7XyhAsMK
# robw3m54nQ==
