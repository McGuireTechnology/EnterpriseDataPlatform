...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>An attacker that has compromised a remote domain can spoof any user or machine in the local domain. This can allow the attacker to access any resource as well as escalate their privileges, thus compromising the entire forest.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_trusts_domain_notfiltered" target="_blank">Unfiltered outbound domain trust relationship | ANSSI</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_trusts_tgt_deleg" target="_blank">Inbound trust relationships with delegation | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_trusts_domain_notfiltered', 'vuln3_trusts_tgt_deleg') }
    )
    Products = @(
...
<#
TAPT:  TRUST_ATTRIBUTE_PIM_TRUST = 0x00000400 = 1024
TAEC:  TRUST_ATTRIBUTE_CROSS_ORGANIZATION_ENABLE_TGT_DELEGATION = 0x00000800 = 2048
#>
...
try {
    ...
    $attributes = @("flatName", "trustAttributes")
    ...
    foreach ($domain in $DomainNames) {
        ...
        $propertyFilter = "trustAttributes"

        $results += Search-ADHelper -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter $filter -StartAttackWindow $StartAttackWindow -EndAttackWindow $EndAttackWindow -PropertyFilter $propertyFilter

        foreach($result in $results) {
            $trustAttribute = $result.Attributes.trustattributes.GetValues("string")[0]
            $trustDomain = $result.Attributes.flatname.GetValues("string")[0]

            $dsetting = [System.Collections.ArrayList]@()

            if (($trustAttribute -band 0x00000400) -eq 0x00000400) {
                $dsetting += "TRUST_ATTRIBUTE_PIM_TRUST"
            }

            if (($trustAttribute -band 0x00000800) -eq 0x00000800) {
            ...
            if ($dsetting.count -gt 0) {
                ...
                $updateSearchParameters = @{
                    ...
                    Metadata = $result.attributes.attributemetadata
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) domains with dangerous trust attribute settings."
            $res.Remediation = "Confirm that the trust attributes are configured as desired."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAKgZvH/wkrV5/J
# gBiHiGucFLeSfkC3SXygThCajrrLpqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg5702X129Jw0cl8tQB63f4wxZGlqQmMu7JmEEWabbY0wwDQYJKoZI
# hvcNAQEBBQAEggIAfEehaMmbnREdj5gmqJIt3FlXQgSebQJEH2BX5ZGHZpb+Hne5
# kToBixTfMaLnUlKbwuwwf4TwDnE3j8eH2QqgjkNR8Gc4iPY2yWNjuzRLSRMVCd8W
