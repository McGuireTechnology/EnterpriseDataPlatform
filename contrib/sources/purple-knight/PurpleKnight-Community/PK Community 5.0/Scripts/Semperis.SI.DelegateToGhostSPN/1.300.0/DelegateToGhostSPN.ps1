# This script looks for msds-allowedtodelegateto to ghost SPNs
...
$Global:self = @{
    ID = 100
    UUID = '55f8f10b-deef-4f24-8567-1da71161340c'
    Version = '1.300.0'
    CategoryID = 5
    ShortName = 'SI000100'
    Name = 'Accounts with Constrained Delegation configured to ghost SPN'
    ScriptName = 'DelegateToGhostSPN'
    Description = 'When computers are decommissioned, delegation configuration to them is often not cleaned up. Such a delegation could allow an attacker that has the privileges to write to the ServicePrincipalName attribute of another service account, to escalate privileges on those services. This could result in escalating privileges by moving laterally across the infrastructure. This indicator looks for accounts that have Constrained Delegation configured to ghost SPNs.'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>This type of attack should be easy to spot as the configured SPN within the msds-allowedtodelegateto attribute will not exist on the domain. However, if they are found, they would represent a significant risk and should be mitigated quickly.</p>
         <h3>References</h3>
    ...
    ResultMessage = 'Found {0} account(s) with Constrained Delegation configured to ghost SPN(s).'
    Remediation = 'Remove any Kerberos delegations to ghost SPN(s).'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $hostSPNMappings = Get-HostSPNMapping -domainNames $DomainNames -forestName $ForestName
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a of all accounts with constrained delegation configured
        $searchParams = @{
            dnsDomain = $domain
        ...
        $delegations = Search-AD @searchParams
        if ($delegations.Count) {
            foreach ($delegation in $delegations) {
                # loop through each configured SPN and search to see if it exists
                foreach ($spn in ($delegation.Attributes.'msds-allowedtodelegateto'.GetValues("string"))) {
                    $searchParams = @{
                        dnsDomain = $domain
                        attributes = ""
                        baseDN = $DN
                    ...
                    $spnTarget = Search-AD @searchParams
                    if ($spnTarget.Count -lt 1) {
                        # if not found check if it is a mapped SPN and if so search for the HOST SPN
                        $service = $spn.Split('/')[0].ToLower()
                        if ($hostSPNMappings.Contains($service)) {
                            $hostFilterSPN = "HOST$($spn.Substring($service.Length))"
                            $searchParams = @{
                                dnsDomain = $domain
                                attributes = ""
                                baseDN = $DN
                                scope = "Subtree"
                                filter = "(serviceprincipalname=$($hostFilterSPN))"
                            }
                            $hostSPNTarget = Search-AD @searchParams
                            if ($hostSPNTarget.Count -lt 1) {
                                # it was a mapped SPN but HOST doesn't exist
                                $thisOutput = new-object psobject -Property @{
                                    DistinguishedName = $delegation.DistinguishedName
                                    DomainName = $domain
                            ...
                            # it wasn't a mapped SPN and didn't exist
                            $thisOutput = new-object psobject -Property @{
                                DistinguishedName = $delegation.DistinguishedName
                                DomainName = $domain
                                ServicePrincipalName = $spn
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) account(s) with Constrained Delegation configured to ghost SPN(s)."
            $res.Remediation = "Remove any Kerberos delegations to ghost SPN(s)."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAFPTgkzz3foM1V
# cd4O+vEONCNUhUdWKoHfrki3RxZkXKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ