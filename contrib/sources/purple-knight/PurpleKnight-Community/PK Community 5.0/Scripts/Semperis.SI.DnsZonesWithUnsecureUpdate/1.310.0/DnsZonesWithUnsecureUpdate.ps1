# This script looks for DNS zones configured with ZONE_UPDATE_UNSECURE
...
$Global:self = @{
    ...
    Remediation = '<p>Reconfigure DNS zones to only allow secure updates using the following command: dnscmd <servername> /Config <zone> /AllowUpdate 2</p>
      <h3>References</h3>
     <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dnszone_bad_prop" target="_blank">Misconfigured DNS zones | ANSSI</a></p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_dnszone_bad_prop', 'vuln3_dnszone_bad_prop') }
    )
    Products = @(
...
public struct DNSProperty {
    ...
    public DNSProperty(byte[] dnsProperty) {
        Data = null;
        DataLength = BitConverter.ToInt32(dnsProperty, 0);
        NameLength = 1;
        flag = 0;
        Version = BitConverter.ToInt32(dnsProperty, 12);
        Id = (PropertyId)BitConverter.ToInt32(dnsProperty, 16);
        if (DataLength != 0) {
            Data = BitConverter.ToString(dnsProperty, 20, DataLength);
        }
        else {
            // Some defaults may be wrong
            switch (Id) {
                case PropertyId.DSPROPERTY_ZONE_TYPE:
                    Data = 1;
                    break;
                case PropertyId.DSPROPERTY_ZONE_ALLOW_UPDATE:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_SECURE_TIME:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_NOREFRESH_INTERVAL:
                    Data = 168;
                    break;
                case PropertyId.DSPROPERTY_ZONE_REFRESH_INTERVAL:
                    Data = 168;
                    break;
                case PropertyId.DSPROPERTY_ZONE_AGING_STATE:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_SCAVENGING_SERVERS:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_AGING_ENABLED_TIME:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_DELETED_FROM_HOSTNAME:
                    break;
                case PropertyId.DSPROPERTY_ZONE_MASTER_SERVERS:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_AUTO_NS_SERVERS:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_DCPROMO_CONVERT:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_SCAVENGING_SERVERS_DA:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_MASTER_SERVERS_DA:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_AUTO_NS_SERVERS_DA:
                    Data = 0;
                    break;
                case PropertyId.DSPROPERTY_ZONE_NODE_DBFLAGS:
                    Data = 0;
                    break;
            }
        }
...
try {
    if (-not ('DNSProperty' -as [Type])) {
        Add-Type $dnsPropertyType
    }
    ...
    # Get partitions in the forest
    $searchDN = "CN=Partitions,CN=Configuration,$forestDN"
    $filter = "(&(objectClass=crossRef)(|(dnsRoot=ForestDnsZones*)(dnsRoot=DomainDnsZones*)))"

    $searchParams = @{
        attributes = "nCName","msDS-SDReferenceDomain"
        baseDN = $searchDN
        scope = "Subtree"
        filter = $filter
    }

    $resultsPartitions, $unavailableDomains = Search-ADConfig -forestName $ForestName -domainNames $DomainNames -searchADParameters $searchParams

    if (!($unavailableDomains)) {
        $unavailableDomains = [System.Collections.ArrayList]@()
    }

    foreach ($partition in $resultsPartitions) {
        $referenceDomain = $partition.Attributes.'msds-sdreferencedomain'[0].replace("DC=","").replace(",",".")
        if ($unavailableDomains.Contains($referenceDomain.ToLower())) {
            continue
        }
        else {
            if (-not (Confirm-DomainAvailability $referenceDomain)) {
                [void]$unavailableDomains.Add($referenceDomain.ToLower())
                continue
            }
        }

        $nCName = $partition.Attributes.ncname[0]

        # Check if this partition has any 'MicrosoftDNS' containers
        $searchDN = "$nCName"
        $filter = "(name=MicrosoftDNS)"
        $attributes = @("")
        $resultDNS = Search-AD -dnsDomain $referenceDomain -baseDN $searchDN -scope "OneLevel" -filter $filter

        if ($resultDNS) {
            $searchDN = "CN=MicrosoftDNS,$nCName"
            $filter = "(&(objectClass=dnsZone)(dnsProperty=*)(!(name=RootDNSServers))(!(name=..TrustAnchors))(!(name=..InProgress*))(!(name=*CNF:*)))"
            $attributes = @("dnsproperty","name")

            $results += Search-AD -dnsDomain $referenceDomain -attributes $attributes -baseDN $searchDN -scope "Subtree" -filter $filter
        }
    }

    if (-not $unavailableDomains.Contains($ForestName.ToLower())) {
        foreach ($domain in $DomainNames) {
            $domainDN = Get-DN $domain
            $searchDN = "CN=System,$domainDN"
            $filter = "(name=MicrosoftDNS)"
            $attributes = @("")
            $resultDNS = Search-AD -dnsDomain $domain -baseDN $searchDN -scope "OneLevel" -filter $filter

            if ($resultDNS) {
                $searchDN = "CN=MicrosoftDNS,CN=System,$domainDN"
                $filter = "(&(objectClass=dnsZone)(dnsProperty=*)(!(name=RootDNSServers))(!(name=..TrustAnchors))(!(name=..InProgress*))(!(name=*CNF:*)))"
                $attributes = @("dnsproperty","name")
                $results += Search-AD -dnsDomain $ForestName -attributes $attributes -baseDN $searchDN -scope "Subtree" -filter $filter
            }
        }
    }

    foreach ($result in $results) {
        foreach ($dnsPropertyEnc in $result.Attributes."dnsproperty".GetValues("Byte[]")) {
            $dnsProperty = New-Object DNSProperty(,$dnsPropertyEnc)
            if ($dnsProperty.Id -eq "DSPROPERTY_ZONE_ALLOW_UPDATE" -and $dnsProperty.Data -eq "01") {
                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $result.DistinguishedName
                    ZoneName = $result.attributes."name"[0]
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage =  "Found $($outputObjects.Count) DNS zones configured with ZONE_UPDATE_UNSECURE."
            $res.Remediation = "Reconfigure DNS zones to only allow secure updates using the following command: dnscmd <servername> /Config <zone> /AllowUpdate 2"
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC