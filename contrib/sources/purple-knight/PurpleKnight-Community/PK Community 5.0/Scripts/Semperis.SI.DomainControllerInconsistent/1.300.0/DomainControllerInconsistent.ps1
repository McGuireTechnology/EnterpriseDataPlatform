# This indicator looks for Domain Controllers that may be in an inconsistent state, indicating a possible rogue or otherwise non-functional DC. DCs in a consistent state are characterized by the following: 1. useraccountcontrol attribute on the DC machine object has one of SERVER_TRUST_ACCOUNT or PARTIAL_SECRETS_ACCOUNT flag set. 2. A corresponding object of type server exists for the DC in the configuration partition. 3. That server object must have a child NTDS Settings object of type nTDSDSA.
[CmdletBinding()]
param(
...
$Global:self = @{
    ID = 69
    UUID = 'fce4fe73-2676-4c3c-bf7e-97529ca7d118'
    Version = '1.300.0'
    CategoryID = 3
    ShortName = 'SI000069'
    Name = 'Domain Controllers in inconsistent state'
    ScriptName = 'DomainControllerInconsistent'
    Description = '<p>This indicator looks for Domain Controllers that may be in an inconsistent state, indicating a possible rogue or otherwise non-functional DC. DCs in a consistent state are characterized by the following:</p>
      <ol>
        <li>UserAccountControl attribute on the DC machine object has the SERVER_TRUST_ACCOUNT flag set.</li> 
        <li>A corresponding object of type server exists for the DC in the configuration partition.</li>
        <li>That server object must have a child NTDS Settings object of type nTDSDSA.</Description></li>
        </ol>'
    Weight = 3
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>Illegitimate machines acting as DCs could indicate someone has compromised the environment (e.g. using DCShadow or similar DC spoofing attacks). At the very least, partially functional legitimate DCs could represent a security risk if they are compromised.</p>
      <h3>References</h3>
     <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dc_inconsistent_uac" target="_blank">Domain controllers in inconsistent state | ANSSI</a></p>'
    ResultMessage = 'Found {0} Domain Controllers that are in an inconsistent state'
    Remediation = 'Ensure that all DCs within your environment are healthy and functional. It is not uncommon for attackers to attempt to stand up "fake" DCs; however, these DCs are generally distinguishable from real DCs by the metadata found by this security indicator.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
        @{ Name = 'DistinguishedName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Error'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AD')
    Permissions = @()
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Privilege Escalation', 'Resource Development') },
        @{ Name = 'ANSSI'; Tags = @('vuln1_dc_inconsistent_uac') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $attributes = @("useraccountcontrol", "serverreferencebl", "name", "objectSid")
        ...
        foreach ($result in $results) {
            $userAccountControl = $result.Attributes."useraccountcontrol"[0]
            $distinguishedName = $result.DistinguishedName
            $name = $result.Attributes."name"[0]
            $serverReferenceBL = $result.Attributes."serverreferencebl"[0]

            # read/write domain controllers =0x2000, RODC=0x4000000
            if ((($userAccountControl -band 0x2000) -eq 0x2000) -Or (($userAccountControl -band 0x4000000) -eq 0x4000000)) {
                if ($null -ne $serverReferenceBL) {
                    # search sites Container in config partition to see if a server object has the same distinguished name in serverReferenceBL
                    $forestDN = Get-DN $ForestName

                    $resultsConfig = Search-AD -dnsDomain $domain -attributes "serverreference" -baseDN "CN=Sites,CN=Configuration,$forestDN" `
                        -scope "Subtree" -filter "(&(objectClass=server)(distinguishedName=$serverReferenceBL))"

                    if ($resultsConfig.Count -eq 1) {
                        $serverReference = $resultsConfig.Attributes."serverreference"[0]
                        # check serverReference matches the DN of the domain controller we found
                        if ($serverReference -eq $distinguishedName) {
                            # check we have a child object of CN=NTDS Settings with object of nTDSDSA

                            $nTDSDSADN = 'CN=NTDS Settings,' + $resultsConfig.DistinguishedName
                            try {
                                $nTDSDSA = [ADSI]"LDAP://$nTDSDSADN"
                                $nTDSDSA.Properties["objectClass"][0] | out-null
                            }
                            catch {
                                # child object of class nTDSDSA not found
                                $thisOutput = new-object psobject -Property @{
                                    DistinguishedName = $distinguishedName
                                    Error = 'nTDSDSA missing'
                                }
                                [void]$outputObjects.Add($thisOutput)
                            }
                        }
                        else {
                            # $serverReference does not match $distinguishedName
                            $thisOutput = new-object psobject -Property @{
                                DistinguishedName = $distinguishedName
                                Error = 'ServerReference missing or misMatch'
                            }
                            [void]$outputObjects.Add($thisOutput)
                        }
                    }
                    else {
                        # could not find a server object in config partition that matches serverReferenceBL
                        $thisOutput = new-object psobject -Property @{
                            DistinguishedName = $distinguishedName
                            Error = 'Server object missing in config partition'
                        }
                        [void]$outputObjects.Add($thisOutput)
                    }
                }
                else {
                    if ($name -eq "AzureADKerberos") {
                        try {
                            $scpResult = $null
                            $scpResult = Search-AD -dnsDomain $domain -attributes "keywords" -baseDN "CN=900274c4-b7d2-43c8-90ee-00a9f650e335,CN=AzureAD,CN=System,$DN" `
                                -scope "Base" -filter "(objectClass=serviceConnectionPoint)"
                        }
                        catch {
                            # serverReferenceBL is empty for "AzureADKerberos" & ServiceConnectionPoint was not found (Azure AD Kerberos)
                            $thisOutput = new-object psobject -Property @{
                                DistinguishedName = $distinguishedName
                                Error = 'AzureADKerberos computer object exists, but ServiceConnectionPoint was not found. There may be errors within your Azure AD Kerberos configuration.'
                            }
                            [void]$outputObjects.Add($thisOutput)
                            continue
                        }

                        if ($scpResult.Attributes.keywords) {
                            $resultSid = (New-Object System.Security.Principal.SecurityIdentifier @($result.Attributes.objectsid.GetValues("byte[]")[0],0)).Value
                            $scpSid = ($scpResult.Attributes.keywords[0] | ConvertFrom-Json).ComputerAccountSid
                            if ($scpSid -eq $resultSid) {
                                continue
                            }
                        }

                        # serverReferenceBL is empty for "AzureADKerberos" & the Azure AD Kerberos configuration is missing
                        $thisOutput = new-object psobject -Property @{
                            DistinguishedName = $distinguishedName
                            Error = 'AzureADKerberos computer object exists, but Azure AD Kerberos configuration is inconsistent. There may be errors within your Azure AD Kerberos configuration.'
                        }
                        [void]$outputObjects.Add($thisOutput)
                    }
                    else {
                        # serverReferenceBL is empty
                        $thisOutput = new-object psobject -Property @{
                            DistinguishedName = $distinguishedName
                            Error = 'Server object missing in config partition'
                        }
                ...
                # computer object did not have correct UAC
                $thisOutput = new-object psobject -Property @{
                    DistinguishedName = $distinguishedName
                    Error = 'UserAccountControl is incorrect'
                }
                [void]$outputObjects.Add($thisOutput)
            }
        }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.count) Domain Controllers that are in an inconsistent state."
            $res.Remediation = 'Ensure that all DCs within your environment are healthy and functional. It is not uncommon for attackers to attempt to stand up "fake" DCs; however, these DCs are generally distinguishable from real DCs by the metadata found by this security indicator.'
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCRSVH1QliZfY+G
# rDwD8PBndfIAyPDGhBdX9Fsdjko2vqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgUyKDTbgVdrHi+FzzwNzerkYlzOLq12fvMVxPcZxDBHgwDQYJKoZI
# hvcNAQEBBQAEggIAQVGFSUPOB25fpgy98QtYrqXeVhQ2vAHhd79qPQWFzYiDOblj
# ewzcM+a6N38uJN1EXX7fJkPfqFHsUeWJX60yYkwOkgrng2FXq60XjbyvIcEzq/qC
# V4uSkIzO3aRG0glM6R8MpOZTz1rQGPxq8N3/+Yq7bMnTMmAR9ee1ocpBOq0yE6Xg
# KSsbv3D79s4KZCdkXI8sk6N0Oq8wHDjIm0SPicupGCKQ6/3LQ3fBb67E/dAtceZl
# UcmUeclK/XgKiZtqA5Zfqa8HvfGyIr82zXYYG0YcC9jlhzBnUaPr6JZduj3WgVNE
# nt0hQzbHmeuftHchdgj8N1SPdKvn7EBRaMOMf7RZhr/Y8MYsgRwjXPSdYqPCrgxh
# FqJIBKZOjsm59NQic+XK7H74Dc2ofvc8DzxqgPTrsnvdVEG/kOV4nwYSz23iEa0J
# ntyBlc16+5ZBibKgykEXhJhs+d8yFzSRrNoWVjYnvfoo7ubwSJ/MvbuOBtXCHJy+
# OmB7nxyVwUSi1aEnVDA0/vvDru+ySjctyxELziH2krObpJvfs+RQ407YZgQ58/pD
# Xa1vxa5iJsES2ULMirDvCBnF09KetyHqagR0qJuyFHCyw0aqAZoetv2RR5hnJFxl
# izhUSNO0q2SHZtHb9C8FldFDEftNvq3uyZ1bJ8QBFiREma/py+IMi37WeBmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCB3D4so3gEx/yu3jZyR1cjXgRqZD3QcmMYB
# YFKStRkXygIUHSNQrednXaitD8m1Ka7I8N0gAQYYDzIwMjUwNTI4MTIzOTI0WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDmwgmyGSmf2X69NPK269tsuGILbeQzN00NaPCE1uEJ
# lDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgJcouU4/+mtDziEFTqRySkUNW7PAfqmPtY93dupJuoE119gumJeGG0PH
# q8sOteXiTk1vaBCMFrGZBucphircVpupwGF41La6Hmn+HApmJLnmUPZh/t+DLO1L
# 5LVLBLDGScapEANX/YQ+U0K5vyrf1iTfgTMaRKbeXkDc4OaJXLc39Pk7BldrZK3Z
# P/wxhtjYYWB33RWU20PfVdxwGkb7dOGMKBVTShhBosPpFhuDx6mFQxW+70Zgo8pq
# feIxhRmDEV3awOwCDhENDLzqF8UIOqWBWm/xPyeJk8/HYYBPJpaZ70M5BXa9Y79C
# uW1UKtMSVwOA2lpgJsOH59jxEyHxj4H8ugvOsOZzujGB4Ocb97pQ3bdziLTZkbcM
# Gpu8SVbRjA5w3B0DQWeEiTFiQ3U+NrVRMRl/TNQ8+9B23Jufl+HCkVDE9D8FEXJN
# Qns39a9GjI0k2JALtv9iOW52Q6N9zQRtfMdLSD/M6Vq/WBLVlaK0o9qBzwOVQR7N
# RC4sl3nyeQ==
