...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Normally the default behavior for AdminSDHolder SDProp should be left intact. If its behavior is modified, this could indicate an attempt at defense evasion.</p>
        <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/e5899be4-862e-496f-9a38-33950617d2c5">6.1.1.2.4.1.2 dSHeuristics</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dsheuristics_bad">Dangerous dsHeuristics settings | ANSSI</a></p>'
    ResultMessage = 'Found non-default configuration on the forest for SDProp''s protected groups.'
    Remediation = '<p>Set dwAdminSDExMask flag (16th byte) on dsHeuristics to 0 or SDProp (AdminSDHolder) behavior will not fully apply to Operator groups.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    if ($result.Count -gt 0) {
        #now check to see if the 16th byte in dsHeuristics is not a 0
        ...
        if ($flag.Count -ge 16 -and $flag[15] -ne "0") {
            ...
            if ("0123456789abcdef" -match $flag[15]) {
                ...
                foreach ($key in $operatorgroups.keys) {
                    if ($operatorflag -band $key) {
                        [void]$grouplist.Add($operatorgroups[$key])
                    }
                }
            }
            else {
                [void]$grouplist.Add("Invalid value")
            }
            ...
            $thisOutput = new-object psobject -Property @{
                ...
                GroupsExcluded = $grouplist.ToArray() -join ", "
                LastChanged = $result.Attributes."whenchanged"[0]
            }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found non-default configuration on the forest for SDProp's protected groups."
            $res.Remediation = "Set dwAdminSDExMask flag (16th byte) on dsHeuristics to 0 or SDProp (AdminSDHolder) behavior will not fully apply to Operator groups. For more information see <a href=`"https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/e5899be4-862e-496f-9a38-33950617d2c5`">6.1.1.2.4.1.2 dSHeuristics</a> "
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAmdhhvXCwWvV/S
# ELc5eelOoaAnHcmLBNxdOzp8nF7CcKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgCETMNKG9xu3bp88vremiLPOxaR3ZcGmg8dgKNYO+5nwwDQYJKoZI
# hvcNAQEBBQAEggIAKToJEla19tb53kdb+Rwses4MPPFpktiM/rJsRx4AQOXU3Y84
# CJGyCt7LbbZKxMj+/2gQHEfB2wgnlztrURX4R8gO6f6qqvQndx9CCA6TMvkTlY8X
# o9IiBIOlH9ytbOiPbUB6hYN4GNd577DcB5CoktiF2rjtkCJZqN7eNs7w9qaLufxe
# YC28cKjx08hoatJlRop2DXBIkRMsTso5APoFr6gNIZ0GM6uCJLUaxuu34iBtKFYf
# d7iFMmDOVRCV7Y54WUjYvXa23mTEzaB0COkpJJsIcVqZgRekzWbiM5cdB+vXZuks
# gVhOAVIeC+t1iCzGfuB73MbxMDMK8PU/uPcjSVs9Y6LvGz4Dwhs7O9KOJJdlR2hS
# 6cbXlhRKzpzV7BcDg2+lCwQcXgbyAcgMLjvj1FJTlyxTAdRGsBCH41AWWU+swQpl
# ucE9YHs+yG2Vp9IaOhEVvBFNAqDtvWBFJM51rYytMuX3mCChD/J0aXbO43lTB5fw
# YLC+Ph+0ZgXoH/hiwNiLrF9vTD3d+w4dU/JWT3X5HyDjPcuMdTmwQ/RxEwe4tj9O
# TiQi70DPVqHXQd/1l5xKQAqta5Ffj4TLUDlYJHimgiVJX+7mm/jiGcoAVE+oPviL
# G9+hydOwwkerN3tFXIREVaPp89fjUB7bLol5OElSkggoYJawK/0S1U1kgcyhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCtUP4ZIzEm6p24AHeZCyCeg0ddbA2km714
# NfIC2d9mqgIUDYGKcJZZf2Yd5g9pG/3UqKS5574YDzIwMjUwNTI4MTIzOTQ2WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDd6El1JdfTvOHZfLMIb32+ienpMYy5besZR9N0BE0D
# 7jCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgBhhGB0vk/FC/yqqrOIZZisWXJCCw2AcvU5e5b0LHLIZvUjZCuroj+QD
# DqKyG4TVLnHS8TQ+4IuLhEN11pVlH34jP7+mA1tJ/zDeeYgihWemOpLwQq0aFmQp
# GKtK9JVPCW3DCiD+CxFuf2Gw1MV57v88viSQGbWcwL+icQkmkoeWCbWb1MWtJ/Sh
# XiAiClmvmnn0LVONdWy1CE/SC2d4kGM9zGz4PcccmABFk+Wzx+Eprv9O6NqOm0hK
# a/cgX8S+uCTu2YQ9IEt4UE5PSdcGga9k5Pb6f9MDG5jOzNleSF8a9A1XBXSw0tBA
# niZKIiwRjqBTcidwE6PtQgHxDOooTRhkUk/AY746zCo70rk3dIw9zsH6I5uw2eoM
# M3zVd+1LxqFR+rL6xIKrIP3qYYP48FY7DowJVKNJjg0BZP5S0P2o0MBbBNNk6RQv
# 3Y0qCBGVoPk9AGLtB+bacMjK8h+FwOtzd3QjmLoL0+7LtJHN1KWnvHDneq+IhVxO
# YWFk/JKRog==
