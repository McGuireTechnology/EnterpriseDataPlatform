# Check if secrets are assigned to certain Microsoft service principals based on UnOAuthorized research
...
$Global:self = @{
    ID = 700
    UUID = '9eccc815-24b8-48d9-b86b-c9a5c12b66ef'
    Version = '1.290.0'
    CategoryID = 6
    ShortName = 'SI000700'
    Name = 'Suspicious credentials on Microsoft service principals'
    ScriptName = 'EID_SuspiciousMSSPCreds'
    Description = '<p>This indicator checks if certain Microsoft service principals have secrets assigned to them.</p>
    ...
    Schedule = '1d'
    Impact = 10
    LikelihoodOfCompromise = '<p>In Entra ID it is possible to assign credentials such as secrets or keys to service principals for Microsoft applications. In certain scenarios, this would then allow you to act with the same rights the application has using OAuth 2.0 client credential grant flows against Microsoft Graph API.</p>
      <p>Through Semperis research, it was discovered that select Microsoft service principals had unexposed, or hidden, permissions that would have allowed for unexpected privileges, including the ability to manage roles such as Global Administrator. An attacker, if they knew of this finding, could have used these permissions to elevate privilege and further implement methods of persistence within the Entra tenant. The full scope of the findings and the attack vector can be found in the associated Semperis reference article.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/unoauthorized-privilege-elevation-through-microsoft-applications/" target="_blank">UnOAuthorized: Privilege elevation through Microsoft applications (semperis.com)</a></p>'
    ResultMessage = 'Found {0} suspicious credentials in Entra ID.'
    Remediation = '<p>Suspicious credentials have been detected on a Microsoft service principal.</p>
      <p>The credentials are no longer valid as Microsoft has fixed the underlying issue in Entra ID. The Entra tenant should still be audited, as there is no known or valid reason to have ever had credentials assigned to the discovered service principal.</p>
      <p>Organizations should investigate any audit log data available in their SIEM to look for any actions performed by the discovered service principal, as well as details on the user that assigned the credential to the service principal.</p>
      <p>Organizations should treat the finding with concern, in particular for Device Registration Service. The UnOAuthorized abuse of Device Registration Service allowed for role management in Entra ID, including highly privileged roles such as Global Administrator.</p>
      <p>For full details on the attack, please refer to the Semperis article in references.</p>'
    Types = @('IoE')
    DataSources = @('AAD.GraphAPI')
    OutputFields = @(
        @{ Name = 'ApplicationName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ObjectId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'CredentialType'; Type = 'String'; IsCollection = $false },
        @{ Name = 'keyID'; Type = 'String'; IsCollection = $false },
        @{ Name = 'StartDate'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ExpirationDate'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/Application.Read.All')
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Privilege Escalation', 'Persistence', 'Defense Evasion') }
    )
    Products = @(
...
Try {
    ...
    # Array of application Client IDs we need to inspect for suspicious credentials
    # Device Registration Service - 01cb2876-7ebd-4aa4-9cc9-d28bd4d359a9
    # Viva Engage - 00000005-0000-0ff1-ce00-000000000000
    $clientIdsToInspect = @('01cb2876-7ebd-4aa4-9cc9-d28bd4d359a9','00000005-0000-0ff1-ce00-000000000000')

    $graphAccess = Get-GraphApiToken @getToken

    $credentialResults = @()

    # Enumerate through Client IDs
    foreach ($clientId in $clientIdsToInspect ) {
        $getServicePrincipalCredentials = @{
            Uri = "https://graph.microsoft.com/v1.0/servicePrincipals(appId='$clientId')"
            AccessToken = $graphAccess
        }

        try{
            $servicePrincipalCredentialsResult = Invoke-GraphApiRequest @getServicePrincipalCredentials
        }
        catch {

            # Exception for 404 as it's not an error if the service principal does not exist
        ...
        if ($servicePrincipalCredentialsResult) {
            if ($servicePrincipalCredentialsResult.passwordCredentials -or $servicePrincipalCredentialsResult.keyCredentials) {
                $credentialResults += $servicePrincipalCredentialsResult
            }
        }
    }

    # Populate results for any found credentials
    foreach ($credentialResult in $credentialResults) {
        $appName = $credentialResult.displayName
        $objectId = $credentialResult.id

        if ($credentialResult.passwordCredentials) {
            foreach ($result in $credentialResult.passwordCredentials) {
                $credentialType = "Secret"
                $expireDate = [DateTime]::Parse($result.endDateTime).ToUniversalTime()
                $startDate = [DateTime]::Parse($result.startDateTime).ToUniversalTime()
        ...
        if ($credentialResult.keyCredentials) {
            foreach ($result in $credentialResult.keyCredentials) {
                $credentialType = "Key"
                $expireDate = [DateTime]::Parse($result.endDateTime).ToUniversalTime()
                $startDate = [DateTime]::Parse($result.startDateTime).ToUniversalTime()
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDrDSlhI5E8gFzP
# mTjSfHd7VpX3ufyMonzZBaarxxjh1aCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgheVH1GbYZCle+CC3EZ6VPx/+rJ7I94x4NmMxYmqvJBIwDQYJKoZI
# hvcNAQEBBQAEggIAE6pvSl/CQkqaD/ADvQ9N4ObSYYKhrt+iihlbVYp9JkGLazIN
# 7t92yaMUwIkROrQE6qwmnvgH0+lCdNm06rSULS/AqcLA80EkubCZXYFFl11Vu040
# WponmcaXTNywqClyONKnwrFobVcnBEoVZInPlKyWIuWbWLguBTw04rKY5R6V2wji
# mB2/Cq2MoeDL3nafzPH1ujdvJbVaO4R8Mce5dikg47X+ukXIxnhEJYX5LuU86OQY
# EJToF4UuZo92bCrCRTml+OuAAQKQSmqnsNraJQqLGoqbtXoiYC1ZZKHwr8v8tSK0
# RcIZAhRaDoA1TRvEffI5q67IZ1qPvk2J3IywhcoBmuPs5M3ggOE7WydU+q18fALV
# eQqs9QjLrSsVbSCIw7NQkxWfkHjhTmyRf1vti/ifqAk57gfQJFYZ+W6gau0ng6Xb
# eJ4kE8XwoPMcbPLaPki4wti2hosmYFfN988cz5Nbt6ig5c9+5JLi9uSa8GimW7ni
# YequLZXa9ZgffLM9c0E6Ll6q9wwVRlnxl0UeZLhjOy00pJ8ylqhzrXe1cW87YihY
# GbvcqfZMuaow+77rQOl89hrBzou+4P2awoI9ZK6+1HfS8yW9T/jox40eTgrpGiKk
# K/qWDDWrkj5aBudJTeLS9tl3c4Ijqxq+YhaFzetRgr9odmKS49P2SIPH7Puhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAfZip3mKPPjsm8CjFZ7bcmnxz07fD1SsWP
# bz/RskmZiAIUMaUoe5yCW/gCLIxrIxMx3Em29/EYDzIwMjUwNTEzMTg0OTMzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDJ0khNGpuO/sHqs2SaTcgDPko1uAkmv28TfZnsFr7S
# WDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgIpWzojY2BW7dgXBzzRvZfG+SDRF/776NXkqJNbquTLG7SKjxO274wQY
# +bOYrHrIeemLjkLN/sTp+njo4rlNJKCjRsIAWJTJ/SnU1ggN5DgyQ/rpNuiCn+Q0
# 3ourdfmAoHytA1HutY8XVU1KzvV/pAdDUQv7yZv68awHAQNuhp8ROUDbH4vlwMYI
# ivWpKT2t3hMmrK+1BvQWoWWw/4vPZ8vSg/KVCDthuBKyL94AhduCI7T/n/HpyPpM
# 5TJTo4z9TNm1Vy015Ne2PDYBk9LI2Oq9FvePZGfjGdGunna1PcygHL3YabUDI7kB
# kveyyj922hArCEMBSUtgl+JSc9YOpV8tePh9qw95a/nOOtiWWldLW/VgHmqneHMa
# cQUvMnORGs1Pi+56ogDdSQEbgZbQY9NMAC+X5QvgfVrwxb0C9eVFqZ7sHkSlaEDP
# Q4Qi7yOxauEgna4Z58VpFBynrqqU+RxgLfTS5pgLiCbNf+RAq+aR85WNxDiJVZea
# 6FNlsm+Hsw==
