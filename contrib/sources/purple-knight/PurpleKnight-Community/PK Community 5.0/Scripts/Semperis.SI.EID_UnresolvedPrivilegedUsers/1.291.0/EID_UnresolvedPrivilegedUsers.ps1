...
$Global:self = @{
    ID = 237
    UUID = '5e9faa14-dd01-4722-b1ed-dd7b5a2ecf46'
    Version = '1.291.0'
    CategoryID = 6
    ShortName = 'SI000237'
    Name = 'Unresolved Entra ID privileged role members'
    ScriptName = 'EID_UnresolvedPrivilegedUsers'
    Description = '<p>This indicator identifies privileged role assignments where the member cannot be found, either due to deleted eligible users or GDAP partner relationships.</p>
    ...
    Schedule = '1d'
    Impact = 2
    LikelihoodOfCompromise = '<p>When privileged role members cannot be found, it can indicate either deleted eligible users or hidden GDAP partner relationships. Deleted eligible users that remain in role assignments can create confusion and complicate role management. GDAP partner relationships, while legitimate, need to be properly tracked and managed to maintain security oversight.</p>
      <p>Regular auditing of these situations is important to maintain clear visibility of who has access to privileged roles in your environment.</p>'
    ResultMessage = 'Found {0} privileged role assignments where the member could not be found.'
    Remediation = '<p>To address ununresolved privileged role members, follow these steps based on the scenario:</p>
      <p>For Deleted Users:</p>
      <ol>
        <li>Navigate to the Microsoft Entra admin center</li>
        <li>Use the provided UserPrincipalName and DisplayName to search in the Users blade</li>
        <li>If confirmed as deleted, remove their role eligibility through PIM</li></ol>
        <p>For GDAP Partner Relationships:</p>
        <ol>
          <li>Access the <a href="https://admin.microsoft.com/Adminportal/Home?#/partners" target="_blank">Microsoft Admin Center</a> (direct link)</li>
          <li>Review all partner relationships and their associated role assignments</li><li>Verify that each partner relationship is legitimate and necessary</li>
          </ol>
          <p>Note: GDAP relationships cannot be viewed through Graph API from the managed tenant, but are visible in the admin portal to Global Administrators</p><p>Regular review of both scenarios is recommended as part of your security governance process.</p>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'ObjectType'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Role'; Type = 'String'; IsCollection = $false },
        @{ Name = 'AssignmentType'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AAD')
    Permissions = @('AAD.GraphAPI/RoleManagement.Read.All', 'AAD.GraphAPI/Directory.Read.All')
    SecurityFrameworks = @(
...
function Write-OutputObject {
    ...
    $assignedRole = $AAD_PRIVILEGED_ROLES[$writeRoleID]
    ...
    if (!($existingObject)) {
        ...
        $thisOutput = [PSCustomObject][Ordered] @{
            ...
            Role = $assignedRole
            AssignmentType = $writeType
        }
        [void]$currentCollection.Add($thisOutput)
    }
}
...
try {
    ...
    foreach ($AADPrivilegedGUID in $AAD_PRIVILEGED_ROLES.Keys) {
        $roleName = $AAD_PRIVILEGED_ROLES[$AADPrivilegedGUID]
        ...
        $activeAdmins = @($activeResults.value)
        if ($activeAdmins) {
            foreach ($activeAdmin in $activeAdmins) {
                $getObject = @{
                    Uri = "https://graph.microsoft.com/v1.0/directoryObjects/$($activeAdmin.principalId)?`$select=id"
                    AccessToken = $graphAccess
                }

                try {
                    $object = Invoke-GraphApiRequest @getObject

                    if ($object.'@odata.type' -eq "#microsoft.graph.group") {
                        $getGroupMembers = @{
                            Uri = "https://graph.microsoft.com/v1.0/groups/$($object.id)/members"
                            AccessToken = $graphAccess
                        }
                        ...
                        foreach ($groupMember in $groupMembers) {
                            ...
                            try {
                                ...
                                if ($_.Exception.Response.StatusCode -eq 404) {
                                    ...
                                    Write-OutputObject -writeRoleID $AADPrivilegedGUID -responseObject $memberObject -writeType "Active" -currentCollection $outputObjects
                                }
                                else {
                    ...
                    if ($_.Exception.Response.StatusCode -eq 404) {
                        $activeObject = @{
                            id = $activeAdmin.principalId
                            Role = $roleName
                            State = "Active"
                        }

                        if ($activeAdmin.principal) {
                            $activeObject.principal = $activeAdmin.principal
                        }

                        Write-OutputObject -writeRoleID $AADPrivilegedGUID -responseObject $activeObject -writeType "Active" -currentCollection $outputObjects
                    }
                    else {
                        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
                    }
            ...
            foreach ($eligibleAdmin in $eligibleAdmins) {
                ...
                try {
                        ...
                        foreach ($groupMember in $groupMembers) {
                            ...
                            try {
                                ...
                                if ($_.Exception.Response.StatusCode -eq 404) {
                                    ...
                                    Write-OutputObject -writeRoleID $AADPrivilegedGUID -responseObject $memberObject -writeType "Eligible" -currentCollection $outputObjects
                                }
                                else {
                                    return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
                                }
                    ...
                    if ($_.Exception.Response.StatusCode -eq 404) {
                        ...
                        if ($eligibleAdmin.principal) {
                            $eligibleObject.principal = $eligibleAdmin.principal
                        }

                        Write-OutputObject -writeRoleID $AADPrivilegedGUID -responseObject $eligibleObject -writeType "Eligible" -currentCollection $outputObjects
                    }
                    else {
                        return $_ | ConvertTo-GraphApiErrorResult -RequiredPrivileges 'Read Directory Objects' -TenantAppId $TenantAppId -AccessToken $graphAccess
                    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDKZuFp4Yp02LPg
# 9xwgiJs64zKEJ6+jtFvFtyFqs+X6nqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQguCEACx6lXmORiZ3hvhwgtrPW1ZEefvNGujHqRhkSPmkwDQYJKoZI
# hvcNAQEBBQAEggIAcNqir/XBB5uF0YjG31XfdF6igCNyZQuS88mjuk3FCnjx6ml0
# 5FDBE23T4aGL0h2Lg3egPXAuM0VL066Sha+two0gKLoFsrMoXvKm55a9Q5wKXsKh
# 8ZHiYylLBLf67MM23rybZmT9yqsNGXHHI79A8OVYFPRlkT9Fwz3M3q+H/qicDjI7
# 4gSnvejmA9r5Mwonxi3BZjHNNpUMWzYw6J/7mJjGbt+lg42NJN8ndI/IOJ+Tk12I
# sipNE5ZRZT+bUmvD0Z25Uv5uYgT7xy05dp/Hvtsq+TFH+PWvLOdWOlQuJnIdSwm9
# wRKP3nDZxjbVUU+0KhZs3A3NSsONCNscWbiddSv92E2nKf5yb10TVJv/WV3Rvxi1
# bXCuUpfCs/EqWf+1x/9lsahytSeWhj17At4pncga+0RyGQ2+efaqjA3cmmWj+Y/q
# qIwNQLVOqssHnjadGjOJylDu/Bd8sYnQv2qhTocsXp4iNxA4ibuMUU412TYwI4QN
# YoNWKLB/vmGkuFK5XHEkpCgni0p8a3wXwW+NmQmbsjnUqnSgqAH75NLTaIR+x00x
# AU84ZYSs1OjoRKVtXdwsS60pDdLpnDpixtlySAEQApxV9r3PwD5Aq6PDSkZDoeub
# +blSOKkIn3Zy4LoJgE5TYD5Sj3ky6v54yaUCu/9dohg/t+iUlwQ3AOci4JWhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBTEoMRmeBEez/5Zu5xJjKpp4yUMwwmD4H/
# MX29pci58gIUHuUrRY1AAjIZdQu7d4wp6qgOCgoYDzIwMjUwNTE1MTIzNzQyWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCfhEAEbE5K5rHi3sPkVOJgYp9FmLSpdlo61RS9rX7+
# VDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgEDTO38U6EQzeopTJQw1ZIMuzdCFMeAADMff8bvCABub6ZF8ZoEs3yVz
# /+dq6Y/HIdE971rdcnqmJK0rA1WpfNMu8UGqxQqduoWU7CAJ06ZouHf0k5hDxIPM
# vJ44QlOBkTWIqYBkX+UCdaYKN6gS0A8jB7UgZ0cmXmJ9pltsDUxsst3JmJ1VN1yR
# R2g8vSd0YpjhD1tUEmY+Z0mEBJnElxRHvYyS/99YQQgh+kZs5gXW+AJjhmnZzQ3V
# XVjYpjleGEvWixtacieE7k3y65bE3ocFf00WoysG+ocQKLnokSWwAZp/AeubDiU/
# WjOZZ6JCC/dOT4aauXEVgxmo/ouLUpMjHWLMEA4qramO/j3U0Eh+UOOmbGKs3KRO
# 2IaWcfMordbRuNJh0ppwnbvdGZkxZRgBFMUnNkSOY7/ZVkiMiGV+oDlbcemh0PAj
# ZJkKBvzuTtKYYZnPQZtE8ohu/t1wI41q/1Lu9L5ZJYkzh6vmMUD5LDyNR/Ko1i6m
# K19eN1nWCw==
