# This script looks for enabled users that didn't logged in for more than a month
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>While the presence of an unused admin account is not automatically a problem, removing these accounts reduces the attack surface of AD.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_user_accounts_dormant">Dormant accounts | ANSSI</a></p>'
...
try {
    ...
    $daysToRemove = 90
    $lastLogonThreshold = ((Get-Date).AddDays(-$daysToRemove)).ToUniversalTime().ToFileTime()
    $whenCreatedThreshold = "{0:yyyMMddHHmmss}.Z" -f (Get-Date).AddDays(-$daysToRemove).ToUniversalTime()
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all enabled users that didn't logged in in the last month
        # The filter is on samaccounttype and not object category and class to filter out trust objects
        $results = Search-AD -dnsDomain $domain -attributes "lastlogontimestamp", "samaccountname" -baseDN $DN -scope "Subtree" `
            -filter "(&(admincount=1)(samaccounttype=805306368)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(lastlogontimestamp<=$lastLogonThreshold)(whencreated<=$whenCreatedThreshold)(!(objectsid=$domainSID-500)))"
        ...
        if ($results.Count) {
            ...
            foreach($result in $results) {
                $thisOutput = new-object psobject -Property @{
                    ...
                    LastLogon = [datetime]::FromFileTime($result.Attributes."lastlogontimestamp"[0]).ToUniversalTime()
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) enabled users that have not logged in in the last $daysToRemove days."
            $res.Remediation = "Admin accounts that are not in use should be removed or disabled."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBqzHbTZ7Kt2Cxd
# F9iA6LARVxB4wwrGCzOcXuHKg/fksaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgCVp7oScgaGbkeoxVJzJ4lIbYtLpDKw9E/sCfOQwwRZswDQYJKoZI
# hvcNAQEBBQAEggIAeG4T52S8b3l/tNLe5SWpM4IT8rmyE9egWGMm3buNswFcqJbB
# BBLWGnggXq9fxFmsTlZ9DTpJmY5EURPHQs0YytQFcaAy3WdQhisb2CSBV17dG7k2
# 8uQ+uAQWqKS+/xyA52Runq713H1sJWt18Dp9K6Ve61o4ALxH7KHCfnvJ1HOyEtWE
# 0GlBV33BFTtpBUkuAkzZfqUHBYUXEUdkgQqOmPUxUxuP/zbt+zoUrSabdpKo8NKE
# e6It69fucY3aonWcIXGdLGq7g/k/GqzG0LFsULcjbhnIJ7kMuiLfMN1XzaKKlS0G
# XrTYKnTnME9BZvadZLwkVChHYH6wqYF3TbunL84FZ5A5fSLekqVBCF3LIPaaP7Da
# V/DOUHQYSAqCHz93dBvXp65yNdLhyegbfHkbv2zzTonYLYwLFbISp//mfFfjr2tk
# pGXS/FOf1v8lE9GIYNgfDaebek8otse7H3Iwy37rOYPhQyZkMLZFIX8zSxxWO0tG
# mJIiBzL5VoRqL5uQBHxzHIbKMiXRooUA86yepUczZrKHVSAGz4r5SQx+z0zlVMZn
# +bmSNJPjazCHPWeUHg0CJQcElxpTk/Z/xPTq4XXHBmwLBTc7FQ+M6mGKG+uiqeI4
# 8AEM95w2XS0BGAOuWvm3L1Ehl7jr+5atidQKvSItPLslx8yT2qYWbh0WcKqhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAQ5Iw0Vaq5hblKYBio4adko/vgEssIkFKs
# QCRBRhKyDgIUcImxgP115Dk06VriT9F28EltGW8YDzIwMjUwNTI4MTIzOTUyWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBxVfo415GJzUidSxD5QhGFBofsaR/Xjck/JPRoOI3Y
...
# CwUABIIBgHtdqIrbPE4dTTBaFv7K0oF2hzhHiQe0JkL4+2bYj6KZ0G0Vs2Adt1yg
# d6giUQLg/8WIqaHV2zvLqaPk0IqQJuBifP+OWI4IwEgN/OCd9ZCiThsYLF3nV5wI
# rs+ojCAXMnvLM/wVfg05sAJlgl09k0vdTKi4T7xuKmI4fxLE0xibCP5N37tEu3l9
# hAzAOF+2wng3dQsVoi+d/Rg3dhwjTCcWXCDEHEIvDvEWiDWOaSUWXlSk/sPBDy0c
# Q2NetaASiSDhk3ICmpjnq4LDT4fOznY8FngALe3VIIFk6dIUXT5zssX2Q8txCdgb
# MMiMASiArQlyrOfckz+Hh5VlNHY+FMdRcytP68zPjOJ3+2f+9RYgJwUO4b5uOd5w
# MgSfgivSdylHhTpdd+yo766bqihcgwyEkEQDDtHRYvw+7hqn5mL41TyA52fU4ZaU
# E0AH9WOsn3biphxYW6U+XpAWChytyMmcDxLhxBMYCZDS1Im9q5dt4FoI1fFMmDaj
# MpQPFh9JPA==
