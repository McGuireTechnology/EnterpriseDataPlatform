...
$Global:self = @{
    ...
    Description = '<p>This indicator attempts to identify AD CS servers in the domain that accept NTLM authentication to Web Enrollment Services. The script enumerates CAs in the Enrollment Services container, resolves their IP and attempts NTLM authentication to https://IP/certsrv and http://IP/certsrv. Note: This indicator currently does not identify EPA or other mitigations suggested by Microsoft.</p>
        <p>The indicator will return a <b>Passed</b> if an enrollment service is contacted, but NTLM authentication is denied (positive effect on the posture score) for any endpoint.</p>
        <p>The indicator will return an <b>IoE Found</b> if NTLM authentication is available on an enrollment service (negative effect on security posture) on any endpoint.</p>
        <p>The indicator will <b>Fail to Run</b> (no effect on security posture score) if one of the following is true for all endpoints:</p>
        <ul>
    ...
    Remediation = 'As per Microsoft guidance, ensure EPA is enabled and NTLM authentication is disabled on all AD CS servers.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAaF/kkiIwE+kNL
# A/4w78r2xw88J0Ch1l+sBL8dat2xQqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgPhEKIQKU45k+cxPNc+tTFXeDK3t8fLtOi/oS3GB9l0YwDQYJKoZI
# hvcNAQEBBQAEggIAIcayJxrdP9yt3mRAp1QJL9Ha2TrbKNBuGM6is6NQ0Vkc1xsh
# I2wCjqIVMyx8VqaNpULIa2/OUBD5c8CudPQt5f4lkiXiK8C/4e3+6JhRTh2sTgtj
# 6F9SJBkzDbd9FfBUjjyLgUibW0LYZDXT+so7CqEUnaByIKJpBku4kx+FyVDYbeUc
# HWeMOBL/QQZtsl2aoZKShLD6xEmlOblyfNEumjqO6T3f5GE03HrQr2WKtkoOx2VF
# +ILwznXpAlE+5xC5n83X/X85uZkgo0HFTBXU7M6qDuea2BZchlm34Vk1vRqrYiVE
# yDEHJmtb6hxhtMPaCc+ogQmAyMe5ommbF+oNKoF3F3kw8Pphlbzx0b7S0v5CPcju
# 9RaPsRs7lZ66wFDnFA81w4YMwdfK1mdsjdUWWMgTnCne9aDIDK0T9N5NXqFVoA0U
# CVnNqRbmE+q8K/hjnaEHUB3kj15MU861bsgaQq60SRlD7W1kYBHpXwB4Bh9gjDmg
# sRyrRpykmt2fjcH4PNtjx9KDkgCVKKS3D8w8RXhqL9wfiW30cs2l7vhAYwvzj1Zr
# VZyf+DSZ/5sN5oUjaaNf03b/ZngIZ+7jHyDiUoDPPWPf8uyFtThx+cqjGrnv+h/O
# zcsS1hLLpul3r2RRWjAZf4joYL2OuMcsmk/Co+B5IrKTqTaNYnFvkrD4Jzehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCANIGX/wINx8bmIFJ4vWj0wfAH4tE+fJbq4
# 9dNrNNCbqgIUVTojCtEZT2LRa/rGVJSLoqBKtgQYDzIwMjUwNTE1MTIzNzQ4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBSUoilaZZCoxQuTTN6G/r3gdYQCT2H9y9noJPGJxAn
...
# CwUABIIBgEtDE4YJ8C06RetifjNgcjMuAQm1SWsCWa7Em5EBmJ+fFvmLZoWUugvS
# jUOGQhGnH5+u6veAOPQbKHoiqNAz8Zqq36z54C7Kn7MXf+XDsk+LNvC+2Bkw23tU
# JyNVQLFrLUo5+UDjNVWnUYqUZ7IfMvJnZLzdt37+3QhbeO78+ka2BiiKotJHd4L7
# 4swVzNT8Q7SSPHsflnbgZtKctCJyLqJwZe8YMlqP9Xn2UmKI2l49YI5rsbEyBr9z
# OvomsjwpU4o+Ngaz6cL0QRw+qsB+q47jj6sBAbm2DeBgoldQprxTrIoy/CsAlOZD
# 7pPquYNRnHFvRVQDuh2eEZyQxq1QUIDFHYBZF7C04NnuifvlBQfnzoDkiuf/F+SV
# 61S9kaP/k//uVj7s7kOjS8gnuUOFSkyW9nrhLa6PLoqZLxBuLylLzDz1gmQRl7oi
# M0ISRZX2mdAUYFFy6Slfbhtsz8+zcn/gpEnsX1oj/TouAcrkgJ0WtUhcbJWOgyZ/
# rrQ+y6SNHQ==
