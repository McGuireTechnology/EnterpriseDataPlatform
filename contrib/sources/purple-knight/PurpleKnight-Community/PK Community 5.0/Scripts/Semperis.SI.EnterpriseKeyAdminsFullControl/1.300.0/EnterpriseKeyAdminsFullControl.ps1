...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>This issue was corrected in a subsequent release of Server 2016 and may not exist in your environment, but checking for it is definitely warranted, since it grants this group the ability to replicate all changes from AD (DCSync Attack).</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_adupdate_bad">Bad Active Directory versions | ANSSI</a></p>'
...
try {
    ...
    # Will be used later to find enterprise key admins SID
    ...
    foreach ($domain in $DomainNames) {
        ...
        $rightsFilter = "GenericAll"

        $enterpriseKeyAdminsSID = @("$forestSID-527")
        ...
        if ($results.count -gt 0) {
            ...
            foreach ($access in $securityDescriptor.Access) {
                ...
                if ($identitySID -eq $enterpriseKeyAdminsSID) {
                    $rights = ($access.ActiveDirectoryRights).ToString().split(",").replace(' ','')
                    if ($rights.contains($rightsFilter) -and $access.ObjectType.Guid -eq "00000000-0000-0000-0000-000000000000") {
                        $failedDomainCount++
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) domains where the Enterprise Key Admins group has full access."
            $res.Remediation = "Ensure that users don't have unnecessary permissions."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA4Brvat7u3MQ87
# pee/I6bk7BShcxWE7eXthLeJFCQKwqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg+mmsqXe/eW9/Rl3dp3/uGJl4zdBcGXni2lWzVSkAnoUwDQYJKoZI
# hvcNAQEBBQAEggIAGCc5dzeYMOOLlFRedO9b1xxswGeDNR2bFCHSXFxEMdw/g1+C
# dsKSvaOQszudmU2BF9qLE9HXAW5Nh3ehxaRYWv9x6qn8qgFxu+SwYdyqsCKd24bX
# RoOzjoic1fB6pVJI2+lUBfyKYErlbSK9+PwWP0v+ArJEIK3raR+iZPM3VkPY1sW0
# aS3knnf5HYYRm63Egrhw8Lk+6I7zm8shiaiQ+c7xYv1Fc4am5wS2GPbCsw70mFur
# +UtglpoOPuBPDHqQC8NFeG7nOSTuluq+Z9WAmxnDsWHmVac1kJTsfwk8PSO6h7lG
# IZPSg37DQ0cjswsr2qTuOJYxJ+zQ502TKxNxPc7wFbgjtpF4Scf6Em/Ndl3LP5sJ
# tFgF+YHV40wKNptvRtZZnuRBR2GDzzUcLcjy2fteoGSYtc99c7dzPxIHeYBAd0y5
# ZNDz2/EnPm8iYdG7kXd5laDCJM9ZGiItjqNCSv8pCUAgDh/8/yc5mi4G9xbVH+s3
# wmUY0kupcHu776pVhg1/UZz4IRMeNYN7g6CirlpU4hZYFe/GVFC4Sjqe8K+XxtE2
# 8+UI8NMSX/KM6kJ7A87qghwSf+3fChTCUum2OScSXqR4N/yixeuWEkDUCJBeYc3H
# lKmsYtiYiur4wUZ6e9t5KhBrOTI0zbjDTg9rdl2Mr6VbVqPPejnREMzfp/Whgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCh3CS+ov/KGExYCicHgX0FLm4c+BrMqJq+
# eModprzw6wIUHIupUah2jOYyBP3s/o7NsU8OuFYYDzIwMjUwNTI4MTIzOTU3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCC+4eB0bybjFTHnXe+6VMT8wSHrDziJEs5cRJF4sAGb
# tjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgKgr1/gofWBJYXCl1gxFUFNdFFmdowVv7j05TakRiun4yLQaosZNOoP/
# 28QW7mA0sJdOq+EhvW5pB1FGvMqCUjbDI/r+HxTX524GaFUO3fKzev/rOgXhqGPl
# R8HxDUIH1SzETUmsCqgo9OUs+Chv32i9AHW27tukFoaz8n1XYG0uj1h3+5GExy0+
# 0ZHqFRH8ANjVg/2FT3VYSz+MDVTpaVU6/vkptbNFu5IJohzOMkC3vaP0Ph/yFmvH
# j0qyrqQ+REE3LgdUDgCx2td42tVeMP7XS1g/99tV+g+yN5eveeCGxJOlDckgHFuz
# P93Fzzfndvt0MBhAKTkWSCIN3FKl/HCdp93w6zDoWocQjoCxDw8YIY6NH0JO+sc7
# FVQtySib2eURcWaUbsjK6JJAKNpiPh5obuuwpZw/Oz2CS3VvRJ+O5/3j+Jy+K9H8
# WN8DWwRzY8TDL27xfWJPceS0e/mzW/xurG+vLTAFuCIdYY89MqYzoeqX/9t7xYHc
# n3q/bY8oHA==
