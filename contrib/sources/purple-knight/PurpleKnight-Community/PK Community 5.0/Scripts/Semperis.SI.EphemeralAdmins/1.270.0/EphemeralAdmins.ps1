# This script looks for any ephemeral admins (added & removed within 48 hours):
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $adminGroupSids = @('S-1-5-32-544',"$domainSid-512")

        if ($domain -eq $forestName) {
            $adminGroupSids += "$domainSid-519"
        }

        # build filter for searching for these sids
        $filter = ""

        Foreach($adminGroupSid in $adminGroupSids) {
            $filter +='(objectSid='+$adminGroupSid+')'
        }
        ...
        foreach($result in $results.attributes.valuemetadata) {
            $createdDate = ([datetime]::ParseExact($result.TimeCreated,$format,[Globalization.CultureInfo]::CultureInfo.InvariantCulture))
            $deletedDate = ([datetime]::ParseExact($result.TimeDeleted,$format,[Globalization.CultureInfo]::CultureInfo.InvariantCulture))

            # Since we're only focused on admins that have been added AND REMOVED within a 48 hour period, we can eliminate ones that are STILL members
            #  Note:  Members that have been deleted *while* being a member will not have deletedDate updated
            #         For this case, we will check if member's DN is within deleted objects container
            ...
            $memberSpan = New-TimeSpan -Start $createdDate -End $deletedDate
            if ($memberSpan.TotalHours -ge 48) {
                Continue
            }
            ...
            if ($result.ValueDN -match "CN=ForeignSecurityPrincipals,$DN") {
                $thisSID = $result.ValueDN.split(",")[0].split("=")[1]
                $member = (new-object System.Security.Principal.SecurityIdentifier($thisSID)).Translate([System.Security.Principal.NTAccount]).Value
            }
            else {
            ...
            $thisoutputCreate = [PSCustomObject][Ordered] @{
                GroupDistinguishedName = $result.ParentDN
                Member = $member
                Operation = "Added"
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count/2) users who have been added & removed from an admin group within 48 hours."
            $res.Remediation = "Confirm that any additions/removals from admin groups are valid and properly accounted for."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAzIBMwTdDoNfLO
# Lp2+RF0yVVqG5HEFCO1kQ71k9+Ks+KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgmme/0BM4Os2gjK7So41nEQUgg7CsYP7FIAbZWPgAqYcwDQYJKoZI
# hvcNAQEBBQAEggIAGpffRWA14St/UnKTYhwD4INrHjeXaSqOVywZ1b/AUhq1sTKP
# Ke0hsBndZNl6byErVW66qt0Uf77ImGBVP5js4jwqBSHLrU5JbhzGAKT5KGv0gKo3
# d0S8RGo+s46Yw30qMYmBhnYjc18K2RnoKmtvE5XnGJGCFQ5wYGlv9rYrA8GS2V84
# GfVR3O7J/xNe1qBxObNPL6j1KmMYafHwpMpEe7bN5Qu+/Jauek1X+rbGckhGxinv
# J18ZQCO+5GqcC/tiNVL4pq55u/T6CdrKM0qEuiDvr9b/zp84tuuvaP9Y8+lfM1N7
# TCzSAqti+P46PpMyLPDnyuE5VDiTRxt/Jtvmpkv/fKYhOIYXYhJDYNeA1TOD0fML
# theFjGyKWvIeRPJe/hoM+ImG1co/GnfaePjdK4TInWWsni8Xx/TJZI5WZZjvgKYl
# 6bsfO+GYpJe/pWgpILu+bjVSpgMMMHYfW2kZ90gK8RmwfMBsDCMom0KMbGusx23n
# 6865qiwYQoZGU7IjOOa5/y82LtlLMx8bdUPUFtToL31nVXS0kpnhiyXJmRVy+KtG
# Vu+EXz8WoSnyLilfUQZvWz5oeeHiM6G1SdIjhqs5hkuivHq2r5WP6ACXvr5VVxxr
# H77G8U980pXrg4jUhH3r7gOvDzK5DcvvFqJrGo5fzLyG1rSalChYC3NC+mahgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCc5RsKIg1qcpw2h1Q6IcW9++iFro6HAOuo
# R6ti4bD6MAIUDxVIw+Me6VO+2y/TcnRiPMgn2sgYDzIwMjUwNDE2MTcxNjE3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCD8IiRf9AJv0pXjbyvVOSgGgfwEp82LgcIMO51HWs41
...
# CwUABIIBgKMMTVloWouSz0hgOco2h0oPCWwZBnuUw1mbqLFGi8fDgL49qhGe2VTj
# 1MzHv4bcn19G944yNxJ/tT2/KF0fAGO8+CEHT6VOidTLFnkq0ywtlOnZVO0B1Pv4
# 8CV7kgsdeqywZx2X15eCsqNNpIaLsOIM+BCK2UbN6m+Ce2GOPhH3DzXqLBCAYjmp
# GG+ms4anoT8bp6Ze8Y2Uc27Qtvwno5hvQIXrD1gvVbaxZT3rL0rbswVqXNgF0C3j
# ioL3JFN4kyiFU4CQMdbpMBP5/bTUu3VZnbNXABs2P54uYdj51Nj38mxRHYwzB5mP
# 1PoQ3qKJRcx5mZPY5Z6MyiEDr3qpTR8zYFhbWoji5V39SNrhiMdz6KqFsnng5pns
# Iji+Bv+Cs5kfpcAMhvVoDab4AEnJw6wcHrP4PLBLo6zNUrddhgDODyX/91MTK9Se
# shrHNCbW7eig2LHyUFcKwBHaRbatfhexd9s4yVoRAvxGMZE/sbxrhLJNqiRy4wU/
# t+k6zVBCdA==
