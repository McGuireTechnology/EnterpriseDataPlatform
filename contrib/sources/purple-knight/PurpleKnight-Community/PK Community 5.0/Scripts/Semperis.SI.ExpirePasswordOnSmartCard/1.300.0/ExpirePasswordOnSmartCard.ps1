...
$Global:self = @{
    ID = 147
    UUID = '04ad3f4e-b67c-49c3-a668-528214dd6c63'
    Version = '1.300.0'
    CategoryID = 3
    ShortName = 'SI000147'
    Name = 'Smart card password rotation disabled'
    ScriptName = 'ExpirePasswordOnSmartCard'
    Description = 'This indicator checks whether the mcDS-ExpirePasswordsOnSmartCardOnlyAccounts attribute, which determines if smart card-only accounts must follow regular password rotation schedules, is disabled.'
    Weight = 4
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>The msDS-ExpirePasswordsOnSmartCardOnlyAccounts attribute is a security configuration that determines if smart card-only accounts must follow regular password rotation schedules. When enabled, it ensures these specialized accounts comply with the domain''s password policy. This security feature is available in domains operating at Windows Server 2016 functional level or higher.</p>
      <p>While not immediately indicative of an attack, when disabled, smart card-only accounts can maintain the same password indefinitely, potentially creating a security vulnerability that attackers can exploit for persistent access and lateral movement.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-ada2/1ba3e699-77ab-4ba6-9d70-74616b1e11d4" target="_blank">Attribute msDS-ExpirePasswordsOnSmartCardOnlyAccounts</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_smartcard_expire_passwords" target="_blank">Missing password expiration for smart card users | ANSSI</a></p>'
    ResultMessage = 'Found {0} domains with msDS-ExpirePasswordsOnSmartCardOnlyAccounts disabled.'
    Remediation = 'To remediate this security issue, enable the msDS-ExpirePasswordsOnSmartCardOnlyAccounts attribute by opening ADSIEdit.msc with an account that has domain administrative privileges. Connect to the Configuration naming context, navigate to the domain configuration object, right-click on the domain object, and select "Properties". Go to the "Attribute Editor" tab, locate the "msDS-ExpirePasswordsOnSmartCardOnlyAccounts" attribute, and change its value to "TRUE" (case-sensitive). Click "Apply" and then "OK" to save the changes.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln4_smartcard_expire_passwords') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        @{
            dnsDomain = $domain
            attributes = @("msDS-ExpirePasswordsOnSmartCardOnlyAccounts","DistinguishedName")
            baseDN = $DN
            scope = "subtree"
            filter = "(&(objectClass=domain)(msDS-ExpirePasswordsOnSmartCardOnlyAccounts=TRUE))"
        }
        $expirePasswordsQuery = Search-AD @searchParams

        if(-not $expirePasswordsQuery.Count){
            $thisOutput = [pscustomobject][ordered] @{
                DistinguishedName = $DN
                Status = "Smart card password rotation disabled."
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCr2dT7m6VYYkxV
# cWWsxWxdVC21iDSwaaoL7fm8xDjj7KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ