...
$Global:self = @{
    ...
    CategoryID = 2
    ShortName = 'SI000103'
    Name = 'FGPP not applied to Global group'
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>Changing a group''s scope settings from Global group to Universal or Domain Local group, will result in FGPP settings no longer applying to that group, and decreasing its password security controls.</p>
      <h3>References</h3>
    ...
    Remediation = 'Confirm that the following groups should be Universal or Domain Local, otherwise change their scope settings to Global for FGPP to apply to them.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
$failedPolicies = [System.Collections.ArrayList]@()
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $accountNames = [System.Collections.ArrayList]@()
        ...
        $containerResult = Search-AD @paramsContainer
        ...
        $appliedFGPP = Search-ADHelper @paramsFGPP
        $countFgpp += $appliedFGPP.Count
        foreach ($policy in $appliedFGPP) {
            if ($policy.Attributes.'msds-psoappliesto'.Length -gt 0) {
                $accounts = $policy.Attributes.'msds-psoappliesto'.GetValues("string")
                foreach ($account in $accounts) {
                    if ($accountNames.Contains($account) -eq $False) {
                        [void]$accountNames.Add($account)
                    }
                }
            }
        }
        ...
        foreach ($accountName in $accountNames) {
            $paramsAccount = @{
                dnsDomain = $domain
                attributes = @("grouptype", "msds-replattributemetadata")
                baseDN = $dn
                scope = "subtree"
                filter = "(&(objectcategory=group)(DistinguishedName=$accountName)(&(|(grouptype=-2147483644)(grouptype=-2147483640))))"
                PropertyFilter = "grouptype"
                StartAttackWindow = $StartAttackWindow
                EndAttackWindow = $EndAttackWindow
            }
            $groupTypeSearch = Search-ADHelper @paramsAccount
            if ($groupTypeSearch) { [void]$results.Add($groupTypeSearch) }
        }

        $groupTypes = @{
            -2147483644 = "DomainLocal"
        ...
        if ($results.count) {
            foreach ($result in $results) {
                ...
                $groupAsInt = $result.Attributes.'grouptype'[0] -as [int]
                $groupType = $groupTypes.$groupAsInt

                $policies = [System.Collections.ArrayList]@()
                foreach ($policy in $appliedFGPP) {
                    if ($policy.Attributes.'msds-psoappliesto'.Length -gt 0) {
                        $accountsCheck = $policy.Attributes.'msds-psoappliesto'.GetValues("string")
                        foreach ($account in $accountsCheck) {
                            if ($account -eq $result.DistinguishedName) {
                                [void]$policies.Add($policy.DistinguishedName)
                                [void]$failedPolicies.Add($policy.DistinguishedName)
                            }
                        }
                    }
                }

                $outPolicies = $policies -join "; "

                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $result.DistinguishedName
                    GroupType = $groupType
                    AppliedFGGPPolicies = $outPolicies
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Group(s) whose FGPPs do not apply to them."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCADMD4nmRLbhC40
# r5tJubvqxE1CxKfkGVFlEriCozvpcqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ