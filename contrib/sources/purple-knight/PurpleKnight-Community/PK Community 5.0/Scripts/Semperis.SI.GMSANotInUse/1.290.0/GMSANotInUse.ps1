# This script looks for domains that don't has gMSA objects
...
$Global:self = @{
    ...
    Description = 'This indicator checks if there are enabled group Managed Service Account (gMSA) objects in the domain.'
    Weight = 4
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 4
    LikelihoodOfCompromise = '<p>The group Managed Service Account (gMSA) feature in Windows Server 2016 allows automatic rotation of passwords for service accounts, making them much more difficult for attackers to compromise. The feature should be used whenever possible for service accounts.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/group-managed-service-accounts/group-managed-service-accounts/getting-started-with-group-managed-service-accounts" target="_blank">Get started with Group Managed Service Accounts</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get all gMSAs
        $results = Search-AD -dnsDomain $domain -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=msDS-GroupManagedServiceAccount)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))"
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) domains with no gMSA objects enabled."
            $res.Remediation = "Group Managed Service Accounts should be used to protect service accounts. See description for more information."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB0X/zTAV06io1M
# CQYZz58o6Bgl6RK7wcrhPlWDGpSrE6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgaXXwDAWZ/1WnTt55Rxp4yrDIzuN5pMtg9exratrpwHgwDQYJKoZI
# hvcNAQEBBQAEggIAeOVWgJHBwK0bfs27E8Rif1ywm0T8wopq5UcLFxWRiZojoOGb
# RhyyhL7IBJzzksgpCpc9MAl9fEQP+47fFqeYbYc0lzhmtSmmbmpmRN/iABF8pqgB
# wvU92lkPyEip1TJrM3zGOQQYJ1HLIUzXzDWoKTswocCm3YPBKPX1X0U/gsyhAy/W
# CTdOFwvkhNALWpUwO3bBMN6SzwbJCxxCk8/RhqxaJluxdsGtIGfnNiroKkc9doko
# MYPsdRzBOZf6boSLwIKnJhTsCiqVEYF+NoSX3E6n8P6c8XC2PzyM+KB5iOunJ8dP
# /6V97fm84mqrywB9P8O8Ril0W7NEkcgJhDCa+XBy8qe90b9VUzN7s3Oz9TlHXgZI
# vgpZLmAmO40+r3PolOYEq6livypCYS1pIdMXTJvkXjvqCbyzOne3Nxm1RQvCu6bO
# 1FDavjDcW7vBAO9JFgjwezJ/8Lso9tTok+GuBMJiDy1Ebaoi17VXqtb24DiH0V3G
# 4HfRlvj1IRdvF6e26QZUe55cUf+/2Rf90G+NjCxCf2GxCu8CKYJFbt2/mDHG5w2W
# WT3xJBKyhlo7XZSkE48yJaxwx5RBSJG8j9PT27ulSgd5Cr6f42sea2Iw0S7cEwHM
# eZQbDXCjxCYaYwABWtVlMDqIloODL2xyItnJmNo6izxTBbVUOVZM283ifV2hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCjZbeJBX7iKR+Nl+IVLG4seP5wyjnQZWIX
# e2qqXqC4pgIUYATCzLq8HD1tr512+oc6JiiiqesYDzIwMjUwNTEzMTg0OTU5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCUyv/yLjxE2xdkzukrY4KWcjmS7uiPbJpbwNTABEeW
...
# CwUABIIBgALKDVU/5myJnVZG1COYEarCL5/N5EylCgyH288jZqa/1QM6NXfzxNE8
# iWqVGIsBUT1CIYVMVjK2WHsTNV+dygBN2UL1tv+6DBFSilhnMF7yPOfSrp4WDa9a
# M6SVPddpXySmRPO8uadaZB5JgTfeB9Wb0Mec/Us2cRhUjlUUHrpnYYJ4c/ylmzWP
# Yb9Q0Xi+0k8uzii1EcS42r54J2byviqC0aMqC5/8tdjxXxiLpWyFIeCIcM9S2v0L
# GZqncoZgGm+oAI4n7mMPj56xobMgDLGgx0BkVp2E+Sf8Qp/kKM1l9qbVVZWw50Mb
# 9haf+sPX4Xr+zgWBeQqDVYTfW95dgV1pOQi9v/BSJHuRtLgNJnV8ys5FET0oqrsN
# epu8MALPOIJPgjn2pIhICW6xhiERWVGs5gj7h7OvaeNNRf+USPwY8e8ltS325QHG
# GzESJTQHXO/1meTtrvOHlWS2a83fCupfm5YfflDIb+Cj4j9GyEKldeLCG72Am+P/
# nUJWle0L3g==
