...
try {
    ...
    $daysToRemoveActive = 45
    $activeThreshold = (get-date).AddDays(-$daysToRemoveActive).ToUniversalTime().ToFileTimeUtc()
    ...
    foreach ($domain in $DomainNames) {
        ...
        $results = Search-AD -dnsDomain $domain -attributes "pwdLastSet", "lastlogontimestamp" -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=msDS-GroupManagedServiceAccount)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(pwdLastSet=0))(pwdLastSet<=$pwdLastSetThreshold)(!(primarygroupid=516)))"

        if ($results.Count) {
            $failedDomainCount++
            foreach ($result in $results) {
                $active = $false
                $lastLogonTimeStamp = $result.Attributes."lastlogontimestamp"[0]
                if ($lastLogonTimeStamp -ge $activeThreshold) {
                    $active = $true
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Group Managed Service Account objects whose password has not changed in the last $daysToRemove days."
            if ($DomainNames.Count -le 0) {
                throw [System.Exception] "Empty domain array"
            }
            $res.Remediation = "Managed Service Accounts' passwords should be changed every 30 days. This is also the default setting. The returned accounts should be investigated for why their passwords were not changed. Note: The 'Active' column shows if the account was active in the past 45 days."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDOkYo+UGnkIJpS
# d+JbDIl0WbuM/FsTR32MrBpdT/Y0jaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgvdFeIYGMvSFduPofDT4VdXG5kWGJVaJmLAWgda5juG8wDQYJKoZI
# hvcNAQEBBQAEggIARAE1IK48Xgndy65fpBSVFz54u43HLA+HG6J8xeMpObB/vHIV
# EZnPCpIAnd4kt20u3z+k4O9DkNbviXV7giN+nnmF8hW1n9AjZQ7eFZfExdJMQ7uf
# 3u/14ylDhlw3iabud/lDdlmivB2LK835YQU0VoSsilJBOQ1BIHzEHwAn9xJQDOze
# /O4Rv7AznmLoXI/Lk4UWPEdBCuoxT/yd1MAAS5qaTJRUnkBs8bcEdEwYS0Bq/ubp
# D50yOzLLCqyubjeMRRNRcP5XmankvMxpUQ3UQ7qjmKyP9h1eAzhwzuZG5ht3ygvK
# kifRYmwFmJPeTomdDzmsnWRqKJMt0w1b39W/AxfgZ1AjRylO+UgPRoUUNQbuTGTI
# 0wM5Oby8WA8LLvSIWUxFVnQLah3dSneR7E0DZmW9IDij165WZpRaR3+9YC5hEvVg
# I0jMzAbGBpaHkH2v8jAmgTeW7itQtgvUbuszBGfuQjCPTWdXwcuDxcVqK7f5jofm
# yC5fS+1ke8LltCPDEN+n/QW2LIVoUrFVqEo/bRrJ80kk/i3zfjrUYkuVwM+WoxAd
# fdNJvmNk6v+vIGdlMrR1qy9Mq4IXx0fo4noeQInWzlUs9bPTd4JgMQ5lpgEtbY5H
# qo13EbsPOdInBCUguzuVTI0LzynCXXD0vnKKZpYH1bJXvo1rBLNow5FC2wmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCUNeDdKlEI1vat8iLXKVGe43A6FTuyKmmN
# ekR5I80DCAIUOCvDeD5CGwO4zJmHItUnELc6FKkYDzIwMjUwNDE2MTMxMDA4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCB/4W84UM8/iifyIm70k/ojSCOzailqA90gmDoIRxlu
...
# CwUABIIBgA29a+x2pIUUPLxaxNhY4XREALoa3x2ieqDhMfJVCLZalQTi+8TV1y4p
# jfLWPZfWutktMMehNjTyKGH7KVrisz5Tz4r2F9b/eUl9LMoIPs5Luzkmvx8N91bR
# Ia5saqgMlJBGwPU8FCVHUziHjzHsnSeNszzsb5g7Zpe4GYk4XPc4fkUgU2hRz0le
# kXMEvK3yNsJWPmTT1Yk6yUHwVj5TdT51v27FlVI8Np+XOLd8G69kTvZBO+dqUcuP
# PhSi6Yd1TIRZ1x1+qwQoigb6ksIRD/1L0R5rgBcGTMn8uQmpDyLFOGVD+oFDGU+U
# TrrlsCHdFsDSHxRsC/LmVp7GYx6v2UxU7E4zeqK7SJ6QjXZvqlwhGSNi9wPW9v11
# bzmlvWxrjONckm/+axWiOaC4bE4zUPYRuXf1sOr+B3V3S/WiPOYN7EeYC7L0f4hT
# ytQUM/p6JQzrjZw1P3Cp2EWz7jqIfBNGTPcXMrfAKgtvmz/56eP46dCHV/dHPzsz
# 83S6c5wDQw==
