...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>An attacker that controls access to the gMSA account can retrieve passwords for resources managed with gMSA.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_permissions_gmsa_keys" target="_blank">Dangerous ACLs expose gMSA key objects (attack path) | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_permissions_gmsa_keys', 'vuln2_permissions_gmsa_keys') }
    )
    Products = @(
...
try {
    ...
    $guidHT = @{"00000000-0000-0000-0000-000000000000"="All Properties";"888eedd6-ce04-df40-b462-b8a50e41ba38"="msDS-GroupMSAMembership"}
    $rightsFilter = "GenericAll|GenericWrite|WriteProperty|WriteDacl|WriteOwner"
    $attributes = "ntsecuritydescriptor", "msds-groupmsamembership"

    $adminsAndErrors = Get-AdminsInDomain -DomainNames $DomainNames -ReturnSIDs -ForestName $ForestName
    $adminSIDs = $adminsAndErrors[0]
    $notEnumerated = $adminsAndErrors[1]
    $largeGroups = $adminsAndErrors[2]
    $unavailableDomains = $adminsAndErrors[3]

    # Remove objects resulting from replication conflict
    $notEnumerated = $notEnumerated | Where-Object {$_ -notmatch "CNF:"}
    ...
    $scriptBlockFilter =  {
        ...
        Update-NTSecurityDescriptor -Entry $Entry -PropagationFlagsFilter "None|NoPropagateInherit" -AccessControlTypeFilter "Allow" `
            -RightsFilter $rightsFilter -ExcludedSIDs $adminSIDs -GuidHTFilter $guidHT
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            PropertyFilter = "msds-groupmsamembership|ntsecuritydescriptor"
        ...
        foreach ($result in $results) {
            ...
            if ($result.Attributes."msds-groupmsamembership") {
                $gMSAMembershipSD = (New-Object System.Security.AccessControl.RawSecurityDescriptor(@($result.Attributes."msds-groupmsamembership"[0],0)))
                foreach($ace in $gMSAMembershipSD.DiscretionaryAcl) {
                    if ($ace.AceQualifier -ne "AccessAllowed" -or $ace.AceType -ne "AccessAllowed") {
                        continue
                    }
                    $objectSID = $ace.SecurityIdentifier
                    if ($adminSIDs.Contains($objectSID.Value)) {
                        continue
                    }
                    try {
                        $ntAccount = $objectSID.Translate([System.Security.Principal.NTAccount]).Value
                    }
                    catch {
                        $ntAccount = $objectSID
                    }
                    ...
                    $attrMetaData = $result.attributes."attributemetadata" | Where-Object {$_.AttributeName -eq "msds-groupmsamembership"}
            ...
            foreach ($access in $result.Attributes.'aces') {
                ...
                if ($strSID.Value -eq "S-1-5-18") {
                    continue
                }

                if ($acl.ContainsKey($result.DistinguishedName)) {
                    $acl[$access.IdentityReference.Value] += ("; " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid])
                }
                else {
                    $acl[$access.IdentityReference.Value] = ($access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid])
                }
            ...
            if ($acl.Count -gt 0) {
                ...
                foreach ($ace in $acl.GetEnumerator()) {
                    $thisOutput = [PSCustomObject][Ordered] @{
                        ...
                        Identity = $ace.Name
                        Access = $ace.Value
                        Source = "nTSecurityDescriptor"
                    }

                    $attrMetaData = $result.attributes."attributemetadata" | Where-Object {$_.AttributeName -eq "nTSecurityDescriptor"}
                    Update-IndicatorOutputAttackWindow -ThisOutput $thisOutput -Metadata $attrMetaData `
                        -StartAttackWindow $StartAttackWindow -EndAttackWindow $EndAttackWindow
                    [void]$outputObjects.Add($thisOutput)
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            $gMSACaount = ($outputObjects.DistinguishedName | Select-Object -Unique).Count
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "There are $($gMSACaount) Group Managed Service Accounts that unprivileged principals can potentially read their password."
            $res.Remediation = "Ensure that there are no unnecessary principals who can read Group Managed Service Account passwords via the msDS-GroupMSAMembership attribute."
            if ($largeGroups) {
            ...
            if ($notEnumerated) {
                $res.ResultMessage += " The following objects were identified in the group enumeration but could not be read due to ACL restrictions on the object or OU. It is suggested to check why these objects are hidden.  $($notEnumerated -join '; ')."
            }
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCPVDT1bJwPuFxu
# Bi5pYC0+my2kK3x5XXFS+DP3x4zmu6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg9KnEJG3pj6zGJlRjrLvG/wb+AEKFjlRlm0gO6QrvSEMwDQYJKoZI
# hvcNAQEBBQAEggIAWNQYuL3ttrdyZbR2ubtVKCzV2Kbo3s9CAmn+3dCENShLSQbq
# ZKDNZfVR41cLehpD6j60Gz32KA1BLxOYqqaWKuUgq2JLuxwj1WJwUhiNbmiQKwc6
# upsOKQdyaNpfbSHKip3txfnGhlqBLmTSJy3VPazyJOeKcibch/9jAKElxj9bU8Yv
# kJ58FYaLLQNH+xx7bBVDCswKWoQ/hFbU//qbh4o38cf9ALWjzzZs+GOOH/vsD2uW
# hG1hJ6RGKCmPbR83XpofJPzSPk0/fj2Nohh3Ti9N5tw4HFU97Tzdtqz6YCOlQ5t+
# 0B+vffNIowQCY6zSQssel7lZou8VssZGjZe9ncwTweXg6q1+MGUmjp3kBOGMeMFu
# MxBaHdo+LZDFeOwcD0cQtYpYNflEBKGbRT5/49r6eFcTkEzrQrpv/6e9neT6WcCx
# oaOriy8d3VEcx7f/kCFgdWabgwdXj+gEhPQvyy2Zx7YRwdHOeZ+YqkuK0GVx2EFH
# ui1uxyu9HR0YP0PB1eR8vlml5dHjIDu1O7WBpYIADBBAf54RUTyUFRnheuAm43fi
# vaP1sjF0XNz/827SRXvzrtH2GEvZqKQlV/f+SgVJw0OIRHqFFvbPXdBNltLoWAA9
# uCQGnD0rcEs9VLirr98HHM4TN2coCebxD61AlMHpxBiiu2gM7JylVkJyr1ehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAfm2nVsDbV7vrdfAsxRB6blGjUJUDWTUaI
# 65na62gZSAIUQ/Bie2heT4AMQ3iE06T2BEg/ICUYDzIwMjUwNTI4MTI0MDA4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCRkoLZt6ifap1Exd+l1OkVdT1xNw+PQN9RipEe5tuJ
# 0jCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgBOhnprV0xXKQJQnLRCpMppFcI8DdKlLhtCBTE0DpO5wlqtC8vI5T1X8
# WrG8qlnMflsheAyNZhhoHT83MbLYUmYf9yfHuQNmGCHR7qgLVdffNEk9nLg4E+pS
# eHtjiXzZ8O+W5IQ74HT0FW3jJS1UfwS79m0eOzIUzLSH0jHC8C1+oYa0jXfse32j
# Ryeg3o4E0SeKSsRCTh3a/XZ1CL25subLZWwSm8gcFlxeTyQiOgwky7LXxPHGVQbu
# QeQZ9jvr/muP+Pk7KFZ/J7Ji8n1GSAKtafAd+qCyt5TRAIExipOLsFbWMNdsEeh/
# 8X+fX6IWzCDYfBHfIOgKfWN5Tc5/flHG6jdTRk6DIzeZoNv+NXOqNXrNt/RQy0Ln
# CWIMaHThbQwrqLHJFK5PANg7S29nNWu2OnCXdVrPg4goOpfd6GC1mGI81IGioG8J
# tYCxh8uUQCPl8kU4+WjFLfHelcZd3YAo6vVJfRUAx1G08XxY1GOqVmYr79cTf4N/
# GVkckB2eUA==
