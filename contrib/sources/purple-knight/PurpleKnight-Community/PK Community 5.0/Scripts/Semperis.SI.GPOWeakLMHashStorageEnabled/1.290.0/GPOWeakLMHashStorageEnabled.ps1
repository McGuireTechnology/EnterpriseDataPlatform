...
$Global:self = @{
    ID = 188
    UUID = '49368362-a307-4d9a-91b5-56eac7be35c5'
    Version = '1.290.0'
    CategoryID = 4
    ShortName = 'SI000188'
    Name = 'GPO Weak LM Hash storage enabled'
    ScriptName = 'GPOWeakLMHashStorageEnabled'
    Description = 'This indicator detects when the "Network security: Do not store LAN Manager hash value on next password change" Group Policy Object setting is disabled within the Windows operating system.'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>When the "Network security: Do not store LAN Manger hash value on next password change" setting is disabled, LAN Manager (LM) hashes continue to be stored during password changes. Enabling this setting strengthens security by preventing the storage of weak LM hashes and promoting the use of stronger password storage mechanisms.
        If an attacker gains access to the system or manages to extract the stored LM hashes, they can employ offline cracking techniques to obtain the original passwords. Therefore, monitoring and enabling this setting is crucial for mitigating the risk of unauthorized access and data breaches caused by the exploitation of weak LM hashes.</p>
        <h3>References</h3>
        <p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/security-policy-settings/network-security-do-not-store-lan-manager-hash-value-on-next-password-change">MS GPO information</a></p>'
    ResultMessage = 'Found {0} Group Policies with Network security: Do not store LAN Manager hash value on next password change value disabled.'
    Remediation = '<p>Microsoft best practices for this value are:</p>
      <ul>
        <li>Set Network security: Do not store LAN Manager hash value on next password change to Enabled.</li>
        <li>Require all users to set new passwords the next time they sign in to the domain so that LAN Manager hashes are removed.</li>
          </ul></a>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        try {
            $ouInfoDict = Get-LinkedOU -ForestName $domain
        }
        catch {
            $ouInfoDict = $null
        }
        ...
        foreach ($result in $results) {
            ...
            if ($result.Attributes.displayname) {
                $gpoDisplayName = $result.Attributes.displayname[0]
            }
            else {
                $gpoDisplayName = "GPO display name is not set"
            }

            $PolicyName[$cn] = $gpoDisplayName

            $policyFiles = Get-ChildItem -path $result.Attributes.gpcfilesyspath[0] -Recurse -Include $gpoFileName -Force
        ...
        foreach ($file in $files) {
            ...
            if ($testMatch) {
                ...
                if ($ouInfoDict) {
                    try {
                        $tempP = $ouInfoDict | Where-Object Policy -eq $policyCN
            ...
            $outputResults = Get-IniContent -Path $file
            $getRegConfig = $outputResults.'Registry Values'
            if ($getRegConfig) {
                $getValue = $getRegConfig.'MACHINE\System\CurrentControlSet\Control\Lsa\NoLMHash'
                if ($getValue) {
                    $isLMHash = $getValue[1]
                    if ($isLMHash -ne 1) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB6+TmjXmzdNI1S
# 2Ppj5InHAvrFa/j6/JYH0vzfO9rC/aCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ