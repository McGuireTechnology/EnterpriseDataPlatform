#This script checks each domain to determine if there are GP Preferences Password entries left in any GPOs
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Description = 'This indicator looks in SYSVOL for GPOs that contain passwords that can be easily decrypted by an attacker ("Cpassword" entries). Until <a href="https://support.microsoft.com/en-us/topic/ms14-025-vulnerability-in-group-policy-preferences-could-allow-elevation-of-privilege-may-13-2014-60734e15-af79-26ca-ea53-8cd617073c30" target="_blank">patch MS14-025</a>, it was possible to store local admin and other high-value credentials in GPOs. The passwords stored in GPOs were encrypted using a global key that was published and easily available to any domain member for decryption.'
    Weight = 8
    Severity = 'Critical'
    Schedule = '1h'
    Impact = 8
    ...
    ResultMessage = 'Found {0} Group Policies Preference password entries.'
    Remediation = 'If Group Policy Preference password entries have been found in one or more GPOs, they should be removed immediately. Their encryption key is well known and can be easily cracked, potentially exposing accounts defined in those GPOs.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'MITRE D3FEND'; Tags = @('Detect - Emulated File Analysis') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $domainfailCount = 0
        $DN = Get-DN $domain
        $results = Search-AD -dnsDomain $domain -attributes "gpcfilesyspath", "displayname" -baseDN "CN=Policies,CN=System,$DN" -scope "Subtree" `
            -filter "(&(objectClass=groupPolicyContainer)(|(gPCMachineExtensionNames=*{91FBB303-0CD5-4055-BF42-E512A681B325}*)(gPCMachineExtensionNames=*{AADCED64-746C-4633-A97C-D61349046527}*)(gPCUserExtensionNames=*{AADCED64-746C-4633-A97C-D61349046527}*)(gPCMachineExtensionNames=*{17D89FEC-5C44-4972-B12D-241CAEF74509}*)(gPCUserExtensionNames=*{17D89FEC-5C44-4972-B12D-241CAEF74509}*)(gPCMachineExtensionNames=*{728EE579-943C-4519-9EF7-AB56765798ED}*)(gPCUserExtensionNames=*{728EE579-943C-4519-9EF7-AB56765798ED}*)(gPCUserExtensionNames=*{5794DAFD-BE60-433f-88A2-1A31939AC01F}*)))"

        if ($results.Count -gt 0) {
            foreach ($gpo in $results) {
                #iterate through each GPO, looking for cpassword entries
                $GPT = $gpo.Attributes."gpcfilesyspath"[0]
                $GPTFiles = Get-ChildItem -Path $GPT -Recurse -Include *.xml
                foreach ($GPPFile in $GPTFiles) {
                    #now look for cpassword with values
                    $lines = Get-Content -Path $GPPFile
                    foreach ($line in $lines) {
                        if ($line.Contains("cpassword")) { #we found a candidate
                            if ($line.Substring($line.IndexOf("cpassword=")+10,2) -ne '""') {
                                $domainfailCount ++

                                if ($GPPFile.DirectoryName.ToLower().Contains("machine")) {
                                    $side = "Computer"
                                }
                                else {
                                    $side = "User"
                                }
                                switch ($GPPFile.Name.ToLower()) {
                                    'services.xml' {$area = "System Services"}
                                    'scheduledtasks.xml' {$area = "Scheduled Tasks"}
                                    'groups.xml' {$area = "Local Users and Groups"}
                                    'datasources.xml' {$area = "Data Sources"}
                                    'drives.xml' {$area = "Drives"}
                                    Default {}
                                }
                                $thisOutput = new-object psobject -Property @{
                                    DomainName = $domain
                                    GPOName = $gpo.Attributes."displayname"[0]
                                    GPOSide = $side
                                    PolicyArea = $area
                                }
        ...
        if ($domainFailCount -eq 0) { #no cpassword GPOs found in this domain
            $scoreObjects += 100
        }
        else {
            $failcount++
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.Count) Group Policies Preference password entries."
            $res.Remediation = "If Group Policy Preference password entries have been found in one or more GPOs, they should be removed immediately. Their encryption key is well known and can be easily cracked, potentially exposing accounts defined in those GPOs."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC