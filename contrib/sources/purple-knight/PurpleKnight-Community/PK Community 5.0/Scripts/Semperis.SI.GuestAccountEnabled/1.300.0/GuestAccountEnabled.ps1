...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 10
    LikelihoodOfCompromise = '<p>Attackers can take advantage of a guest account to enumerate open shares that are accessible to the "Everyone" setting, as is often the case. Additionally, attackers may utilize the limited access these accounts provide to conduct additional scanning for vulnerable users, shares and other network resources.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_guest" target="_blank">Guest account enabled | ANSSI</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # consider chaning the filter to objectsid=<Guest SID>

        $searchParams = @{
            dnsDomain = $domain
            attributes = @("whenchanged","useraccountcontrol")
            baseDN = $DN
            scope = "Subtree"
            filter = "(&(primaryGroupID=514)(iscriticalsystemobject=TRUE)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))"
        ...
        if ($results.Count -gt 0) {
            $failcount++
            $scoreObjects += 0
            $uac = $results.Attributes."useraccountcontrol"[0]
            $thisOutput = new-object psobject -Property @{
                ...
                LastChanged = [datetime]::ParseExact($results.Attributes.whenchanged[0],"yyyyMMddHHmmss.0Z",$null)
            }
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.Count) domains in which guest account is enabled."
            $res.Remediation = "The guest account should be disabled to prevent the associated security risks."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC0pyvUtnVqIl+l
# 2SIf3b2LB6EUSnpM5Wwo8h8xltf3xaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQglgJdwWZGhrujmGHc29rN6e6s5INifEiXrmods452t3gwDQYJKoZI
# hvcNAQEBBQAEggIAVKH/6LQ2TXbcaRxKE2S9V9Oilfb7xq6rJK4R5WWeQG/UdWuA
# fn+/6Elf8dzWprh/wyp5V/Dy6aWmWExLG+NHOwopYFwmxNctvW0Tjc9cENrqsZsg
# soCPl2t9z0w9ixuQNFRQXEwe1NjWJrxcnGSrEuP9GJ1B9rkSdJqqjdVanDy9cwgf
# lIO3HFQM6OwUR2W++dOtCLnklyqEyfE1SExfM0k6WLu+Xe31ncoARCMI+B4E06t2
# nNDDUbRCg/22L7s7fJp975p3va5A455/M8158OIS/G9VBNx4v8xVwLLdcoPGfILV
# rNa+bDmiQLvtM9vARsF2OlqU67PkOZtEOcIOt4ddjEImGFkON3kj8NMhui9l6Bot
# FC8HYy4qgDUACWUG8HYnAO0NYT2vJ3QB1DcBnCnIks8MhEeaRnFFUR9hijUmQCkL
# mP2vXJB1PfQ7R/22+bujdFkW6ao2PWVsSlKwAd+2oMTY6djy2lWCncEg8IUfozfa
# RAJ2w3sHph1qJF/BNlH9643oodcBqu2KKs5I6axgRyhFdcvPSbbeywjqAZEVMSXK
# rBzQL40Q8tsIm13sgPb0CXkB92lMNU+aP6t1rV/6Me7xfNwY1nEbKjGw0ngo0dDP
# naa1CL6rbuXDthUE5XJYu6BQPKRn+X3QXa9IySpG4C0uODiS/3Gybxy5rgyhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDcbY1+azGVYx0EBNDRuPbdP4ezNqawYQpg
# JfPyMi5sfAIUTe67nHDorFPd9ZRsjT44wwPsYqIYDzIwMjUwNTI4MTI0MDE5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBVqY8WfiDbjuMwsShIqmeoO3loK21JG24S9oJBJoX2
# LTCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgJjLh57Bn5i5UhbcvDS6nNHhjvOy6gxBTriCjBaWz+5MQvVNmSiWrIxQ
# r1iZHgLqlajJIHtjXKVWr4JhIp6U1Ao4qsUm79vfdyWxTK6U+1UgwQM9rb8Om0Ms
# 0KgqiJuS/wjJzgRERsvk9tgqSl04Lag3DIm53O155dDSdMCac9TTV68XA0SEYYau
# femkswopd+8TJB+eGF7vGyhHlWRPRBJf9P7lj4aMWKp/GkkZuna5E6iy/Wf2iJhE
# zoZBscW2MyyiuuDYjwqapK3hlWtoz39K9Ps+kj+ep+dsbc2ccWIBMPUC7jo+rSUx
# 2bMIdIYiwhdnVR2x2aEZCJcS5QhL+6scHxCUt4hB3aYhUSEktk0dz3OOGxOA/b5e
# 3N9jgm5yHGqTm6iefpHjx1+5kqjFiX89g+f/DSNuzJTkN4I07R7XgubvvvpsCfzg
# vH3Iee/oMJmCjj8XChPNxMa73FNdc/JcQT0L7IBfOI86/pnnMaLI1WqJQEtI+6wT
# 9mTgsxWtVw==
