# This indicator looks for Domain Controllers that haven't authenticated to the domain in the last 45 days, or more - depends on syncinterval
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Domain Controllers that are not active in the domain would likely be out-of-sync with functional DCs and therefore a compromised offline DC may be of little value to an attacker. However, if an attacker could compromise an offline DC and crack credentials or re-connect it to the domain, they may be able to introduce unwanted changes to production AD that could compromise its security.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_password_change_inactive_dc" target="_blank">Inactive domain controllers | ANSSI</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Determine the sync interval
        $searchParams = @{
            dnsDomain = $domain
            attributes = "msds-logontimesyncinterval"
            baseDN = $DN
            scope = "base"
            filter = "(distinguishedname=$DN)"
        }

        $syncInterval = Search-AD @searchParams

        if ($syncInterval.Count) {
            if ($syncInterval.Attributes."msds-logontimesyncinterval"){
                if($syncInterval.Attributes."msds-logontimesyncinterval"[0] -gt 45) {
                    $daysToRemove = $syncInterval.Attributes."msds-logontimesyncinterval"[0]
                }
            }
        }

        $lastLogonThreshold = (Get-Date).AddDays(-$daysToRemove).ToUniversalTime().ToFileTimeUtc()

        $searchFilter = "(&(PrimaryGroupID=516)(lastlogontimestamp<=$lastLogonThreshold))"
        $attributes = @("lastlogontimestamp","serverreferencebl")
        ...
        if ($results.Count -gt 0) {
            foreach ($result in $results) {
                ...
                if ($result.Attributes."lastlogontimestamp"[0]) {
                    $lastLogon = [datetime]::FromFileTime($result.Attributes."lastlogontimestamp"[0])
                }
                else {
                    $lastLogon = "Never"
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.Remediation = "Ensure that any DCs that have not been in communication with the domain for more than 30 days are either de-commissioned or re-promoted into the environment to ensure consistency with the production AD."
            $res.ResultMessage = "Found $($outputObjects.Count) Domain Controllers which did not authenticate to the domain in the last $daysToRemove days."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCI+XJ80RQk0T02
# AdT5lGcJvZsWp1UqneyFXKD2W6tAKqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgSGAA72S2ZaDhiSEgJtCkawUauw1d06ypXlkuocehrOAwDQYJKoZI
# hvcNAQEBBQAEggIAbe5MB3kq90JgPVm887tcelsrvOvv1Y1VbArrSUnEmH0vYtME
# HVtaf3wjaA/oLKyKdcisUlGIzc4Bba8c1h/Alh1dZeG3gnSvnzr5+42Yuttz4BBF
# ODoTcNxH24mXCCOfNH4XW1Gu7lwVY9GmHoAac4skosLgU0OpfFx1gDoxJzntjXYf
