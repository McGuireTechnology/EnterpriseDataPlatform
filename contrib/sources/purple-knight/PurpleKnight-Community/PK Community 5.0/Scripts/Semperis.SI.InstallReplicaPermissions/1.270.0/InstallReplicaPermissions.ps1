# This script looks for non default objects with DS-Install-Replica permissions (required permission to set SERVER_TRUST_ACCOUNT UAC)
...
try {
    ...
    # DCSync rights GUIDs
    $guidHT = @{"9923a32a-3607-11d2-b9be-0000f87a36b2" = "DS-Install-Replica";
    ...
    foreach ($domain in $DomainNames) {
        ...
        $allowedSIDs = @("S-1-5-32-544", "$domainSID-512", "$forestSID-519", "S-1-5-18")
        $UsersACL = @{}
        $scriptBlockFilter =  {
        ...
        if ($results.count -gt 0) {
            ...
            foreach ($access in $results.Attributes.'aces') {
                ...
                if ($domainSIDHT.ContainsKey($identityDomainSID)) {
                    ...
                    $identityDomainName = $identityDomainDN.Substring(3).Replace(",DC=",".")
                    $identityObject = Search-AD -dnsDomain $identityDomainName -attribute "useraccountcontrol" -baseDN $identityDomainDN -scope "Subtree" `
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects with install replica permissions."
            $res.Remediation = "Ensure that there are no unnecessary install replica permissions and investigate suspicious permissions."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAIO640eHsFU8k5
# CBTFnh7/7n5dps1f+ngWVRCw4OR8y6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgBL9s6tn8RJNhH2j9z3pXwuMJMVArtD4tT25uOod9nDAwDQYJKoZI
# hvcNAQEBBQAEggIAfjZrEb7/YpWK4ow9KKR8VGJj3eVuOLkp4HXE7Sq+j5zoEH5/
# +D08YSyr2gux/btbbOy9GJYKt4eoJwOwS5h0NtZspQXx8kVCWyyPxHHVEvfeccov
# h/pdPQAG54UciCpgqwKWkLaYFiHIdEcl+/a8H10V/6S3pzpqRfMHNKGB08WmxFjI
# 0XKKhVjwCJ447zLj7EvhFlQHOQv+6eSScX+M0cwwDaeVUqctS2xDpz6VoAuDhwBW
# rKGBqx6vehras33+IkC42+BvjdzsLJcg5SNGXTScAA3Ut9YhVgml4+Ig+Fc671ym
# dJ8PeLzRp828cUKEaAAtFpvW6XY3virVEvsCxQStPqRQpgsU/NrtSg4h9rEFT6VN
# Zrs781Hz8WCnV1HYIttu3QQ8a+LqrbALu7A2g7Oy5sP3Md+Szdom4vs3OPSEGr3i
# D8RRh5VilCg8EfKOfN+eJBHs/JFi4V4KmhW6VkjQDs/xNtZcRCtdjWUOJcBplj8o
# xMKuFhBRD5UpJGtpFZeKFPl5nxiPGVdkietTs+lxDgV79aHwUdZZre69vX9CBTjX
# kbJlQm24U3ybF7nvuNkpijlhEQDqLKPM7w+VU41C7JIJ6EiwyMD/rpf21HPR6Qqn
# C8Za3g9X1KfWNNHwwslrKfeFrUhVZd3wEMcwDydWB+KNjVU+Z/eSpbiNB2ehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCKhikMFZa/rEk/n5KwRBmk+cndTMeTrA6W
# AS86q26WVwIUad3yT1a/wFEg08lQCjumDkJy+2gYDzIwMjUwNDE2MTcxNzIwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCARBe6eVlqcpuMPY+7ggSjwRYeWjQuUrAQclYmeeUoK
...
# CwUABIIBgF/ZXtMJ4r5TaIPaa8uqv688LgL10NT3OmxAWlkW5ict4k12zElKB5MB
# cdyiQ+AdCq73pDC8agiP4fBy2fIMKlhJFEVe7STrDe8DbVQwNrTJ+v+OD5xD//el
# zhH0TDjWeq/KcZh9wqYLbA6/mrS7XBiWLXa4JdNlx9PwOFRM9hxIxX5UmP5+pRe4
# Kz6LUpktxXgak4OuH1Jn/ie/kVEDUL97WrQ4U6xpvseWxkDHmwDRlyQcRCizJjJv
# CtuyQfAD2HB0FUdq8kIKpcTXhgtdEARp2yRLRj6HKtYSkzCF89uAfSfKZhhozkLt
# H9+0SvlYLj2/pAyrDGqpXYgZVFcglnln9jXbRSNeSgIxt3Z1P0eJV/imSmODx71D
# /yECM04rxs7tXC1BX/vy1o80+C5XmwvR+TNUQUaYmtQsUO2iN08UVH3G9UBbh2h/
# vJDpjpjqk5t9MGleCklU9W9vODs+swJXYahy916qYluotKln9WOzVg5/f0tPqeWM
# b7bFWbiAgA==
