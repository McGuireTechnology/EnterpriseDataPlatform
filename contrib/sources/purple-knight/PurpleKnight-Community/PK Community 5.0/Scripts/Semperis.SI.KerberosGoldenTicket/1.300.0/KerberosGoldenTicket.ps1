# This script checks each domain to determine if the krbtgt account password has changed within the last 180 days
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>The KRBTGT user account is a special user account in Active Directory that is a service account for the Key Distribution Center (KDC).</p>
      <p>If an attacker is able to compromise the KRBTGT account, they will be able to perform golden ticket attacks in Active Directory, which allow the attacker to impersonate any user.</p>
      <p>Beyond rotating the password twice a year, it is recommended that the password is rotated every time someone who had privileged access in Active Directory, such as a Domain Admin, leaves the organization. There are specific steps that are required to successfully rotate the KRBTGT password, please see the reference article from Semperis for further details.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/golden-ticket-attacks-active-directory/" target="_blank">How to Defend Against Golden Ticket Attacks on Active Directory - Semperis</a></p>
      <p><a href="https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/understand-default-user-accounts" target="_blank">Active Directory Accounts | Microsoft Learn</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_krbtgt" target="_blank">Krbtgt account password unchanged for more than a year | ANSSI</a></p>'
    ResultMessage = 'Found {0} domains whose KRBTGT''s password has not changed in the last {1} days.'
    Remediation = '<p>It is recommended that the KRBTGT account password is rotated as it is over 180 days old.</p>
      <p>Further, it recommended that the KRBTGT account password is rotated anytime someone that had privileged access in Active Directory, such as Domain Admins, leave the company, that the password is rotated.</p>
      <p>There are certain considerations that must be taken into account to ensure a successful rotation of the password. For security purposes the KRBTGT password needs to be reset twice, however, if it is reset before the password has fully replicated across Active Directory you can negatively impact the functionality and availability of Active Directory. Refer to the reference article from Semperis for further details.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $daysToRemove = 180
    $temp = (get-date).AddDays(-$daysToRemove).ToUniversalTime()
    $lastOriginatingChangeThreshold = get-date $temp -Format "yyyy-MM-ddTHH:mm:ssZ"

    # The attribute we search for
    $propertyFilter = "unicodepwd"

    $filter = "(samaccountname=krbtgt)"
    $attributes = @("msds-replattributemetadata", $propertyFilter)
    $minDwVersion = 0
    ...
    foreach ($domain in $DomainNames) {
        ...
        $scriptblockFilter = {
            ...
            Update-AttributeMetadata -Entry $entry -PropertyFilter $propertyFilter -MinimumVersion $minDwVersion -EndOriginatingChangeThreshold $lastOriginatingChangeThreshold
        }
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter $filter -scriptBlockFilter $scriptblockFilter

        foreach ($result in $results) {
            if (!($result.Attributes.'attributemetadata')) {
                continue
            }
            $attrMetaData = $result.Attributes.'attributemetadata'[0]
            $failedDomainCount++
            $thisOutput = new-object psobject -Property @{
                DistinguishedName = $result.DistinguishedName
                Attribute = $attrMetaData.AttributeName
                LastChanged = [datetime]::Parse($attrMetaData.LastOriginatingChange).ToUniversalTime()
            }
    ...
    # Calculate the score, you could change it
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputobjects.count) domains whose krbrtgt's password has not changed in the last $daysToRemove days."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC