...
$Global:self = @{
    ...
    Schedule = '7d'
    Impact = 5
    LikelihoodOfCompromise = 'Unauthorized or unexpected entries in the LDAP IP deny list could suggest a security breach or an attempt to limit access to critical resources maliciously. The likelihood of compromise depends on the following factors:
      <ol>
        <li>Unexpected Changes: Unauthorized modifications to the LDAP IP deny list.</li>
        <li>Unknown IP Addresses: Presence of IP addresses that are not recognized or authorized by the network administration team.</li>
        <li>Security Policy Violations: Entries that violate the organization''s established security policies.</li>
        </ol></a>'
    ResultMessage = 'Found {0} query policies with ldap deny list set.'
    Remediation = 'To mitigate potential security risks associated with the LDAP IP deny list, the following steps are recommended:<ol><li>Regular Audits: Conduct regular reviews of the LDAP IP deny list to ensure all entries are authorized and legitimate.</li><li>Change Management: Implement strict change management procedures for modifications to the LDAP IP deny list. (Semperis DSP for example).</li><li>Security Policy Violations: Entries that violate the organization''s established security policies.</li><li>Security Monitoring: Utilize security information and event management (SIEM) tools to monitor for unauthorized changes or access attempts blocked by the deny list.</li></ol></a>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try{
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("ldapipdenylist", "DistinguishedName")
            baseDN = "CN=Query-Policies,CN=Directory Service,CN=Windows NT,CN=Services,CN=Configuration,$($DN)"
            scope = "subtree"
            filter = "(objectClass=queryPolicy)"
        ...
        try {
            ...
            if ($_.Exception.Message -match  "The object does not exist.") {
                continue
            }
            throw
        }
        ...
        foreach ($result in $results) {
            if ($result.Attributes.ldapipdenylist) {
                $denyList = $result.Attributes.ldapipdenylist
                $resultDn = $result.DistinguishedName
                $ldapDenyBytes = $denyList.GetValues([System.Byte[]])
                $denyListString = [System.Text.Encoding]::ASCII.GetString($ldapDenyBytes[0])
                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $resultDn
                    DomainName = $domain
                    LDAPDenyList = $denyListString
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBgKP0sTmDx9cCk
# tSe9f5DPJEr7LoQHK/BwIpbtGFf75KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgHL335KiCClr5uMX+Fru+3wJjdtTsZGShL/dyE9nJRBQwDQYJKoZI
# hvcNAQEBBQAEggIAK8tcUzmne314il8G/llr+wpw3zJ1tTXiALoezcnC71kcqe+E
# gm+hkpYkPlqUKiba2KBI1X2Vn9EwMB9HcqbOqIOZRSBzMnPm1U3iyJXhD7wgC1tl
