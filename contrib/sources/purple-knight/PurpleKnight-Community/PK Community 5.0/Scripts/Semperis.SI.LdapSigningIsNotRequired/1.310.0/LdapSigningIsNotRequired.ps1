...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>Unsigned network traffic is exposed to MiTM attacks, where attackers alter packets and forward them to the LDAP server, causing the server to make decisions based on forged requests from the LDAP client.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/security-policy-settings/domain-controller-ldap-server-signing-requirements" target="_blank">Domain controller LDAP server signing requirements - Windows 10 | Microsoft Learn</a></p>
      <p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/security-policy-settings/network-security-ldap-client-signing-requirements" target="_blank">Network security LDAP client signing requirements - Windows 10 | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} DCs that do not require LDAP Signing.'
    Remediation = '<p>To remediate follow the steps below:</p>
      <p><b>If the steps are not followed in order, disruption to Active Directory may occur.</b></p>
      <ol>
        <li>Configure clients to request LDAP signing. Group policy: <b>Network security:LDAP client signing requirements</b>, select <b>Negotiate signing</b>.</li>
        <li>When all clients request signing, configure domain controllers to require LDAP signing. Group policy: <b>Domain Controller:LDAP server signing requirements</b>, select <b>Require signing</b>.</li>
        <li>Configure clients to require LDAP signing. Group policy: <b>Network security:LDAP client signing requirements</b>, select <b>Require signing</b>.</li>
        <li>For full details on the process please see the reference articles.</li>
      </ol>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
namespace LdapSigning
{
    ...
    class SSPILDAP
    {
        ...
        const int LDAP_OPT_PROTOCOL_VERSION = 0x0011;
        const int LDAP_OPT_ERROR_NUMBER = 0x0031;
        const int LDAP_SUCCESS = 0;
        const int LDAP_STRONG_AUTH_REQUIRED = 8;
        const int LDAP_SASL_BIND_IN_PROGRESS = 14;
        const int ISQ_REQ_ALLOCATE_MEMORY = 0x00000100;
        const int ISQ_REQ_CONNECTION = 0x00000800;
        const int ISQ_REQ_NO_INTEGRITY = 0x00800000;
        ...
        public class LDAP
        {
            private SECURITY_HANDLE _phCredential;
            private SECURITY_INTEGER ptsExpiry;
            private UInt32 fCredentialUse = 0x00000002;
            private uint _pfContextAttr = 0;
            private uint _contextReq;
            private SECURITY_HANDLE _phNewContext;
            private string _server;
            private uint _port;
            private uint _version = 3;
            private SECURITY_INTEGER _ptsExpiry;
            private SecBuffer output;
            private SecBufferDesc _pOutput = new SecBufferDesc(MAX_TOKEN_SIZE);
            private SecBufferDesc _pOutput2 = new SecBufferDesc(MAX_TOKEN_SIZE);

            public LDAP(string pszPackage, string server, uint port, uint contextReq = 0 )
            {
                _pOutput = new SecBufferDesc(MAX_TOKEN_SIZE);
                _pOutput2 = new SecBufferDesc(MAX_TOKEN_SIZE);
                output = new SecBuffer();
                _ptsExpiry = new SECURITY_INTEGER();
                _server = server;
                _phNewContext = new SECURITY_HANDLE();
                _port = port;
                _phCredential.HighPart = IntPtr.Zero;
                _phCredential.LowPart = IntPtr.Zero;
                ptsExpiry.LowPart = 0;
                ptsExpiry.HighPart = 0;
                _contextReq = contextReq;

                Int32 res = AcquireCredentialsHandleW(
                    null,
                    pszPackage,
                    fCredentialUse,
                    IntPtr.Zero,
            ...
            private SecBuffer outputFromSecurityContext(bool serverResponse)
            {
                if (!serverResponse)
                {
                    InitializeSecurityContext(
                                ref _phCredential,
                                IntPtr.Zero,
                                String.Format("LDAP/{0}", _server),
                                ISQ_REQ_ALLOCATE_MEMORY|ISQ_REQ_NO_INTEGRITY,
                                0,
                                0x00000010,
                                IntPtr.Zero,
                                0,
                ...
                else
                {
                    ...
                    output = _pOutput2.GetSecBuffer();
                }
                return output;
            }

            private IntPtr bervalToCred()
            {
                var berval = new berval
                {
                    bv_len = output.cbBuffer,
                    bv_val = output.pvBuffer
                };

                var cred = Marshal.AllocHGlobal(Marshal.SizeOf(berval));
                Marshal.StructureToPtr(berval, cred, false);
                return cred;
            }

            public string checkSigning()
            {
                int errorNumber = 0;
                IntPtr ldapCon = IntPtr.Zero;
                ldapCon = ldap_init(_server, _port);
                ldap_set_option(ldapCon, LDAP_OPT_PROTOCOL_VERSION, ref _version);
                uint res;
                var timeout = new LDAP_TIMEVAL { tv_sec = 20 };
                res = ldap_connect(ldapCon, timeout);
                ...
                var MessageNumber = IntPtr.Zero;

                for (int i=0; i<=3; i++)
                {
                    if (i == 3)
                    {
                        ldap_unbind_s(ldapCon);
                        return false.ToString();
                    }

                    if (!bindStarted) { output = outputFromSecurityContext(bindStarted);  bindStarted = true; } else { output = outputFromSecurityContext(bindStarted); }

                    // using the output form the bind creating a berval struct
                    var cred = bervalToCred();
                    ...
                    ldap_get_option(ldapCon, LDAP_OPT_ERROR_NUMBER, out errorNumber);

                    if (MessageNumber != IntPtr.Zero)
                    {
                        berval msg = (berval)Marshal.PtrToStructure(MessageNumber, typeof(berval));
                        byte[] msgidbytes = new byte[msg.bv_len];
                        Marshal.Copy(msg.bv_val, msgidbytes, 0, msg.bv_len);
                        _pOutput = new SecBufferDesc(msgidbytes);
                    } else { _pOutput = new SecBufferDesc(16384); }
    ...
    public class SigningEnforced
    {

        public static string[] ResultFromCheckInternal(string host, uint port, string DN)
        {
            SSPILDAP.LDAP ldap = new SSPILDAP.LDAP("Negotiate",host,port);
            string[] resultsArray = new string[3];
            resultsArray[0] = host;
            resultsArray[1] = DN;
            resultsArray[2] = ldap.checkSigning();
...
if (-not ('LdapSigning.SigningEnforced' -as [Type])) {
    Add-Type $code
}
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $dcs += Search-AD -dnsDomain $domain -attributes "dnshostname" -baseDN $DN -scope "Subtree" -filter "(&(objectCategory=computer)(primaryGroupID=516))"
    ...
    foreach ($dc in $dcs)
    {
        ...
        $tasks += [LdapSigning.SigningEnforced]::Check($dcHostName, $dc.DistinguishedName)
    }
    ...
    foreach ($result in $tasks)
    {
        ...
        $isLdapSigningRequired = $resultsArray[2];
        $dcDN = $resultsArray[1]
        $hostName = $resultsArray[0]

        if ($isLdapSigningRequired -eq "False"){
            $domainControllersFailedCount++
            $thisOutput = [PSCustomObject][Ordered] @{
                DistinguishedName=$dcDN
                HostName = $hostName
                State="Ldap Signing Not Required"
            }
            [void]$outputObjects.Add($thisOutput)
        }
        elseif($isLdapSigningRequired -eq "Unreachable"){
            $domainControllersFailedCount++
            $unreachables++
            $thisOutput = [PSCustomObject][Ordered] @{
                DistinguishedName=$dcDN
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            if ($unreachables -eq 0 )
            {
                ...
                $res.ResultMessage = $self.ResultMessage -f $domainControllersFailedCount
                ...
                $res.ResultObjects = $outputObjects
                $res.ResultMessage = "Found $($domainControllersFailedCount-$unreachables) DCs that do not require LDAP Signing., And $($unreachables) DCs are Unreachable."
                $res.Remediation = $self.Remediation
                $res.Status = 'Failed'
                $res.Score = $scoreOutput.Score
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUAB