# This indicator  looks for forest with more than 50 privileged accounts
[CmdletBinding()]
Param(
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>In general, the more privileged accounts you have, the more opportunities there are for attackers to compromise one of those accounts. 50 is an arbitrary number, but the number should reflect the absolute maximum allowed. If business needs dictate many privileged accounts, consider implementing a tiered administration model to further isolate those privileged accounts and their potential impact from compromise.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/security/privileged-access-workstations/privileged-access-access-model">Enterprise access model | Microsoft Learn</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_privileged_members">Large privileged group member count | ANSSI</a></p>'
    ResultMessage = 'Found {0} privileged accounts in the forest.'
    Remediation = 'It''s always best to keep the number of privileged accounts as low as possible. Consider using solutions that allow for Just-in-time administration or implement admin tiering to ensure that only certain accounts can access domain controller, workstation and server resources.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_privileged_members', 'vuln2_privileged_members') }
    )
    Products = @(
...
$countThreshold = 50 # Approved number of privileged accounts in the forest.
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $admins = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "subtree" `
            -filter "(&(objectCategory=person)(objectClass=user)(adminCount=1)(!(objectsid=$domainSID-502)))"
        if ($admins.Count -gt 1) {
            [void]$results.AddRange([System.Collections.ArrayList]$admins)
        }
        else {
            [void]$results.Add($admins)
        }
    }
    ...
    if ($outputObjects.count -gt $countThreshold) {
        $configArgs = @{
        ...
        if ($badObjectCount -gt $countThreshold) {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($results.count) privileged accounts in the forest."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBchmnIeHhC6UKE
# D7GP1RKUI5lTWHfE7DfVn4HkCU/kvqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgPXzOLvcmPOtPWdLY9r4MDNT1KqfGGI7+OArVwIETzx4wDQYJKoZI
# hvcNAQEBBQAEggIAU+teZx3H4kAmf2jI0dW+fJSAbebnu77Z/pDU8Qiycku0yZVy
# cUoFVvAVwA0DXNqBQO1MKLRusucs0mGExAoCY3E1b/xZIOpI8Mqc5gzkYF0sVZU1
# nYFlXsna5Zctacxq3vueGYayUMuAFbWdxpjAWN4Oy5cvgzl5B690B/FYjHuVfK26
# UWTezzLJ/H6bdveSI12OrHPZhXMLVF1QfimAxjrf+1uALTQKhtxfSbZgP2OI/2rE
# 0DOBuPyVkvN58gckNoyiw8LzyQ5dhk3Q+RkTzgNZ6r5XzDif6n42G4GdK7vd9eLO
# JdxG2nYgGCPlgRNEbl/FwSaZjEnocZueh6JZxB0xtwxMbhnSlmDHmFaDDSPjNP6E
# 22IQDXstNuJ0wV7ng2G0xyOyQMwu7pKXta1sUOtSQDK/OXvwUWCWFEKOwsQ6g7G1
# bwkQ1NwizYyn2bz1YB7lIkXGccvMErA1+nFF4TEVvOaAnGHJb7ySJg90+BEtEdQY
# 0KbDeVZADCVIfMBW9SNq4uV4CcoTvUWv8W9zR+CqxInMzGLxnXIZ6y1Olytwf+BR
# cPgNSLY0fjV/kC2PwiYSexnNm2O2ysc2DEWt/IpFsZX7eOnHLXfCZxlHGy7L7DIY
# mE2ug+uwkJTDPNt3r8aM/GYzuQOqinHL8nMYyk7aKvjV10XhcvYrf0H/7JGhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBlAjF6/Bnf0YbSppWqKH0QEWYBfR3Knbja
# 7vEWn7q2/gIUfECiAlPSJJhS/3s75oOpbKSWFAIYDzIwMjUwNTI4MTI0MDM1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBKY5yuuAsuf1RICCBWMvJ+VSrangXIt6Ug0o0/KaWY
...
# CwUABIIBgLVUx3Q7xIJqW7yS3FDi+8un4oSTaNjdrjY9ymlOlirCKZ4VbCUaAS26
# SEm43Asowr9ax/Bo8fSeRCD1bj1XVGxZwtyCS3a0JnmQ4EErvfAbdEtSf9Z2eL+O
# SMUfyb2B8KznGTlGdy10bt8RjSTM+9N2/GiCCSJGj0NjovxE9dsl+l58OWRboQ6B
# vKGL/e4lUL4B1zO0SI21b8iGR1KWb3fUCEEBnn7GxvUhrRNuU8sOqCaXo4A/e0jZ
# Yk2NiM/EiKz3kj6CXcdGX82sbMaOqSgNmJ0Uyjcq34YzzxZ4upa3TfjiECZ84Lba
# hx1IHYIYV7izeNssBtKdKGDTuV+gWCUmgMDD5ta+95O1yMH7eS3Tcv32uNeqSUwh
# FpA2NinNEDnNHhDmeQOeiVNMPj+jpLL6Cky4ZpNYbxwDd1nxckY5NayxDT+5Lkkr
# 1xWPvFGbQVm7yIGNcUYHRZRL07Etbnwbm02Elf2+UJdfF4rxpDtfqzfNe5nVKc2M
# +/U5fFMHOA==
