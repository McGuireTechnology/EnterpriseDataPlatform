...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = 'NTFRS is an older protocol that has been replaced by DFSR. Attackers that can manipulate NTFRS vulnerabilities to compromise SYSVOL can potentially change GPOs and logon scripts to propagate malware and move laterally across the environment.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/windows-server/storage/dfs-replication/migrate-sysvol-to-dfsr">Migrate SYSVOL replication to DFS Replication | Microsoft Learn</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_sysvol_ntfrs">SYSVOL replication through NTFRS | ANSSI</a></p>'
    ResultMessage = 'Found {0} domains that are suspected to use NTFRS for SYSVOL replication.'
    Remediation = 'This protocol is obsolete since Windows Server 2008.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DFSRSettingsDN = "CN=DFSR-GlobalSettings,CN=System,$domainDN"
        $getSettings = @{
            dnsDomain = $domain
            baseDN = $domainDN
            filter = "(DistinguishedName=$DFSRSettingsDN)"
            Attributes = @("msDFSR-Flags")
        }

        $DFSRResults = Search-AD @getSettings

        if ($DFSRResults) {
            if (!($DFSRResults.Attributes.'msdfsr-flags'[0] -eq "48")) {
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) domains that are suspected to use NTFRS for SYSVOL replication."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCALtbe4rG0cMqeA
# pl1xTQG7W7I/bNUVTbKDLg2IQwIbBqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgU/ANBYrgASIp+bUmc0DnZXYksfs6KckAk/wUGDShxdwwDQYJKoZI
# hvcNAQEBBQAEggIAFlVUlbRraG1bYXdwIhI3jVYa9dZ05LDN5HaaG67G88gn6vho
# 08l4xuSHEIQg4IqepWEfYFNN3fQJ8poAbTjMLLGgBi5FDfG/7smoGtLWEZmqZrsl
# 8xH1aqVDdCDhEHOjyHFzkN8U490gsT1syJ5XiNe9QdeZDNJwhLQNd1/4lT6bBCkO
# jHdpOXsP1scgDkMoscBnYmqG+ZWIFICWBaK/hgwnG8TWoN4TB3ZISca5isbJUvxi
# FACC/XPQLCelT3TYytTy4LCvLrasOP6XppH7sMxolwauiQnQnvMf+ZetsQcShg6W
# JPfQtSrG8JjIkI8h+59pTj4NuuQhU0Pv+7wqTNtm7D4zKGQ/CAGjJtUkmNChIKEk
# sWi8uar4NeSbdcrCifQDAbg2uCod2qwf7k48N6HfS2ztTL+/jMMKCMhB8jJ8TGSs
# +7X68EEvWg48PIQNyWRWIl/wjdmXIv57QxUZ4KdJASlxRAweoKB3trGzkZIzyVoj
# Se9DfAtspy3QAiY4RdtbjUSSPcZuvlgBAntiloFawQUiw600LQDg4cM1xjspL3ni
# G+nx3NGjeZqlRJO4Z0VO5HOM7OB+IaMY6mwV0UJNgo7KqEJBVo05+iSBFHJHRAhk
# j9TuaGU8yOrnAKH8JKv7//PsWk9NBiUzRHDpQ23vs3AcZbsLdd0CVS7+2vehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCcxW+1ZN9gPPloQsMDGirdCKziIkD3GeMl
# ZnK2fVeXGgIUKr6TjdfU0nzziz/9Bh3p3QuzUZYYDzIwMjUwNTI4MTI0MDUyWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCB81dNBCdoPLbIkvv9BAc6DSC1kgbdm8JUOjfVvAsy9
# LjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgJ1M0fv2iW4oSCUzqYpZ0t3Pc/36fQSyyAiWCxbOgs5Nn3fTI7gusU67
# 5eNA7TnHk/Z3sNd5WqCCKKnFmdw1V9U9jmMdirjke9I6tI0NpEj2LmTT8OFLg+Uc
# xb9GDKG/zfNUQ2dZL0XnL2NzyErFruFD2S58z9WPR+YPigMPpGKAsiXIyspMM6Ji
# eV8Cep0N96KQvEnfFL2RBy0KiwQ/RR5BlwDsOfUhi+oOyy0TC4qNQMHLhzeTwkRp
# d2UrFVbubCGUASZF+b3e0e5ibyfhqgmL3NuB4FCrKrFHDwR6tXqMQojWadC+4W3D
# Q4x+iJ3XfAl6FEEz0SVt5kjKA5LQKfN/+gVHVHD9kPrnruHbqQ2Z6VvuW002YpYC
# TjnEulK9dEj+UlOtH8ZbHeQs5rg2X5O38lxkd0a1ycfMj9nkMnVCdxI59cizyLT+
# jVaceNChOyAgJ4jgABjYHvfaQU4hRB+96VFoPHRMtJZg/5BdI33TuV6N4MKEANx6
# awtPQeZdHw==
