# This script looks for objects that was created in the last 10 days
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 1
    LikelihoodOfCompromise = '<p>In some environments, object creation happens consistently; however, recently added accounts should be reviewed to ensure they are legitimate.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/auditing/audit-user-account-management" target="_blank">Audit User Account Management | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} objects that were created in the last {1} days.'
    Remediation = 'Ensure that the new objects are known and legitimate.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("name", "objectclass", "whencreated")
            baseDN = $DN
            scope = "Subtree"
            filter = "(whencreated>=$wcdate)"
        }
        ...
        if ($results.Count) {
            foreach ($result in $results) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    ObjectName = $result.Attributes."name"[0]
                    ObjectClass = $result.Attributes."objectclass"[-1]
    ...
    # Count is the number of domains that has objects that were created in the last 10 days
    # This is not much of a security indicator but just information, so score is always 100

    $res.Score = 100
    if ($outputObjects.Count -gt 0) {
        $res.ResultObjects = $outputObjects
        $res.ResultMessage = "Found $($outputObjects.Count) objects that were created in the last $daysToRemove days."
        $res.Status = 'Failed'
    }
    else {
        $res.ResultMessage = 'No new objects.'
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCSxJBOBfjfGO5U
# 6PCPFOknxtFJVx+ZTSQH/QcMjwaEPaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgjKq7mVWJdlmrGPfG1TQouhC2cUYqhD25BcgaorNziQcwDQYJKoZI
# hvcNAQEBBQAEggIADCScEI09Sh60wuvbXN5yx0a/YaviaLqpzY439+SmdwOhxbY4
# 6LDknQgB0W3WVFnc81VatvAn/ARrJtErmuKpxSnK7Nj1BSuOXPejhUE0IkQY/YPm
# DmockXSO+w+6vzR13Z5PQDjxH1dKlXUNr513u3eh51t/FTVG3y8gzuxOupuAR4+Z
# wHhc+2aqtqPq8B0Fvv9mzzA0MQDWcFKWiwMO6XzJXZKDOCheJ+ArXWDpTocaBlui
# o4HRdCYlVpoXpocV3WWcFATg1lBpMkFK2yuecTGnwHRYR8LW3sAURU5mLdDbXiUV
# OIDflM99WzRwSvMI/J60ad4My4iD5Xo7yP1UaxMdIEsaSgY9lQoKHnaJHXmjefBr
# qtXa2O17ZeLif6KAuD3fENThWmd5mNelF7hfoyUNK5DsZYzfyE84egjqZ6SMOhkM
# hJmlY+50q9FCzdBHlOxA6Zq0eF08FO42P1uVLkheMxqAZA1MShT2q1y7M9Ej+6a4
# 7HjR8Gnmn9r4B+wqvIvRu6HDSBOvNs4aEFvkmWFBDOmecFs/4457sZQT5CNpust9
# wtajEgGoqJ1wTPBL0Ci/Ex6b2Mt09PrgLsMrk/Zkf3/cjDzfpCwOJWtRfpIoDflu
# eKJtaWOW3demsWEqLEGevqVDBvRtORsGdVjJM6lJFxpFNMIh5QkfVHTkad2hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBKniEwImBc1uqfjsn0QVQe9Q6rJ22SkDjO
# yBGTwnxn5wIUaJwoFE495HS1LQzWxYhi7kbOZooYDzIwMjUwNTEzMTg1MDQ5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCFROlWWfpNxFz+e0M8M9mID7SN2xW1GMKypbLFzggJ
# vjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgCxSH6T7W10RKZtYhHhVlw4ae2ZQPpLF6MiWh9+ZY66uluE/tU/kuTyJ
# 68k+Je6KefpIcXU+7SxupuY6ent/FTMmkUBr2jXkbtpLu4CgAh9awsne7k4nIiXR
# gYzrhoIgotdCWIS8WcCriwRocTnjuosOUBMUsWquLRqNNfGpY/K8dyP4s4Zh7zG/
# MvYpmSh+UH8TY6DCAmOxLpsJislGiV/A2wgLPeH55fx/THHuzmt8/08s1Gl+yp4K
# CVxN95AJ5DF+eBU2TI3Re8FXwBIdl2Ft4/7oTAkLAwUwuiUMa1AVbJWai4h9x7Tp
# sRvtoOuE/W8MXmTQdGmtkm5XfCBtEZUsCqk1JywMjZwWloya6o1LSWsAw5clTBbO
# 4+SuPw6zPk8tv+FI2B5JEasPmaGdmxNzvFH+35N5WjGQnrkb9slYe3/WqgzNL+Y6
# 3RRhsA0v984OBKKFEtRzP+Ldt7JWODallP0eGDVvqrPJUKYXi+/JTeqZlBHFJUjq
# Q+yrxyJHnw==
