# This script looks for objects that was created in the last month and have admincount=1
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>In most environments, creation of privileged accounts and groups is tightly controlled and audited. This indicator provides a fast method to create a list of new privileged accounts (where adminCount = 1) for investigation and review.</p>
      <h3>References</h3>
    ...
    Remediation = 'Review the list and verify that all privileged accounts and groups that were recently created are valid. Ideally, all privileged account creation goes through an approval-based workflow and gets periodic attestation.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
$startDate = (Get-Date).AddDays(-$daysToRemove).ToUniversalTime()
if ($StartAttackWindow -and ($StartAttackWindow -lt $startDate)) {
    $startDate = $StartAttackWindow
}
...
try {
    ...
    $wcdate = "{0:yyyyMMddHHmmss}.Z" -f $startDate
    $res = New-Result
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all new objects with admincount
        $searchParams = @{
            dnsDomain = $domain
            attributes = $attributes
            baseDN = $DN
            scope = "Subtree"
            filter = "(&(whencreated>=$wcdate)(admincount=1)(objectCategory=person)(objectClass=user))"
        }
        ...
        if ($results.Count) {
            foreach ($result in $results) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    EventTimestamp = [datetime]::ParseExact($result.Attributes.'whencreated'[0],"yyyyMMddHHmmss.0Z",$null)
                }
                Update-IndicatorOutputAttackWindow -ThisOutput $thisOutput `
    ...
    # Count is the number of domains that has a new user who is also a privileged one
    if ($outputObjects) {
        $configArgs = @{
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects that were created in the last $daysToRemove days and are members of a privileged group."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB2WZC364mnqAw4
# iIkeE63+ULmpScq+PNPvgqxCzTvMXaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgK3QqNB++wXVJqwQYXABBuoN2BSNy6elJHfT93qeuDyIwDQYJKoZI
# hvcNAQEBBQAEggIAE05ZK2xU6FvKu1jVV9cuZpwxEnadcsKJtx+vGFbFcFBvzPJ4
# tQGjB7i97p4/ZsAsvCC3RJSLuksV3MkDJS1XWOLDPLVdlTjMKOEybkam14WdROXQ
# 1oNYwWBu/58fIre9xLWeHXxxV5FKJz55AATxQSv2LVx4QTNWuGEmMy4OB8Ugwn6E
# jigXavsXt+2pqP2SnK0I2w6d2RQ+ar9wa5u6Z6wMkvt2pe/dFTFYdDneBg/4zVQe
# /R43pHy/r+wIYloZeUdDBw+nfrDG+M5rbJPxNwxF5JzEAftM9ZD0r/C0On4aN/C9
# p72aTAB42dtty6JbBOOBCqIzp4aJFJCWXf5Ut3EPBLk7w5dRyGqKEi6J3Fv0tWsh
# Tj6hRWT3kZ+QZOz/pZeqD547QEQmDorhIwRolWPsj/pUDi2Z5x7TFY5OEqhxp2PY
# L5hmMMS/AbXZXKa0rjtgdak4b+Lh2zxorp0zajO1xaWH0BVWRoZQgicpm8zb7iUs
# VfQfIHsZFv3F01Q8TccBDGM7K583TFOZvzYomLgmcHEo2zUavhWsHdcVdQFRpBlt
# 1VFa6O3AV2lMnUx+cl/fDuGZqAtU0j/ihLESWPXfDLhGbmrixAJaeEq7UIrQifjE
# imqM3IJk4iXYhIP+2EBwDCtjDERbtNieS2IBpl7kl5+AQGqdmRDlKt8ZStKhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDfFF3PXeL0G5dUfo5DnNmG25T6+QXJKP0R
# 5L561sixPAIUFybyGuYRW+XM0G1CCJVHuv6UuJ4YDzIwMjUwNTEzMTg1MDUzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCD+wmOBcZF1YRbb3TeiR35hfbbOiSL2Z+WU9DjsowAl
# jjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgFXTHi/efaySHal/lUWr3RwOjBRKxOFkVSG2k2XPND6tkDiZgvYl8Q6a
# DblfpH0ECTFh0CgduTtwJ3IK0RKQ5JvKRRSnFkCPYMR/AqevA6QPC9fMHOXEaz9P
# aT8ULNGiPtRjso/vBCSrjya5ahkc5MOnOlJnZ9Yb7wJRcQKACxVrtAfrcOhBl8zX
# dAkhOJoSUXt0Cnyido0jUPKL2E/v2FSLKhA3tZ0Zu8a0BJVhGTuXADqxh5KsTUs2
# vIxOkX9xKf+Yz+clfzcnuS/miB4D74X0XXWvZGgG6gzPpGOk8W2p2dICuP7L4yF5
# 0dNw+6ThuBuowjE5uW/npL+itGwXkquOhYEJsqTIIMglqLrWo45rCYCF8H3scAlX
# HK3YxbemkHgie+SGP5CvDAaxvDNfJXhGQq6KEWI2k9u6/Np/j3Mz/N7Z4F0hiR1y
# Awr1khNqQ7StHNWR20QE/49K+ap7bi91GOyHHdcARqAV1b60pQtmntprGj9mHICT
# 9UeNwFdlAA==
