# This script looks for users with unreadable Primary Group ID
...
$Global:self = @{
    ...
    ScriptName = 'NoPGID'
    Description = 'This indicator finds users and computers for whom it can''t read the PGID. This may be due to the default permission of Read access having been removed, which could indicate an attempt to hide the user (in combination with removal of the memberOf attribute).'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = 'Can be used for hiding users in certain groups (non SDProp protected).'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $userFilter = "(&(objectClass=user)(objectCategory=person)(!(primaryGroupID=*)))"
        $computerFilter = "(&(objectCategory=computer)(!(primaryGroupID=*)))"

        # Get a list of all users and computers without PGID
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("primarygroupid", "samaccountname")
            baseDN = $DN
            scope = "Subtree"
        ...
        $noPGID = Search-ADHelper @searchParams

        if ($noPGID) {
            $failedDomainCount++
            foreach ($result in $noPGID) {
    ...
    # Count is the number of domains that had a user with a PGID that was not readable
    if ($outputObjects){
        $configArgs = @{
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects whose Primary Group ID cannot be queried."
            $res.Remediation = "Ensure that these objects are known and legitimate and check their nTSecurityDescriptor to see if some ACE is causing the Primary Group ID attribute to be unreadable."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDvId91uqnmjE2+
# JgZeKsS+pf2YIYFOd8ffcNekSFBgcaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgqfofEAXyIoMzZbFEudOFWbVY/CswyphbyyVLnNt0CAQwDQYJKoZI
# hvcNAQEBBQAEggIAHIBMBGY65uLAlje8mUydK5q2OUSt+DHlj9xDKkbF+rCWhhP+
# 6/Bb0etsUzmbVtpotU1lDq+i16TaPRd8AxihPMiLOKaDrks7iNA84Q6mi+5pR7fM
# XWXejQmEQgvaB4V7V0BKFRqxzVCOyBTsPw3iZz6ewUaPmLAMkbjFtpHf7W/EuZyM
# 61tqgh918s/V5Dn50uZbwzSlBtZRE4v6v12xlc8Sb16GUD8TZvquxV5AbXZ6Yh/+
# DSbV7pLMSaozt6jmUpbdYOhMX8fC+F9bWAAUDkeyM/ExK3941AmWz1N+xKX5QUyI
# BNAC3SaK+n1QGDBcJHAxCqfKoNV+4tP+e/AkYkzqelJKA721I+pVspevGYW42nZS
# eBuP9SrWJZGabulvaNm0GVIqCnbX0anBzm9A17f3juKsdy17ZD7p5V+XVGIIcjT+
# JVCQYj9O5aVLe+499+cQniAzZCFW+DDSbNtGb6LyEUC/BA7f/OJOQUtdxD4gyiU3
# +KxmBfoEu5Z4WytembQFv3sMxSNcWd37oOogoUYFxHuaHmwq2j+2xs6u2nIv0pfQ
# 8HJTkhZlO6nmlrAZ+Mg5IAJJntmNcKA9fSzCyGfr8slhriqQkwjJZBj7s37F21sl
# T8F3HaQqOkviVWVxlAqMuuJT49BfPyrL7g4ijdg9jEhSYRFIUiVTR33IYzOhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAJHfy59xNA8v93tynoNv88gbbqIeP9O/Tc
# 2TEYoFNMcwIUArSfPSvKJg6UUjDhCc+j8nTYBscYDzIwMjUwNDE2MTMxMjExWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBYpOVQn9F1N9FQDczbJqrtCoVHCRpUf4de9+JBllna
...
# CwUABIIBgCiGhDUQmdksa0a/CiHI/YgpZTLsMqyWrOUec0d/+8pzEX+d8HxP6wph
# 5x/P/v6ohkdnBNCMIcc9FLUzYvRDJP57Lj/CxHvAxEMao7ehitHLVGr5An9sy6IG
# NxBCjH1HrpTzmB15GHLm74qAjR+BbCzI5XNiovT8Vx+iM7acF3pga8zuRT4ExvLJ
# sPRt7rNRJBOKjDOSJBhaT1bfqhBBWYIWH7R57EbQK5DyEW22i1bmVO7ADg8F3m2v
# 9tvBxP7AS6yJHbpJxQap2XXwdE5YmJ11RHqIe6HlRFD9Fr3uxBGQBQACn6chg0bw
# PfY3cRdKxjFM73Kn3h68/L1QAe8p9lLi84qJMfJQtGcUoXvwzBi9/93WiSpGInQh
# rkhQe/U+hdogRM850Zwo1nmvlcTJAGUcMDz4ggziK11COYFb6/BiXK4jKalgmSlo
# F3jZpfjuAsq8XzyKJqiKXIryNJHzeTEHdZ23gy99qwqg3mn4lwZMg1K2pcJ2BRF5
# gcOdcjy61g==
