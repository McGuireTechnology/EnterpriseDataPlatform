...
$Global:self = @{
    ID = 355
    UUID = '0e61db1b-b877-457e-9550-607330143d92'
    Version = '1.290.0'
    CategoryID = 2
    ShortName = 'SI000355'
    Name = 'Distributed COM Users group or Performance Log Users group are not empty'
    ScriptName = 'NonEmptyDcomAndPerformanceLogGroups'
    Description = '<p>This indicator checks if either the Distributed COM Users group and/or Performance Log Users group in Active Directory are populated.</p>'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>Members of the Distributed COM Users or Performance Log Users Groups can remotely trigger actions and relay the authentication of users connected to the target server, including Domain Controllers.</p>
      <h3>References</h3>
      <p><a href="https://decoder.cloud/2024/04/24/hello-im-your-domain-admin-and-i-want-to-authenticate-against-you/" target="_blank">Hello: I''m your Domain Admin and I want to authenticate against you</a></p>'
    ResultMessage = 'Found {0} members in Distributed COM Users and/or Performance Log Users groups'
    Remediation = '<p>Organizations should evaluate the use of these groups and restrict the membership of these groups to trusted users.</p>
      <p>Organizations that are using these groups should implement an appropriate rights delegation to the necessary OUs in Active Directory as an alternative.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    # S-1-5-32-562 - Distributed COM Users , S-1-5-32-559 - Performance Log Users
    $groupsSIDs = @("S-1-5-32-562","S-1-5-32-559")
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $groupsDNs = Search-AD -dnsDomain $domain -attributes "" -baseDN $DN -scope "subtree" -filter $filter

        foreach ($groupDN in $groupsDNs.DistinguishedName) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCiynK4YhRz+Nwi
# u+mLBB4QOcRvxvM9LK4kMJHlZaj8RqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgLKOXohoSex7/5BWNWPhLiUbnAR/QkphiT6Xt8Z1UkmQwDQYJKoZI
# hvcNAQEBBQAEggIAHK2CPrqcSBS8FcwtxEpR7VWzh0SGuNe9M/BfWXh+e6+icayw
# 4Sx0T1xIZs+ZExlf6kHrtntT4aLu5uAocf51uCrGO24j0IfKGgNCKTZbjRNpAQdv
# PwNJjkNNp5XwUEYS3PilLKO5uNeHFry3POrTGSvTcRnV6dDe2b9xCw3eVhkO3U/T
# NY6RAB5yesJe/bnYblujBOQS9sFx6csbzFvesFOpKKNCBySk92aRC0c61Wl9MP0x
# NymQYNH8PtUC9Nk2NlBiuLyAdgg0sKSIz9Xu46lLAz9fXhEzuslFXYERpP0MXpob
# k14aXo1kFliFg6usPxKcr1533zIXMIUvqGfEIVL9w0ycZKNg5G98teSXtn0N5OfS
# um2FrQM/48EXHnMto+4Lz/a6sysAINMoadVeIjr3UX78yAjREnW1rD6xo+DSeNyh
# bZmz8BmCAzl2jm6RJOaUJLHoIHGoXYsgeUlvF+XmoI5/wc+4ZJyfGNCmdUz2lZzP
# TRqQf7h3Y+gx+rueBm5TCxg+sfGpdUKEa5PXhHVIFEmTb+jPIlHWyklGyzbw32W4
# LJNR0j/04c4waDrpeUucfwXvfhIAOeACpYQxBZa8JkuNjm1HNPgW5hERPiFfcqeA
# IBBGYCjMNYaNDEhAcyhakDKsrsPNZUo6Bi3Gn3L4K8E1hHT99oH8yhfQMB+hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDqGuA3SBWsuCeGR5GtNNerhGx8gFY59byU
# /9lIrKYHvQIUfdzHwY02jqn5POj5p5/TVufQcrIYDzIwMjUwNTEzMTg1MDU4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCClwmZXGZAGGpL21cdz5nbT6D8mc6I+Z5TqwE0aF/A4
...
# CwUABIIBgFPfLee+in62Zc60HOo3wgumOnSy90VIWdMJPZV3DUbxOTwGXP6oFeGa
# T8Z/PrI/CuLCbHuoEQXNmGcKhFzscF4Zzk/+wGGd9xauwUzFeVyS3/TOkGcGWwy7
# BU43I16PY13EyZBQhYj2mUPrOQpyWZ9Xkxjdw/fdO/8AVAtAFhj2ea5ymb/mBUCo
# hO3slfSKDqpQ5U1FIR7EZfKhVhqVsaHx8beXoEQU5QSkhO5dZ7xua9rimybTbx2e
# cOa9NMMGJlkVvuSWUyx1lBPxlI7zdDygerODWnhIfzNSbPT/HmOkrVQBteJpuH5x
# wrgVUCFw2X1Fz0dYWb4p6KxGVnreRsdcX0/ZPl5mLv1mn6iWW+doxhLu3jMZar5F
# d9EJ+LE1MTkDQlVCqz8MRxrOxAwKDou2k/xtuSuJh8TDtHHdg7VJaVI2Jnhg3j8L
# DO58GKm9w61mt5dgFIKbBz+9opwaRCcXglpnfWCAJPLkWWzrX7q5YHsImLG1+0hx
# 9YXq/+06Cw==
