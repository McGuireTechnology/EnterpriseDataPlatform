# This script looks for objects with admincount=1 that are not members of a privileged group
...
try {
    ...
    $privilegedRIDs = @()
    $privilegedRIDs += $builtinPrivilegedGroups | ForEach-Object { $_.split("-")[-1] }
    $privilegedRIDs += $privilegedGroups
    $privilegedRIDs += $forestPrivilegedGroups
    ...
    foreach ($domain in $DomainNames) {
        ...
        foreach($groupRID in $privilegedGroups) {
            $groupObject = Search-AD -dnsDomain $domain -baseDN $DN -scope "Subtree" `
        ...
        # Build Filter starting with Primary Group
        $filter = "(&(admincount=1)(!(samaccountname=krbtgt))"
        foreach($privilegedRID in $privilegedRIDs) {
            $filter += "(!(primaryGroupID=$privilegedRID))"
        }
        foreach($groupDN in $privilegedGroupsDN) {
            $filter += "(!(memberOf:1.2.840.113556.1.4.1941:=$groupDN))(!(distinguishedName=$groupDN))"
        }
        $filter += ")"
        ...
        if ($results.Count) {
            ...
            foreach($result in $results) {
                ...
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    Type = $result.Attributes."objectclass"[-1]
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects with adminCount=1 that are not members of a privileged group."
            $res.Remediation = "Remove the adminCount = 1 attribute from these users. Investigate unknown users with this attribute."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD3jfI25Dxzi1IT
# LgenKl3fiZgwNp32rgOindH3IvBSIKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgI2KmjyxZV4BrnSPJZToeIJBzaMdd8uNr983OJJcGUq0wDQYJKoZI
# hvcNAQEBBQAEggIActZSQxyKvk9fKFb87bUSnGYbxjgwIrMqKgLdqKiKlvh1sG10
# IHI/mtSKo4T4NR1FkOxa3euNyQoKcPEOLjAK33wEhDhTqAqGzI7F4mxBzmxQZTs3
# jPrpFZDoe5Qf9S1J0Bnht4P8LCxSh0v7mbc3bfuWDVhyMCimQhf5kR4P4vV2D6Sj
# gUy7dP8Wclo1ia/kCuZzKPh/IG46bV7liB0Zg7WhOENiLiLPnu0vDVsDDoRQ/P8n
# 8yzZGpZYPkIdwECvwit0h3I28gUM20bJ1CNOnw5uj22OSxdiCVaKwE+kCOHeHMTE
# W/JpIMJhEmwnDZOZKWYnq8CWW4pOGPlyIYhAfwnqm7Cv3N/LYY3gZxYUEIm2PZvT
# AhRJX2ZpAxjxEGxgQ4z5xX2SVAb4LEOnWJGMHGNxufRroZFEC+iaWZqKB6eMsOTw
# NIOsEL9AfFUKYiHEuv4a01G5RA15U2ynORhNFhMor26ycIQPbPcAmb3rTtAKu5Bl
# DbYtKnbuRm8Qqw/mEYhhdEn5tOw9/HlUS0EeziYlT3Y0W+kCsv29geXCA47EZhob
# CChvlX9mi8KEOk3ZSkO4yNNUovvpZWeR0YdTabU7cwzo3GKjDfSAmneBMeHsO5OY
# NZfT6alzJZY9dPGMo398K3Xl2m8OHHJK2rkJv0ZG7CvugbNE8plM3yy3BXyhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCoYsplD6zGldwScB82qgLIRsK+5qEJuytG
# kn25XO7r9wIUZ2imU0DLFeU3UQDOrhyPaYvbPnsYDzIwMjUwNDE2MTMxMTU3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDjaZlnREmwRNZaUc8xmmG7l349kyycAlhVvOSkr5gj
...
# CwUABIIBgD8uWCFt6vZrVQxMprvjcvFUD0hP/ukHxmujqld2ozMECYAZsMnWtz3t
# EIq/MvNGb+Go4ww1MDc9zmCWlNvupsBdNvZQfXUyFv4L5mKvhw7gnGdcJsTqW4U2
# v7RnQEsXeYql7SSKDgPvX0lyytPzQfDaUwx1awmYWBaD/VWH4U/uI5lBmQtB92NT
# BIsnCuQz/yXHZd+OSf67SHsTE2wXTuIHNVbeUsZ5MsLS+b2W37pVyWEYjpiFSmRL
# Zrnv7vlX/kN/1KO/gW5ZEIyUxHXWqwBy4jjAvIJkS5dvQ44VZiQdfmbOr7uoroNi
# 5YnAYqNk1N+IXP6j5UJtm3o2iBlWfGziOMzk0UtDHYaz5+OoOMlqJlzxZ32hEEza
# lcP0VdM4Oof5Wb1AZnATCclutEytd6M7oDkYnVWb/zGX3WA2oOBOrTb15JKRtkWo
# D46V+I141ONv/4iiampYXkntDSW6aUao+bI4WqKGRIzcIHX3hgdVlMuERJmorKqV
# 2YaSCaE3Sg==
