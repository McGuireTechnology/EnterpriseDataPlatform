# This script looks for users with Primary Group ID of a group they are not member of
...
$Global:self = @{
    ...
    Description = 'This indicator returns a list of users and computers whose Primary Group IDs (PGIDs) are not the defaults for domain users and computers. Users created in the domain will have a default PGID of 513 (Domain Users) or 514 (Domain Guests) while computers are 515 (Domain Computers), 516 (Domain Controllers), or 521 (RODC). The Primary Group ID is not automatically changed when a user is moved to a different group (i.e. a user moved into Domain Admins will not be assigned PGID 512). This fact can be used to hide users with privileges to systems that rely on PGID, while hiding the user from queries that rely on enumerating the member attribute without the PGID. Additionally, group objects'' member attribute will not list the user objects with PGID of those groups.'
    Weight = 3
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>Modifying the Primary Group ID is a stealthy way for an attacker to escalate privileges without triggering member attribute auditing for group membership changes.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_primary_group_id_1000" target="_blank">Accounts with PrimaryGroupID lower than 1000 | ANSSI</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_primary_group_id_nochange" target="_blank">Accounts with modified PrimaryGroupID | ANSSI</a></p>'
    ResultMessage = 'Found {0} objects with Primary Group ID of a group they are not member of.'
    Remediation = 'Ensure these PGIDs are needed. When unneeded change the Primary Group to Domain Users (can be done through the MemberOf tab in dsa.msc).'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $userFilter = "(&(objectClass=user)(objectCategory=person)(!(primaryGroupID=513))(!(primaryGroupID=514))(!(primaryGroupID=528))(primaryGroupID=*))"
        $computerFilter = "(&(objectCategory=computer)(!(primaryGroupID=515))(!(primaryGroupID=516))(!(primaryGroupID=521))(primaryGroupID=*))"

        # Get a list of all users with PrimaryGroupID different then 513 (Domain Users) or 514 (Domain Guests) or Computers different from 515 (Domain Computers) or 516 (Domain Controllers)
        $searchParams = @{
            dnsDomain = $domain
            attributes = "primarygroupid"
            baseDN = $DN
            scope = "Subtree"
            filter = "(|$userFilter$computerFilter)"
            PropertyFilter = "primarygroupid"
            StartAttackWindow = $StartAttackWindow
            EndAttackWindow = $EndAttackWindow
        }

        $PGIDResult = Search-ADHelper @searchParams

        # Check each non standard PGID if the object is indeed a member of the group
        foreach ($result in $PGIDResult) {
            $groupRID = $result.Attributes."primarygroupid"[0]
            $group = Search-AD -dnsDomain $domain -attributes "samaccountname" -baseDN $DN -scope "Subtree" `
                -filter "(objectSid=$domainSID-$groupRID)"

            if (!($group)) {
                $groupSamAccountName = "No group found that matches the RID"
            }
            else {
                $groupSamAccountName = $group.Attributes."samaccountname"[0]
            }
            ...
            $thisOutput = [PSCustomObject][Ordered] @{
                ...
                PrimaryGroupID = $result.Attributes."primarygroupid"[0]
    ...
    # Count is the number of domains that had a user with a PGID that do not match any of his groups
    if ($outputObjects){
        $configArgs = @{
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects with Primary Group ID of a group they are not member of."
            $res.Remediation = "Ensure these PGIDs are needed. When unneeded change the Primary Group to Domain Users (can be done through the MemberOf tab in dsa.msc)."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC