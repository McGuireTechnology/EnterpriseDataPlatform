# This script checks the schema for non default permissions
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Description = 'This indicator looks for additional  principals with any permissions beyond generic Read to the schema partitions.Schema is one of three main Active Directory naming context. It contains every object attribute definitions of the forest.'
    Weight = 7
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>By default, modification permissions on schema are limited to Schema Admins. These permissions grant the trusted Principal complete control over Active Directory.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_permissions_schema" target="_blank">Dangerous ACLs expose schema objects (attack path) | vuln_permissions_schema | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_permissions_schema', 'vuln2_permissions_schema') },
...
try {
    ...
    $schemaDN = "CN=Schema,CN=Configuration,$forestDN"
    ...
    # Get-ForestSID will also check if not removed here
    ...
    if ($result.Count -gt 0) {
        ...
        $allowedSIDs = @("S-1-5-9", "S-1-5-32-544", "$forestSID-518", "$forestSID-498", "S-1-5-18")
        ...
        # Check if owner is different than Schema Admins
        try {
        ...
        if ($ownerSID -ne "$forestSID-518") {
            $acl[$securityDescriptor.owner] = "Owner"
        }
        ...
        foreach ($access in $securityDescriptor.Access) {
            ...
            # Continue if it's one of the allowed SIDs, if it is deny access or if it is only read
            if ($allowedSIDs.Contains($identitySID) -or $access.AccessControlType[0] -eq "Deny" -or $access.ActiveDirectoryRights[0] -eq "GenericRead") {
                continue
            }

            if ($acl.ContainsKey($access.IdentityReference.Value)) {
                $acl[$access.IdentityReference.Value] += "; " + $access.ActiveDirectoryRights.ToString()
            }
            else {
                $acl[$access.IdentityReference.Value] = $access.ActiveDirectoryRights.ToString()
            }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) non default ACEs on the schema."
            $res.Remediation = "It is recommended to revert schema permissions to default state. See indicator description for additional info and link."
            $res.Status = 'Failed'
    ...
    elseif ($DomainNames.Count -eq 0) {
        $res.Status = 'Error'
        $res.ResultMessage = "Unable to retrieve the Schema partition. The following domains were unavailable: $($unavailableDomains -join ', ')"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAkCdZeukeyo9ru
# l2yTXmCmqaf/n6v0DobIxR6hTcV9kqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg