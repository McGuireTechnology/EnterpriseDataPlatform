#Identifies when a new access token is created
[CmdletBinding()]
param(
...
$Global:self = @{
    ID = 303
    UUID = 'fce1590d-01ea-4c23-a653-7d4d395ae36a'
    Version = '1.219.0'
    CategoryID = 8
    ShortName = 'SI000303'
    Name = 'New API token was created'
    ScriptName = 'OKTA_NewAppAccessToken'
    Description = 'In the last 7 days,a new API token was created.'
    Weight = 4
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 4
    LikelihoodOfCompromise = 'API tokens with high privileges can allow unauthorized access and actions in Okta. If an attacker gains access to the token''s password, they can leverage it to query and perform actions, potentially leading to persistence and compromising the environment.'
    ResultMessage = 'Found {0} new API tokens in the last week.'
    Remediation = 'Ensure that the API token was created and used legitimately. Regularly review and audit API tokens to verify their necessity and permissions. Implement proper access controls and token management practices to mitigate the risk of unauthorized access.'
    Types = @('IoE')
    DataSources = @('Okta.OktaAPI')
    OutputFields = @(
        @{ Name = 'Operation'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Tokens'; Type = 'String'; IsCollection = $false },
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'MITRE D3FEND'; Tags = @('Harden - Application Configuration Hardening', 'Harden - User Account Permissions') }
    )
    Products = @(
...
try
{
    ...
    $logsUri = '/api/v1/logs'
    $filterTime = "$($since)"
    $filterEvent = 'event_type eq "system.api_token.create"'

    $getNewAppToken =  @{
        Uri = "$($logsUri)?since=$($filterTime)&until=$($now)&filter=$($filterEvent)"
        DomainUrl = $OktaDomain
        TokenValue = $OktaApiToken
    }
    $newAppToken = Invoke-OktaApiRequest @getNewAppToken

    foreach($token in $newAppToken)
    {
        $tokensOut = ""
        foreach($t in $token.target)
        {
            $tokensOut += $t.id + " ," + $t.displayName + ";"
        }
        $actorId = $token.actor[0].id
        $actorName = $token.actor[0].displayName
        $thisOutput = [PSCustomObject][Ordered] @{
            Operation = "Add Access token"
            Tokens = $tokensOut
            EventTimestamp = $token.published
            ActorID = $actorId
            ActorName = $actorName
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDHacHcLOzBVWBO
# 1FpamiDQUeXYrNX/ctDs/QhVqLLc7qCCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# FTAvBgkqhkiG9w0BCQQxIgQg4qUJkaI1zDfVAlYb6uUtDtyg+drgRLAfKJZNLSeL
# zuYwDQYJKoZIhvcNAQEBBQAEggIAh4OaU6gnGVa6hNN7e84r58NxXFGSw7NW7BIT
# AV7wUfGyov5qyf7sKdzBLMMUmlGfaUt+7izsfs1pqlImBs+JKMezYI2dM3OBRlNg
# 63W+6Ih451VKQTXcD2wBsTjGdlAxYHjhuFMZiwUPGCKzpjRQQZSF1dpwgEJ5TYBC
# 9Q0QH0PnGr+ixp2GHUaLzV3hAjjYhzMVUGF4mN122orgfClaQZEshDxGfDduO3+p
