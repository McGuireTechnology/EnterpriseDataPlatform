...
$Global:self = @{
    ID = 306
    UUID = '74e7dd15-b715-4926-bb7e-71c12d3ced5a'
    Version = '1.219.0'
    CategoryID = 8
    ShortName = 'SI000306'
    Name = 'New Super Admin permission has been granted to a group'
    ScriptName = 'OKTA_NewSuperadminGroup'
    Description = 'In the last 7 days, "Super Admin" permission was given to a group.'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = 'Members of a group with "Super Admin" privileges have extensive access and can perform significant actions in Okta. This includes managing user accounts, configuring security settings, and controlling critical aspects of the environment. It is important to closely monitor and control which groups are granted these strong privileges to prevent unauthorized access and potential compromise of the Okta environment.'
    ResultMessage = 'Found {0} events granting "Super Admin" permissions to groups in the last week.'
    Remediation = 'Make sure that the "Super Admin" permissions given to the group are indeed necessary and were done legitimately.'
    Types = @('IoE')
    DataSources = @('Okta.OktaAPI')
    OutputFields = @(
        @{ Name = 'Operation'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Groups'; Type = 'String'; IsCollection = $false },
...
try
{
    ...
    $logsUri = '/api/v1/logs'
    $filterTime = "$($since)"
    $filterEvent = 'event_type eq "group.privilege.grant"'
    ...
    foreach ($admin in $adminAssignment) {
        ...
        if($priv -match "Super administrator"){
            $thisOutput = [PSCustomObject][Ordered] @{
                Operation = "Grant group privilege"
                Groups = $groups
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAqiVaud4SD7aEY
# XyijshNA9zNSgJaMqOKdPBI6UVeYq6CCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# FTAvBgkqhkiG9w0BCQQxIgQgyrVvNzbD72e/X5R6ihkOwndALuAxRoJmoG/MnboR
# kCYwDQYJKoZIhvcNAQEBBQAEggIADeaJRqW7Ao9VWhpu/LL8673dFBwoJ0oR07f+
# 97mFqOMXpEE5WZqoHfe0EjKg5/r64YQV4bNI2yLDJkNmvC3V84WrudDpg3eZ//OW
# 6R+4f6gVbPM6HsyLDgPkSy1nJl0xviniyIEm9pUmkdFCtZTwcm6c280jurfG6s+P
# c5ArT8kRy0d61l/nXjAVMYCrQqzuNYvL3ZIkbFr+AOZojQQ8Gj00IiR0wK5IFNaw
# q+uQgIrRW9+gn8B8doPJ7e4XNY1pTENZI9P6NaUjaH/htkjBuHuYgpZPYpKiudX2
# 3oHZb0MZfqVjmPAog08il+r/mW+GlVzsbvgTZcyH42b2jSoG6iUOS8hcmEZNzliR
# gLpCi+qmZLj1m+bZ3uIav4/At5qg4sNC80Pgd/ryeVYYNUaq+wZqNdT5OW+JHTsv
