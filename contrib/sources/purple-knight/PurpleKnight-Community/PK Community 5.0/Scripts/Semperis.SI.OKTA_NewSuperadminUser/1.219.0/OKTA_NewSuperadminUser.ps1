#Identifies when an administrator role is assigned to an Okta user.
...
$Global:self = @{
    ID = 305
    UUID = '4e25afc3-f7e1-4c1f-b092-eb7287c0f2d1'
    Version = '1.219.0'
    CategoryID = 8
    ShortName = 'SI000305'
    Name = 'New Super Admin permission has been granted to a user'
    ScriptName = 'OKTA_NewSuperadminUser'
    Description = 'This indicator checks if the "Super Admin" permission was given to a user in the last 7 days.'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = 'Granting the "Super Admin" permission to a user grants them extensive privileges and control over critical aspects of the Okta environment. This includes managing user accounts, configuring security settings, and accessing sensitive information. Unauthorized or excessive granting of the "Super Admin" permission can significantly increase the risk of compromise and unauthorized access to Okta.'
    ResultMessage = 'Found {0} events granting "Super Admin" permissions to users in the last week.'
    Remediation = 'Regularly review and audit the assignment of the "Super Admin" permission to ensure it is granted only when necessary and to authorized individuals. Implement strong access controls, such as multi-factor authentication and least privilege principles, to limit the impact of a compromised super admin account. Monitor and analyze user permission changes, and promptly investigate and address any unauthorized or suspicious activities related to the "Super Admin" permission.'
    Types = @('IoE')
    DataSources = @('Okta.OktaAPI')
    OutputFields = @(
        @{ Name = 'Operation'; Type = 'String'; IsCollection = $false },
        @{ Name = 'Users'; Type = 'String'; IsCollection = $false },
        @{ Name = 'PrivilegeGranted'; Type = 'String'; IsCollection = $false },
        @{ Name = 'EventTimestamp'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ActorID'; Type = 'String'; IsCollection = $false },
        @{ Name = 'ActorName'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('Okta')
    Permissions = @()
    SecurityFrameworks = @(
...
try
{
    ...
    $since = Get-Date $week -Format "yyyy-MM-ddTHH:mm:ssZ"
    $defaultRequestArgs = @{
    ...
    $logsUri = '/api/v1/logs'
    $filterTime = "$($since)"
    $filterEvent = 'event_type eq "user.account.privilege.grant"'
    $getNewAdminAssignment = $defaultRequestArgs.Clone()
    $getNewAdminAssignment['Uri'] = "$($logsUri)?since=$($filterTime)&until=$($now)&filter=($filterEvent)"


    $adminAssignment = Invoke-OktaApiRequest @getNewAdminAssignment

    foreach($admin in $adminAssignment){
        $user = $admin.target
        $users = ""
        foreach($u in $user)
        {
        ...
        $priv = $admin.debugContext.debugData.privilegeGranted
        $actorId = $admin.actor[0].id
        $actorName = $admin.actor[0].displayName
        if($priv -match "Super administrator"){
            $thisOutput = [PSCustomObject][Ordered] @{
                Operation = "Grant user privilege"
                Users = $users
                PrivilegeGranted = $priv
                EventTimestamp = $admin.published
                ActorID = $actorId
                ActorName = $actorName
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDZJ/uMSo3h3KVf
# gyGRNGwrSOhYFLYAOPyamafAHj8rHqCCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
