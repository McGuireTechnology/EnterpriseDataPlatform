...
$Global:self = @{
    ID = 310
    UUID = 'a4a97716-bca9-498e-844b-eef6ac402efa'
    Version = '1.290.0'
    CategoryID = 8
    ShortName = 'SI000310'
    Name = 'Password policy check'
    ScriptName = 'OKTA_PasswordPolicyBestPractice'
    Description = 'This indicator evaluates the password policies in place and verifies if they adhere to Okta''s recommendations.'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = 'A strong password policy is crucial in preventing unauthorized access to the environment through brute-force attacks.'
    ResultMessage = 'Found {0} Password policies that do not meet Okta recommendations.'
    Remediation = 'To ensure a robust password policy, please consider the following recommendations:
        <ul>
        <li>Set a password lockout threshold of 10 or more failed login attempts. This helps protect against brute-force password attacks.</li>
        <li>Implement a minimum password history of 24 passwords. This prevents users from reusing previously compromised or weak passwords.</li>
        <li>Establish a minimum password age of one hour. This ensures that users cannot immediately change their password to bypass the password history requirement.</li>
        <li>Specify a minimum password length of 12 characters or more. Longer passwords offer increased resilience against brute-force attacks.</li>
        <li>Consider restricting the use of common or easily guessable passwords, as recommended by Okta.</li>
        </ul>'
    Types = @('IoE')
    ...
    OutputFields = @(
        ...
        @{ Name = 'Priority'; Type = 'String'; IsCollection = $false },
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'MITRE D3FEND'; Tags = @('Harden - credential Hardening') }
    )
    Products = @(
...
try
{
    ...
    $uri = "/api/v1/policies?type=PASSWORD"
    ...
    $passwordPolicies = Invoke-OktaApiRequest @getPassPolicy

    foreach($policy in $passwordPolicies)
    {
        $passSettings = $policy.settings.password
        $lockout = $passSettings.lockout.maxAttempts
        $minHistory = $passSettings.age.historyCount
        $minAge = $passSettings.age.minAgeMinutes
        $minLen = $passSettings.complexity.minLength
        $commonRestriction = $passSettings.dictionary.common.exclude
        $isPolicyNotTheBest = 0
        $policyNote = ""
        if ($lockout -lt 10)
        {
            $isPolicyNotTheBest += 1
            $policyNote += " Lockout: $($lockout);"
        }
        if ($minHistory -lt 24)
        {
            $isPolicyNotTheBest += 1
            $policyNote += " History: $($minHistory);"
        }
        if ($minAge -lt 60)
        {
            $isPolicyNotTheBest += 1
            $policyNote += " Minimum age: $($minAge);"
        }
        if ($minLen -lt 12) {
            $isPolicyNotTheBest += 1
            $policyNote += " Minimum length: $($minLen);"
        }
        if ($commonRestriction -eq $false) {
            $isPolicyNotTheBest += 1
            $policyNote += " Common Password restriction: False;"
        }
        if($isPolicyNotTheBest -gt 0)
        {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBqhDrmsIgAzfDV
# CITvjz+CX+sl8g/Tzi9iUGw469K/0KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg