...
$Global:self = @{
    ID = 307
    UUID = 'ad1b3358-d94e-4e3d-abdc-5cf49a7b784f'
    Version = '1.219.0'
    CategoryID = 8
    ShortName = 'SI000307'
    Name = 'User activation in the last 7 days'
    ScriptName = 'OKTA_UserActivate'
    Description = 'This indicator checks if a user has been activated within the last 7 days.'
    Weight = 1
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 1
    LikelihoodOfCompromise = 'Activated users have the ability to authenticate and perform actions within the environment. It is important to monitor and verify the activation status of users to ensure that only authorized individuals have access.'
    ResultMessage = 'Found {0} new activated users.'
    Remediation = 'Review the activation status of users to confirm that they should indeed be active. Deactivate any users who no longer require access or whose activation may be unauthorized.'
    Types = @('IoE')
...
try
{
    ...
    $logsUri = '/api/v1/logs'
    $filterTime = "$($since)"
    $filterEvent = 'event_type eq "user.lifecycle.activate"'
    $getactivate = $defaultRequestArgs.Clone()
    $getactivate['Uri'] = "$($logsUri)?since=$($filterTime)&until=$($now)&filter=($filterEvent)"

    $activate = Invoke-OktaApiRequest @getactivate

    foreach($user in $activate)
    {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBjeQhbCTvUAY6t
# V4fSCNx2l43CNKLNjA87GIT8gbYPyaCCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# FTAvBgkqhkiG9w0BCQQxIgQgYhIqU+LaCJDZbNEXH2PAY9FssCFl6vU8+NaLDQFg
# +1QwDQYJKoZIhvcNAQEBBQAEggIAHqpNwXMFL9xjsJCU6HiJAsLwzrRwvZpfTk9J
# 0Q8lebLprbVZG2QvO1hQIcCdC1Nf1TXc9x35NCfquGkkW0SdjCpqnCZYSYHHYL/u
# juLMI74i7SwKf6ouOz5E14ZcAAeEjxAO81tSRS0zYF2Ei5jdS6iQNvlvMMmBU47K
# WG20nmZBIN9sXoKRceqwlGjs5Rb3oPtBpnMDjstb/gjgg9BGARPO62cWsZOy3BmK
# RaUibcCLKORiaEXzR5ag8Y22NVWsrAOcI4QIAxxWglcOCisi57+/3QMYaG2TSLMU
# +gPzLqZbf5WXb9Gq9H7KG6NZ3tlwsI3jqpi0+LPc1hXzvl+zMAzgM2nLPF9CCySg
# AwhW21xmhZU/73QB/QeebRBNw7OfmVZuQTz3e+dEGjjf+JhkztKEbDXQ67F6kCpv
# 7cqWKxm8CEFqGFffWnR4U1/QBjH6Ppba1lolve+2vn/QIcxxZumnZsBDakOYFC9e
# OCXISikP2vo7Nz6pesGXQMBuyyf33Y6Ptwm3u8/xK5qPEUEmO7B/0bAkeF9Vz5AH
# k/CUb9XKJB94c9TB4KYMQ2Rj9wcareYuq601D2cEKUHke/oJgkKAmEw8etVgRyVj
# +++GYaiJvgMC0EebtnGU3LUvm/bjqJQ9bTeGuGGugL7qfL7FcXmCpL+EXIsmWq4/
# R4jBg7ShghbJMIIWxQYKKwYBBAGCNwMDATGCFrUwghaxBgkqhkiG9w0BBwKgghai
# MIIWngIBAzENMAsGCWCGSAFlAwQCATCB5QYLKoZIhvcNAQkQAQSggdUEgdIwgc8C
# AQEGCSsGAQQBoDICAzAxMA0GCWCGSAFlAwQCAQUABCAu30Fh22wr2esg28OTl586
# xpmEATYdpZGxqBe4EnUDegIUDdanXwzZ59gBcvQdm9bw/lE0xhcYDzIwMjUwMjEy
# MDUyMTQ2WjADAgEBoGCkXjBcMQswCQYDVQQGEwJCRTEZMBcGA1UECgwQR2xvYmFs
...
# CwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEiBCC+Ok/T
# AlFcVh1mq2X7+dMpw4psv3qiURH24UVSmcHNaDCBsAYLKoZIhvcNAQkQAi8xgaAw
...
# GXV0ccmS10TfpZbruXAVMA0GCSqGSIb3DQEBCwUABIIBgHt3oTJXCIi8pl/mK9+r
# yWkRx9dC1jQlft+a/ar8A9+91gMxCJoVTc9Ezg8RG3mhKb4C7MLIJdVojMdhzthK
# NkJAauZgxBfpjIUvpJar6YQwO6weqjBX5h7KJTi1wPlZ7MaQddEetFGC/sQrLVzd
# z3aiHisUYsP2IFD06NMybdWz2urIkiE5mw6+VXY2EUWUJ+tF64DPYq6rNVDXOZ2m
# JJPIUcc6yQqsuo08B2FwDviMB3CB3VZQFO4XTgZXROurpU3u5fGMbbAqVsmXTGVj
# xshTi4oqR0RvIUJhuyehdh8rqkfNQnP5IXsJ5rRInNCuzfnbkAXA8E8MlHOWkGwL
# J7mIY12FB9FQ326EuCBTIhyX89u/b7Q16ccXnc2x7gc85hV8oR7lHfITq94satZD
# PfsptQExkOlZAwwrmO85TZo+JP9oKZUy0Q0vrBc4+bMrR6EMsAV3Wfq3osZCTtgW
# yvFRBWsIuSVcEnN0KW61o56vhgD1LXX3HUWNE55mlKVZiA==
