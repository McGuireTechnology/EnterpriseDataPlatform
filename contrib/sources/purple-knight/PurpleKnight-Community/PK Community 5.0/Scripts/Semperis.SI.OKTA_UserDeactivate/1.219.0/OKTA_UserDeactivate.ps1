...
$Global:self = @{
    ID = 308
    UUID = '9db6aed0-726a-4384-9d31-3cc64c9dbb76'
    Version = '1.219.0'
    CategoryID = 8
    ShortName = 'SI000308'
    Name = 'User deactivation in the last 7 days'
    ScriptName = 'OKTA_UserDeactivate'
    Description = 'This indicator checks if a user has been deactivated within the last 7 days.'
    Weight = 1
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 1
    LikelihoodOfCompromise = 'Deactivated users no longer have the ability to authenticate and perform actions within the environment. However, an attacker may intentionally deactivate a user to disrupt the functioning of the environment or to hide their activities. It is important to monitor and verify the deactivation status of users to ensure that it aligns with the intended access controls.'
    ResultMessage = 'Found {0} new deactivated users.'
    Remediation = 'Review the deactivation status of users to confirm that they should indeed be deactivated. Reactivate any users who require access and whose deactivation may be unauthorized. Additionally, investigate any unauthorized deactivations as they may indicate malicious activity.'
    Types = @('IoE')
...
try
{
    ...
    $logsUri = '/api/v1/logs'
    $filterTime = "$($since)"
    $filterEvent = 'event_type eq "user.lifecycle.deactivate"'
    $getDeactivate = $defaultRequestArgs.Clone()
    $getDeactivate['Uri'] = "$($logsUri)?since=$($filterTime)&until=$($now)&filter=($filterEvent)"

    $deactivate = Invoke-OktaApiRequest @getDeactivate

    foreach($user in $deactivate)
    {
        $actorID = $user.actor.id
        $actorName = $user.actor.displayName
        $usersOut = ""
        foreach($du in $user.target)
        {
            $uid = $du.id
            $un = $du.displayName
            $usersOut += $uid + " ," + $un + ";"
        ...
        $thisOutput = [PSCustomObject][Ordered] @{
            Operation = "Deactivate Okta user"
            Users = $usersOut
            EventTimestamp = $user.published
            ActorID = $actorID
            ActorName = $actorName
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCkbUbVbp6JGstL
# vyl040Mk9NIwkvLgQPQSzbJdpLw95aCCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# FTAvBgkqhkiG9w0BCQQxIgQgpYp5V7voT8WCgbG4pzkVdzUbbcTs1KmAtIhLVDJh
# J2EwDQYJKoZIhvcNAQEBBQAEggIAPjzOr23cNNwdwI4tTXXx+AVhYPMgMg5m3sG9
# RwjDf9+ZjlEtGMMutjaksCGsJL3tqTirrMM4CTTSOEvyK13IXAIhDcbH/O71AcpX
# omd6z9Xf9eknlWugqnQr8caPq2UvXB3ZqiaO+4yGkxJklbDe6agflLOYImSnG0rK
# xd5ialFHR+iL/l64SAsgeO6H7bD6O1DKA4M7KiIWMCiVxfT6g0/rURlOGpb11Wll
# KuwelSe7oVfQ94XpAXBcoHFIBlVgByhX1u66G8up0Quba6Q+sq74tPtLRX4fukTN
# Lhhvu4MTLJ9nBFQb7P5H/iiVW2xs5e57DUR2lj/xLW4dD9EB174Kzwc9wfJL+TLm
