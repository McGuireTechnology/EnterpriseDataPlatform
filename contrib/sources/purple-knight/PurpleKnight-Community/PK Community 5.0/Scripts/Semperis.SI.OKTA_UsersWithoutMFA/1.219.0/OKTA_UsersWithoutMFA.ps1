...
$Global:self = @{
    ID = 309
    UUID = '6e11c380-dd30-4407-88f8-c3f1037b292c'
    Version = '1.219.0'
    CategoryID = 8
    ShortName = 'SI000309'
    Name = 'Users without Multi-Factor Authentication (MFA)'
    ScriptName = 'OKTA_UsersWithoutMFA'
    Description = 'This indicator checks all users to identify those who have not registered for Multi-Factor Authentication (MFA).'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = 'Users who have not configured MFA are at a high risk of being compromised. This poses a significant threat not only to the individual user but also to the entire environment.'
    ResultMessage = 'There are {0} Okta users without MFA configured.'
    Remediation = 'Ensure that MFA is enabled and enforced for all users.'
    Types = @('IoE')
    DataSources = @('Okta.OktaAPI')
    OutputFields = @(
        @{ Name = 'UserID'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserStatus'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserLastLogin'; Type = 'String'; IsCollection = $false },
        @{ Name = 'UserDisplayName'; Type = 'String'; IsCollection = $false },
...
try
{
    ...
    {
        $uid = $user.id
        $factorsUri = "/api/v1/users/$($uid)/factors"
        ...
        $factors = Invoke-OktaApiRequest @getUserFactors
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBwtqNsNPENWENO
# 32/gMIEyewLqGuaH0qyTMjuHY+2oAaCCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# FTAvBgkqhkiG9w0BCQQxIgQgRrHn4ANQ5rdhcxUtK78TSueR3o1O0TriDLSRSJ8x
# XucwDQYJKoZIhvcNAQEBBQAEggIAcX5A6xM8bXB3DFv5g84cjRxxsHzx5w4KpH/G
# 2LdeHGlt6znsPsv2uoILalvA6rw3/Ss76NGwyvcml3c8W2SqruLxAgXod1Q7DvIx
# UDnBBNPDeVUdkulAmYDWfwgwfj32KzpfNv4IFQkxvL46VSOB3uS6AT58T1vC/t90
# ztr7CpTvRvHu/WBPs1ZB1XH3xPDXgGSey5oUa1mLr09iBxGFhVfQbwM7c4gh1BPs
# I/VcWF4jayfVjgsstwx9AQVq7tFYn6doDXROfNgIoCFpUcWdddldkKbI1GIsgo7W
# DrVj9OgUGW9pwytxp+xdo96j+XLnd4/6Yiy6atpEUcvTwLUDKQf7yWGhY8ZlNDNP
# uELnwsoEmVNkaBMwRTSqbwVgzW0eLEk5d7jhIOH3WQwPA9jajY1l4sbd9pcdf3Em
