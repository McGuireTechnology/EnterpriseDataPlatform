...
$Global:self = @{
    ID = 185
    UUID = '1a220cc3-cadb-4960-840d-37cf7b07e940'
    Version = '1.318.0'
    CategoryID = 1
    ShortName = 'SI000185'
    Name = 'Objects with Reanimate-Tombstones extended right'
    ScriptName = 'ObjectsCanReanimateTombstones'
    Description = '<p>This indicator checks if there are any non-default principals with the Reanimate-Tombstones extended right configured.</p>'
    Weight = 4
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 4
    LikelihoodOfCompromise = '<p>When an Active Directory object is deleted, it is by default transformed into a tombstone object and loses most of its attributes. If the Active Directory Recycle Bin optional feature is enabled, the object is instead transformed into a deleted-object, which maintains the state of the object before deletion and can be undeleted without loss of information.</p><p>By default, only domain administrators can restore deleted objects. For other users, restoring these types of AD deleted objects requires having the "Reanimate-Tombstones" extended right. It is not recommended to delegate this right to other users since tombstone reanimation is a highly privileged and infrequent operation.</p><p>Furthermore, this permission could assist attackers with privilege escalation and lateral movement opportunities, as it could permit them to restore account objects with access to more resources. As tombstones don''t maintain the state of the original object, reanimating them essentially grants control over the account because the user must set a password for the restored account.</p><h3>References</h3><p><a href="https://learn.microsoft.com/en-us/windows/win32/ad/restoring-deleted-objects" target="_blank">Restoring Deleted Objects | Microsoft Learn</a></p><p><a href="https://learn.microsoft.com/en-us/windows/win32/ad/creating-and-deleting-objects-in-active-directory-domain-services" target="_blank">Creating and Deleting Objects in Active Directory Domain Services | Microsoft Learn</a></p><p><a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/e9482178-ee53-4b6c-841d-5259d6261985" target="_blank">Recycle Bin Optional Feature | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} principal(s) with the "Reanimate-Tombstones" extended right.'
    Remediation = '<p>By default, only domain administrators can restore deleted objects, and it is not recommended to delegate the "Reanimate-Tombstones" right to other users since tombstone reanimation is a highly privileged and infrequent operation.</p><p>The "Reanimate Tombstone" control access right is granted at the Naming Context root. To remove the detected privilege assignments, perform the following steps:</p>
      <ol>
        <li>Open Active Directory Users and Computers (dsa.msc) and check the <b>Advanced Features</b> option under <b>View</b>.</li>
        <li>Right-click the domain root object and select <b>Properties</b> to view its properties.</li>
        <li>Click the <b>Security</b> tab and select the detected object in the "Group or user names" pane.</li>
        <li>Scroll down in the permissions pane to locate the "Reanimate tombstones" permission entry. If the permission isn''t visible, you may need to find it using the <b>Advanced</b> button below the pane.</li>
        <li>Deselect the "Allow" checkbox for the permission, then click <b>Apply</b> to save the change.</li>
      </ol>
      <h3>References</h3><p><a href="https://learn.microsoft.com/en-us/windows/win32/ad/restoring-deleted-objects" target="_blank">Restoring Deleted Objects | Microsoft Learn</a></p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    Permissions = @()
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Lateral Movement', 'Privilege Escalation', 'Persistence') },
        @{ Name = 'MITRE D3FEND'; Tags = @('Isolate - User Account Permissions') }
    )
    Products = @(
...
try {
    ...
    # Reanimate-Tombstones right guid
    $rightGuidHT = @{"00000000-0000-0000-0000-000000000000"="All Properties";"45ec5156-db7e-47bb-b53f-dbeb2d03c40f" = "Reanimate Tombstones"}
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Exlude NT AUTHORITY\SYSTEM, Administrators, Domain Admins and Enterprise Admins
        $domainSID = Get-DomainSID $domain
        $excludedSIDs = @("S-1-5-18","S-1-5-32-544", "$domainSID-512", "$forestSID-519")

        $scriptBlockFilter =  {
            param ([System.DirectoryServices.Protocols.SearchResultEntry] $Entry)
            Update-NTSecurityDescriptor -Entry $Entry -PropagationFlagsFilter "None|NoPropagateInherit|InheritOnly" -AccessControlTypeFilter "Allow" `
                -RightsFilter "ExtendedRight|GenericAll" -ExcludedSIDs $excludedSIDs -GuidHTFilter $rightGuidHT
        }
        ...
        $searchParams = @{
            ...
            PropertyFilter = "ntSecurityDescriptor"
            StartAttackWindow = $StartAttackWindow
            EndAttackWindow = $EndAttackWindow
        }
        $searchResult = (Search-ADHelper @searchParams)

        if ($searchResult) {
            foreach ($access in $searchResult.Attributes.aces) {
                $identityReference = $access.IdentityReference.ToString()

                # Translate the identity reference to get the SID
                ...
                $searchIdentityReferenceDNParams = @{
                    dnsDomain = $domain
                    attributes = ""
                    baseDN = $DN
                    scope = "subtree"
                ...
                # Search by SID to get the DN
                $identityReferenceDN = (Search-AD @searchIdentityReferenceDNParams).DistinguishedName
                if (-not $identityReferenceDN) {
                    $identityReferenceDN = $identityReference
                }
                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $identityReferenceDN
                    DomainName = $domain
                }

                if ($searchResult.Attributes.attributemetadata){
                    $attrMetaData = $searchResult.Attributes.attributemetadata[0]
    ...
    if ($outputObjects) {
        ...
        $scoreOutput = $outputObjects | Get-Score -Impact 4
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCApCK2XuB8v1ov7
# CsAxN0nLMqwuF146MFnwzrrsmAQ/XqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgNGnCz67JCKAhV4PVjVjwx7g9aiUI+fRLMCJBGc39T24wDQYJKoZI
# hvcNAQEBBQAEggIAWE/V2f784J8VBw/89ogHm9wlasl0iFaszLtXqCHtYmvoBgC6
# jB31kiIkUZ9Kk34o6Qa2DaTyN3m4tHxOE9vWFJOoQeqDJZq9WvJpWPaxt2kvvRap
# PavFYW3vu+vZ7pM0dj6hg/VbKplfMt2qpKXKhnBR0rIQPw7+ELhmv6AfW8ofKLX9
# XBWYKXDcEU6NCrUyCvBLibzqBgBTHZQbkGSB9Su9hw9XxKliejh8XAZXVJbMyU8o
# 5q8mko+gJYUWJObbwZ0i7KASLo155F40fmjwhp7C0yndoceegacT1zk2omPIMPYB
# Ki6P6fjiROeWPY9pkb08pmU0LMWx5NiwVouOJ34lMUeu9Q3YsUTRgo9oLS6YufhH
# UZWQ4OBqgc5p40jwGt8jAyP6DpSTfMhAjMtADwoXGI0C0prpHPeLFNQPTFX1+LaX
# Feyp1VbnJRwf1U63eegq7P+4MloaN8/ZirX8ptgQrnBAONkdFErYC/A1CecjEHjv
# 62P+8Ca6VsyePq3Iw2V379GvhS5iKREO4fVbUuA6ATdROJOksB6fOJOednWiq8zB
# tCG2t2BMub7HHXOGmpFBjHPGxO35fkQZfn6X4mJ+/fXLPSmgm9zgEkOIe+zORV1c
# HTBjersD+FsExwc+9S/gH9IvOhgqk/2PLv1yzB4c/UD0M4KGvwprp7TNWnChgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBoUb/u5U8IB2BmoUAsBt/MMskYBegkopUx
# O2jEX6XRZAIUAKMtl4n4MwZwlWTIS0A7hdnTfu8YDzIwMjUwNjEwMDc1MjEwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAxfwKDmTaM4oM+K8/YdI8TNP9ySAeCU2vDThFktRDQ
# ojCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgLECdNQcgb7mSlN9ecylqNTN133XDxXafF0gu8HpfutX2KOEbLtDWp2f
# 3KRsGXe1xUOxdFx/Vqy/WxC6fUF9Kz1eGnaZUMtHjNNR08qQHwMSrFc+FvpH7pDG
# s530O6COfKKaSR7MX5sHwqUZ9xZQB3ewrYz3U2MYux1NNQsw+GWU1h0bxNN+dObj
# oBAvqAmzg1K3HV48k2Pn278oGMojzq9cU8juNlW+nruWJdIBCmV8hD9Vf9L5aJ5c
# d4GIO6eanRTpc36uW8f6Us5KlubC7kyXLBYrherO/Gu60IUbC8UsVp9/xUbK37Ma
# sWLrheonrRbJcfmDEO1Uv3dmbHywC1beVUAjo+S8gyQYPNbub0R8e/CLpuczNQ57
# mRDKZVd6A2mMLoE5C0BrFGE1HJcj1dXqMCwdlGMSxEAL/ykK1mCa9UwGmEroy+qa
# 9m9Fz4K9IhzLXv/FRXbgatrTh9O2xY5E/1T4163c+iiD9QSbYKRG+BXVesKcGiTh
# i8Ak6S0kEQ==
