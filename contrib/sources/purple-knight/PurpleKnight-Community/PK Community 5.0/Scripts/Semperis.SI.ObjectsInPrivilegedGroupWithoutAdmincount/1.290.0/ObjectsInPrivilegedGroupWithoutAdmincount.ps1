# This script looks for memebers of privileged groups that dont have admincount=1
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 4
    LikelihoodOfCompromise = 'While not immediately indicative of an attack, privileged users that are not clearly marked as such (adminCount =1) represent an exposure in that they may be used nefariously without being detected. Additionally, an attacker may add a privileged account and attempt to hide it using this method.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/previous-versions/technet-magazine/ee361593(v=msdn.10)" target="_blank">AdminSDHolder, protected groups and SDPROP | Microsoft Learn</a></p>'
...
function Search-PrivilegedGroup {
    ...
    $targetGroupScope = $targetGroup.split(";")[1]
    ...
    try {
        ...
        foreach ($adminDN in $groupMembers.Attributes.$memberAttribute.GetValues("string")) {
            if ($script:privMembers.Contains($adminDN) -or $adminDN -match "CN=ForeignSecurityPrincipals") {
                continue
            }

            $script:privMembers += ($adminDN)
            $privMemberDomain = $adminDN.Substring($adminDN.IndexOf("DC=")).replace("DC=","").replace(",",".")

            if ($targetGroupScope -eq "DomainLocal") {
                $targetGroupDomain = $targetGroupDN.Substring($targetGroupDN.IndexOf("DC=")).replace("DC=","").replace(",",".")
                if (!($privMemberDomain -eq $targetGroupDomain)) {
                    continue
                }
            }
            ...
            try {
                $searchParams = @{
                    dnsDomain = $privMemberDomain
                    attributes = @("admincount","useraccountcontrol","objectSID")
                    baseDN = $adminDN
                    scope = "Base"
                    filter = "distinguishedName=$adminDN"
                }

                $testAdminCount = Search-AD @searchParams

                if ($testAdminCount.Attributes."admincount") {
                    if ($testAdminCount.Attributes."admincount"[0] -eq "1") {
                        continue
                    }
                }

                if ($testAdminCount.Attributes."useraccountcontrol") {
                    $uacValue = $testAdminCount.Attributes."useraccountcontrol"[0]
                    $uac = "$uacValue [$( (Get-UACSet $uacValue) -join ", ")]"
                }
...
try {
    ...
    if (Confirm-DomainAvailability $ForestName) {
        ...
        foreach ($groupRID in $forestPrivilegedGroups) {
            ...
            if ($forestGroupObject) {
                $group = $($forestGroupObject.DistinguishedName)
                Search-PrivilegedGroup -targetDomain $ForestName -targetGroup $group
            }
        }
    }
    else {
    ...
    $domainPrivilegedRIDs = @("512")

    $privilegedRIDs = @("512","518","519")
    $primaryGroupFilter = "(|"
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            dnsDomain = $domain
            attributes = @("adminCount","useraccountcontrol","primarygroupid")
            baseDN = $DN
            scope = "subtree"
        ...
        foreach ($primaryResult in $primaryResults) {
            $primaryFailed = $false
            $primaryRID = $primaryResult.Attributes.primarygroupid[0]
            $primaryDN = $primaryResult.DistinguishedName

            if ($primaryResult.Attributes."admincount") {
                if (!($primaryResult.Attributes."admincount"[0] -eq "1")) {
                    $primaryFailed = $true
                }
            }
            else {
            ...
            if ($primaryFailed -eq $true) {
                if ($script:privMembers.Contains($primaryDN)) {
                    continue
                }

                $script:privMembers += ($primaryDN)
                ...
                $searchParams = @{
                    ...
                    baseDN = $DN
                    scope = "subtree"
                    filter = "(objectSID=$domainSID-$primaryRID)"
                }

                $primaryGroup = Search-AD @searchParams

                $primaryGroupDN = $primaryGroup.distinguishedName

                if ($primaryResult.Attributes."useraccountcontrol") {
                    $uacValue = $primaryResult.Attributes."useraccountcontrol"[0]
                    $uac = "$uacValue [$( (Get-UACSet $uacValue) -join ", ")]"
                }

                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $primaryDN
                    GroupDistinguishedName = $primaryGroupDN
                    UserAccountControl = $uac
                }
                [void]$outputObjects.Add($thisOutput)
            }
        ...
        foreach ($sid in $builtinPrivilegedGroups) {
            ...
            if ($groupObject) {
                [void]$domainGroups.add("$($groupObject.DistinguishedName);DomainLocal")
            }
        }

        foreach ($group in $domainGroups) {
            Search-PrivilegedGroup -targetDomain $domain -targetGroup $group
        }
    }

    # Calculate score
    if ($script:privMembers.Count) {
        $res.Score = 100 - (($outputObjects.Count / $script:privMembers.Count) * 100)
    }
    else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDjHTlYIHNz0m00
# rpFnrt0JbAW3wGzf68yNLQ+n4+QbJKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg15HPXF40kjnea9PdllGIe/HsCWjcHYgPPSgSF2j0yd0wDQYJKoZI
# hvcNAQEBBQAEggIATtrh4jiX44lMy5n3MlmcWR7spL1IaMiltuvyLcW55TbIPDiG
# ir3FdtnzmBbBlqm+EwhaER4ccmBzz7RX7JzdZqHrZi6DMQvAmb+/GlVNRSfawFi/
# 6LKokAJWfRMI70f+E2iXKPvPHQ2+4I36k3su9mJNQPUemO6NyJjJtgRaCahVMauU
# l0KmD6h8Ra6o3scJNHSUuhFMB7vEVSId0DX4NnKMDAaNp9kYkHnoxM0gscrKU7Ig
# 0a5YeGigh6YjB166kVEChXLNuWaBMDlZ1U7TtMljje32MlWWNe7ZLIDOTj1SSu4+
# mors+HrmQAo7fRe3576OZZChc+rVrSxnLgj3gCHI8+6p5nDQ7VuW47xfIJqe9ry5
# SGpGVm84yj7UgGN7Gpu5pCFTEZzhUoifXxfh3Ab5Ijq8KWve8Jn4bJB7Q8meInHV
# s9aKd1UCpYXIGFOAffo5HY0ZrSQWvRrfEuUSNtc5WLPnztAtrf3r665tTyxYUF9a
# w9ny5liBc7LvgdVrNGPHbvQ1AYayrExPTGF/wGvUYVePn24RPNCrNCxcyTpXIt6i
# fkjt5tYoimKV3t332PQzDzfD3R07SoIuUx2poRSs/0aPcLVwmPRjK7l3LLgefv2S
# PASG8ELs4E6GQ9Pzo2q2J3hbQVtyHdfb5bORxUCVE8htmypd03IOs8qMFrChgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBMo/klZn4vdJRmZhhMkB8MD6g9sgPFWo3w
# JzCqJr7tpQIUMDjJOYTWwsFaAufS37m9vCyF6V8YDzIwMjUwNTEzMTg1MTE0WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDNTiYJasSWDCtBPbbXe5JAK7gxIUyiYxoqlg1cTz+M
...
# CwUABIIBgE+NFvV57DhnoxtcJmxiOYIhFHtHUMuy81Ote9iXtzX0tEvFRxgk6Y8H
# 6nlcWP5HmPypH8NqZ/uaGjIjpMLiHeQYEcU8G2AyVtI3KUv4OzctAQfNZBUrnGfL
# +DBkYXZ1sMM3o66hYOib7nnwhdvC5mtrDeau6Oj3kg4ZedCTVltmAqqDapmCQ/8l
# zQlYzc34yXrPhyYEf6MLznZ4tgX5waBq6bHHWLSMbBDMnu/4t6P6xT1QBehWkAI0
# MU98b09RIo/muGiB8ke+4dkRsmMoODgwhZBDRneNwB4CQ8XXgyHA8iY5oM94YRb4
# pQHCxeNPvey0e3IN4fJRoFjgbSAhBj7SQcth3GIz3LBoBT4gxVGOAxtBEu4w8fVh
# iXjdZcDIkD/paHwEOA54glPdh7k1gaOWjoLWwvmqhkpubChoI79J/2K3/+iwT4tO
# QObx71twGrYfHX+kFae8nQbqh58kmY5fM8TM6XsUUacO+ppPXT8C8AuFZjqZBpb8
# sD+MhaWwHg==
