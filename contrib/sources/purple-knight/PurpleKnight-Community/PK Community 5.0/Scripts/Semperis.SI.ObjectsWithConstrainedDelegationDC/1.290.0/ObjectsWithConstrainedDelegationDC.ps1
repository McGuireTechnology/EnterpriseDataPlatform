...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = 'Constrained delegation allows a service to act on behalf of an authenticated user to another service. While this is sometimes necessary and requires the user to authenticate to the delegating service first, delegation to such services on domain controllers greatly increases risk. An attacker that is able to compromise such a service can significantly elevate their privileges in this way and infiltrate Active Directory.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/auditing/audit-user-account-management" target="_blank">audit-user-account-management of Microsoft</a></p>'
    ResultMessage = 'Found {0} objects with constrained delegation configured to Domain Controller.'
    Remediation = 'Remove any delegations, constrained or otherwise, that are defined against remote DC services.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            baseDN = $DN
            scope = "subtree"
            filter = "(&(msds-allowedToDelegateTo=*)(!(userAccountControl:1.2.840.113556.1.4.803:=16777216))(serviceprincipalname=*))"
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects with constrained delegation configured to Domain Controller."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCADRinpHq2P04Vm
# hqOozURu5cxOuikOXfC8sqolERq6pKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgeCaiY1Kp1XiIeGXbUuNxpasQPpSMGF3/mF4IBV4q2I0wDQYJKoZI
# hvcNAQEBBQAEggIAI2GxExDnuS9aFqInB8FOXAAFZ/kSmtgeg5/WVmMDDvXtsjX7
# qHK3++vwBxYqkN5zypAtcrFXMHnYgrMugUtnqgiUeScxI09R5qpHj2g0cDEFxxOC
# kd/jVn5FJmRdVxpA5Fh3AidfLUMkGxh+6bt4RZ2d7fHP8GsNHV9QhtwITa+kzqto
# 4JIumtLArcbrzhSSB1Oen04RuuSeOz2UuBcqD9eAPn3wM/YQhXc1y3kqUn/tJFoX
# 9iuxPQujKKRqm8Elz8iDiPIGgZKJ7Rw9ZHgkTpUf5sTDLTPKX9/t4KNpPWgpxbzp
# IuCU9P4vU74PFTgo20xaKvH8YHz+FCJr0PY90ozswTfkdMb42p5St0K4coi37Kcw
# Jj2Ll0Eu+7DLKjHEcJP1pKuxsPwathUWA6aC63pwyKAhODIh4YB60QnA2MNTZdAR
# Tsn/9MsUpm/qP6BY2G5mIxEaBf4JYS09oVoWGcxOeNqb1y/5qZ6tdiZmYa45ZvR9
# gejCmyNQ+4w26Ugpj8t8YAu2Nl93650YdEjeDa3wBrv2m3BBXUShPf49hYUM6slq
# 3nXjwWYBh47Wj4aFotJtXrSNNyGyFoe3wF3QQY/y6aERSuMp2Zp4KN4KcyQT/dJ7
# NsKGxuq6t2Q/f3gnLbQtqjEOAHVdk0+9EOS2Rxqjy4LXXv4ciVfkDi63Irehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAir5h3U5I0k144N4gJIWJG6uM0PtltwiQ+
# B0nf/VUTxgIUIa/5PK21GqR/IlMdx4CKkxQdgDoYDzIwMjUwNTEzMTg1MTIzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCAaUUz/zzmWn3+2IwxlLC///Q76xkAoTzxmZcQdVcx
...
# CwUABIIBgHbBEKpbvcnQKGnnmoaBwM1w2o6L9xlYqNT9RxrK/XcxgGXfI1DjtcOK
# MK+s30tN7P1QGLLTlnjbgccz8pMcITzII1qzomu9rBeifrGleUWSpCoYL8V9ODcG
# zJeFVSDaVvAcIOg42o5ktwN81f4yP0YzFT12rVQDFAkd+tyzzRRHiuqXHnYrVbGy
# J+U3LmQxUmium5G+9DMooTuOHIzweiJS+t5/Tv5zECsq8+Hxgjsdshiu5g337Q2Q
# IqcSbW5u5OGj79+1vbwSwcDSsZMd9ff4GPKLYU6p/UKWGlZmEJALxc9twEdpd1tl
# kERUcdUpPArNZf1RZCVw3OUeQRmDUZEm8GlAObxYIQfZQ/WKU1p8sAZ9KEjOtJLn
# ryUf6KDBl6xzpqL5yRIAf7Cz5c6FW72m21yDQ9qHh0F5gkDGdG8VeE/UXHRzRb0N
# BSRmlIKyqFZgaXYqJ1COsoD9DaYJpHfCB7ODFtPH0y65Ltb8kbsL3qABVRv9AmfH
# 5IB2+Csf/w==
