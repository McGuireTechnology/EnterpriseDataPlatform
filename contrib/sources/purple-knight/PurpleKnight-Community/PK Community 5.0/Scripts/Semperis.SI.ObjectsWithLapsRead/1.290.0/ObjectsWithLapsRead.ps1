# This script returns every computer with laps and the principals who can read his password
...
$Global:self = @{
    ...
    Description = 'This indicator looks for permissions on computer accounts that could allow inadvertent exposure of local administrator accounts in environments that use the Microsoft LAPS solution. These permissions include Read access to ms-Mcs-AdmPwd as well as Write DACL and Owner (which would allow provisioning the read access). LAPS provides a method to rotate local administrator account passwords on servers and workstations.'
    Weight = 3
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>Only authorized administrative users should have access to LAPS passwords. Attackers may use this capability to laterally move through a domain using local compromised administrator accounts.</p>
      <h3>References</h3>
      <p><a href="https://www.microsoft.com/en-us/download/details.aspx?id=46899" target="_blank">Local Administrator Password Solution (LAPS)</a></p>'
...
try {
    ...
    $laps, $unavailableDomains = Search-ADConfig -forestName $ForestName -domainNames $DomainNames -searchADParameters $searchParams

    if (-not $laps) {
        $res.Status = 'NotRelevant'
        $res.ResultMessage = 'This indicator is not applicable as LAPS was not found in the forest.'
        return $res
    }
    ...
    if ($DomainNames.Count -gt 0) {
        $lapsGuid = (New-Object guid(,$laps.Attributes."schemaidguid"[0])).Guid
        $guidHT = @{"00000000-0000-0000-0000-000000000000"="All Properties";$lapsGuid="ms-mcs-admpwd"}
        ...
        foreach ($domain in $DomainNames) {
            ...
            $results = Search-AD -dnsDomain $domain -attributes "ntsecuritydescriptor" -baseDN $DN -scope "Subtree" `
                -filter "(&(objectclass=computer)(ms-mcs-AdmPwdExpirationTime=*))" -scriptBlockFilter $scriptBlockFilter
            foreach ($result in $results) {
                ...
                foreach ($access in $result.Attributes.'aces') {
                    if ($acl.ContainsKey($result.DistinguishedName)) {
                        $acl[$result.DistinguishedName] += ("; " + $access.IdentityReference.Value + " " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid])
                    }
                    else {
                        $acl[$result.DistinguishedName] = ($access.IdentityReference.Value + " " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid])
                    }
            ...
            if ($acl.Count -gt 0) {
                foreach($result in $acl.GetEnumerator()) {
                    $thisOutput = new-object psobject -Property @{
                        DistinguishedName = $result.Name
                        Access = $result.Value
                    }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) computers on which some normal users can read their LAPS password."
            $res.Remediation = "Ensure that there are no unnecessary principals who can read computer administrator account passwords via Extended Rights on the ms-Mcs-AdmPwd attribute."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD1OV7nT+jPphoc
# w+I0auh3dv7wShTP88F3vcE7WK4cEKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ