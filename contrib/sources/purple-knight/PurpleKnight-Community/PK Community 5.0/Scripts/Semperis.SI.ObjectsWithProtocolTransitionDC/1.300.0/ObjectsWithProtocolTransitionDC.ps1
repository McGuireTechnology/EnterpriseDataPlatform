# This script looks for objects that has Domain Controller SPN in msds-allowedToDelegateTo and has the userAccountControl bit of protocol transition not set
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>Protocol transition (also known as T2A4D) allows any user to authenticate to a delegated service using any protocol such as NTLM. This allows the delegated service to request a TGS from Kerberos for any user without any proof such as that user''s corresponding TGT or TGS. If an attacker can create such a delegation for a service that they control or compromise an existing service, they can effectively gain a TGS for any user with privileges to the DC.</p>
        <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_delegation_t2a4d" target="_blank">Constrained delegation with protocol transition to a privileged service | ANSSI</a></p>'
...
try {
    ...
    # Get a list of all DC names
    $dcHostnames = [System.Collections.ArrayList]@()
    $dcSPNsWithoutDCName = [System.Collections.ArrayList]@()
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $dcsFQDN = Search-AD -dnsDomain $domain -attributes "dnshostname", "serviceprincipalname" -baseDN $DN -scope "subtree" `
            -filter "(|(primaryGroupID=516)(primaryGroupID=521))"
        foreach ($dcFQDN in $dcsFQDN) {
            if (!($dcFQDN.Attributes."dnshostname")) {
                continue
            }
            $dcHostname = $dcFQDN.Attributes."dnshostname"[0].Split('.')[0]
            [void]$dcHostnames.Add($dcHostname)
            if ($dcFQDN.Attributes."serviceprincipalname"[0]) {
                foreach ($spn in $dcFQDN.Attributes."serviceprincipalname".GetValues("string")) {
                    if ($spn -match "\/$dcHostname\.{0,1}") {
                        continue
                    }
                    [void]$dcSPNsWithoutDCName.Add($spn)
                }
            }
        }
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all objects with constrained delegation configured and protocol transition not configured
        $searchParams = @{
            dnsDomain = $domain
            attributes = "msds-allowedToDelegateTo"
            baseDN = $DN
            scope = "subtree"
            filter = "(&(msds-allowedToDelegateTo=*)(userAccountControl>=16777216)(userAccountControl:1.2.840.113556.1.4.803:=16777216))"
            PropertyFilter = "msds-allowedToDelegateTo"
        ...
        if ($results) {
            foreach ($result in $results) {
                $spns = $result.Attributes."msds-allowedtodelegateto".GetValues("string")
                $delegateToDC = $false
                foreach ($spn in $spns) {
                    foreach ($dcHostname in $dcHostnames) {
                        if ($spn -match "\/$dcHostname\.{0,1}" -or $dcSPNsWithoutDCName.contains($spn)) {
                            $delegateToDC = $true
                            break
                        }
                    }
                    if ($delegateToDC) {
                        ...
                        $thisOutput = new-object psobject -Property @{
                            ...
                            AllowedToDelegateTo = $result.Attributes."msds-allowedtodelegateto".GetValues("string") -join "; "
                        }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects with protocol transition constrained delegation configured to Domain Controller."
            $res.Remediation = "Remove any delegations that are defined against remote DC services."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD0OX/5IVUAWDH2
# fMx+SjOYv8ZtWd/S4ijLr+qoU016dqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ