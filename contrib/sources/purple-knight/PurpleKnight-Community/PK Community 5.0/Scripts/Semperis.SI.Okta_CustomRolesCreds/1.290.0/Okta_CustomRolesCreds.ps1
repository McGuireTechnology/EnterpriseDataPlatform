[CmdletBinding()]
param(
    [Parameter(ParameterSetName='Execution')][string] $OktaDomain,
    [Parameter(ParameterSetName='Execution')][string] $OktaApiToken,
...
$Global:self = @{
    ID = 311
    UUID = '9f09273e-cbef-43c0-877f-020ad9503089'
    Version = '1.290.0'
    CategoryID = 8
    ShortName = 'SI000311'
    Name = 'High-privileged custom roles'
    ScriptName = 'Okta_CustomRolesCreds'
    Description = 'Custom roles were found that allow a user to perform actions on users'' passwords. These custom roles grant elevated privileges and potentially pose a significant security risk if not properly managed.'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = 'Users with this role can perform operations on other users'' passwords and MFA. Therefore, these are very strong permissions that can lead to compromising a user and taking complete control of the environment. An attacker who gains access to a user account with these high privileges can potentially manipulate or exploit sensitive user information.'
    ResultMessage = 'Found {0} custom roles that allow a user to perform actions on users'' passwords.'
    Remediation = 'To mitigate the risks associated with high privilege custom roles, it is recommended to take the following actions:
      <ul>
        <li>Review and assess the necessity of the custom role. Ensure that the role is essential for specific business requirements and that it is not overly permissive.</li>
        <li>Limit the assignment of the custom role to only those users who genuinely require these high privileges to perform their duties.</li>
        <li>Regularly review and audit the membership of the custom role to ensure that it remains appropriate and necessary.</li>
        <li>Implement the principle of least privilege by granting users only the minimum privileges necessary to perform their assigned tasks.</li>
        <li>Monitor and log activities performed by users with high privilege roles to detect any suspicious or unauthorized actions.</li>
        </ul>
        By following these remediation steps, you can help reduce the potential risks associated with high privilege custom roles and enhance the security of your Okta environment.'
    Types = @('IoE')
    DataSources = @('Okta.OktaAPI')
    OutputFields = @(
        @{ Name = 'RoleId'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RoleName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'CredentialsPermissions'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('Okta')
    Permissions = @()
    SecurityFrameworks = @(
    ...
    Products = @(
        @{ Name = 'PK'; MinVersion = '4.0'; MaxVersion = '10'; Licenses = @('Community', 'Post-Breach', 'BPIR') }
    )
...
Import-Module 'Semperis-Lib'

try
{
    $result = New-Result
    $outputObjects = [Collections.ArrayList]::new()
    $uri = '/api/v1/iam/roles'

    $getCustomRoles = @{
        DomainUrl = $OktaDomain
        Uri = $uri
        TokenValue = $OktaApiToken
    }
    $customRoles = Invoke-OktaApiRequest @getCustomRoles

    foreach($role in $customRoles.roles)
    {
        $roleId = $role.id
        $roleLabel = $role.label
        $permissionsURI = "$($uri)/$($roleId)/permissions"
        $getRolePermissions = @{
            DomainUrl = $OktaDomain
            Uri = $permissionsURI
            TokenValue = $OktaApiToken
        ...
        $rolePermissionsList = [Collections.ArrayList]::new()
        $rolePermissions = Invoke-OktaApiRequest @getRolePermissions
        foreach($permission in $rolePermissions.permissions)
        {
            if($credsHighPrivs.contains($permission.label))
            {
                [void]$rolePermissionsList.add($permission.label)
            }
        }
        if($rolePermissionsList)
        {
            $permissionString = $rolePermissionsList -join " ;"
            $thisOutput = [PSCustomObject][Ordered] @{
                RoleId = $roleId
                RoleName = $roleLabel
                CredentialsPermissions = $permissionString
            }
    ...
    if ($outputObjects){
        ...
        {
            $result.ResultObjects = $outputObjects
            $result.ResultMessage = $self.ResultMessage -f $outputObjects.count
            $result.Remediation = $self.Remediation
            $result.Status = 'Failed'
            $result.Score = $scoreOutput.Score
        }
        if ($scoreOutput.Ignoredcount -gt 0)
        {
            $result.ResultMessage += " ($($scoreOutput.Ignoredcount) Objects ignored)."
        }
    }
}
catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC