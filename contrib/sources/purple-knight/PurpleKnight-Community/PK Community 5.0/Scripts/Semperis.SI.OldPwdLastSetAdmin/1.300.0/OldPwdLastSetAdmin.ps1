# This script looks for admins that haven't changed their password in a while
...
$Global:self = @{
    ID = 62
    UUID = 'dc8f6889-7df9-4414-9bf5-865f1e3e9e83'
    Version = '1.300.0'
    CategoryID = 2
    ShortName = 'SI000062'
    Name = 'Admins with old passwords'
    ScriptName = 'OldPwdLastSetAdmin'
    Description = 'This indicator looks for admin accounts whose password has not changed in over 180 days. This could make these accounts ripe for password guessing attacks.'
    Weight = 2
    Severity = 'Informational'
    Schedule = '1w'
    Impact = 2
    LikelihoodOfCompromise = '<p>An administrator account whose password hasn''t changed in a while could be a target for attackers looking for privileged accounts that can provide elevated access to the environment.</p>
      <h3>References</h3>
        <p><a href="https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf" target="_blank">NIST.SP.800-63-3</a></p>
        <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_password_change_priv" target="_blank">Privileged account passwords age too old | ANSSI</a></p>'
    ...
    OutputFields = @(
        ...
        @{ Name = 'ReplicationMetadata'; Type = 'Integer'; IsCollection = $false }
    )
    Targets = @('AD')
    Permissions = @()
    SecurityFrameworks = @(
...
try {
    ...
    # Property filter for Replication attribute meta data
    ...
    foreach ($domain in $DomainNames) {
        ...
        $results = @()
        $DN = Get-DN $domain
        $domainSID = Get-DomainSid -dnsDomain $domain

        # Get a list of all enabled users that didn't changed their password in a while
        # The filter is on samaccounttype and not object category and class to filter out trust objects
        $attributes = @("samaccountname", "msds-replattributemetadata", "pwdlastset")
        $filter = "(&(admincount=1)(samaccounttype=805306368)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(pwdLastSet=0))(!(objectSid=$domainSID-500)))"

        $scriptblockFilter = {
            param ([System.DirectoryServices.Protocols.SearchResultEntry] $Entry)
            Update-AttributeMetadata -Entry $entry -PropertyFilter $propertyFilter -EndOriginatingChangeThreshold $lastOriginatingChangeThreshold
        }
        ...
        if ($results.Count) {
            foreach ($result in $results) {
                if ($result.Attributes.'attributemetadata') {
                    ...
                    $pwdLastSet = [datetime]::FromFileTime($result.Attributes."pwdlastset"[0]).ToUniversalTime()
                    $lastChange = [datetime]::Parse($attrMetaData.LastOriginatingChange).ToUniversalTime()
                    $daysSinceLastSet = ((get-date).ToUniversalTime() - $pwdLastSet).Days
                    $daysSinceLastSetMeta = ((get-date).ToUniversalTime() - $lastChange).Days
                    $thisOutput = [PSCustomObject][Ordered] @{
                        ...
                        PasswordLastSet = $lastChange
                        DaysSinceLastSet = $daysSinceLastSet
                        ReplicationMetadata = $daysSinceLastSetMeta
                    }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "The following $($outputObjects.Count) users were returned. Note the following: users with DaysSinceLastSet and ReplicationMetadata higher than $daysToRemove days have not changed passwords in over $daysToRemove days. Users with PwdLastSet over $daysToRemove days and ReplicationMetadata is N/A - permission was denied to read these users' metadata. These users may be using smartcard for interactive logon instead of passwords - in which case it is ok that their passwords have not changed."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDFkQY+2MPF7LiX
# Y5Ehdff+/qKCPkjv2xGmaZIsKxInTKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ