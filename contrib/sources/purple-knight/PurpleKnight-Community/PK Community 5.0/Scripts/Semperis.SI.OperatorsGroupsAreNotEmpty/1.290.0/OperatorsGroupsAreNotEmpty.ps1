...
$Global:self = @{
    ...
    LikelihoodOfCompromise = '<p>The operator groups in Active Directory, Account Operators, Server Operators, Backup Operators, and Print Operators, all provide users within these groups certain privileges over different critical domain resources in Active Directory and different levels of access to domain controllers.</p>
      <p>An attacker may target users in these groups for further privilege escalation and lateral movement in Active Directory.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/understand-security-groups" target="_blank">Active Directory security groups | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} Members in Operator groups'
    Remediation = '<p>Organizations should evaluate the use of these groups, and work to reduce and remove membership of these groups.</p>
      <p>Organizations that are using these groups should implement an appropriate rights delegation to the necessary OUs in Active Directory as an alternative. For Backup Operators, organizations should implement a different group and manage access to lower tier servers or workstations through group policy objects.</p>
      <p>If the organization accepts the risk of these groups being populated, they should use attack path analysis software, such as Forest Druid, to determine the attack paths that exist due to the population of these groups, and further consider members of this group to be Tier 0.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    # S-1-5-32-548 - Account Operators , S-1-5-32-551 - Backup Operators, S-1-5-32-550 - Print Operators, S-1-5-32-549 - Server Operators
    $operatorsGroupsSIDs = @("S-1-5-32-548","S-1-5-32-551","S-1-5-32-550","S-1-5-32-549")

    # Build Filter for Search-AD
    $filterSIDs = ""
    foreach ($groupSID in $operatorsGroupsSIDs) {
        $filterSIDs += "(objectSid=$groupSID)"
    }
    $filter = "(&(|$filterSIDs)(objectCategory=group))"
    ...
    foreach ($domain in $DomainNames) {
        ...
        $operatorsGroupsDNs = Search-AD -dnsDomain $domain -attributes "" -baseDN $DN -scope "subtree" -filter $filter

        foreach ($groupDN in $operatorsGroupsDNs.DistinguishedName) {
            $groupMembers = [System.Collections.ArrayList]@()
            $groupMembers = Search-AD -dnsDomain $domain -attributes "msds-membertransitive" -baseDN $groupDN -scope "base" -filter "distinguishedname=$groupDN"
            ...
            $domainFailed = $true
            foreach ($member in $groupMembers.Attributes.'msds-membertransitive'.GetValues("string")) {
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Members in Operator groups"
            $res.Remediation = "It is recommended to have these groups empty. Assign administrators into administrators group. Other accounts should have proper delegation rights in an OU or in the scope they are managing."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA90BJwv/vZvEJo
# SCUtFKTkJjq4CnXOkqbYPbJrxMB2hKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ