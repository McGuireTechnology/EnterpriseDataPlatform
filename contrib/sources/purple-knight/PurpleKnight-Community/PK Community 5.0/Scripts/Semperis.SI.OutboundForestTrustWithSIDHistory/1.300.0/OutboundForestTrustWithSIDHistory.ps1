# This script looks for outbound forest trusts that has EnableSIDHistory = yes
...
$Global:self = @{
    ID = 88
    UUID = '13b327cf-1283-4362-8815-6c1acb24f4de'
    Version = '1.300.0'
    CategoryID = 3
    ShortName = 'SI000088'
    Name = 'Outbound forest trust with SID History enabled'
    ScriptName = 'OutboundForestTrustWithSIDHistory'
    Description = '<p>This indicator checks for outbound forest trusts that have the TRUST_ATTRIBUTE_TREAT_AS_EXTERNAL flag set to true.</p>'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>If TRUST_ATTRIBUTE_TREAT_AS_EXTERNAL is set to true, the security of a cross-forest trust is relaxed and trusts are treated as equivalent to external trusts.</p>
      <p>An attacker that compromises the remote Active Directory forest, with this set to true, can spoof most users and computers in the local Active Directory forest. This would allow the attacker to act as local users or computers and access resources they have privileges in. An attacker can also use this for privilege elevation and compromise the local forest.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/active-directory-security/active-directory-security-best-practices-checklist/" target="_blank">Active Directory Security Best Practices | Semperis</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_trusts_forest_sidhistory" target="_blank">Outbound forest trust relationships with sID History enabled | ANSSI</a></p>'
    ResultMessage = 'Found {0} outbound trusts where SIDHistory is enabled.'
    Remediation = '<p>Unless a forest migration is currently in process, SID filtering should be enabled on the trust.</p>
      <p>Organizations should understand the risk of leaving this enabled for an extended period of time.</p>
      <p>The following command can be run to enable: <code>NETDOM TRUST <ForestName> /domain:<TrustedForest> /EnableSIDHistory no</code></p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_trusts_forest_sidhistory') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # TrustAttributes 0x20 (32) flag means in-forest trusts, 0x40 (64) flag means TRUST_ATTRIBUTE_TREAT_AS_EXTERNAL, TrustDirection 2 means outbound
        $results = Search-AD -dnsDomain $domain -attributes "trustattributes", "trustpartner" -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=TrustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=64)(trustDirection:1.2.840.113556.1.4.803:=2)(!(trustAttributes:1.2.840.113556.1.4.803:=32)))"
        ...
        foreach ($result in $results) {
            ...
            $thisOutput = [PSCustomObject][Ordered] @{
                ...
                TrustAttributes = "$trustAttributes [$( (Get-TrustAttributesSet $trustAttributes) -join ", ")]"
            }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) outbound trusts where SIDHistory is enabled."
            $res.Remediation = "Unless forest migration is currently ongoing, use the following command to disable SIDHistory for the trust: NETDOM TRUST <ForestName> /domain:<TrustedForest> /EnableSIDHistory no"
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAli8/TsOkIe+7u
# k79oatAmZL3M8El2vOIfyGQe63d+/KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ