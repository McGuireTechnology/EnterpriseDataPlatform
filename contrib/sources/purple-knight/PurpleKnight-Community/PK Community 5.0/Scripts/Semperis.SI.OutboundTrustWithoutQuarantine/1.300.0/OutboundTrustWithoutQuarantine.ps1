...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>An attacker having compromised the remote domain can spoof any user or machine on the local domain (except for accounts with a RID lower than 1000, excluding built-in accounts and groups). This attacker can therefore access every resource on the local domain. If a dangerous control path is exposed to any "spoofable" account (virtually any account other than the built-in ones), the attacker could also escalate his privileges up to "Domain Admins" and compromise the entire forest.</p>
      <h3>References</h3>
    ...
    Remediation = '<p>Unless domain migration is currently ongoing, use the following command to enable Quarantine for external trusts: <code>NETDOM TRUST <ForestName> /domain:<TrustedForest> /Quarantine yes</code></p>
      <p>And use the following command to disable SID history for forest trusts: <code>NETDOM TRUST <ForestName> /domain:<TrustedForest> /EnableSIDHistory:No</code></p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD8yfMxk3mWB88M
# QHxQ1fNBKOv03ZWyxiIdh470EHgbeqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgQtJz3laNFty9isbq9MBWfc+HHNq2e/Z486HSoFaMJ5YwDQYJKoZI
# hvcNAQEBBQAEggIAPstbxmdSHvNlOkECXipvmx0ZfDMtxhvCDEtLoDk5mdB4oCJj
# MjCBzKAjPt5ixbTmPMiv0yEXQIrx73SCXOyH6JJ+tziuIe9bzIMmNflohaPsuMLW
# jNm/j30X3nSo7eYf54B58AOpp4pgHC8pdbXrZ6fALPjx8IGQT0eQ4+dAv6zgLans
# J7i4YqTQ+006Q15yQ9tnJ08OnvnpXxXhfMYvR0f62ACjO/tLpl4+TINPcdUEkKmf
# ZCchgbC/avSDaYSlrv+LAYtBmd7AMrSl3iuwkPANLn4cREaYWtCLED3ImPRiRYIq
# m11OqSluHIvemLJiOh3+VJ8bmtiaBJfn1GYL2GzO8OGrqhAOOGcXg704KBSaO7ZM
# +AYL9Ywx45xgoFp6Ir9y1waEidbRUTyhzm+7jS5hWp9Gei6H7FB7LF405N70kZOW
# vfYjWJYMNE6W2K1arRNKJkSxbihnXhkNR+DqRsFNhRcwdrMlkuYN13WdBHR8f1KB
# BiKEx7Fuv4xT2/Ctl6qIluEzfPmn73TwBhnzlLMMOwHdY19Dh6SuseLCgDDHgZFQ
# Fup+2Rffg1Hdew3B6kHL5It6HfquRCJW5yMCWZX19iXfgJoFWcSsp6rhPbjHx5uj
# 9Vlp5Jvy9j1UJeeOqFL8JELq/I0E8IvhYf/KZygcdr2mbgyiWSvHeQnwgWKhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDGZeJTssGuH/dGUvATpBRsZM5jewsfShP2
# ne4V0Hy1wgIUa+hUD/GnyO3xLQl6x4HdsLlHNVsYDzIwMjUwNTI4MTI0MTE5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBkCFva0mfxgjwZaH+B3PLyLAT1Lv3GtgTKM+xUBfeL
...
# CwUABIIBgFs9d1cUTqL2snwHTvVcIS8q1H1LMD1inlX6decWzngyH8LN4TK70Jh4
# /zgP3bjMoDQV1FXzSvQ+Q4asOpgrr0Ut04uCj439biRynyQEMYOrLBSiivJYx5x4
# 3byu7ueftw6tHhHKtR6qdNhftW6zj7Jv9eMxPX2zP3pMC805ty8y3ADC/3W7dvZR
# BwpXbBH0JmBERvHsGVjqkEzuatkjqMnsgtzG8mH1m9ceSdMP5Gi+7h6BU7GRfJ/T
# pOyatsNMzKaMExjD0yWn4aRcvEnmzWOfnQFfDqBx+XMjkLjY5ZJ2iLbHMlbdQXL2
# CnmtfSc7YGnNufS3ffvZAz9vC80eboIGH6qifOoZfWWK1O9+shqHoP2l8MzyfID4
# qGbH6X943Z9EJ1Mv8an1STA0QVMsHC0hcu8tttsz2yT9UOsapn8a7wU9wuGmlmTE
# FRcNF71pSn9fi3iVCtqZb0NilG3cqfYxlEF72IlbarO9CKgA//JFQujdgCMdkwVj
# Xf/jpBqEMg==
