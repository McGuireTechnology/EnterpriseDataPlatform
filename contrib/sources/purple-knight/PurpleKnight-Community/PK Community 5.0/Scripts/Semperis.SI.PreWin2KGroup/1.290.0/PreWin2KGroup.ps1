# This script looks for membership of the group "Pre-Windows 2000 Compatible Access"
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Description = 'This indicator looks for changes to the built-in group "Pre-Windows 2000 Compatible Access". This group grants read-only access to Active Directory.'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>As part of a layered approach to security and to ensure that non-authenticated users cannot read Active Directory, it''s best to ensure this group does not contain the "Anonymous Logon" or "Everyone" groups.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/security-risks-pre-windows-2000-compatibility-windows-2022/" target="_blank">Understanding the Risks of Pre-Windows 2000 Compatible Access Settings | Semperis blog</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $domainFailedCount = 0
        $DN = Get-DN $domain
        $deletedContainer = "CN=Deleted Objects,$DN"

        $results = [System.Collections.ArrayList]@()

        $thisSid = "S-1-5-32-554"
        $filter = '(objectSid='+$thisSid+')'
        $propertyFilter = "member"
        ...
        $results += Search-ADHelper @ADHelperParameters

        foreach($result in $results.attributes.valuemetadata) {
            $addedDate = ([datetime]::ParseExact($result.LastOriginatingChange,$format,[Globalization.CultureInfo]::CultureInfo.InvariantCulture))
            $deletedDate = ([datetime]::ParseExact($result.TimeDeleted,$format,[Globalization.CultureInfo]::CultureInfo.InvariantCulture))
            $authUsersAtDomainCreation = $false

            $accountDeleted = $false
            $accountIsMember = $true

            if ($deletedDate -lt (Get-Date "01/02/1601")) {
                if ($result.ValueDN -match $deletedContainer) {
                    if ($StartAttackWindow) {
                        $userCNFilter = "(cn=*" + $($result.ValueDN.split(":")[1].split(",")[0]) + "*)"
                        $deletedUserResult = Search-AD -dnsDomain $domain -attributes "whenChanged" -baseDN "$deletedContainer" -scope "Onelevel" `
                            -filter $userCNFilter -includeDeleted
                        $deletedDate = ([datetime]::ParseExact($deletedUserResult.Attributes.whenchanged[0],"yyyyMMddHHmmss.0Z",[Globalization.CultureInfo]::CultureInfo.InvariantCulture))
                        $accountDeleted = $true
                        $accountIsMember = $false
                    }
                    else {
                        Continue
                    }
            ...
            if ($result.ValueDN -match "CN=ForeignSecurityPrincipals,$DN") {
                $foreignPrincipal = Search-AD -dnsDomain $domain -scope "base" -baseDN $result.ValueDN -attributes "msDS-PrincipalName" -filter "(distinguishedName=$($result.ValueDN))"
                try {
                    $member = $foreignPrincipal.Attributes.'msds-principalname'[0]
                }
                catch {
                    $member = $result.ValueDN
                }


                if ($member -eq "\Everyone") {
                    $member = "Everyone"
                    $riskyMember = $true
                }

                if ($member -match "CN=S-1-5-7," -or $member -eq "NT AUTHORITY\ANONYMOUS LOGON") {
                    $riskyMember = $true
                    $member = "NT AUTHORITY\ANONYMOUS LOGON"
                }

                if ($member -eq "NT AUTHORITY\Authenticated Users") {
                    $filter = "(distinguishedName=$($DN))"
                    $domainCreationResult = Search-Ad -dnsDomain $domain -attributes "whenCreated" -baseDN "$DN" -scope "Base" -filter $filter
                    $format2 ="yyyyMMddHHmmss.0Z"
                    $domainCreated = ([datetime]::ParseExact($($domainCreationResult.Attributes.whencreated[$GET_STRING]),$format2,[Globalization.CultureInfo]::CultureInfo.InvariantCulture))

                    if (($addedDate - $domainCreated).TotalDays -lt 1) {
                        $authUsersAtDomainCreation = $true
                    }
            ...
            $thisoutputCreate = [PSCustomObject][Ordered] @{
                DistinguishedName = $result.ParentDN
                Member = $member
                Operation = "Added"
                EventTimestamp = $addedDate
            }

            if ($riskyMember -eq $true) {
                if ($authUsersAtDomainCreation -eq $true) {
                    $thisoutputCreate.Operation = "Risky Member Added During Domain Creation"
                }
                else {
                    $thisoutputCreate.Operation = "Risky Member Added"
                }
            }
            ...
            $updateSearchParameters = @{
                StartAttackWindow =$StartAttackWindow
                EndAttackWindow = $EndAttackWindow
                ThisOutput = $thisoutputCreate
                Metadata = $result
                ValueMetadata = $true
            }

            Update-IndicatorOutputAttackWindow @updateSearchParameters

            if ($accountIsMember -eq $true) {
                $domainFailedCount++
                [void]$outputObjects.Add($thisOutputCreate)
            }
            else {
                if ($thisoutputCreate.InAttackWindow -eq "True") {
                    $domainFailedCount++
                    [void]$outputObjects.Add($thisOutputCreate)
                }
            }

            if ($deletedDate -gt (Get-Date "01/02/1601")) {
                $operation = "Removed"

                if ($accountDeleted -eq $true) {
                    $operation = "Removed (Member Deleted)"
                }

                $thisoutputRemove = [PSCustomObject][Ordered] @{
                    DistinguishedName = $result.ParentDN
                    Member = $member
                    Operation = $operation
                    EventTimestamp = $deletedDate
                }

                $updateSearchParameters.ThisOutput = $thisoutputRemove
                ...
                if ($startAttackWindow) {
                    # If the account was added IN attack window, we show that it's removed
                    if ($thisoutputCreate.InAttackWindow -eq "True") {
                        $domainFailedCount++
                        [void]$outputObjects.Add($thisoutputRemove)
                        $resultAdded = $true
                    }

                    # We also want to notify on removed members, but only if not already notified above
                    if ($resultAdded -eq $false -and $thisoutputRemove.InAttackWindow -eq "True") {
                        $domainFailedCount++
                        [void]$outputObjects.Add($thisoutputRemove)
                    }
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects in the Pre-Windows 2000 Compatible Access group."
            $res.Remediation = "Confirm that any addition or removals from Pre-Windows 2000 Compatible Access group are valid and properly accounted for."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAIAwth0bN2k+xL
# JPdZgIGmsb8Xl5DAZJHf+Xf3130CLqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg