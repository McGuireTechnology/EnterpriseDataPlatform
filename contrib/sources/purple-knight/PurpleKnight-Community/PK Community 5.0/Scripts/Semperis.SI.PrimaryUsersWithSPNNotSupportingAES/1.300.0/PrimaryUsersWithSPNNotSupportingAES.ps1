...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>AES encryption is stronger than RC4 encryption. Configuring primary users with SPN to support AES encryption will not mitigate attacks such as Kerberoasting but does force AES by default, meaning that it is possible to monitor for encryption downgrade attacks to RC4 (Kerberoasting attacks)</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/security-policy-settings/network-security-configure-encryption-types-allowed-for-kerberos" target="_blank">Network security: Configure encryption types allowed for Kerberos | Microsoft Learn</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_kerberos_properties_encryption" target="_blank">Service accounts supported encryption algorithms | ANSSI</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_kerberos_properties_preauth" target="_blank">Kerberos preauthentication disabled | ANSSI</a></p>'
    ResultMessage = 'Found {0} Primary users with SPN not supporting AES encryption'
    Remediation = 'Best practice is to enable AES encryption support on service accounts.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
$attributes = @("msDS-SupportedEncryptionTypes","serviceprincipalname","samaccountname")
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # In MSDS-SupportedEncryptionTypes the decimal value of 8 is signifying AES 128 and 16 is AES 256
        $filter = "(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(objectSid=$krbtgtSID))(!(|(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.804:=8)(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.804:=16))))"

        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "subtree" -filter $filter
        ...
        if ($results){
            ...
            foreach ($result in $results) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    SamAccountName = $result.Attributes.samaccountname[$GET_STRING]
                    ServicePrincipalName = $result.Attributes."serviceprincipalname".GetValues("string") -join "; "
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Primary users with SPN not supporting AES encryption"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBimy0QIDhLsFu/
# 9FqF6VKbBE+ZhHeds21f5efNj916O6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgHLsChQ3wYhux5cQmiS2ZNdmBr0drnYd1O63rjEzY8PgwDQYJKoZI
# hvcNAQEBBQAEggIAb+3sbOSJsAyZB0/ObwjjTwgl3XYyppxjenMSmPhD64h0GGSK
# 4MQC+Skh/wEcVAyZXbNyFWrD8hi13C0jd9N0gaoDORqU8ayckQFX3CBgAomD8j+0
# 9MiH/5fl/bC+LERVm0ZUqVfUFxgghD18w8xRtEi2y3A3xDLAkP9KNn29p0NkVg7X
