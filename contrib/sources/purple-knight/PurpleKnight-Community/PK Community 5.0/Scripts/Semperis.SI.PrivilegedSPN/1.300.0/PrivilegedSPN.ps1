# This script looks for objects with admincount=1 that also have SPN
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>This is a significant issue that can allow an attacker to elevate privileges in a domain. Audit all accounts where privileged access is possible looking for anomalous access. If found, a breach or ongoing attack should be further investigated.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_spn_priv" target="_blank">Privileged accounts with SPN | ANSSI</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all objects with admincount flag set to 1 that also have SPN
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter "(&(adminCount=1)(servicePrincipalName=*)(!(servicePrincipalName=kadmin/changepw)))"
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) privileged users with associated SPN."
            $res.Remediation = "Remove SPN from privileged accounts when not required or mitigate by other means."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB/ae5e72Ctozcp
# t+Y468W7CB0kP9hAJvG7GU3GV110SqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgiUDGpIYIvx3G0A6xo1q4m+PIcspxrvMxi4E/Gq8Xa5QwDQYJKoZI
# hvcNAQEBBQAEggIAVriaHv6CXZMEestCyH/IN+beOvdDHvzV9h2qvPoAUCVMjBfd
# S8f92QS89GmiLCTUIo9ZYYZ2rJxXcCeGMMYmerT8Cir+E0XVVX1QxXyHRzJRipXC
# rnLKOONUMxpI3ZhHYi8+TsvjkPQk+I9JeYe3OWEBCumlc8RehL9wWOcrdVKL5jkm
# TTtQ7fv5FQWJFUcQUu+ZytVP/LxIJij7e2qp7wzDmE4tMlGbMjxvLZMTbhZBe41i
# /+p3x6mjSEVpJXIxk8KMcP4afLTfrRt/Kb4+vVWPao9J5gSr1Zd6iCkmSJG+sWcj
# ilga+QZed0VfbaPovPZsF9tDsketis4D1uCWaPWaBTBr6VGU/2NJHsQr6dlbfEEf
# Xp1ywCdedkr91hLvO5682YskKCIRbniDQ241d4P6xVq9xYDtYpA1ozu9XFkGonpA
# ja6ZA8CpUlxsb1DsIpYamvSnigmKJXxwY2d0LIWP/5z0gmuDp/Lzm0X8T3TVjK5y
# RBOUr8u0c5dj+ieKboaXnQA3M9jH5vBKv7lQu3iwvJ3nQSYmw9aVTM7PN8g3sFp/
# OnYoPe4TAxwTzzQ5wNkayWTjWA2uc27XmM+6jrJ9AIl1lezAUdxT8A7a7lIhFORT
# At4Gk9q4s6ehoJ0w5My7OneDdDKX2OBvuzOE12128FDOeqHb3ElH34h7SNOhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCC16AmI3cc+jUQkYtSH7xEMNxZfXVmUPmUR
# gusLxGk78AIUbkcE6TrGMXSUkL9WkCq5KucTjs4YDzIwMjUwNTI4MTI0MTMwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAo+ZP96lHMt4wNmwZcUOV1qLEo/M2t/RQNwhaxH7zP
# 0zCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgLQtcHSXA+Ifp12RDBLEkhAf5RbtojNvjteCVITBcbwyGKsGGY+yI8Nl
# znm1ps5N9vjF58ulmqLP2T1M9TCIQeN+53YwFtELMZMBOlCswJgAHx0epe9iLJQp
# LX/Lsxq8O7Z/T3qkuki1mN6ZwA6VeBCdFogF33OCq5BrL5EsEHnx07QzAWcdr/7F
# fBVzyVq+G9MVWIvybtShlFnVfQ3pU7ATxybHOSFk4rHOAvMLlG0m/J7oT0Uts37j
# 8nh/7ksdij8cYVstVuLreBiTaJCMyWY7G0ieWB887qhHu7XZyHgIRZBIk61n2cyg
# KAJ6ZEOqoV7lFmt2hiyVHXTrr7KIdn0jwsDwldx1yqm9zyJdjG5oi9rUhDMXt8PB
# NKgpxnkXAQh88EcMsQKZMXJzt6xYBTPnnH1oAQ17KpJJqBabF4UMaDqSOioxwmAk
# xUK/AS0hnTIcViavOU9IodQG78SbO4ACwtIAQGWmcRuJ2BqicUvKoKKDxbNjhTy3
# 12M8RjBBaQ==
