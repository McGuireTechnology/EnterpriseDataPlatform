# This indicator looks for privileged users that have weak password policy applied to them. it determined by checking both FGPPs and the password policy applied to the domain.
...
$Global:self = @{
    ...
    LikelihoodOfCompromise = 'Weak passwords are easier to crack via brute-force attacks, they can provide attackers opportunities for moving laterally or escalating privileges. The risk is even higher for privileged accounts, for when easily compromised, they improve the attacker''s chance to quickly advance within the network.
        <h3>References</h3>
      <p><a href="https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf" target="_blank">NIST.SP.800-63-3</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_privileged_members_passwordf" target="_blank">Privileged group members with weak password policy | ANSSI</a></p>'
    ResultMessage = 'Found {0} privileged users with a password policy but whose compliance with them could not be determined'
    Remediation = 'Apply appropriate password policies for privileged users.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    # Scriptblock that gets a policy object as input and output a boolean that indicate if the policy is weak for our standards
    $configurationCheck = {
        param (
            $MaximumPasswordAgeDays,
            $MinimumPasswordLength
        )
        if ($MaximumPasswordAgeDays -is [int] -and $MinimumPasswordLength -is [int]){
            if ($MinimumPasswordLength -ge 8 -and $MaximumPasswordAgeDays -lt 1095) {
                return $true
            }
            else {
                return $false
    ...
    $PSOUnreachable = 0
    $passwordAttributesFGPP = @("msds-maximumpasswordage", "msds-minimumpasswordage", "msds-minimumpasswordlength", "msds-passwordcomplexityenabled", "msds-passwordhistorylength",
        "msds-passwordreversibleencryptionenabled")
    $passwordAttributesNCHead = @("maxpwdage", "minpwdage", "minpwdlength", "pwdhistorylength","pwdproperties")
    ...
    foreach ($domain in $DomainNames) {
        $privilegedUsersResult = Get-AdminsInDomain -DomainNames $DomainNames -ForestName $ForestName
        $privilegedUsers += $privilegedUsersResult[0]
        $largeGroups += $privilegedUsersResult[2]
        $unavailableDomains += $privilegedUsersResult[3]
    }

    $privilegedUsers = $privilegedUsers | Select-Object -Unique
    $largeGroups = $largeGroups | Select-Object -Unique
    $unavailableDomains = $unavailableDomains | Select-Object -Unique

    foreach ($adminDN in $privilegedUsers) {
        $adminDomain = $adminDN.Substring($adminDN.IndexOf("DC=")).replace("DC=","").replace(",",".").tolower()

        if ($adminDN -match "CN=ForeignSecurityPrincipals") {
            continue
        }

        $admin = Search-AD -dnsDomain $adminDomain -attributes "msds-resultantpso" -baseDN $adminDN -scope "base" `
            -filter "(&(objectCategory=person)(objectClass=user))"
        if (!$admin) {
            continue
        }

        $PolicyDN = $admin.attributes."msds-resultantpso"[0]

        if ($PolicyDN) {
            [void]($PolicyDN -match "(CN=System,DC=.*)")
            $SystemDN = $Matches[1]
            $passwordPolicyFGPP = Search-AD -dnsDomain $adminDomain -attributes $passwordAttributesFGPP -baseDN $SystemDN -scope "subtree" -filter "(distinguishedName=$($PolicyDN))"

            if ($passwordPolicyFGPP) {
                $passwordPolicyDN = $passwordPolicyFGPP.DistinguishedName
                $maxPwdAge = ([TimeSpan]::FromTicks($passwordPolicyFGPP.attributes."msds-maximumpasswordage"[0]).days * -1)
                $minPwdAge = ([TimeSpan]::FromTicks($passwordPolicyFGPP.attributes."msds-minimumpasswordage"[0]).days * -1)
                $minPwdLength = [int] $passwordPolicyFGPP.Attributes."msds-minimumpasswordlength"[0]
                $pwdComplexityEnabled = $passwordPolicyFGPP.Attributes."msds-passwordcomplexityenabled"[0] -eq "TRUE"
                $pwdHistoryLength = $passwordPolicyFGPP.Attributes."msds-passwordhistorylength"[0]
            }
            else {
                $PSOUnreachable += 1
                $passwordPolicyDN = $PolicyDN
                $maxPwdAge = "N/A"
                $minPwdAge = "N/A"
                $minPwdLength = "N/A"
                $pwdComplexityEnabled = "N/A"
                $pwdHistoryLength = "N/A"
            }
        }
        else {
            [void]($admin.DistinguishedName -match ".*?(DC=.*)")
            $domainDN = $Matches[1]
            $passwordPolicyNC = Search-AD -dnsDomain $adminDomain -attributes $passwordAttributesNCHead -baseDN $domainDN -scope "base" `
                -filter "(DistinguishedName=$domainDN)"
            $passwordPolicyDN = $passwordPolicyNC.DistinguishedName
            $maxPwdAge = ([TimeSpan]::FromTicks($passwordPolicyNC.attributes."maxpwdage"[0]).days * -1)
            $minPwdAge = ([TimeSpan]::FromTicks($passwordPolicyNC.attributes."minpwdage"[0]).days * -1)
            $minPwdLength = [int] $passwordPolicyNC.Attributes."minpwdlength"[0]
            $pwdComplexityEnabled = [bool]([int]$passwordPolicyNC.Attributes."pwdproperties"[0] -band 1)
            $pwdHistoryLength = $passwordPolicyNC.Attributes."pwdhistorylength"[0]
        }
        if (!(& $configurationCheck -MaximumPasswordAgeDays $maxPwdAge -MinimumPasswordLength $minPwdLength)) {
            $thisOutput = [PSCustomObject][Ordered] @{
                DistinguishedName = $admin.DistinguishedName
                PasswordPolicyDistinguishedName = $passwordPolicyDN
                MaxAge = $maxPwdAge
                MinAge = $minPwdAge
                MinLength = $minPwdLength
                ComplexityEnabled = $pwdComplexityEnabled
                History = $pwdHistoryLength
            }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            if ($PSOUnreachable -eq $outputObjects.Count) {
                $res.Status = 'Error'
                $processUser = $ENV:USERNAME
                $res.ResultMessage = "$processUser does not have sufficient permissions to access FGPP."
                if ($largeGroups) {
                ...
                $res.Remediation = "Allow $processUser to read FGPP policy container and child objects."
            }
            elseif ($PSOUnreachable -gt 0) {
                $res.Status = 'Failed'
                $res.ResultMessage = "Found $($outputObjects.Count - $PSOUnreachable) privileged users who do not comply with strong password and $($PSOUnreachable) privileged users whose compliance with the policy could not be determined."
                if ($largeGroups) {
                    $res.ResultMessage += " While processing group membership, group(s) were discovered to have more than 4500 transitive members. Only the first 4500 members were processed by this indicator. A secure environment should not have such a high number of privileged group members. Please review membership for the following group(s) to ensure only necessary objects are contained:  $($largeGroups -join '; ')."
                }
                ...
                $res.Status = 'Failed'
                $res.ResultMessage = "Found $($outputObjects.Count) privileged users who do not comply with strong password policies."
                if ($largeGroups) {
                    $res.ResultMessage += " While processing group membership, group(s) were discovered to have more than 4500 transitive members. Only the first 4500 members were processed by this indicator. A secure environment should not have such a high number of privileged group members. Please review membership for the following group(s) to ensure only necessary objects are contained:  $($largeGroups -join '; ')."
                }
                $res.Remediation = $self.Remediation
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC