# This script looks for users with admincount=1 that are not member of the Protected Users group
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 1
    LikelihoodOfCompromise = '<p>The Protected Users security group was introduced in Server 2012 R2 Active Directory to minimize credential exposure for privileged accounts. As a defense in depth measure privileged accounts, such as Domain Admins, should be added to the Protected Users security group.</p>
      <p>Attackers targeting privileged accounts in Active Directory will find a much higher level of friction in certain types of attempts to compromise the accounts due to the protections offered.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/windows-server/security/credentials-protection-and-management/protected-users-security-group" target="_blank">Protected Users Security Group | Microsoft Learn</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_protected_users" target="_blank">Privileged accounts outside of the Protected Users group | ANSSI</a></p>'
    ResultMessage = 'Found {0} privileged users that are not members of the Protected Users group.'
    Remediation = '<p>It is recommended to ensure that all privileged users are members of the Protected Users security group.</p>
      <p>If the organization is already using the Protected Users security group functionality, it is recommended to add the identified privileged users.</p>
      <p>If the Active Directory schema is pre Server 2012 R2, this functionality does not exist, and it is recommended to upgrade the schema.</p>
      <p>For full details on the protections offered and the implementation of the Protected Users security group, please refer to the reference article.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get the DN of the Protected Users group to be used later in the filter
        $protectedUsersDN = (Search-AD -dnsDomain $domain -baseDN $DN -scope "Subtree" -filter "(&(name=Protected Users)(objectClass=group))").DistinguishedName

        # Get a list of all privileged users that are not member of the protected users group
        # memberOf:1.2.840.113556.1.4.1941 walks the chain of ancestry in objects all the way to the root until it finds a match.
        if ($protectedUsersDN) {
            $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "subtree" `
                -filter "(&(objectCategory=person)(objectClass=user)(admincount=1)(!(memberOf:1.2.840.113556.1.4.1941:=$protectedUsersDN))(!(samaccountname=krbtgt)))"
        }

        if ($results.Count) {
            $failedDomainCount++
            foreach($result in $results) {
                $uac = $result.Attributes."useraccountcontrol"[0]
                $enabled = $True
                if ($uac -band 2) {
                    $enabled = $false
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) privileged users that are not members of the Protected Users group."
            $res.Remediation = "Ensure that all privileged users are members of the Protected Users group. If using a pre 2012-R2 schema, then the protected users group does not exist. This is an exposure, but the remediation is to upgrade the schema."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAwi1vwVoanUJDF
# w3Hyy8sdqa4yYy9yBkWVTN02qQclxqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ