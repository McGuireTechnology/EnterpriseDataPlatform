# This script looks for Domain Controllers that has values in msds-allowedtoactonbehalfofotheridentity
...
$Global:self = @{
    ID = 66
    UUID = '6df6c2ee-a77b-411b-8540-4357e5d0a1fb'
    Version = '1.300.0'
    CategoryID = 5
    ShortName = 'SI000066'
    Name = 'Domain Controllers with Resource-Based Constrained Delegation (RBCD) enabled'
    ScriptName = 'RBCDOnDC'
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>An attacker needs to know the Service Principal Name (SPN) of the object they want to delegate, as well as be able to populate the msDS-AllowedToActOnBehalfOfOtherIdentity attribute with a computer account that they control. This is sometimes possible when unprivileged users are by default allowed to create computer accounts (MachineAccountQuota) and write the attribute to the target computer.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_delegation_sourcedeleg" target="_blank">Resource-based constrained delegation on domain controllers | ANSSI</a></p>'
    ResultMessage = 'Found {0} Domain Controller objects with Resource Based Constrained Delegation configured.'
    Remediation = 'Ensure that the msDS-AllowedToActOnBehalfOfOtherIdentity attribute on any DC is empty - this is the normal state for a DC.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    Permissions = @()
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Defense Evasion', 'Lateral Movement', 'Privilege Escalation') },
        @{ Name = 'ANSSI'; Tags = @('vuln1_delegation_sourcedeleg') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            baseDN = $DN
            scope = "Subtree"
            filter = "(&(msds-allowedtoactonbehalfofotheridentity=*)(primarygroupid=516)(samaccounttype=805306369))"
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Domain Controller objects with Resource Based Constrained Delegation configured."
            $res.Remediation = "Ensure that the msDS-AllowedToActOnBehalfOfOtherIdentity attribute on any DC is empty - this is the normal state for a DC."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD0Jv0dHd2urL+m
# 0TB4rlfJZGfrlco7F+Y3mtRoklmrcqCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgcnGzgVSOfYIGSXKUKqnzKSuCcy0p7x1e+Z1DhEZKv3IwDQYJKoZI
# hvcNAQEBBQAEggIAVe5MdWdj79J6HitaXvQyhhNXdo/dO/N3nP4z5CwlN82OhXx2
# BQr/rnCuWzlOhDT5I+f36dIwrK8E4xPEcJLe0g5nz/p/8T6YLizSxGA7twpY1+Eh
# Af5BGiphlg4Vo/czTxznm6ImwpHLilX0N5+HqSInIVXGXXd38gMIH4N2Bwl+qvlP
