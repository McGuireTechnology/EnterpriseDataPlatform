# This script looks for objects with the right to add RBCD on the krbtgt account
...
try {
    ...
    $guidHT = @{"00000000-0000-0000-0000-000000000000"="All Properties";"3f78c3e5-f79a-46bd-a0b8-9d18116ddc79"="msds-AllowedToActOnBehalfOfOtherIdentity";"4c164200-20c0-11d0-a768-00aa006e0529"="Account Restrictions"}
    ...
    foreach ($domain in $DomainNames) {
        ...
        $scriptBlockFilter =  {
            ...
            Update-NTSecurityDescriptor -Entry $Entry -PropagationFlagsFilter "None|NoPropagateInherit" -AccessControlTypeFilter "Allow" -InheritanceTypeFilter "None|All|SelfAndChildren"`
                -RightsFilter $rightsFilter -ExcludedSIDs $allowedSIDs -GuidHTFilter $guidHT
        }
        $result = Search-AD -dnsDomain $domain -attributes "ntsecuritydescriptor" -baseDN $DN -scope "Subtree" `
            -filter "(ObjectSID=$domainSID-502)" -scriptBlockFilter $scriptBlockFilter
        ...
        foreach ($access in $result.Attributes.'aces') {
            if ($acl.ContainsKey($access.IdentityReference.Value)) {
                $acl[$access.IdentityReference.Value] += ";" + $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid]
            }
            else {
                $acl[$access.IdentityReference.Value] = $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid]
            }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) principals that can write to msds-AllowedToActOnBehalfOfOtherIdentity on krbtgt."
            $res.Remediation = "Review the list and ensure that there are no unnecessary principals who can set resource-based constrained delegation on krbtgt."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBto/TPQCpQ2I0B
# XjgJEu8a+nZK81b6s6dvkggoMueBbKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgbU5KHCCe0ljGvH1dkJbKmDPKwDGenBUoNJuvd0RR+5MwDQYJKoZI
# hvcNAQEBBQAEggIAQ/U0ZSjz5LzuG4+R45hF8IFCkjni/Ehujvmz66ePjMLoz7pZ
# E5hvGYF8+NELOdRGNRUP80ybAOq2HFeSH8ipXKQhTjDwf09whVIg8F0oxCE4xPDw
# mCiLOUGigmLE6Vv3ba6cRnG7KSz2hl7nHAB8WbKVEOw/376Q2Snyz2wWsb/jDnCz
# kKy8Ik4BhkOCo1ExrxnixyS37XZaqS67ZCDn/vg/wxP1mWNtsv4mOUz6R9hBwsvu
# oaJ/65z23LifImJtr9SKuDceGYpQFUEeNIMKhiMkvtfazjc1O1bvTkSgoAOLDI3y
# gHRk4CV3aAifMJ7Srh0glho7qiQ2fPbkWUVlzxaN9IqiCByUxpZ9I51uoEba78vu
# eudsQHwKsMBDAUB6FCPLWpr8lrj6JDcveFSdgis4uXUFtPMmuXk8NF1WzGlKwgpM
# C5hOd3P52bgVYSyEd55x1+tKLo4b6sgB1K7so57BWSOxK8A9MVq3ZpP2utpNQYiY
# ctxz80wDa+ym1wy27X1OT67yE7jYBWGW1ts6lMMBBzHTw+byFmw3qvB6xAAnCQm9
# 5z6YC5/GMlziEQDbj2YYQjvIg66PcF6R4FzbRV66y/eUNn7vu+1FNXlZJJq0Cq+Z
# 8HTKrdKrpT2vfzWKhPE42PO4nSqP3legw4h7rHVlXNNsgaiCHCK65B+R1cmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCNCRBbCxcLOqJbvoRy9jYV6/StptVUurlx
# ZTMGNQnnIgIUbcJPeE/OToWHekQgKxXAr9Lq9ocYDzIwMjUwNDE2MTMxNDQ1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDye4sxQGYKDPAU8QEtf3YDG9saeDe9eo4bGvdJq5qw
...
# CwUABIIBgHF3+i7s2v5WnQYBfda1b1lk/Cj3nW0/QNweJ72mE1OM5XqeXQI1f+Wt
# UPsYFeFkZLTFuprhkr0NQ4C+to8UkqxYgUOhg3KPnGpLUPUZX1JCv8JGUKpSiHXY
# SJPwrVzR5ibN5Ka1SlkV0xKgPcw2s1TNxmBztbezn6ztBv15TntLhMiYT1dc+mbo
# YtMISFkHYr8Vl4zwjZXrd8QUApJmzIFyD7qba4W5iqvsgDi2TRK9au45nru9/PXl
# FlfNW9RnG2+9JBvQnjhGQ7rkDZXrIK3mAMxpKSE03CuQz3xbKcRo8v6L1LBgNfcQ
# gfi/WsnhKZYp1PPkCIKuAIl3wszHYaF35KvrwxV15BLWkziZ2GEdcxYlKAetzYAc
# BwDnGFKCgg4gr8LCZwFcACS+a2GSOTzHVbiLh9s90a7L6vAq7KJJiC9hUUMdh7eB
# kd2isS01G49/YbBWOi2EAAt502O8vaHIoN4W0x4BU6JCRkutThjrgaltpxDfVHLR
# nOA46dvSCw==
