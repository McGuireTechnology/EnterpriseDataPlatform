...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>RC4 and DES are considered an insecure form of encryption, susceptible to various cryptographic attacks. Multiple vulnerabilities in the RC4 and DES algorithms allow MitM and deciphering attacks.</p>
      <h3>References</h3>
      <p><a href="https://nvd.nist.gov/vuln/detail/CVE-2013-2566" target="_blank">CVE-2013-2566</a></p>
      <p><a href="https://nvd.nist.gov/vuln/detail/CVE-2015-2808" target="_blank">CVE-2015-2808</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dc_crypto" target="_blank">DC/RODC supported encryption algorithms | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln2_dc_crypto', 'vuln3_dc_crypto', 'vuln4_dc_crypto') }
    )
    Products = @(
...
# LDAP Request Params
$filter = "(&(|(primaryGroupID=516)(primaryGroupID=521))(|(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.804:=4)(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.804:=2)(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.804:=1)(msDS-SupportedEncryptionTypes=0)(!msDS-SupportedEncryptionTypes=*)))"
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            scope = "Subtree"
            filter = $filter
            PropertyFilter = "msDS-SupportedEncryptionTypes"
        ...
        if ($results){
            ...
            foreach ($result in $results) {
                $supportedEncryptionType = $result.Attributes.'msds-supportedencryptiontypes'[0]

                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $result.DistinguishedName
                    SupportedEncryptionTypes = "$( (Get-SupportedEncryptionTypesSet $supportedEncryptionType) -join ", ")"
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) Domain Controllers that support RC4 or DES encryption"
            $res.Remediation = "It is best practice to disable support for RC4 and DES on domain controllers. Proceed with caution, as this can cause clients that request RC4 encrypted kerberos tickets by default to fail.
        Disable it by adding the group policy Network security: Configure encryption types allowed for Kerberos and select only AES-128, AES-256 encryption types, to a GPO that affects the Domain Controllers container.
        The group policy path is Computer Configuration\Windows Settings\Security Settings\Local Policies\Security Options."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDxuMlw9xvWZPdk
# tK/HIReqih667IVlEhksEchKuMzhN6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ