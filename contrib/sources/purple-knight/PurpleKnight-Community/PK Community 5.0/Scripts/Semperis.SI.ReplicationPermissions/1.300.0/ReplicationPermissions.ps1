...
$Global:self = @{
    ...
    Schedule = '1h'
    Impact = 8
    LikelihoodOfCompromise = '<p>DCSync is an attack for accessing credentials through this method. If an attacker gets ahold of these privileges, it is straight-forward to retrieve credential material using tools like Mimikatz, for any user in a domain.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_permissions_naming_context" target="_blank">Dangerous ACLs expose a naming context root (attack path) | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln1_permissions_naming_context', 'vuln2_permissions_naming_context') }
    )
    Products = @(
...
try {
    ...
    $unconditionalRightsFilter = "GenericAll|WriteDacl|WriteOwner"
    $conditionalGuids = @("DS-Replication-Get-Changes-All", "DS-Replication-Get-Changes")
    ...
    foreach ($domain in $DomainNames) {
        ...
        if ($UsersACL.Count -gt 0) {
            foreach ($result in $UsersACL.GetEnumerator()) {
                # Continue if the access is ONLY extended right for only ONE of DS-Replication-Get-Changes-All and DS-Replication-Get-Changes
                if ($result.Value -notmatch $unconditionalRightsFilter) {
                    if ($result.value -notmatch "All Properties") {
                        if (!($result.value -match $conditionalGuids[0] -and $result.value -match $conditionalGuids[1])) {
                            continue
                        }
                    }
                }
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.Remediation = "Ensure that there are no unnecessary replication permissions and investigate suspicious permissions. Under certain situations (e.g. Microsoft PAM Tiering), an empty group may appear in the results - this is normal but keep in mind that this is a highly privileged group."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAnglvehya1a81p
# 768Ei+4CJrLiJ/QCfSOwiSv3oKk4DKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgRGYEpOOHsrJ+3/SUYfE4cHEBF5JtNcg/63RQ/6xlr+MwDQYJKoZI
# hvcNAQEBBQAEggIAQE4bpDAhmaPay4OSH5rWlFRuWVqgge2KtGkS2qDNJ5gm29Wx
# qxruwqNVMxv8RN3Qnz2EZwUNj1/MePIf4FH7EAHmgMjRz3obXwZMImv5BDP4wOQO
# juDvMsQHVGbPmkKqYkO1Vd6lZbxn8ZemLX9Ov7PhwSEqBMDZLuu4eaiBB7pJdRiz
# lkIeVquJvujq6jJ1nZlmvL8qajDadFmlNg0Su75iHHQ76lOjscxTSfRHc2MGowYk
# Jv3+2gYByJGSku1iQdHPsp/F7HPxHeoe50XaQi4vS7i3A+9XK4Va0uR47FPFSnvq
# yfMGuEyPFbFjvv0abx2aqTSqiIUkoamOhA25Pb02syRC/Qptw7hjVwpQH+qPsxge
# bHl4exI67jDi45zuABJHAoqfaeczIT8KGuaqdxdTvUz2Bjz3qKgBGCFB2rlpRWlW
# 5SajktzZ0Vih6RKVjPH5psHoXmXT48zizj7iLb9BQgnLNLg2JDauF7Oqkwhp/zko
# G8gOfp07cW15ixRmGwolEqEsY1cfUZpmNYYaQ1uj7h7/KvDJHksgfTE3RRMXQWKf
# 3T9VPx+OTPfSSZe0uk3rPxO5uXAbyd3t/yDEiCr3tPjDBnE48Ih244id2erShA33
# JuuX4AaKEl8/JoravSLESBgisrIvkgMmSAgAESZC4jkWVcBargSJqWuLbaehgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCCeQ8YRiXWPaiFeJTEfOm3PnsGt8XguZq4c
# tUOBcqkVNAIUOWVZeSNjar6QVEc+Q+rRnVpXmlUYDzIwMjUwNTI4MTI0MjA4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAHdHZh+3ikI2LAPLLq4/85YPAn7A2MpPPHaW6yRF0+
...
# CwUABIIBgDSNOyRgqjSFbP4sTRzhP8LTHSXCcm7Y0Dm8rA4BsuIxFRwqOOuOxKMA
# vcWfFuwDIBS8EMP90g8NesAqUPU5OAsVewb5BILz8nYTDcS9QVmxnNCsmRyosTnl
# q83bv3dVO5oMn5gRj/nrRtrol586xqfiMuocPywo5Z1x4uxv9L1w0PbsXa6+AhsS
# PqneZyHETuIw1wCnbh/fB9UZ3Wq1P5WEQSq08cvPNpzVo+/GPZAFSDJ49AEKCjaj
# qZKi+jwioAak0TuFrKLBEdPf46WWRzsn/JU5Iks5yWrvi0TQWdCSfNEgH913UFv4
# hY4Wc50aD00Sy+ADLWAcGyd1GLJ0vHxcT9DxQsZYHQHwlbemIZULcL5fAQTM1inp
# 7yLMRf2jJ79RyTyabebaLtYIGKJHZgFBg4cBZxH89AwH0lXNfY9uXxIoIt5gm6se
# D/Q4tb2uaxzJtiaL7dKcst3dc3M1ugimRAqPGrHNNYu5nVgG5dbMy2vRAug2RSze
# HO6oCSdsgg==
