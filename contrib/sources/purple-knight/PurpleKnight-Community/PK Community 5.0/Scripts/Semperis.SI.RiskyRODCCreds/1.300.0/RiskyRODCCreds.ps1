...
#This script checks each domain to determine if there are privileged users/groups added to RODC Password replication policy
...
$Global:self = @{
    ID = 22
    UUID = 'f84225d1-4f05-4cb1-bf3f-f87413006214'
    Version = '1.300.0'
    CategoryID = 3
    ShortName = 'SI000022'
    Name = 'Risky RODC credential caching'
    ScriptName = 'RiskyRODCCreds'
    Description = '<p>This indicator checks the password replication policy on RODCs.</p>'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = '<p>In many instances, read-only domain controllers (RODCs) are deployed in area where there is a lower level of physical security to the servers.</p>
      ...
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_rodc_never_reveal" target="_blank">Dangerous configuration of read-only domain controllers (RODC) (neverReveal) | ANSSI</a></p>'
    ResultMessage = 'Found {0} privileged objects added to RODC Password Replication Policy.'
    Remediation = '<p>It is recommended to adjust the password replication policy on the RODC to deny replication of privileged user credentials to an RODC.</p>
      ...
      <p>You may also need to review the membership of the group <b>Denied RODC Password Replication Group</b>, which by default holds many privileged user groups that should be denied replication.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
        @{ Name = 'DistinguishedName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'RODC'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AD')
    Permissions = @()
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln2_rodc_priv_revealed') }
    )
    Products = @(
...
# This function is called when there is a suspected IoE
# Below we validate whether the caching of the user's credential is *also denied*
function Get-CacheResult {
    param (
        $targetObject,
        $rodcDN
    )
    ...
    $memberOfs = @()
    $memberOfs += Search-AD -dnsDomain $domain -baseDN $DN -scope "Subtree" -filter "(member:1.2.840.113556.1.4.1941:=$($targetObject.DistinguishedName))"

    if ($memberOfs) {
        foreach ($rodcDenyObject in $rodcDenyObjects) {
            if ($memberOfs.DistinguishedName.Contains($rodcDenyObject)) {
                return
            }
        }
    }
    ...
    $thisOutput = new-object psobject -Property @{
        DistinguishedName = $targetObject.DistinguishedName
        RODC = $rodcDN
    }
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $attributes = @("msds-revealondemandgroup","msds-neverrevealgroup","serverreferencebl")
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" -filter "(&(objectCategory=computer)(primaryGroupID=521))"

        if (!($results)) {
            continue
        }

        foreach ($rodc in $results) {
            $rodcDenyObjects = @()
            $rodcDN = $rodc.DistinguishedName

            if ($rodc.Attributes."msds-revealondemandgroup") {
                if ($rodc.attributes.serverreferencebl) {
                    $nTDSDSADN = 'CN=NTDS Settings,' + $rodc.attributes.serverreferencebl.GetValues("string")[0]

                    $nTDSDSA = Search-AD -dnsDomain $domain -baseDN $nTDSDSADN -scope "base" -filter "DistinguishedName=$nTDSDSADN"
                    if (!($nTDSDSA)) {
                        continue
                    }
                }
            ...
            if ($rodc.Attributes.'msds-neverrevealgroup') {
                $rodcDenyObjects = $rodc.Attributes.'msds-neverrevealgroup'.GetValues("string")
            }

            $allowedIDs = $rodc.Attributes."msds-revealondemandgroup".GetValues("string")

            foreach ($principalDN in $allowedIDs) {
                try {
                    $attributes = @("admincount", "objectClass")
                    $principalObject = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $principalDN -scope "base" -filter "(distinguishedName=$principalDN)"

                    if ($principalObject.Attributes."objectclass"[-1] -eq "group") {
                        if ($principalObject.Attributes."admincount"[0] -eq "1") {
                            if ($outputObjects.DistinguishedName -notcontains $principalObject.DistinguishedName) {
                                $thisOutput = new-object psobject -Property @{
                                    DistinguishedName = $principalObject.DistinguishedName
                                    RODC = $rodcDN
                                }
                                [void]$outputObjects.Add($thisOutput)
                            }
                        ...
                        if ($memberStore.ContainsKey($principalDN)) {
                            $principalMembers = @()
                            $principalMembers = $memberStore[$principalDN]
                        }
                        else {
                            $principalMembers = @()
                            $principalMembers += Search-AD -dnsDomain $domain -baseDN $DN -scope "Subtree" -filter "(&(memberOf:1.2.840.113556.1.4.1941:=$principalDN)(admincount=1)(objectClass=user))"
                            $memberStore[$principalDN] = $principalMembers
                        }

                        foreach ($principalMember in $principalMembers) {
                            if ($outputObjects | Where-Object {$_.DistinguishedName -eq $principalMember.DistinguishedName -and $_.RODC -eq $rodcDN}) {
                                continue
                            }

                            Get-CacheResult -targetObject $principalMember -rodcDN $rodcDN
                        }
                    }
                    else {
                        if ($principalObject.Attributes."admincount"[0] -eq "1") {
                            if ($outputObjects | Where-Object {$_.DistinguishedName -eq $principalObject.DistinguishedName -and $_.RODC -eq $rodcDN}) {
                                continue
                            }

                            Get-CacheResult -targetObject $principalObject -rodcDN $rodcDN
                        }
                    }
                }
                catch {
                    if ($failedObject -notcontains $principalDN) {
                        [void]$failedObject.Add($principalDN)
                    }
                }
            }
        }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) privileged objects added to RODC Password Replication Policy."
            $res.Remediation = "Privileged users and groups should be removed from the RODC Password Replication Policy to prevent those secrets from being replicated to RODCs, which tend to be less secure than read-write DCs. This is managed on a per-RODC basis."
            if ($failedObject) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAHKec45Chjk4d9
# XSH1elOkdF9RSzfhWCi13e19GTERO6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg