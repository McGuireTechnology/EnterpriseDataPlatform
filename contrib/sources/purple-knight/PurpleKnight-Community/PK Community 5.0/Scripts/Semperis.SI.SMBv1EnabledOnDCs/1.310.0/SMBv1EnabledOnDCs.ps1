...
$Global:self = @{
    ...
    Schedule = '1h'
    Impact = 8
    LikelihoodOfCompromise = '<p>SMBv1 is an old protocol, considered unsafe and susceptible to all kinds of attacks. It was publicly deprecated by Microsoft in 2014.</p> 
      <h3>References</h3>
...
namespace SmbVersioning
{
    public class SmbVersion
    {
        ...
        public struct SMB_DATA
        {
            ...
            //public byte[] Data; // Dialects maybe change it Dialect
        ...
        public static string GetDialectVersionSMB2(byte[] bytesReceived)
        {
            SMB2_NEGOTIATE_RESPONSE smbResponse = new SMB2_NEGOTIATE_RESPONSE();
            smbResponse.DialectRevision = BitConverter.ToUInt16(bytesReceived, 72);

            if (smbResponse.DialectRevision == 0x302 || smbResponse.DialectRevision == 0x300 || smbResponse.DialectRevision == 0x311)
            {
                return "SMB3";
            }
            else if (smbResponse.DialectRevision == 0x2FF || smbResponse.DialectRevision == 0x202 || smbResponse.DialectRevision == 0x210)
            {
                return "SMB2";
            }
            return "SMB2";
            ...
            if (version == SMBVersionType.SMB2_PROTOCOL)
            {
                try
                {
                    ...
                    using (Socket socket = ConnectSocketNew(server, port))
                    {
                        ...
                        if (version == SMBVersionType.SMB2_PROTOCOL)
                        {
                            string dialect = GetDialectVersionSMB2(bytesReceivedSMB2);
                            return dialect;
                        }
                        socket.Close();
                    }
...
if (-not ('SmbVersioning.SmbVersion' -as [Type])) {
    Add-Type $code
}
...
try {
    ...
    $dcs = @()
    $attributes = @("dnshostname","serverReferenceBL","primarygroupid")
    ...
    foreach ($dc in $dcs)
    {
        ...
        if ($dc.attributes.serverreferencebl) {
            ...
            $nTDSDSA = Search-AD -dnsDomain $ForestName -attributes "objectguid" -baseDN $configDN -scope "subtree" -filter "DistinguishedName=$nTDSDSADN"
        ...
        $tasks += [SmbVersioning.SmbVersion]::Check($dcHostName, $dc.DistinguishedName)
    }
    ...
    foreach ($result in $tasks)
    {
        ...
        if ($smbVersion -eq "SMB1"){
            ...
            $thisOutput = [PSCustomObject][Ordered] @{
                ...
                HostName = $hostName
                State = "SMBv1 is enabled"
            }
            [void]$outputObjects.Add($thisOutput)
        }
        elseif($smbVersion -eq "Unreachable"){
            $domainControllersFailedCount++
            $unreachables++
            $thisOutput = [PSCustomObject][Ordered] @{
                DistinguishedName = $dcDN
    ...
    if ($outputObjects){
        ...
        if($scoreOutput.Score -lt 100)
        {
            if ($unreachables -eq 0 )
            {
                ...
                $res.ResultObjects = $outputObjects
                $res.ResultMessage = "Found $($domainControllersFailedCount-$unreachables) DCs with SMBv1 Enabled, And $($unreachables) DCs are Unreachable."
                $res.Remediation = $self.Remediation
                $res.Status = 'Failed'
                $res.Score = $scoreOutput.Score
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCncndu56QWKSZ9
# /PMqz+BhNwjHSJpYhjDeUOP6LhAQ4KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg