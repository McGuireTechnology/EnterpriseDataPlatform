# This script checks SYSVOL for recently changed executables
...
$failedSYSVOLCount = 0
$scoreObjects = @()
$sysvol_extensions = [System.Collections.ArrayList]@()
$test_extensions = [System.Collections.ArrayList]@()
...
# Which extensions will we look at
$extensions = "BAT","BIN","CMD","COM","CPL","DRV","EXE","GADGET","INF1", `
    "INS","INX","ISU","JOB","JSE","LNK","MSC","MSI","MSP","MST","PAF","PIF","PS1", `
    "PSM1","REG","RGS","SCT","SHB","SHS","U3P","VB","VBE","VBS","VBSCRIPT","WS","WSF","DLL"

$extensions | ForEach-Object { $sysvol_extensions.AddRange(("*.$_", "*.$($_).DISABLED"))}
$extensions | ForEach-Object { $test_extensions.AddRange((".$_", ".$($_).DISABLED"))}
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # DETECT SYSVOL UNREACHABLE
        try {
            Get-ChildItem -Path "\\$domain\SYSVOL" -ErrorAction Stop | Out-Null
        }
        catch {
            $failedSYSVOLCount++
            ...
            $thisoutput = [PSCustomObject][Ordered] @{
                FileName = "N/A"
                FileDirectory = "Unable to access SYSVOL for $domain"
                GPODisplayName = "N/A"
                EventTimestamp = "N/A"
            }
        ...
        $GPOdict = [System.Collections.ArrayList]@()
        $getGpoforDict = Search-AD -dnsDomain $domain -attributes "displayName","cn" -baseDN "CN=Policies,CN=System,$DN" -scope "Onelevel" -filter "(objectCategory=groupPolicyContainer)"
        foreach ($gpo in $getGpoforDict) {
            $attr = $gpo.Attributes
            if ($attr.displayname) {
                $display = $attr.displayname[0]
            }
            else {
            ...
            [void]$GPOdict.Add($gpoOutput)
        }

        @(Get-ChildItem -Path "\\$domain\SYSVOL" -Recurse -Include $sysvol_extensions).foreach({
                if (($test_extensions.Contains([System.IO.Path]::GetExtension($_.Name.ToUpper()))) -or (([System.IO.Path]::GetExtension($_.Name.ToUpper()) -eq ".DISABLED") -and ($test_extensions.Contains([System.IO.Path]::GetExtension([System.IO.Path]::GetFileNameWithoutExtension($_.Name.ToUpper())))))) {
                    # do the work
                    if ($_.CreationTime -gt $startSearchWindow -or $_.LastWriteTime -gt $startSearchWindow) {
                        $failedSYSVOLCount++

                        # The executables may or may not be within a GPO folder.
                        # If they ARE, we get the GPO NAME
                        if ($_.DirectoryName -match [regex]::Escape("\\$domain\SYSVOL\$domain\policies\{")) {
                            $thisGPOGuid = "{" + "$($_.DirectoryName.split("{")[1].split("}")[0])" + "}"

                            $GPOMatches = @()
                            $GPOMatches += $GPOdict | Where-Object {$_.CN -eq $thisGPOGuid}

                            if ($GPOMatches) {
                                $GPODisplayName = $GPOMatches.DisplayName -join " ; "
                            }
                            else {
                                $GPODisplayName = "GPO NOT FOUND IN ACTIVE DIRECTORY"
                            }
                        }
                        else {
                            $GPODisplayName = "N/A"
                        ...
                        Update-IndicatorOutputAttackWindow -StartAttackWindow $StartAttackWindow -EndAttackWindow $EndAttackWindow -ThisOutput $thisoutput
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) SYSVOL executables that have been recently modified."
            $res.Remediation = "Ensure all recently created/modified executables are safe."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC