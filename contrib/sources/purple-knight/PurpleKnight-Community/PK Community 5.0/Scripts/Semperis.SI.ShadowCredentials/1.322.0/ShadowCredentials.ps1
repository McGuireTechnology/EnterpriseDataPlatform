...
try{
    ...
    $searchGUIDs = @{"5b47d60f-6090-40b2-9f37-2a4de88f3063" = "ms-DS-Key-Credential-Link";
        "00000000-0000-0000-0000-000000000000"="All Properties"}
    $rightsFilter = "GenericAll|WriteDacl|WriteOwner|WriteProperty|GenericWrite"
    ...
    foreach ($domain in $DomainNames)
    {
        ...
        $adminsAndErrors = Get-AdminsInDomain -DomainNames $domain -ForestName $ForestName
        $admins = $adminsAndErrors[0]
        $notEnumerated = $adminsAndErrors[1]
        $largeGroups = $adminsAndErrors[2]


        $filter = "(|"
        foreach($adminDN in $admins) {
        ...
        $ignoredSIDs = @("S-1-5-9", "S-1-5-32-544", "$domainSID-516", "$forestSID-498", "$domainSID-512", "$forestSID-519", "S-1-5-18","S-1-5-32-548","$domainSID-526","$forestSID-527")

        $scriptBlockFilter =  {
            param ([System.DirectoryServices.Protocols.SearchResultEntry] $Entry)
            Update-NTSecurityDescriptor -Entry $Entry -PropagationFlagsFilter "None|NoPropagateInherit" -AccessControlTypeFilter "Allow" -RightsFilter $rightsFilter -ExcludedSIDs $ignoredSIDs -GuidHTFilter $searchGUIDs -InheritanceTypeFilter "None|All|SelfAndChildren"
        }
        ...
        if ($results)
        {
            ...
            foreach ($result in $results){
                $dnOfTheUser = $result.DistinguishedName
                $acesOfUser = $result.Attributes.aces
                $usersWithWriteAccess = @()
                if ($acesOfUser)
                {
                    foreach($userWithWriteAccess in $acesOfUser)
                    {
                        $usersWithWriteAccess += $userWithWriteAccess.IdentityReference

                    }
                    $usersWithWriteAccess = $usersWithWriteAccess -join ", "
                }
                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName=$dnOfTheUser
        ...
        foreach ($dc in $dcs)
        {
            $dnOfTheUser = $dc.DistinguishedName
            $acesOfUser = $dc.Attributes.aces
            $usersWithWriteAccess = @()
            if ($acesOfUser)
            {
                foreach($userWithWriteAccess in $acesOfUser)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBIRCNneUYkXWom
# 6zNFXJ0tDmPiJVtNxSqkIH5Iok0CQaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgxQArcQ3G4TxQa8Tqw7BRDabsMIVfKHIs+ywjihTYzX0wDQYJKoZI
# hvcNAQEBBQAEggIAfjQmAlLPzb/wb5j1eiLxHW+l7pUVogpUj7Gtl8yKEKfn/RNK
# AhvBqyHUXhtSrOHhCVWCoXxysc6HU7pfGvpOy9EUQx/FfXxsW9jjnTgOJ89TsS1Q
