...
$Global:self = @{
    ID = 236
    UUID = 'baeb89cc-e32d-4f17-ad37-29a6a45ff724'
    Version = '1.291.0'
    CategoryID = 2
    ShortName = 'SI000236'
    Name = 'User accounts using Smart Card authentication with old password'
    ScriptName = 'SmartCardUserPasswordNotChange'
    Description = '<p>This indicator looks for user accounts that are using Smart Card authentication whose password has not changed in the last 90 days.</p>'
    Weight = 6
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 6
    LikelihoodOfCompromise = '<p>A user may use a Smart Card for authentication instead of a password. By default, accounts with the "Smart Card required" flag enabled have disabled password rotation, which may weaken your organization''s password rotation practices. This static password hash is used for signing Kerberos tickets and if compromised may lead to credential-theft based attacks including Silver Ticket attacks.</p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/powershell/module/activedirectory/set-addomain?view=windowsserver2022-ps" target="_blank">Set-ADDomain</a></p>'
    ResultMessage = 'Found {0} users using Smart Card authentication whose password has not changed recently.'
    Remediation = '<p>Implement regular password changes for users. Additionally, for Windows 2016 or later domains, leverage the msDS-ExpirePasswordsOnSmartCardOnlyAccounts attribute to enforce periodic password hashing even for smart card users. This enhances security by mitigating the risk associated with static password hashes.</p>
      <p>To enable this attribute, use the following command (to enable the attribute msDS-ExpirePasswordsOnSmartCardOnlyAccounts):</p>
      <code>Set-ADDomain -Identity DomainName -PublicKeyRequiredPasswordRolling $True</code>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
        @{ Name = 'DistinguishedName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'SamAccountName'; Type = 'String'; IsCollection = $false },
        @{ Name = 'DaysSinceLastSet'; Type = 'String'; IsCollection = $false }
    )
    Targets = @('AD')
    Permissions = @()
    SecurityFrameworks = @(
        @{ Name = 'MITRE ATT&CK'; Tags = @('Credential Access') },
        @{ Name = 'MITRE D3FEND'; Tags = @('Harden - Credential Rotation') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $attributes = @("samaccountname","pwdlastset","whencreated")
        $searchParams =
        @{
            dnsDomain = $domain
            ...
            baseDN = $DN
            scope = "subtree"
            filter = "(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=262144))"
        }
        ...
        foreach($result in $results){
            ...
            $formatOfDay ="yyyyMMddHHmmss.0Z"
            $userCreated = ([DateTime]::ParseExact($($result.Attributes.whencreated[$GET_STRING]),$formatOfDay,[Globalization.CultureInfo]::CultureInfo.InvariantCulture))
            $timeFromCreated = ((Get-Date).ToUniversalTime() - $userCreated).Days

            if(($result.Attributes."pwdlastset"[0] -eq 0) -and ($timeFromCreated -gt 90)){
                $thisOutput = [pscustomobject][ordered] @{
                    ...
                    DaysSinceLastSet = $timeFromCreated
                }
                [void]$outputObjects.Add($thisOutput)
            }
            elseif(($daysSinceLastSet -gt 90) -and ($timeFromCreated -gt 90)){
                $thisOutput = [pscustomobject][ordered] @{
                    DistinguishedName = $result.DistinguishedName
                    SamAccountName = $result.Attributes."samaccountname"[0]
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC