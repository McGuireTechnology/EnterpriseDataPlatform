# Trust account password wasn't changed in the last year
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>Trust accounts facilitate authentication across trusts. As such they should be protected just like privileged user accounts. Normally trust account passwords are rotated automatically so a trust account without a recent password change could indicate an orphaned trust account.</p>
      <h3>References</h3>
      <p><a href="https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf" target="_blank">NIST.SP.800-63-3</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_trusts_accounts" target="_blank">Trust account passwords unchanged for more than a year | ANSSI</a></p>'
    ResultMessage = 'Found {0} trusted domain objects whose password has not changed in the last {1} days.'
    Remediation = 'Old passwords on trust accounts usually indicate the trust is no longer valid. Verify that the trust account is no longer needed and then delete it.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        foreach ($result in $results) {
            if ($result.Attributes.'attributemetadata') {
                ...
                $daysSinceLastSet = ((get-date).ToUniversalTime() - $lastChanged).Days
                ...
                if ($result.Attributes."pwdlastset"[0] -lt $temp.ToFileTimeUtc()) {
                    $domainFailCount++
                    $lastChanged = "Enumeration failed."
                    if ($result.Attributes."pwdlastset"[0] -ne 0) {
                        $passwordLastSet = [datetime]::FromFileTime($result.Attributes."pwdlastset"[0]).ToUniversalTime()
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) trusted domain objects whose password has not changed in the last $daysToRemove days."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAgHqMRoMglTPIn
# G98b+XGBj5Yd6KTyUsQcklE1v1UZ2aCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgtmq5AgzOKiqNs4y631UvRFtGDIsX1iagSA3PQP2rX+wwDQYJKoZI
# hvcNAQEBBQAEggIABIrF4s914pJtng/V5z0D0eFNDvBma1mYY6WHc+JxbkzTlf3z
# +mhqL+v8eCMYPiDn131V+JGIrJEdbYLZ3q1RNEw+baoqpZA6Ah2nAke88cVU6F0+
# rMjHwDIhqAxcRp5Nv3cwASfgpxSpR2E86eA2t2IeFmBKVsSeK3tkn09JhV912Ye8
# t2fqJbrRMDKNpVRZYNC1ItQ88XRr9L76zyO2kI4d+P+HzbtYYN2RM6o5G/yCVYBc
# HAsd1UgYgYrEla5nVeONTH96mRp+Z9KBaZeJ0h8WWFjbdR+rf3fy3QHlnM1KNAtX
# ICzs686VEm/+Gp6qd7L1J3pZwxIYcPjVYDZIwXgGcpiQkvtdOMhtbsWG1YOvXza2
# jHRemn8ZkEGg+IpKupd5G4Gap034sSXLKgFvC4V6uRLt60UtyACLXeNhX/VO+U3C
# OLOCQr+xfQvpWIXII4vVrdiz/A2IS+5K/qqvSL7Qu2oa8hoS3CEB+eqnc4TKb4IC
# Eg6j/ik8OuY3w3FLGqRPdXGG+Z75dN5m6c7/FUDkIb+TkI82W4ZqRPTIQ+SETBcB
# glwrZSDDim2f7GFv3N8cfD+KTcsnEGompDUjgc2i2otlMt3+wWGWdkuAFfifacCQ
# JP1DP/iLnEgQ+LV/i6S1+sM5YoK+SN3P6tM0TTYTkec301eys9dasxU+yQ6hgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDIdc4hAfNfCiho9k8I4OBMnqmrxU0IYPSi
# 6i9TqjILhQIUYe2toKYKNQAnhqsZIy5sCXqkQagYDzIwMjUwNTI4MTI0MjM3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBjGQ1wTSxkmiZgdvotCxPFkmLlYoaS1yToOYya/Bvo
...
# CwUABIIBgGe6h7suvmfADoLLijbWYRRFy6eTjGk+iyf4EsO31WYn2X77o1F8s1ZA
# PyB8fqTYg+ghoOWnDEDFDmkca/3n0E38V82YcaI6gonw0XiuAX33VvVGecYl731J
# p2r9URuSh1IXOwYy3K8Zt9IbKUFeDdRZaf03XQhOd0GZLvgyOAfcPYoACcqRW4Km
# ZDn9TB3lohWB5O5FX1ElUKR2BKOoIrWEAYUacEeEKIdnBsYnzlLXwMiHVJSRjROv
# iG2aMGW2wpMvQ3P0DnfVNLzH4JdSZOLA+2aHbdNVeic59chn+Sq60WwU8D3rHLiJ
# 5CiPm4UsRi0aYQXmWYhb3ADTZ4kL/oD484amI3NCoFjXEbAYssdPNWz5BtV8W07q
# NPFEnq8qAq2b8P+/Dg1O93hDATRUZni7i+vWTKxGWdo7/Hj9D1U3ZYmt7Vxge4rB
# hNCecPVks8C7Yc6nPIkfii7+QXGsl3AA9SuxKLMtFyfjxCh5OMvIRoF6OMd6Mzrp
# ddNjyiZ3jw==
