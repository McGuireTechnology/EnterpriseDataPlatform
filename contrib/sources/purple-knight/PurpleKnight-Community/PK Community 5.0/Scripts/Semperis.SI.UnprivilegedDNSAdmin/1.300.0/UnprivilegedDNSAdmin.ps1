# This script looks for members of the DNSAdmins group that don't have admincount=1
...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>Administration of DNS is often delegated to non-AD administrators (i.e., administrators with job responsibilities in networking, DNS, DHCP, etc.). These administration accounts may not have the same security controls as the AD administrator accounts, making them prime targets for compromise.</p>
      <h3>References</h3>
      <p><a href="https://www.semperis.com/blog/dnsadmins-revisited/" target="_blank">How Potential Attackers Can Achieve Privileged Persistence on a DC through DnsAdmins | Semperis blog</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_permissions_msdns" target="_blank">Dangerous ACLs expose MicrosoftDNS server objects (attack path) | ANSSI</a></p>
       <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dnsadmins" target="_blank">Dangerous permissions on the DnsAdmins group | ANSSI</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        foreach($admin in $admins) {
            [void]$adminsSID.Add((New-Object System.Security.Principal.SecurityIdentifier @($admin.Attributes."objectsid".GetValues("byte[]")[0],0)))
        }
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get the DN of the DNSAdmins group to be used later in the filter
        $dnsAdminsDN = (Search-AD -dnsDomain $domain -baseDN $DN -scope "subtree" -filter "(&(samAccountName=DnsAdmins)(objectCategory=group))").DistinguishedName
        ...
        $dnsAdmins = Search-AD -dnsDomain $domain -attributes "msds-membertransitive" -baseDN $dnsAdminsDN -scope "base" `
            -filter "distinguishedname=$dnsAdminsDN"

        # Group has no members
        if (!($dnsAdmins.attributes."msds-membertransitive"[0])) {
            continue
        }
        foreach ($dnsAdminDN in $dnsAdmins.Attributes.'msds-membertransitive'.GetValues("string")) {
            try {
                $dnsAdmin = Search-AD -dnsDomain $domain -baseDN $dnsAdminDN -attributes "objectsid", "samaccountname" -scope "base" `
                    -filter "distinguishedname=$dnsAdminDN"

                $SIDobject = New-Object System.Security.Principal.SecurityIdentifier @($dnsAdmin.attributes."objectsid"[0],0)
                if ($SIDobject -notin $adminsSID) {
                    [void]$results.Add($dnsAdmin)
                }
            }
            catch {
                [void]$failedObject.Add($dnsAdminDN)
            }
        }
        if ($results.Count) {
            $failedDomainCount++
    ...
    if ($outputObjects) {
        ...
        if($scoreOutput.Score -lt 100)
        {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) objects which are not privileged but are members of the DNS Admins group."
            $res.Remediation = "Remove unprivileged principals that are a member of the DNS Admins group."
            if ($failedObject) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAiBmHBWV0sI9vI
# CaB3qllPEjtbpiygXYBK7Vz1lrT/j6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQ