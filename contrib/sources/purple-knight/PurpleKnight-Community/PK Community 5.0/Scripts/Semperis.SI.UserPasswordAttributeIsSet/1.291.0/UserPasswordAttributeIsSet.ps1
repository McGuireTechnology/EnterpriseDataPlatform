...
$Global:self = @{
    ID = 339
    UUID = 'dfa45647-c2b5-4066-b7db-3a8890868cef'
    Version = '1.291.0'
    CategoryID = 5
    ShortName = 'SI000339'
    Name = 'Users with the attribute userPassword set'
    ScriptName = 'UserPasswordAttributeIsSet'
    Description = 'This indicator checks for the attribute of userPassword existing on accounts.'
    Weight = 5
    Severity = 'Warning'
    Schedule = '1d'
    Impact = 5
    LikelihoodOfCompromise = 'The attribute is saving passwords in clear text and can be queried using LDAP by everyone, which can potentially expose passwords.  This attribute is not supposed to be visible but can be queried using LDAP. These days Active Directory is using another attribute named unicodePwd (unless the heuristic fUserPwdSupport is set).A misconfigured application can change the user password using this old mechanism and change the userPassword attribute which, and as a consequence, set the user password in clear text. </p>
      <h3>References</h3>
      <p><a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/f3adda9f-89e1-4340-a3f2-1f0a6249f1f8" target="_blank">3.1.1.3.1.5.2 userPassword | Microsoft Learn</a></p>'
    ResultMessage = 'Found {0} users with the attribute of userPassword set.'
    Remediation = 'It is recommended to remove the value from this attribute on all found users, Unless required by the environment.</p></p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $attributes = @("samaccountname", "unixUserPassword","userPassword")
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchParams = @{
            ...
            baseDN = $DN
            scope = "subtree"
            filter = "(&(objectCategory=person)(objectClass=user)(userPassword=*))"
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBejk1R1gYSfDNG
# 7h+ukvkqmZt2slqVrbYCG0BdR8WL8qCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgCiS2dXVVztlGJpbJR//bbAUyd/KQiHUVXHF7sutxYKEwDQYJKoZI
# hvcNAQEBBQAEggIABOmaJ1JPqeFjNulhI5lVN+GhD+medGhWr7dYXGh+o+VvmY7r
# UZlEXYl8uuUNJETXqoEWxyfDSR4MNFxe49d4bqdfjctIvkR86A+ogOqyyPLvLGX+
# FZHEdv/HLhXWEpC7NCKDOwcZkzlNlB7fDqDyPfVEseda9COmc2j4IIpcTxvQdWkb
# s3DY2AMJRYfLLvp2sb/MHQmb74GwUdoTkXuEd9xnL6GYJXkSeW4zi8Np7lY9QUnV
# lTh3FOTpbBJUiAdoLOrfJyMCftLRswrlL8jAO5KWcPBnL6FGnIb/1qWR4zlNIJBv
# 5SNXuLFD4MDYbfYuuhnm/lOjEu1oQTpmSZteMSkt/2fG9+Ct3gu/kfidBvnEnETG
# I5CNYbnllC4EXABjCro8KgCO3XgPNlQ1esJU4LxEcxsLi0mAO+o2Mpz8iWPZ8W7u
# jYUE5yf9+25Al5zU/eZsJLVYpL77QgLPGfRNVqEWJJnEz4AP0mBKHMsn0b7Xv7lP
# QbmZweEiTfH8PWZZSaw7KUo+ZqhwbaFcCdQ6hk/eGhlrHjtQWmm7MZoGOZrzn5FL
# THEyMDw59fbTs2dwJ4znaJ57h5MxCYVEZauQczvlsoPcWTIj7bLIjeheJdonD4C6
# 5CIL7Tze+6NBC/jQXhfi7z1QE243LJEqIjn9zm0yE1HPAxnt17RYdqvqBtyhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBp7YtKY647eYQmAz/O30BOEOA7cuBalTE2
# BXuFzHDDuwIUL60rDb9p/XepXUlLN7qJc0Jb/yIYDzIwMjUwNTE1MTIzOTAzWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCD5o9U41qZ6DUNqtZ6qC236CnRBnYdmEs8uDnBDeXpS
...
# CwUABIIBgDUMn4JImCJvrJZyfD8TEf21W6JrqwZN/FwjOkPGniDNkbqPxM0yCGes
# RLWQlgoP7f9YEOa38b8Dw2XBXnVdLZx/X9nqJOI4uPpD8b+n03rbzgStuj4kj5QF
# gANnYeg+CHt9zS4Ea+2G8SSJ61yI04Icpv3ZLsOk9CX0AZ02/IFgzbjPHTf6ok0F
# pLRfOd7rjei8q6+zj9iq//HLL6NSlRk1WV3YR2r7D6MBzFAqyvJc6UknroVQachl
# xquuCy54kfAKhyUDZ5/lV2YOARiH5j56SISm0rD12CKNmBm5BFBenGlqVtQAoTfj
# LJty841NQYEi+vdOdqbr+sVwjpjTOHBlYXZjd9UEBx5QmjaCWovdN8vHPb7uqd4P
# 6/l35Tn6dftq66ycW1KhCq4ndVePy+CPQX8160KcE0Kr1Gd8v+nhoAh4YiclIQkf
# /7T+AOSKmlq+mV4BhZe2U4FCAOGH35aE+PZHVgHW572kOm5CbtrJ2dA5OLZf6Ojh
# g7xjDjDF4Q==
