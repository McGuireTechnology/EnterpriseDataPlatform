# This script checks each domain to determine if normal users can add machine accounts to the domain
...
$Global:self = @{
    ...
    Schedule = '1w'
    Impact = 3
    LikelihoodOfCompromise = '<p>The ability to add computer accounts to a domain without restrictions or monitoring present opportunities for attackers to add their own accounts or take advantage of uncontrolled computers with vulnerabilities, thereby extending their reach and entrenching themselves in the environment.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_user_accounts_machineaccountquota" target="_blank">Unrestricted domain join | ANSSI</a></p>'
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln4_user_accounts_machineaccountquota') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $results = Search-AD -dnsDomain $domain -attributes "ms-ds-machineaccountquota" -baseDN $DN -scope "Base" `
            -filter "(objectClass=domainDNS)"

        if ([int]$results.Attributes."ms-ds-machineaccountquota"[0] -gt 0) {
            $failcount++
            $scoreObjects += 0
            $thisOutput = new-object psobject -Property @{
                DistinguishedName = $results.DistinguishedName
                MachineAccountQuota = $results.Attributes."ms-ds-machineaccountquota"[0]}
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.Count) domains in which regular users can add computer accounts."
            $res.Remediation = "Set the ms-DS-MachineAccountQuota attribute on the domain NC head to 0 to disable regular users' ability to add computer accounts."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCzEyyhrFUM7KKQ
# HmIAXkEXxovcut7a/6IZ3N5f922/J6CCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQg6UdmWg2b88HhL3F4HDaN8bXqdNO9cH+x3msYVYBUVPgwDQYJKoZI
# hvcNAQEBBQAEggIACfVPSapTnSRmRn23ZEDZeMktEKae9EC1LWOqSJsgs6cstWGU
# UTnI36gwD+v2chYA803+K8itDQNhl2u9/BqqbKOL9kK0/eYHkEabkyXP8MBIyDoE
# gJoPveXgxb0NUvnqv7XWt2cfs56aw54ru8pYpfVAd8u13MGtmknjULhtvNd27MWj
# kc/EPf7h+NZiXcRyTwlWPeKIqr+TjfzjfjimUMtZLU45RUw8mnd/TXcUKNHEp36v
# 9UNSLhlCeJKRDV2E/x/qI2MFLs1JS6WWOgJkv2ShcC++P9kxy6BVSv3rUmZbBWmU
# IaHLD9pgV8IM3Xr5u3qGDn0T+iVtns5GX6CRpb1SqLfGWRehGJtWqqVBwYtugEbu
# pxgJpkyo4m/qD/19ZL117bu72eW7WkEmQiJn8HBLr6BTwgomOBcDEsF8/i6hVZUb
# 0sYVJrFLWSemliqbPGBd3+0/W+apwn0ezxjv/+0Znb/nnn991bDw62Vwz5jHvQZJ
# d5YyvZmWvL/K1cT793Ez5PiDVOEH1PlzUs2CzmJsJZiWUY7WtapbqGWm5LUMzAWm
# thpF90kXXuzCuR7wSdorD9cQTCloX/E89gZt6yoWl+ARNT5bEjZmoDLsDr37D4/O
# PohlP8l6izA8w6sdCE81Cu8A/aJmY1P1DrDwcMTS1AnwrVYl9uwK9I59S2yhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCA37BQNR21IvaJ4YbFGhtjN10MFAvFEI5ON
# rzVtYcq1RAIUBDG8bM1AkkfPLNdH1OGiazF3IF0YDzIwMjUwNTI4MTI0MjU5WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCD2pAxjdwoDkxlBAX1rk4tZ5fS9HIIy4gwk/1hxZwHY
# BDCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgFkNmvQDbyjphIb4mkUEfLSMR6qg/ibbtWwN9zXpsPr4tWrXrQr+pO3n
# MizvmWTpXKmd1qI5Ye43kzsq9x0LO9+L0vZfT+X+UpsxRpJSTP6A572HmnfZZwl3
# V3jfBzmJB3DT9cNAAniYU3IpIseCIqog2RtAafY9RMpeFlg9LoUhQGaHZfk2zaao
# 4rvkd3aj1r6cl0x81MztWbgOPGQQsE3qqNc15zvQ/Ctb5HA75UcYrbWU0qr36DvV
# MvcPnKGooujOdWsU4oNgn7hG23CtGiHq/cX6Kr6BptW7Mx6vG2Myk9Mq2HgnZ/Ug
# URQOin3KU1R3zwgZJ79tryXlvsEI/cER093M/9cvQzk3JEQflTXVoPP29bYGCda4
# NOGZ260Ur6BUrhUl3dpyCt+HvEImYJveY4PqsIpCt7GjBPN+BPhs3ajXuFP/pKOM
# K6H0f/Vj+uh7vmZmYP8lwtqt+22O9XSlwDOyxr6BBobZxsRPQ4QaUHf4YTkjZAiz
# EjqBIYQbng==
