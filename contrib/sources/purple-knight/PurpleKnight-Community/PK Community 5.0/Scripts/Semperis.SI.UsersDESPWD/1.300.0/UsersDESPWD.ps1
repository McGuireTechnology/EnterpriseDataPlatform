# Users with USE_DES_KEY_ONLY User Account Control flag
#This script checks each domain to determine if there are any user accounts that have USE_DES_KEY_ONLY user account control flag set
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '3d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Attackers can easily crack DES passwords using widely available tools, making these accounts ripe for takeover.</p>
       <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_kerberos_properties_deskey" target="_blank">Use of Kerberos with weak encryption | ANSSI</a></p>'
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=person)(objectClass=user)(!(UserAccountControl:1.2.840.113556.1.4.803:=2))(UserAccountControl>=2097152)(useraccountcontrol:1.2.840.113556.1.4.803:=2097152))"
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.count) users with USE_DES_KEY_ONLY flag set on their User Account Control value."
            $res.Remediation = "This flag represents a potential weakness in user accounts, which if left in place, could make these accounts targets of takeover attacks. It is strongly advised to move to AES-256 or AES-128. If the flag is required, ensure that these accounts have the least privileges possible and are closely monitored."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCYTra1CWafx9fF
# krTyKHXjvq6m1VZfSg/Jx6ZO2yYaaaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgcNhJ762lsbomJXe+N+OpW5bWbTRRMok/XOyJjOfP+f8wDQYJKoZI
# hvcNAQEBBQAEggIARzVCC+fj+NJXc17qtoKT6hpa2jMJPaTj9+eeDHTkF8ZQraIB
# T6cxGLDB04cvLZUyJSMHyND6Yd/TSMHjTJiFV5PGMdcD/6qd1cdoJIxI9CYpnCc8
# yvSGexhTcxG01pbtpBEcJlYt0WlP+ONd53YsrWCVMABeESrqug7ACtrDRz34lFW7
# IWsS0Hd2biT1/j3bzRUw6VDIdQJ5tcKN8CKo9Mi+0evc9Tv+XJY9lDp6p1C0dklt
# Lt9AANQHgtjoaMhanNSlBY+Rd6hG09wf5QgpEqFcqmInBmYSOCZMNjG2XoO1QRxE
# kTXz0YysCZSDzUEVmgCO0zgCJ5kOXvVubqDMeSKYzf1k4CGtNIBBeNAotPkvide1
# 94vXtnRH9CNw8TQ1980JfT7cHbBI36xAKGwWO93jcBjNN6FtdMEb4QQ5gslQhj7/
# ie13VoWE0M5ufHBJTCJizLQq4eaEKBwDeDb0jV2P6Mqq+A4CJ+SlRiS1itgJE43v
# aMlZozgAPa8cMgB5uAZCIxZWML20kmwNqQeXQxczbCfSrG93UZ6JSMH/AKxh6XnT
# k7uLe+EtHO9UNAvf5G3CdfCVrnSjXJkxfTYnGDC+rs+ZSfY/xlAkpX6inMKW0fyq
# 5DDeLHlYwuYWrZdIRPIS5z5mlvxam9FHVLaZzH9G4hQg0CBN9/9zzg7w7Eihgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDt5N6PnORw16RR94+4g4RxHzHtR9M3xN1S
# WtfHqRfRbwIUE95G/0kuRQ1lyV/gQlgzCiKsO+UYDzIwMjUwNTI4MTI0MzA1WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCBRlwpc1Wtf1JXLPY0jpGw/p+pLLF8zylKz7YNwMYR
...
# CwUABIIBgDF7Fo/kG078ej2PAfq4bOllXv7kh7QbeWkcHT0hLLyM7kPlQa8h8XwV
# EkCCJj0qiXyQUd+/sZWMDxRbQftTn5kcPyOEkVex97ea1Qnvwoucy9hHo3go19wF
# Tf37B6pSq7/DTV0ovLRuG6ZYJ4SK72fMTWeRMVPQh4F8HSpXA5J5O9Mt5jfYoQgP
# PLRFH0CtL8s8kTIIhmkPQ5Xra5o864Kbp3heuuqsVnOMcKlz0rGKWEaLPhe7Z5tP
# OnCMQE+R//mqeO5f9v2w7CjPJ5waxVjMOr581GkiHejPEk1AYr3N0Vvs0hXkhhmY
# 4b/AsicBMcgF2rdwSYryXgLvcwJAvaReH6tJW4zr4exFh7ioGOxUNR2g03VB2Y3k
# 4BtuNYsux6MVbzoUaPlQyFioTBhe91an1t1IT8GozePsN8vwTVHit8ER41qVMDMD
# dwIG30GTDlGFJ44hE1xZkLyNP4HSYEObfpCK54YA9qPjLwz95SdAq+QOVXM08vg/
# 4hmpehF0Ow==
