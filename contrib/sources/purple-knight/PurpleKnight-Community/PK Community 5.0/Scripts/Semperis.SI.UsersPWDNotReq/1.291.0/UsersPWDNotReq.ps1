# Users with PASSWD_NOTREQD User Account Control flag
#This script checks each domain to determine if there are any user accounts that have PASSWD_NOTREQD user account control flag set
[CmdletBinding()]
param(
...
$Global:self = @{
    ...
    Schedule = '3d'
    Impact = 6
    LikelihoodOfCompromise = '<p>Accounts with weak access controls are often targeted by attackers seeking to move laterally or gain a persistent foothold within the environment.</p>
      <h3>References</h3>
    ...
    Remediation = 'This flag represents a potential weakness in user accounts, which if left in place, could make these accounts targets of takeover attacks. If the flag is required, ensure that these accounts have the least privileges possible.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=person)(objectClass=user)(!(|(UserAccountControl:1.2.840.113556.1.4.803:=2)(useraccountcontrol:1.2.840.113556.1.4.803:=2048)))(useraccountcontrol:1.2.840.113556.1.4.803:=32))"
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.count) users with PASSWD_NOTREQD flag set on their User Account Control value."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA7eNEX9J+3jX6z
# TI6gL8KzwhCQOO4Yb5ivgl7ugImacKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQgKOCjtUBgMUW6Ubfl4PhLoHBzPalYzSrdnC3uOyuBaswwDQYJKoZI
# hvcNAQEBBQAEggIAEkMNsAjZOGgma8wU4iepUkO0JRj1D2KfYAptXFf24mzizGrU
# DrZiFUcb4ITrggPuIny7OFXp8/bNT7dgkchu/hvl6Iwbng9HHN6BdMYObuU+TkEK
# wnb+cLQVFRwwAp9aBlDa+GbMyB0Ltap1jY4HoPCFSC5Gj277N/meQH69tYVH1Zq2
# 2RJsPdBrQPecXKfjPupitA06OitmiXl0yolHtKjqFyB+uQ+A5laFl8lPnWrV8YNl
# cRzMOLWUVRrMcfK0XrnsppG/wP/10/1A99cJYnXwRD94QJ3Maj4SSOkrkyG8UMXf
# ko06GGsqgpgszsSLx+8PYIu/5hqfSIbZ+UGyMHmSAZLm3IJHVElKTvrix6PQS0Xk
# X5drfU642TEMuka/WBGg7kdwp2puaHTF3wmBZnbeN3cqm2PYw2t2BWlJmt20/fXN
# VZ4nng/G/OQWO/KgN0igZb6+rFWNRZ2daPzIlPncwMM9wVoM+Oy2E3TihpxlJOdU
# ByBBWI2r3G/xfaZ5sE3VW4uI5Jrj5KdILaBqbYAhXtpZ9HUULypzLBZe2SSOxRVk
# ud0K+h4agn5zilnsemzM1wpV/wZIuQqZe0U8Izt6IdV8RawOb6BoLl5QgHoxo5h1
# qlfRVIHFiGPGtqh8k9WaLeWBPEui9YwcFi112xo2i+Ie5u8KeqJxq5cUWeGhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCAODVDmLX0M1skbrDR0oR+41BGbTdR1LZDo
# +Re+yoRmfwIUc3wd+l6XbiBo7SNuTQOW75L9fcoYDzIwMjUwNTE1MTIzOTIwWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCBwY5GUjNf7Ugq+/7F3404aiKIc1zqkf642fmyQoELb
# wjCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgGnoeuI1dWMhhKh2P0b6t+Fq/KjKxmqadTJC331zVRoJs8b6mP2zdYX2
# rp+L096O1dUlzFDHT0VxCi2C0aLgCXUBJZ7RUEWLmjFByFauc5Rh2d/yZGpz9lxT
# vglFEX6JPAF0Grrab4uqUCaYi/3YUXqr2zu6pvt8tjS8lT3JTdKY8cmX/ZTS9lEH
# X0uUJEmt/Z8rHwWynQF11CFd422VmT/YCDso00j99gPEeTiE2RUUYySf+wJo74DL
# qUJb0lTdpRkHFnLnXDmLuSlL3/6XpOTWWMhGlxnY6b29coFS9n/qrU7hlyTsPa/Y
# 01JoW8kVJro/XvQat+cJihvkJ3DBGgeTh5FExYoZAq26iwyqLMv8L8vhSvcokfOo
# uv3ZNx3y09JRvJj+7wmo2qhrpz5D4FDQunWT+nLl3Vn78dE+Kb12QOh1lqYy7d/+
# sGoNb/rh4ZxD4hljJVHd0tS0rkbuDL4AkAMF8nVjx80CwaHjajraVA3IpE8baIKc
# /jkjw6NRCw==
