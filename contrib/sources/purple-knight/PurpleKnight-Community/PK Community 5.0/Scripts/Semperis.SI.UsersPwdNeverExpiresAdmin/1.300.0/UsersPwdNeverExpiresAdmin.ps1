...
$Global:self = @{
    ...
    Schedule = '3d'
    Impact = 6
    LikelihoodOfCompromise = '<p>User accounts whose passwords never expire are ripe targets for brute force password guessing. If these users are also administrative or privileged accounts, this makes them even more of a target.</p>
      <h3>References</h3>
      <p><a href="https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf" target="_blank">NIST.SP.800-63-3</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_dont_expire_priv" target="_blank">Privileged accounts with never-expiring passwords | ANSSI</a></p>'
    ResultMessage = 'Found {0} users with password never expires.'
    Remediation = 'Enforce that users with privileged access must change their passwords on a regular basis and ensure that those passwords are complex and ideally require MFA to authenticate.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=Person)(objectClass=user)(userAccountControl>=65536)(userAccountControl:1.2.840.113556.1.4.803:=65536)(admincount=1)(!(useraccountcontrol:1.2.840.113556.1.4.803:=2))(!(objectsid=$domainSID-500)))"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDSqA6B58jf7yi6
# Ri4BNCVMOLuCvmtmZf25yYj8r02pMKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQghA3RTfeTsR636tqcCXM1eQrlyB39WxD+LlNIZbatkRUwDQYJKoZI
# hvcNAQEBBQAEggIAiJ1YB7fL7jB0duDAVSmVaLkVUNDsochxa7f9E3ck2cJY3Jzt
# BLozmND62dMpT/KBeEZ574xfn3/PK5YGKEGb/A6pTAvXZyulRBpRVwRnvYsO8hvo
# nrqmHBnIfcZOD0He95ttfcGSIf+a4hHfZf8ouQNcL8vF+rES+fkkiIzyXh4StpCV
# eyQ573/pUYu+Bmu2Y56nadTRsGQ8ybeXE1di+9M+bLiSeSRavlKz4ygDzZSHNa0d
# g4W6Hcmpb/ohN9nE2VOy+cHBxTGQ0mc/+evvfOlBpKhFho2XSAkrkGrl9yrXZg3s
# MRXNcAXZ/ZsiEppcOjjFvZOQYvY/SBaeXVu6z6Zfq/etf3yDO8eAJBueZbq/FkVj
# 3Tzl18tdC8492EWH7VQQBAPEWFTwnatxiGO3mlc0pwATdXEr4s69+EBl9IlfOPDq
# 7Bn0UQyPIR0Ulix+e0+KCcoiFrzl9mgOs21Tl+CKu94IvmQ0uTTT6574MBw9dIyi
# pXB5a7SlAV7QV/SVDPh3IpMvZJnqN9xF2t8qm8xMYazTuwJh2auTk2uE9EcxATqx
# 8x0OjCm8U9pOVZ62YQSHyV8iNiYZ70qvttFk22s6Mpb29S/8jubEtme+UnPMQ1x1
# 1KMPHn2e8X2uIdr3m9APKhHfLpHwSpyRb+8G2sBzOe+45WdZV+l9CL8ewAmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCB7xBdkMwT7UxGUYmg8Z+izylVJBhdfWzgm
# ctor9dDe5AIUbgGoViWU5rSxi3v9mh5mnI914n0YDzIwMjUwNTI4MTI0MzE2WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCDtpE0r2Fmb+WoXJX5q0AML6PiVa3knaGfgIZd822OF
...
# CwUABIIBgJeg3iyAgvFbapmzfBDqTUM7bWCkS4sHuIkf22QFJmxDD7agWJiT8jSW
# MIDhFDc2+jlKPtjJKI7sXAyOrOJL2pZRmR3K21K/6sfOcXSxFOMZUJmN+UPqMA9q
# shANX5o2w9xrDmX45HaJXvvLlRjhvsjqNzoVceNi5lJZJ208w/RttlRljgQmJrZe
# JUAKOu8e+rVueOcBRkzO27zSkNMkgj9x13/eCSLU3kg1xB+m3mahTO8gQ+kGKQNu
# 4Q0pc7xR+I2CqaaI+nJq/E26bBKZMYH9pUMXBxH7QcXl49xV6O1ykJql0emqpiIX
# rZ8ZluCdhG/4XzLcd32plFEbDVnDctZtwoblhTFWAooJsr09T2cupCGqgGBrN9Q5
# X1Rjyr3YJbvska3GWwQOzHO1V144zmEnT8aft8yUodK9uV0GNaPnj/48+4fhS4Jq
# Ps5bK2JkyTSpK5wJxt76m59YbyInnYqW7Vh+5zrZhDcC9SodTotmyQmT+sk+2fhH
# D9F+8fRzjQ==
