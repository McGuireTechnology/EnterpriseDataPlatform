# Users with ENCRYPTED_TEXT_PWD_ALLOWED User Account Control flag
#This script checks each domain to determine if there are any user accounts that have ENCRYPTED_TEXT_PWD_ALLOWED user account control flag set
[CmdletBinding()]
param(
...
$Global:self = @{
    ID = 76
    UUID = '2c62cbf1-ebdb-4ac4-9c5c-6507a15fd9e2'
    Version = '1.300.0'
    CategoryID = 2
    ShortName = 'SI000076'
    Name = 'User accounts that store passwords with reversible encryption'
    ScriptName = 'UsersReversiblePWD'
    Description = 'This indicator looks for user accounts with the ENCRYPTED_TEXT_PWD_ALLOWED flag enabled. The secure way of storing passwords is by utilizing one-way encryption where it is mathematically impossible to derive the original password from the ciphertext. This setting encrypts the source password such that it is possible to derive the original. This setting is used when an application or service utilizes authentication protocols that require the original password, e.g. CHAP or IAS.'
    Weight = 4
    Severity = 'Informational'
    Schedule = '3d'
    Impact = 4
    LikelihoodOfCompromise = '<p>Attackers may be able to derive these users'' passwords from the ciphertext and take over these accounts.</p>
      <h3>References</h3>
      <p><a href="https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf" target="_blank">NIST.SP.800-63-3</a></p>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_reversible_password" target="_blank">Accounts with passwords stored using reversible encryption | ANSSI</a></p>'
    ResultMessage = 'Found {0} users with ENCRYPTED_TEXT_PWD_ALLOWED flag set on their User Account Control value.'
    Remediation = 'This flag represents a potential weakness in user accounts, which if left in place, could make these accounts targets of takeover attacks. It is strongly advised to disable reversible encryption if no service explicitly requires it. If the flag is required, ensure that these accounts have the least privileges possible and are closely monitored.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
    ...
    SecurityFrameworks = @(
        ...
        @{ Name = 'ANSSI'; Tags = @('vuln3_reversible_password') }
    )
    Products = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $DN = Get-DN $domain
        $attributes = @("samaccountname", "useraccountcontrol", "managedby", "whenchanged", "whencreated")
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=person)(objectClass=user)(!(UserAccountControl:1.2.840.113556.1.4.803:=2))(useraccountcontrol:1.2.840.113556.1.4.803:=128))"
        ...
        if ($results.Count -gt 0) {
            ...
            foreach ($result in $results) {
                ...
                $thisOutput = [PSCustomObject][Ordered] @{
                    ...
                    UserAccountControl = "$uac [$( (Get-UACSet $uac) -join ", ")]"
    ...
    if ($outputObjects){
        ...
        {
            $res.ResultObjects = $outputObjects
            $res.ResultMessage = "Found $($outputObjects.count) users with ENCRYPTED_TEXT_PWD_ALLOWED flag set on their User Account Control value."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC1p8QoNYmMo13P
# 4e7q1zdbs1Fww6dBOphbA6y3FBr3maCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIg