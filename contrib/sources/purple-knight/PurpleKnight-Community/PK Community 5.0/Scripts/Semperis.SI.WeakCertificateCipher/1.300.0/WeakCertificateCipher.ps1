...
$Global:self = @{
    ...
    Description = 'This indicator looks for  certificates stored in active directory with keysize smaller than 2048 bits or utilize DSA encryption.'
    Weight = 8
    Severity = 'Critical'
    Schedule = '1h'
    Impact = 8
    LikelihoodOfCompromise = '<p>Weak certificates can be abused by attackers to gain access to systems who use certificate authentication.</p>
       <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_certificates_vuln" target="_blank">Weak or vulnerable certificates | ANSSI</a></p>'
    ResultMessage = 'Found {0} certificates with weak configuration.'
    Remediation = 'Problematic certificates need to be revoked and re-issued. Children certificates must also be re-issued. Expired certificates should also be purged from trusted certificates stores. When issuing certificates, the following requirements should be met: Use RSA or ECDSA for certificate signature (don''t use DSA), RSA key length is equal to or greater than 2048 and an up-to-date library is used to generate the RSA key.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    $searchParams = @{
        ...
        baseDN = "CN=Configuration,$forestDN"
        scope = "SubTree"
        filter = $filter
    }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) certificates with weak configuration."
            $res.Remediation = "Problematic certificates need to be revoked and re-issued. Children certificates must also be re-issued. Expired certificates should also be purged from trusted certificates stores. When issuing certificates, the following requirements should be met: Use RSA or ECDSA for certificate signature (don't use DSA), RSA key length is equal to or greater than 2048 and an up-to-date library is used to generate the RSA key."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBBcPsRS8MHJwLw
# hYo0fUKfVE9PRqa0e6/OQgLW7j1Z9KCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQga5CNPFTxPA95YZjkCvttETSZbBAyaw0tNWx3RhTncXwwDQYJKoZI
# hvcNAQEBBQAEggIAJcmjJb/kDehCBK/UYGqiMEgED+8vV30hB1QBpQzpelDCmi2w
# pgHY2CBvQ0YEr0UktMOQ8bcb37zXM4/XjjiNg31e0vuIHidlH3cDqkgiHanikZg7
# IYVxzb/gU3wjEVRFwJZid4zu93M61wJoxM0Y05B/8Yv7wvCm7kJZi5aUo6C2N0iL
# Ywu2+T8eXoFXAe0ng7MJ1DGzeiWMO0eLRfysqxJ87MJs0Kcze5m6bvdV728+jSwu
# PIKbIKy69b+WKGTueFJxjRS3f+3XQjzMf3EUBxVV1EsvIOU6O1MJfV0p+we+Eg4j
# Uwmr+iopIo/WPqg++VAtamAJ47OFc8ohMuhteqobcy4AletNvWddmXgrCNwx9YLt
# Wwb43t6btahDGu+QjZnpyU6tAsi3bSN7nbWOqyyd3Vr2GSfu8LYCuJNRImIuzyht
# 5kx3cd5OFvZnrJRuoXkooBqx8N5t++KJ0rdTEx95Cl/fZVhSMmpZpJbhubodV4P7
# 0uYk0HKjWse7xDgLvAEd+3FvOFTqOGSTMfcpSkbTa/u+s/STVrSblqeFTS1B4Jec
# 14jvm+YnBXBllanHQgWdKGbLZ8zqixoz6PZszdmZ003AbxCh2X1mcPafC1sxEcwW
# 9UkXRtcqg9+jJccF817uRGbNgLrfNZb4LtgWSuWrLSf/oJzmKYd5vsgoiLChgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCBdOf8DzKbSoEMo8y+/GH+amyFNSX7bKQOI
# LwGyljG2WwIUR8buqGuNWr5vDOURPcE3VoAJiU0YDzIwMjUwNTI4MTI0MzMyWjAD
...
# MC8GCSqGSIb3DQEJBDEiBCCOYfOd4mXAg5dASLW8NyEzPJ3xSRKRuK9Kuct16iGD
...
# CwUABIIBgIGZBEFlkoxhnyo9WmbAFSRvJFJBbT9A8a2pAbqKaEOMtVFqjnipsz45
# 20wA5YiYCKe4dPIZOnIwgaQHaj8s+/mk5JfbM7y383RIwkgDF5f4YPTpApMFQLNv
# SMldxx3Njqm13Nv0DrzKsD371td8zBwfA8dI76M9pFIkwO7Al9nnq9un5algpNK6
# K2EANbJBe06vo478TQslHJRR7vl64ihDCB0mK50cQTrW6pBeQP/i5TPZU1pJlkR+
# quX5PaieJ3xf3JjObZqPXj8RiXlQsPRvcdm/iAG8FWfaijByonurPMlae4q+4D1J
# pCTdkzxs+mgE+rz6KYHZhI4kfTGftqjNEUpaenQEyOvHEM3QrFYtEhEdFqtGpWjm
# eTrgMjKFT4zynNg4yr/T9354/rEJ/BmVTiwxf/vzprvpNcbcEP8r/F6w4kJrcWzO
# cs1KpSG9J8lGvjUPiZEfya8/H2mCGyuRpVch2rQ54uMa+Um/+/RNGl0Ewdk28Oxx
# pD726ie38Q==
