...
try {
    ...
    if ($DomainNames.Count -gt 0) {
        ...
        $results = Search-ADConfig -forestName $ForestName -domainNames $DomainNames -searchADParameters $searchParams

        $usersACL = @{}
        if ($results.Count -gt 0) {
            foreach ($site in $results) {

                if (!($site.Attributes.'aces')) {
                    continue
                }
                foreach ($access in $site.Attributes.'aces') {
                    $siteIdentity = $access.IdentityReference.Value + ";" + $site.distinguishedName
                    if ($UsersACL.ContainsKey($siteIdentity)) {
                        $UsersACL[$siteIdentity] += ";" + $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid]
                    }
                    else {
                        $UsersACL[$siteIdentity] = $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid]
                    }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.Remediation = "Unprivileged users should not be able to link GPOs at the AD Site Level. Doing so essentially gives them the ability to escalate their access, change domain-level security posture, and use GPOs to effect all systems and users in AD. Remove unnecessary permissions from the AD Site."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD3bcpqs3G1P+aT
# otG9y/xthkM59mxB/Qebm4UlnLbyTaCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQguG22OLzyLm3fT4jMRtylWo1TiOyBGUn/kM1b9i7MCgMwDQYJKoZI
# hvcNAQEBBQAEggIAemthStV58glZ5CZZ60ONpVvA4x1tfonCZZ51TIgv+w5Fu8uj
# 3BajqmeRVUcZ12a1jOuwn7CBr2Kxi/cx7h6MzYlWG8QEPLRoUxZ3IQn1ZKH1Y7TE
# vc/AdtoCeqaDNvFt5MADUaW/OqaA2Yvq3gbC9nDEIYdTtBhKvQdtUaeegdi+T8Hw
# 5cSwxB7/rXRx+roJw10CWtG+5/7hw8K5JmeKWZJ8ayKOlIjVJtlvSRlTtSpbXb9C
# DDkkT+27SSl4heKhjBbpH8rmqfTTj6h6vKRsR/8OSaxCYwvhvAEL98dADeFg9/nH
# ++e0UMHfD+F4OSEzHW+pg9i0OaA9HdwGg3Pr0COZf193hQ0VpUXXRfdIzpfN1Kfx
# RY45K0yMqOtkvp6ad7knPceHA1ZGhJyQSoJwE6SEqIafU28AXiuJllBuWXHlcT+U
# DQ0yQ6By7upLzw+mOAUQvA3TrH1SPiJGcfjWMuM73/1bcLLVNTA0HWlzem9oH+OC
# RhKHr6Yn8do2rPi0+uaYaGMfJhiGOkRekxeRlTOt/luSm2bqvjupSIsL2CfiouhE
# xwdM7WAuiei38MM5FZ40kMo119CvRjtSZBThKnAik7jvQl9FyqBngRTdOylQczZ9
# Y301E7tAtjd222T+/co+ln7c6yL4PU//4j9kllfq44DLwWCxvE4Zqhk8yjKhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCDNjET7vK5plRew0AHiRPjMxC2hp5b4MMOl
# EYX0SnMe/AIUUghyXMUMtDKDxS+Ojb/aCMXnilwYDzIwMjUwNTI4MTI0MzM3WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCAuvvOJd6j+ueFizb+DunxmWzn8yMbBW4MMFmBSugCz
# 7TCBsAYLKoZIhvcNAQkQAi8xgaAwgZ0wgZowgZcEIJGSR5tiNbl2Jr+2AW14CJGD
...
# CwUABIIBgGZaZCcgmWG8eGEAUzV6qZEMxZONloiyKDaw15Nz6t0pVR4Of/12OJKg
# WRKva3XUwPB93XccAiwbp0XBIDjaT+nm/L5jWk9TO6HhYhvSKBm6YrmBnUCMBfzs
# ACNG9wW5oi6QS7qOf+CUXM2CHxhDKBk1HFRMLhbFTZddIHoyxta3bb6PeUf+xT32
# 3jqBE2LHypp8uct92SkLuLo787bkiWw1e/OEcnMI0TwCBx/vbqRyY79w0rcstg91
# hMEc3L8qg2mAfUOy8s4dS8rEUdtOxCEFiS+KjHDU33JLLH1EOKBs1ByFfv2PKxxT
# nV1J/3rH2DwVSt94cevqeHrwgFJcrt81jibzUzidzIDPy2UCAl3gEWISeW4TLXQ3
# 0zwKf55yH8yOAK96yq6ecSrcIoZ1bAlQFx/M7VjWoM/Tfz2m8dR/I08JIGfOZVco
# GuarDEBC+YpxdYPIUtSwnVdGZvL7yLWpqQgUpizP1C6RTYWqNRa7wkHsCRwth54T
# bfPzqpOhPg==
