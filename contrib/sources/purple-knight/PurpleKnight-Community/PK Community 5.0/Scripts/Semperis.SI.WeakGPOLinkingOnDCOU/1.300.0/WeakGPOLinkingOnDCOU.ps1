...
$Global:self = @{
    ID = 40
    UUID = '154c93b1-02fc-435b-88b4-a5e3b0467f85'
    Version = '1.300.0'
    CategoryID = 4
    ShortName = 'SI000040'
    Name = 'GPO linking delegation at the domain controller OU level'
    ScriptName = 'WeakGPOLinkingOnDCOU'
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>Just being able to link GPOs doesn''t provide the whole picture. An attacker would need to find or edit a GPO that contains the instructions they want to achieve. However, if an attacker can find an existing GPO that meets their needs, then having this write permission gives them the keys to the kingdom.</p>
      <h3>References</h3>
      <p><a href="https://www.cert.ssi.gouv.fr/uploads/ad_checklist.html#vuln_permissions_gpo_priv" target="_blank">Dangerous ACLs expose GPOs applied to privileged group members (attack path) | ANSSI</a></p>'
    ResultMessage = 'Found {0} objects with write permissions on the GPLink attribute at the Domain Controller OU level.'
    Remediation = 'Unprivileged users should not be able to link GPOs at the Domain Controllers OU level. Doing so essentially gives them the ability to escalate their access, change domain-level security posture, and use GPOs to effect all systems and users in AD. Remove unnecessary permissions from the DC OU.'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $searchDN = "OU=Domain Controllers,$DN"
        $filter = "(objectClass=organizationalUnit)"
        $allowedSIDs = @("S-1-5-18", "S-1-5-32-544","$domainSID-512", "$forestSID-519")
        $UsersACL = @{}

        $scriptBlockFilter = {
            param ([System.DirectoryServices.Protocols.SearchResultEntry] $Entry)
            Update-NTSecurityDescriptor -Entry $Entry -PropagationFlagsFilter "None|NoPropagateInherit" -InheritanceTypeFilter "None|All|SelfAndChildren" -AccessControlTypeFilter "Allow" `
                -RightsFilter $rightsFilter -ExcludedSIDs $allowedSIDs -GuidHTFilter $guidHT
        }
        $results = Search-AD -dnsDomain $domain -attributes "ntsecuritydescriptor" -baseDN $searchDN -scope "Subtree" `
        ...
        foreach ($result in $results) {
            ...
            foreach ($access in $result.Attributes.'aces') {
                $ouIdentity = $access.IdentityReference.Value + ";" + $result.distinguishedName
                if ($UsersACL.ContainsKey($ouIdentity)) {
                    $UsersACL[$ouIdentity] += ";" + $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid]
                }
                else {
                    $UsersACL[$ouIdentity] = $access.AccessControlType.ToString() + ": " + $access.ActiveDirectoryRights.ToString() + " on: " + $guidHT[$access.ObjectType.Guid]
                }
        ...
        if ($UsersACL.Count -gt 0) {
            foreach ($result in $UsersACL.GetEnumerator()) {
                $thisOutput = [PSCustomObject][Ordered] @{
                    DistinguishedName = $result.Name.Split(";")[1]
    ...
    # Count is the number of domains that failed the testif ($outputObjects){
    if ($outputObjects) {
        $configArgs = @{
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.count) objects with write permissions on the GPLink attribute at the Domain Controller OU level."
            $res.Remediation = "Unprivileged users should not be able to link GPOs at the Domain Controllers OU level. Doing so essentially gives them the ability to escalate their access, change domain-level security posture, and use GPOs to effect all systems and users in AD. Remove unnecessary permissions from the DC OU."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC