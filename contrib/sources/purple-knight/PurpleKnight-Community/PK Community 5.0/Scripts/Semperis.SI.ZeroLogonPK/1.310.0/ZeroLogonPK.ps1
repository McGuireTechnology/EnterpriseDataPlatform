...
$Global:self = @{
    ID = 36
    UUID = '9c6bb8f6-7950-4d87-8e49-87fc89bf6ba6'
    Version = '1.310.0'
    CategoryID = 3
    ShortName = 'SI000036'
    Name = 'Zerologon vulnerability'
    ScriptName = 'ZeroLogonPK'
    Description = '<p>This indicator checks for security vulnerability CVE-202-1472.</p>'
    Weight = 10
    Severity = 'Critical'
    Schedule = '1d'
    Impact = 10
    LikelihoodOfCompromise = '<p>Commonly referred to as <b>Zerologon</b>, CVE-2020-1472, is a vulnerability an attacker can use to gain Domain Administrator access against an unpatched domain controller. This vulnerability is considered critical because the attacker can perform the attack unauthenticated against Active Directory.</p>
      <h3>References</h3>
      <p><a href="https://msrc.microsoft.com/update-guide/vulnerability/CVE-2020-1472" target="_blank">CVE-2020-1472 - Security Update Guide - Microsoft - Netlogon Elevation of Privilege Vulnerability</a></p>'
    ResultMessage = 'Found {0} DCs that are vulnerable to ZeroLogon.'
    Remediation = '<p>It is critical to ensure that the indicated domain controllers are patched for CVE-2020-1472, and have been restarted after the patch has been applied.</p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
...
$zerologon = @"
    ...
    namespace ZeroLogon
    {
        ...
        public static string CheckInternal(string hostname, string fqdn, bool isrodc)
        {
            ...
            var channelType = NETLOGON_SECURE_CHANNEL_TYPE.ServerSecureChannel;
            if (isrodc)
            {
                channelType = NETLOGON_SECURE_CHANNEL_TYPE.WorkstationSecureChannel;
            }
            for (int i = 0; i < 2000; i++)
            {

                if (I_NetServerReqChallenge(fqdn, "ZeroLogon", ref ClientChallenge, ref ServerChallenge) != 0)
                {
                    break;
                }

                if (I_NetServerAuthenticate2(fqdn, hostname + "$", channelType,
                    "ZeroLogon", ref ClientChallenge, ref ServerChallenge, ref NegotiateFlags) == 0)
                {
                    return fqdn;
                }
            }
        ...
        public static Task<string> Check(string hostname, string fqdn, bool isrodc)
        {

            return Task.Run(() => CheckInternal(hostname, fqdn, isrodc));
        }
    }
}
"@;
...
try {
    if (-not ('ZeroLogon.ZeroLogonCheck' -as [Type])) {
        Add-Type $zerologon
    }
    ...
    $dcs = @()
    $attributes = @("dnshostname","serverReferenceBL","primarygroupid","operatingsystem")
    ...
    foreach ($dc in $dcs) {
        $fqdn = $dc.Attributes.dnshostname[$GET_STRING]

        if ($dc.attributes.operatingsystem) {
            $versionName = $dc.attributes.operatingsystem[0]
            $testableOS = ($versionName -like '*Server 2008*') -or ($versionName -like '*Server 2012*') -or ($versionName -like '*Server 2016*') -or ($versionName -like '*Server 2019*')
        ...
        if (!($dc.attributes.serverreferencebl)) { continue }

        $hostname = $fqdn.split(".")[0]
        $pgid = $dc.Attributes.primarygroupid[$GET_STRING]
        $isrodc = $false
        if ($pgid -eq "521") {
            $isrodc = $true
        }

        $tasks += [ZeroLogon.ZeroLogonCheck]::Check($hostname, $fqdn, $isrodc)
    }
    ...
    foreach ($result in $tasks) {
        ...
        if ($checkResult -ne "NotVulnerable") {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC