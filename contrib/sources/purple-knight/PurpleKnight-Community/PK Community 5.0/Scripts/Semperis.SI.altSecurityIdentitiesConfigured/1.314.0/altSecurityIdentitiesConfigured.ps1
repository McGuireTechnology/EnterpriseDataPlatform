...
$Global:self = @{
    ...
    Schedule = '1d'
    Impact = 7
    LikelihoodOfCompromise = '<p>This type of attack may be easy to spot as it is rarely configured during normal operations. However, it is possible for this attribute to be configured genuinely.</p>
      <h3>References</h3>
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all objects with altSecurityIdentities configured
        $searchParams = @{
            dnsDomain         = $domain
            attributes        = "altsecurityidentities"
            baseDN            = $DN
            scope             = "Subtree"
            filter            = "(&(altsecurityidentities=*)(!(objectclass=msDS-Device)))"
            PropertyFilter    = "altSecurityIdentities"
        ...
        if ($results.Count) {
            foreach ($result in $results) {
                ...
                @($result.Attributes.altsecurityidentities.GetValues("string")).foreach({
                ...
                $thisOutput = new-object psobject -Property @{
                    ...
                    AltCertificates   = $configuredCertificates -join "; "
                    AltExternal       = $configuredExternal -join "; "
                    AltUnknown        = $configuredUnknown -join "; "
                    DomainName        = $domain
                }
    ...
    if ($outputObjects) {
        ...
        if ($scoreOutput.Score -lt 100) {
            ...
            $res.ResultMessage = "Found $($outputObjects.Count) account(s) with altSecurityIdentities configured."
            $res.Remediation = "Remove any entries in altSecurityIdentities attributes that are not explicitly required."
            $res.Status = 'Failed'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCANU7KJApeMrnkB
# MRXQJ1nXPbqeK9IkdsdeDKXQv4toIKCCE6MwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# 9w0BCQQxIgQghQbucJAOUva8NNmWhJiLLPIpOVYEOX7TC7BMUwvzqqQwDQYJKoZI
# hvcNAQEBBQAEggIAX7jROIx6/m2HUNslLrwiPru9QR6xOkx8eS6DOzscqykn7bmE
# UipGn3GC4+mnxtlQtYJF3ZpnTLfC+hx6SOn6c4cj1O0/6Zjyad8i8sH83u0MvSYi
# RW7o93IGCU9M76CGJKAcO65i9c3cTPd9RBDbCB1FPdhyCt0ArkGXiZVcuNzij6Jk
# VgKwFbStSmUxdquqM3bYxQsG7Q4bAEFTRDwYhh0TXlwlhKh3JXDy9JuNhrWP0F8d
# eo6wftNSqp2uC/whiDuEItjIpGpw9R9tZT/bN6foiPp1GMy9DLUCZqr8dC5CnYcE
# YhMbM9KQCbgRilrb5gopPMmr6NuNtmjsiGgh3EHrM2UbfPJQPxuSlMBQNS68Z8cN
# v9CVB491rrQ8srHXXXi1WkA5v/PwUYdcvcBzWZvQHJDLm3oztWcyC76jknABUpSh
# LVOaIrJmZxTh5s5VIC0Qi18IR9rpropppoSm7pUcuFo5Rt9Ct91vCcyPk/XhJTfu
# V5vsqy3Z6KfRiT+YNPdv8PH1+8/+QfMmuY1X5r38OUK1vTm8qp2fuDBFD97NRQCi
# YJQrpSjkZV2iYSgK27LLejxXJr4c86iGqHFrAowPVnWTlnzXWdBGuPZTHyUwG2Ik
# qkqtvjL/8/wbBUiekau6RVvxeevmFi6x4uhijm6J/fJ3Jt/TyQ3JB5ZdjXmhgha3
...
# oDICAzAxMA0GCWCGSAFlAwQCAQUABCD0vaQyC3FqrmBq9TMQZ4fPZvaM+PJBKd0Z
# c+jkvHRSfAIUAM1HFqvaCXBmhOea/WAlhX87+NkYDzIwMjUwNjEwMDc1MTE4WjAD
...
# MC8GCSqGSIb3DQEJBDEiBCD++HYzvFYFf+9YJ8nqzCKchQcm7+oQUzyK9OcOGtkY
...
# CwUABIIBgF+3pPwEQkXevSacGhE+yxZedHhkJ6BruM5RsNLBcZPTOsWY5LMnixs2
# IyGIbddElX7eiicwVE+2HDLhDdiruoFtB3Y/Da6rzvtkWNMdFIymv8yZ4j/l783G
# U8YvqEo9JiB46Sr3GQTcMX/XP9CN3gWstRlTe3Ly4P+vhLSPnLF1VmADSkwEU7os
# B11DOwJ7a70MHZuTeDSz0v6RY22LDQQUpZ49nBY0vJp8ui3ISRLPA3UxMjms4Zx+
# U9XWVNHFsbVL8dnS9khcpIMa6/nJIGxiD7tk/0qsdhZ5A09kxOtLDEtIH2O+a2oZ
# eedsHEk1GAqX/dipxgAx6oLmvbCASRMu7MxpP2vo5k84ailvl+3icfwTxki7fGns
# QzwrHQTe0I07viuNvtwXs1jNVE/2dxjLHvCE9BSQdWGu9CXkHkPcGFEI6f3h45YY
# VuybzNgSI/hbsZ8aIP2EIdSth2ggFM7pmVb3lM3V/Up9J5MSM3c7QzWHi4PdMWGV
# ChmbQZeCsQ==
